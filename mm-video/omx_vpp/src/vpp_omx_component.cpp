    ...
    if (param_index == OMX_IndexParamPortDefinition && !mVppBypassMode) {
        ...
        if (portDef->nPortIndex == 1) {
            ...
            if (mState == OMX_StateLoaded) {
                 ...
                 if (eRet != OMX_ErrorNone) {
                     DEBUG_PRINT_ERROR("SetParam on output port failed");
                     return eRet;
                 }
            }
                                      ...
                                      OMX_PTR pEventData)
{
    ...
    switch ((OMX_U32)eEvent)
    {
        ...
        {
            DEBUG_PRINT_LOW("OMX_EventConfigUpdate");
            eRet = super_omx->mOmxCallbacks.EventHandler(&super_omx->m_cmp,
                                  super_omx->mAppData,
                                  eEvent,
                                  nData1, nData2, pEventData);
    ...
    switch ((enum hqv_control_type)vExt.extensionIndex()) {
        ...
        case HQV_CONTROL_FRC:
        {
            ...
            if (!mVppHqvCtrlValue.frc.num_segments || !mVppHqvCtrlValue.frc.segments) {
                setStatus &= vExt.setParamString(ext, FRC_MODE, "FRC_MODE_OFF");
                setStatus &= vExt.setParamString(ext, FRC_LEVEL, "FRC_LEVEL_OFF");
                setStatus &= vExt.setParamString(ext, FRC_INTERP, "FRC_INTERP_1X");
                setStatus &= vExt.setParamInt64(ext, FRC_TIMESTAMP_START, 0);
                setStatus &= vExt.setParamInt32(ext, FRC_FRAME_COPY_ON_FALLBACK, 0);
                setStatus &= vExt.setParamInt32(ext, FRC_FRAME_COPY_INPUT, 0);
                break;
            }
