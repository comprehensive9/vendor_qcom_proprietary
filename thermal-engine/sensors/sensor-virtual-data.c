...
static char *vs_list_scuba[] = {
	"s3-die-temp-usr"
};

static char *vs_trip_list_scuba[] = {
	"cpuss-0-usr",
	"cpuss-1-usr"
};

static char *vs_trip_logic_scuba[] = {
	"||",
};

static struct setting_info vs_cfgs_scuba[] =
{
	{
		.desc = "VIRTUAL-S3-LEVEL1",
		.algo_type = VIRTUAL_SENSOR_TYPE,
		.dynamic = 0,
		.data.v = {
			.trip_sensor_list = vs_trip_list_scuba,
			.trip_cnt = ARRAY_SIZE(vs_trip_list_scuba),
			.trip_sensor_logic = vs_trip_logic_scuba,
			.sensor_list = vs_list_scuba,
			.list_cnt = ARRAY_SIZE(vs_list_scuba),
			.t[0] = {
				.lvl_trig = 80000,
				.lvl_clr =  75000,
			},
...
static char *vs_sens_list_lahaina[] = {
	"cpuss-0-usr",
	"cpuss-1-usr",
};

static char *vs_trip_list_lahaina[] = {
	"nspss-0-usr",
	"nspss-1-usr",
	"nspss-2-usr",
};

static char *vs_trip_logic_lahaina[] = {
	"||",
	"||",
};

static struct setting_info vs_cfgs_lahaina[] =
{
	{
		...
		.dynamic = 0,
		.data.v = {
			.trip_sensor_list = vs_trip_list_lahaina,
			.trip_cnt = ARRAY_SIZE(vs_trip_list_lahaina),
			.trip_sensor_logic = vs_trip_logic_lahaina,
			.logic_cnt = ARRAY_SIZE(vs_trip_logic_lahaina),
			.sensor_list = vs_sens_list_lahaina,
			.list_cnt = ARRAY_SIZE(vs_sens_list_lahaina),
			.list_size = 1,
			.t[0] = {
				.lvl_trig = 94000,
				.lvl_clr =  90000,
...
void virtual_sensors_init_data(struct thermal_setting_t *setting)
{
	...
	switch (therm_get_msm_id()) {
		...
		case THERM_SA410M:
			cfg = vs_cfgs_scuba;
			arr_size = ARRAY_SIZE(vs_cfgs_scuba);
			break;
		case THERM_LAHAINA:
			cfg = vs_cfgs_lahaina;
			arr_size = ARRAY_SIZE(vs_cfgs_lahaina);
			break;
		default:
