...
#define QMI_MODEM_BW_BACKOFF "modem_bw_backoff"
#define MAX_MODEM_BW_BACKOFF_LEVEL  (255)
...
static struct qmi_dev_info modem_devs[] = {
	...
	{
		.dev_name = QMI_MODEM_BW_BACKOFF,
		.debug_name = MODEM_BW_BACKOFF_DEV_NAME,
	},
	{
...
static struct qmi_client_info fusion_clnt = {
	.name  = "FUSION",
	.instance_id  = 0xfff,   /* Dummy instance ID */
	.mtx          = PTHREAD_MUTEX_INITIALIZER,
...
/*===========================================================================
FUNCTION modem_bw_backoff_request

Action function to throttle modem bandwidth.

ARGUMENTS
	level - 0-255 throttling request for modem bandwidth
...
int modem_bw_backoff_request(int level)
{
	...
	/* Modem bandwidth level: 0 - No action
	 *		Every increasing level triggers mitigation with absolute bandwidth reduction from max bandwidth in 100MBbs resolution.
	 */

	ret = generic_qmi_request(level, MAX_MODEM_BW_BACKOFF_LEVEL,
				  &modem_clnt, QMI_MODEM_BW_BACKOFF,
				  MODEM_BW_BACKOFF_DEV_NAME);
