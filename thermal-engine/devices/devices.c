...
static int modem_bw_backoff_action(struct devices_manager_dev *dev_mgr);
...
static struct tmd_generic_dev_info_t gen_dev_list[] = {
	...
	{
		.name = MODEM_BW_BACKOFF_DEV_NAME,
		.num_of_lvls = 256,  /* number of levels, not max state */
		.action = modem_bw_backoff_action,
	},
	{
...
static int modem_bw_backoff_action(struct devices_manager_dev *dev_mgr)
{
	return modem_bw_backoff_request(dev_mgr->active_req.value);
}
