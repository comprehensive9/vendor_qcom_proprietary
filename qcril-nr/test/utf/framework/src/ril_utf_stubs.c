...
pthread_mutex_t malloc_list_mutex;
uint8_t malloc_list_mutex_inited = FALSE;
...
void ril_utf_lock_malloc_list_mutex() {
  if (!malloc_list_mutex_inited) {
    pthread_mutex_init(&malloc_list_mutex, NULL);
    malloc_list_mutex_inited = TRUE;
  }
  pthread_mutex_lock(&malloc_list_mutex);
}
void ril_utf_unlock_malloc_list_mutex() {
  pthread_mutex_unlock(&malloc_list_mutex);
}
...
)
{
  ...
  ril_utf_lock_malloc_list_mutex();
  ...
  ril_utf_unlock_malloc_list_mutex();
