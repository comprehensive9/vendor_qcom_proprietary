...
test_runner::test_runner()
{
  ...
  utf_log_file = stdout;
  utf_results_log_file = fopen("log/test_results.txt", "w");
 ...
 * run a single test case defined in the function pointer table
========================================================*/

int test_runner::execute_single_test(func_ptr_t ptr, vector<test_result> &total_result)
{
    ...
    if ( failed != 0 )
    {
        RIL_UTF_ESSENTIAL("\n Testcase Failed (please see errors in log folder above)");
    }
    RIL_UTF_ESSENTIAL("\n@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@\n");
  ...
  for (i = 0; func_ptr_table[i] != NULL; ++i)
  {
    ...
    else
    {
      ...
      if (shutdown_after_execution) {
        ...
        utf_property_init();
        qcril_config_clear_all_user_settings();
      }
    }
  }
  if (!shutdown_after_execution) {
