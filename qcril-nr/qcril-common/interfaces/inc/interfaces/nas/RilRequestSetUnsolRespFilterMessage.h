...
class RilRequestSetUnsolBarringFilterMessage
    : public QcRilRequestMessage,
      public add_message_id<RilRequestSetUnsolBarringFilterMessage> {
public:
  static constexpr const char *MESSAGE_NAME = "RilRequestSetUnsolBarringFilterMessage";

  RilRequestSetUnsolBarringFilterMessage() = delete;
  ~RilRequestSetUnsolBarringFilterMessage() {}

  explicit inline RilRequestSetUnsolBarringFilterMessage(std::shared_ptr<MessageContext> context,
                                                      bool enable, int32_t indFilter)
      : QcRilRequestMessage(get_class_message_id(), context), mEnable(enable), mIndicationFilter(indFilter) {
    mName = MESSAGE_NAME;
  }
