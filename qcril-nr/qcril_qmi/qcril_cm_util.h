...
static const uint8_t qcril_ascii_to_def_alpha_table[] =
{
...
void qcril_cm_util_rssi_to_gw_signal_strength( uint16_t rssi, int *signal_strength_ptr );

void qcril_cm_util_rsrp_to_lte_signal_strength( uint16_t rsrp,  int *signal_strength_ptr);

int qcril_cm_util_convert_2s_complement_to_int(uint8_t);

uint8_t qcril_cm_util_ussd_pack(uint8_t *packed_data, const uint8_t *str, uint8_t num_chars);
...
(
    ...
    uint8_t num_uint8_ts
);

void qcril_cm_util_bcd_to_ascii(const uint8_t *bcd_number, uint8_t * ascii_number);

uint8_t qcril_cm_util_ascii_to_gsm_alphabet(uint8_t *gsm_alphabet_string, const uint8_t *ascii_string, uint8_t num_chars);
