...
typedef struct qcril_cm_ss_bs_mapping_s
{
  ...
  uint8_t  bs_type;   /* BS type - bearer services, teleservices */
  uint8_t  bs_code;   /* BS Code */
...
typedef struct qcril_cm_ss_sc_table_s
{
  ...
  uint8_t net_sc;      /* Network value of Service Code */
...
typedef struct qcril_cm_ss_bsg_table_s
{
  ...
  uint8_t service_class;    /* Service Class as per 27.007 */
...
extern  uint32_t qcril_cm_ss_cw_allowed_classes;
...
uint8_t qcril_cm_ss_get_ss_ref( void );

uint8_t qcril_cm_ss_get_cfw_ss_code( int reason, const char **cf_reason_name );
