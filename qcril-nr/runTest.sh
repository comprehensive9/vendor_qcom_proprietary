...
LIBS=( libc++.so libqcrilNrFramework.so libcutils.so liblog.so libprotobuf-cpp-full.so libqcrilnr.so libqcrilNrLogger.so libz-host.so libril-qc-ltedirectdisc.so librilutils.so libril-qc-radioconfig.so libicuuc-host.so libdiag.so)
