...
RilServiceModule::RilServiceModule(qcril_instance_id_e_type instance)
    ...
    mMessageHandler = {
        ...
        HANDLER(RilUnsolCdmaSubscriptionSourceChangedMessage, RilServiceModule::handleCdmaSubscriptionSourceChangedMessage),
        ...
        HANDLER(rildata::RadioDataCallListChangeIndMessage_1_0, RilServiceModule::handleRadioDataCallListChangeIndMessage),
        HANDLER(rildata::GetDataCallListResponseIndMessage, RilServiceModule::handleGetDataCallListResponseIndMessage),
#endif
    };
}
...
void RilServiceModule::handleQcRilUnsolOnUssdMessage(std::shared_ptr<QcRilUnsolOnUssdMessage> msg) {
  ...
  auto ri = getRadioImpl();
  if (ri) {
    if (msg->isCsDomain() || !(ri->isUssdOverImsSupported())) {
...
#ifndef QMI_RIL_UTF
        setNitzTimeReceived(android::elapsedRealtime());
#endif
...
void RilServiceModule::handleSignalStrengthMessage(
    ...
    auto ri = getRadioImpl();
    if (ri) {
        // No need to grab wakelock for this UNSOL
...
void RilServiceModule::handleCdmaSubscriptionSourceChangedMessage(
    ...
    if (ri) {
        ...
        ri->sendCdmaSubscriptionSourceChanged(msg);
...
void RilServiceModule::handleRadioDataCallListChangeIndMessage(
    std::shared_ptr<rildata::RadioDataCallListChangeIndMessage_1_0> msg) {
    auto ri = getRadioImpl();
    if(msg && ri) {
...
void RilServiceModule::handleGetDataCallListResponseIndMessage(std::shared_ptr<rildata::GetDataCallListResponseIndMessage> msg) {
    auto ri = getRadioImpl();
    if(msg && ri) {
        QCRIL_LOG_DEBUG("Handling handleGetDataCallListResponseIndMessage %s", msg->dump().c_str());
        static_cast<RadioImpl*>(ri.get())->getDataCallListResponse(msg->getDCList(), msg->getSerial(), msg->getStatus());
    } else {
