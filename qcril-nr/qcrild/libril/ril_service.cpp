...
static int64_t nitzTimeReceived;
...
void RadioImpl::serviceDied(uint64_t cookie,
    ...
    auto msg = std::make_shared<ClientDisconnectedMessage>(qmi_ril_get_process_instance_id());
...
using namespace rildata;
    ...
    GenericCallback<DataCallListResult_t> cb(
        ([serial](std::shared_ptr<Message> solicitedMsg, Message::Callback::Status status,
            ...
            if (solicitedMsg && responseDataPtr)
            {
                auto msg = std::make_shared<rildata::GetDataCallListResponseIndMessage>(responseDataPtr, serial, status);
                msg->broadcast();
            }
        }));
  ...
  do {
    ...
    GenericCallback<QcRilRequestMessageCallbackPayload> cb(
        [this, serial](std::shared_ptr<Message> msg, Message::Callback::Status status,
          ...
          RadioResponseInfo respInfo = {};
          populateResponseInfo(respInfo, serial, RESPONSE_SOLICITED, errorCode);
          RADIO_HIDL_SEND_RESPONSE(mSlotId, mRadioResponse, explicitCallTransferResponse, respInfo);
        });
    msg->setCallback(&cb);
    msg->dispatch();
  } while (FALSE);
...
bool dispatchImsCdmaSms(const ImsSmsMessage& message, RequestInfo *pRI) {
    ImsCdmaSms temp;
    memset(&temp, 0, sizeof(ImsCdmaSms));
...
Return<void> RadioImpl::sendImsSms(int32_t serial, const ImsSmsMessage& message) {
    ...
    do {
        ...
        std::shared_ptr<RadioContext> ctx = getContext(serial);
        auto msg = std::make_shared<RilRequestImsSendSmsMessage>(ctx, message.messageRef,
            tech, message.retry);
        if (msg == nullptr) {
            sendResponse = true;
        ...
        GenericCallback<QcRilRequestMessageCallbackPayload> cb(
            ([this, serial](std::shared_ptr<Message> solicitedMsg, Message::Callback::Status status,
                ...
                if (solicitedMsg && resp) {
                    ...
                    auto sendSmsResult = std::static_pointer_cast<RilSendSmsResult_t>(resp->data);
                    if (sendSmsResult) {
                        result = makeSendSmsResult(responseInfo, serial,
                            RESPONSE_SOLICITED, sendSmsResult, errorCode);
                    } else {
                        populateResponseInfo(responseInfo, serial, RESPONSE_SOLICITED, errorCode);
                    }
    ...
    if (msg != nullptr) {
        GenericCallback<QcRilRequestMessageCallbackPayload> cb(
            ...
            if (resp != nullptr) {
                ...
                RADIO_HIDL_SEND_RESPONSE(mSlotId, mRadioResponse,
                        setRadioCapabilityResponse, respInfo, result);
            }
        });
        msg->setCallback(&cb);
        msg->dispatch();
...
int RadioImpl::sendNewCdmaSms(std::shared_ptr<RilUnsolIncoming3Gpp2SMSMessage> shared_msg)
{
    ...
    if (ind) {
        ...
        Return<void> ret = ind->cdmaNewSms(RadioIndicationType::UNSOLICITED_ACK_EXP, msg);
...
int RadioImpl::sendNewSmsOnSim(std::shared_ptr<RilUnsolNewSmsOnSimMessage> msg)
{
    ...
    QCRIL_LOG_INFO("ind null: %s", ind ? "false" : "true" );
    if (ind) {
        Return<void> ret = ind->newSmsOnSim(RadioIndicationType::UNSOLICITED_ACK_EXP,
                msg->getRecordNumber());
        checkReturnStatus(ret);
    }
    QCRIL_LOG_FUNC_RETURN();
...
int RadioImpl::sendNewSmsStatusReport(std::shared_ptr<RilUnsolNewSmsStatusReportMessage> msg)
{
    ...
    if (ind) {
        ...
        Return<void> ret = ind->newSmsStatusReport(
                RadioIndicationType::UNSOLICITED_ACK_EXP, pdu);
        checkReturnStatus(ret);
    }
    QCRIL_LOG_FUNC_RETURN();
    return 0;
...
int RadioImpl::sendStkCCAlphaNotify(std::shared_ptr<RilUnsolStkCCAlphaNotifyMessage> msg)
{
    ...
    if (ind) {
        ...
        Return<void> ret = ind->stkCallControlAlphaNotify(
                RadioIndicationType::UNSOLICITED_ACK_EXP, payload);
        checkReturnStatus(ret);
    }
    QCRIL_LOG_FUNC_RETURN();
    return 0;
...
int RadioImpl::sendOnUssd(std::shared_ptr<QcRilUnsolOnUssdMessage> msg)
{
  ...
  if (ind && msg && !msg->isUnknownDomain()) {
    ...
    QCRIL_LOG_INFO("ind null: %s, msg null: %s", ind ? "false" : "true",
                      msg ? "false" : "true");
  }
  QCRIL_LOG_FUNC_RETURN();
  return 0;
...
#endif
      Return<void> ret = ind->cdmaOtaProvisionStatus(RadioIndicationType::UNSOLICITED_ACK_EXP,
...
#endif
      Return<void> ret = ind->srvccStateNotify(RadioIndicationType::UNSOLICITED_ACK_EXP, (SrvccState)state);
      checkReturnStatus(ret);
    }
  } else {
    QCRIL_LOG_INFO("ind null: %s, msg null: %s", ind ? "false" : "true", msg ? "false" : "true");
...
#endif
      Return<void> ret =
          ind->onSupplementaryServiceIndication(RadioIndicationType::UNSOLICITED_ACK_EXP, ss);
      checkReturnStatus(ret);
  } else {
    QCRIL_LOG_INFO("ind null: %s, msg null: %s", ind ? "false" : "true", msg ? "false" : "true");
  }
...
int RadioImpl::sendNitzTimeReceived(std::shared_ptr<RilUnsolNitzTimeReceivedMessage> msg)
{
    ...
    if (ind) {
        ...
        int64_t timeReceived = nitzTimeReceived;
#if VDBG
        RLOGD("nitzTimeReceivedInd: nitzTime %s receivedTime %" PRId64, time.c_str(),
                timeReceived);
#endif
        Return<void> ret = ind->nitzTimeReceived(RadioIndicationType::UNSOLICITED_ACK_EXP,
                time, timeReceived);
        checkReturnStatus(ret);
    } else {
...
int RadioImpl::sendVoiceRadioTechChanged(std::shared_ptr<RilUnsolVoiceRadioTechChangedMessage> msg)
{
    ...
    QCRIL_LOG_INFO("ind null: %s", ind ? "false" : "true" );
    if (ind) {
        Return<void> ret = ind->voiceRadioTechChanged(RadioIndicationType::UNSOLICITED_ACK_EXP,
...
int RadioImpl::sendNetworkScanResult(std::shared_ptr<RilUnsolNetworkScanResultMessage> msg)
{
    ...
    if (ind) {
        ...
        Return<void> retStatus = ind->networkScanResult(
                RadioIndicationType::UNSOLICITED_ACK_EXP, result);
...
int RadioImpl::sendEmergencyCallbackMode(std::shared_ptr<RilUnsolEmergencyCallbackModeMessage> msg)
{
    ...
    if (ind) {
        ...
        if (mode == RilUnsolEmergencyCallbackModeMessage::EmergencyCallbackMode::ENTER) {
            Return<void> ret = ind->enterEmergencyCallbackMode(RadioIndicationType::UNSOLICITED_ACK_EXP);
            checkReturnStatus(ret);
        } else {
            Return<void> ret = ind->exitEmergencyCallbackMode(RadioIndicationType::UNSOLICITED_ACK_EXP);
            checkReturnStatus(ret);
        }
    }
    QCRIL_LOG_FUNC_RETURN();
...
int RadioImpl::sendRadioCapability(std::shared_ptr<RilUnsolRadioCapabilityMessage> msg)
{
    ...
    if (ind) {
        ...
        Return<void> ret = ind->radioCapabilityIndication(
                RadioIndicationType::UNSOLICITED_ACK_EXP, rc);
        checkReturnStatus(ret);
    }
    QCRIL_LOG_FUNC_RETURN();
    return 0;
...
int RadioImpl::sendRestrictedStateChanged(std::shared_ptr<RilUnsolRestrictedStateChangedMessage> msg)
{
    ...
    if (ind) {
        int32_t state = msg->getState();
        Return<void> ret = ind->restrictedStateChanged(RadioIndicationType::UNSOLICITED_ACK_EXP,
...
int RadioImpl::sendUiccSubsStatusChanged(std::shared_ptr<RilUnsolUiccSubsStatusChangedMessage> msg)
{
    ...
    if (ind) {
        ...
        Return<void> ret = ind->subscriptionStatusChanged(
                RadioIndicationType::UNSOLICITED_ACK_EXP, activate);
        checkReturnStatus(ret);
    }
    QCRIL_LOG_FUNC_RETURN();
    return 0;
...
int RadioImpl::sendRadioStateChanged(std::shared_ptr<RilUnsolRadioStateChangedMessage> msg)
{
    ...
    if (ind) {
        ...
        Return<void> ret = ind->radioStateChanged(
            RadioIndicationType::UNSOLICITED_ACK_EXP, radioState);
        checkReturnStatus(ret);
    }
    QCRIL_LOG_FUNC_RETURN();
    return 0;
...
int RadioImpl::sendCdmaSubscriptionSourceChanged(std::shared_ptr<RilUnsolCdmaSubscriptionSourceChangedMessage> msg)
{
    auto ind = mRadioIndication;
    QCRIL_LOG_INFO("ind null: %s", ind ? "false" : "true" );
    if (ind) {
        Return<void> ret = ind->cdmaSubscriptionSourceChanged(RadioIndicationType::UNSOLICITED,
            static_cast<CdmaSubscriptionSource>(msg->getSource()));
        checkReturnStatus(ret);
    }
    QCRIL_LOG_FUNC_RETURN();
    return 0;
    ...
    if (ind && msg) {
        ...
        switch(stk_ind.type) {
            ...
            case RIL_GSTK_UNSOL_STK_CALL_SETUP:
                ...
                ret = ind->stkEventNotify(RadioIndicationType::UNSOLICITED_ACK_EXP, stk_ind.cmd);
                ret = ind->stkCallSetup(RadioIndicationType::UNSOLICITED_ACK_EXP,
...
bool RadioImpl::isUssdOverImsSupported() {
  ...
  qmi_ril_get_property_value_from_boolean(QCRIL_USSD_IMS_RUNTIME_SUPPORT,
                                          &ussdImsRuntimeSupport,
                                          TRUE);
...
void radio::registerService(RIL_RadioFunctions *callbacks, CommandInfo *commands) {
    ...
    for (int i = 0; i < simCount; i++) {
        ...
        getQcrildServiceFactory().initRadioImplListIndex();
        radioService[i] = getQcrildServiceFactory().getNextRadioImpl(static_cast<qcril_instance_id_e_type>(i));
        android::status_t status;
        while (radioService[i]) {
            ...
            if (status != android::OK) {
                QCRIL_LOG_INFO("Error registering service %s %s",
                        ...
                        serviceNames[i]);
                //get the next lower version
                radioService[i] = getQcrildServiceFactory().getNextRadioImpl(static_cast<qcril_instance_id_e_type>(i));
            } else {
                radioService[i]->createRilServiceModule();
                QCRIL_LOG_INFO("Registered to service %s %s",
                            radioService[i]->descriptor, serviceNames[i]);
                break;
            }
        }

        if (radioService[i] == nullptr) {
    ...
    if (qmi_ril_get_process_instance_id() == QCRIL_DEFAULT_INSTANCE_ID) {
        ...
        getRadioConfigFactory().initRadioConfigListIndex();
        radioConfigService = getRadioConfigFactory().getNextRadioConfigImpl();
        while (radioConfigService) {
            ...
            if (status != android::OK) {
                ...
                radioConfigService->createRadioConfigModule();
                QCRIL_LOG_INFO("Registered to service %s %s",
                            radioConfigService->descriptor,
                            "default");
                break;
        ...
        if (radioConfigService == nullptr) {
            QCRIL_LOG_INFO("registerService: Unable to find implementation for IRadioConfig");
        }
    }
#endif
}
...
Return<void> RadioImpl::getDataCallListResponse(std::shared_ptr<rildata::DataCallListResult_t> responseDataPtr,
    ...
    auto rsp = mRadioResponse;
    if(rsp) {
        QCRIL_LOG_INFO("getDataCallListResponse");
        qtimutex::QtiSharedMutex *radioServiceRwlockPtr = radio::getRadioServiceRwlock(mSlotId);
        ...
        responseInfo.error = e;
        radioServiceRwlockPtr->lock_shared();
        Return<void> retStatus = rsp->getDataCallListResponse(responseInfo, dcResultList);
        checkReturnStatus(retStatus);
        radioServiceRwlockPtr->unlock_shared();
    } else {
        RLOGE("getDataCallListResponse: radioService[%d]->mRadioResponse == NULL", mSlotId);
    }
#endif
    QCRIL_LOG_FUNC_RETURN();
    return Void();
