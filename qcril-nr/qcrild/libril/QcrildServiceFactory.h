...
class QcrildServiceFactory
{
    ...
    private:
        ...
        RadioImplListType::iterator radioImplIter;
    ...
    public:
        ...
        sp<RadioImpl> getNextRadioImpl(qcril_instance_id_e_type instance) {
            QCRIL_LOG_INFO("getNextRadioImpl");
            sp<RadioImpl> ret;
            if (!radioImplList.empty()) {
                if (radioImplIter != radioImplList.end()) {
                    FactMethod<RadioImpl> m = radioImplIter->factMethod;
                    if (m) {
                        ret = (this->*m)(instance);
                    }
                    radioImplIter++;
        ...
        void initRadioImplListIndex() {
            radioImplIter = radioImplList.begin();
        }
};
