...
static void qcril_qmi_voice_setup_num_1x_wait_timer()
{
  ...
  uint32_t               num_1x_wait_timer_id = QMI_RIL_ZERO;
...
void qcril_qmi_voice_auto_answer_if_needed()
{
  ...
  if( answer_ms > 0 )
  {
    ...
    auto_answer_timeout.tv_sec = answer_ms/1000;
...
)
{
  ...
  if( ind_data_ptr != NULL )
  {
      ...
      if (getVoiceModule().setAudioStateBeforeCallStatusReport()) {
      ...
      if (getVoiceModule().setAudioStateAfterCallStatusReport()) {
        qcril_am_handle_event(QCRIL_AM_EVENT_CALL_STATE_CHANGED, NULL);
      }
  }
...
void qmi_ril_voice_ended_call_obj_phase_out(void * param)
{
    ...
    timer_id = ( uint32_t )(uintptr_t)param;
...
)
{
  ...
  else
  {
     ...
     * for dial request timer. Send this msgs only if DIAL request is success.
     */
    auto ind_msg_ptr =
...
void qmi_ril_voice_ims_command_oversight_timeout_handler(void * param)
{
  ...
  cause_tmr = (uint32_t)(uintptr_t)param;
