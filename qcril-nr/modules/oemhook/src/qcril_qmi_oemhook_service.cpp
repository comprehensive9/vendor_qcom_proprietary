...
)
{
    ...
    if ((NULL != data) && (1 < data_len))
    {
      ...
      QCRIL_LOG_INFO("data length is %d", data_len);
...
Return<void> OemHookImpl::uimSendPersoReq(int32_t serial, uint8_t *data, uint32_t dataLen)
{
  ...
  tempPersoType = (int32_t)*((uint8_t *)data + ctrl_key_len);
...
Return<void> OemHookImpl::uimSendDePersoReq(int32_t serial, uint8_t *data, uint32_t dataLen)
{
  ...
  tempPersoState = (int32_t)(atoi((char *)data));
...
Return<void> OemHookImpl::uimSendApduReq(int32_t serial, uint8_t *data, uint32_t dataLen)
{
  ...
  /* Allocate memory for the destination pointer and copy the data and convert it into java string*/
  const char* data_offset = reinterpret_cast <char*>(data) + (sizeof(int32_t) * 7);
  uint32_t data_payload_len = dataLen - (sizeof(int32_t)*7);
  char *temp = new char [data_payload_len + 1]{0};
  strlcpy(temp,data_offset, data_payload_len + 1);
  apdu_data.data = std::string(temp);

  /* Oemhook uses basic logical channel for streaming the APDU Updating the second paramter
   * as True
   */
  auto msg = std::make_shared<UimTransmitAPDURequestMsg>(apdu_ptr->slot_id, true, false, apdu_data);
  if (msg) {
                        ...
                        std::shared_ptr<RIL_UIM_SIM_IO_Response> rsp_ptr) -> void
    {
          ...
          status == Message::Callback::Status::SUCCESS)
      {
        ...
        resp_len = sizeof(apdu_rsp.slot_id) + sizeof(apdu_rsp.ril_err) +
                   sizeof(apdu_rsp.sw1) + sizeof(apdu_rsp.sw2);
        if (rsp_ptr->simResponse.length() < sizeof(apdu_rsp.simResponse))
        {
          rsp_ptr->simResponse.copy(apdu_rsp.simResponse, rsp_ptr->simResponse.length());
              resp_len += rsp_ptr->simResponse.length();
        }
        /*Copy only the original response from the modem*/
        data.setToExternal((uint8_t*)&apdu_rsp, resp_len, false);
 ...
 */
void OemHookImpl::sendUnsolAudioStateChanged(std::shared_ptr<QcRilUnsolAudioStateChangedMessage> msg) {
  sp<IQtiOemHookIndication> indCb = getIndicationCallback();
  QCRIL_LOG_INFO("ind null: %s", indCb ? "false" : "true");
  if (indCb && msg) {
    auto payload = msg->getAudioParams();
    uint32_t bufLen = 0;
    uint8_t *buf = constructOemHookRaw(&bufLen, QCRIL_REQ_HOOK_UNSOL_AUDIO_STATE_CHANGED,
