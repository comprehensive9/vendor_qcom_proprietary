...
android::status_t ImsRadioImpl_1_6::registerService() {
...
void ImsRadioImpl_1_6::notifyOnUssdMessage(std::shared_ptr<QcRilUnsolOnUssdMessage> msg) {
  if (msg) {
    sp<V1_6::IImsRadioIndication> indCb = getIndicationCallbackV1_6();
    if (indCb) {
      V1_5::UssdModeType modeType = V1_5::utils::convertUssdModeType(msg->getMode());
      hidl_string ussdMsg = "";
      if (msg->hasMessage()) {
        ussdMsg = msg->getMessage();
      }
      V1_0::SipErrorInfo errorDetails;
      V1_5::utils::convertSipErrorInfo(errorDetails, msg->getErrorDetails());
      imsRadiolog("<", "onUssdMessage: type=" + toString(modeType) + " ussdMsg=" +
          toString(ussdMsg) + " errorDetails="  + toString(errorDetails));
      Return<void> ret = indCb->onUssdReceived(modeType, ussdMsg, errorDetails);
      if (!ret.isOk()) {
        QCRIL_LOG_ERROR("Unable to send response. Exception : %s", ret.description().c_str());
      }
    }
...
void ImsRadioImpl_1_6::notifyOnHandover(std::shared_ptr<QcRilUnsolImsHandoverMessage> msg) {
  ...
  if (!msg->hasErrorCode() && !msg->hasErrorMsg() &&
      msg->hasCauseCode() && !msg->getCauseCode().empty()) {
...
Return<void> ImsRadioImpl_1_6::sendUssd(
  ...
  do {
    if (!V1_6::utils::isUssdOverImsSupported()) {
      err = RIL_E_USSD_CS_FALLBACK;
      sendFailure = true;
      break;
    }
    auto msg = std::make_shared<QcRilRequestSendUssdMessage>(getContext(token));
    if (msg == nullptr) {
      QCRIL_LOG_ERROR("msg is nullptr");
      sendFailure = true;
      break;
    ...
    GenericCallback<QcRilRequestMessageCallbackPayload> cb(
        [this, token](std::shared_ptr<Message> /*msg*/, Message::Callback::Status status,
          ...
          sendUssdResponse(token, errorCode, errorDetails);
        });
    msg->setCallback(&cb);
    msg->dispatch();
  } while (FALSE);
  if (sendFailure) {
    sendUssdResponse(token, err, nullptr);
  }
  QCRIL_LOG_FUNC_RETURN();
  return Void();
}

void ImsRadioImpl_1_6::sendUssdResponse(int32_t token, uint32_t errorCode,
                                    std::shared_ptr<qcril::interfaces::SipErrorInfo> data) {
  V1_0::SipErrorInfo errorDetails = {.errorCode = INT32_MAX};
  if (data) {
    V1_5::utils::convertSipErrorInfo(errorDetails, data);
  }
  sp<IImsRadioResponse> respCb = getResponseCallbackV1_6();
  if (respCb != nullptr) {
    imsRadiolog("<", "sendUssdResponse: token = " + std::to_string(token) + " errorCode = " +
                         std::to_string(errorCode) + " errorDetails = " + toString(errorDetails));
    Return<void> ret = respCb->sendUssdResponse(token, errorCode, errorDetails);
    if (!ret.isOk()) {
      QCRIL_LOG_ERROR("Unable to send response. Exception : %s", ret.description().c_str());
    }
  }
...
Return<void> ImsRadioImpl_1_6::cancelPendingUssd(int32_t token) {
  ...
  bool sendFailure = false;
  do {
    auto msg = std::make_shared<QcRilRequestCancelUssdMessage>(getContext(token));
    if (msg == nullptr) {
      QCRIL_LOG_ERROR("msg is nullptr");
      sendFailure = true;
      break;
    ...
    GenericCallback<QcRilRequestMessageCallbackPayload> cb(
        [this, token](std::shared_ptr<Message> /*msg*/, Message::Callback::Status status,
          ...
          cancelPendingUssdResponse(token, errorCode, errorDetails);
        });
    msg->setCallback(&cb);
    msg->dispatch();
  } while (FALSE);
  if (sendFailure) {
    cancelPendingUssdResponse(token, RIL_E_GENERIC_FAILURE, nullptr);
  }
  QCRIL_LOG_FUNC_RETURN();
  return Void();
}

void ImsRadioImpl_1_6::cancelPendingUssdResponse(int32_t token, uint32_t errorCode,
                                    std::shared_ptr<qcril::interfaces::SipErrorInfo> data) {
  V1_0::SipErrorInfo errorDetails = {.errorCode = INT32_MAX};
  if (data) {
    V1_5::utils::convertSipErrorInfo(errorDetails, data);
  ...
  sp<IImsRadioResponse> respCb = getResponseCallbackV1_6();
  if (respCb != nullptr) {
    imsRadiolog("<", "cancelPendingUssdResponse: token = " + std::to_string(token) + " errorCode = "
        + std::to_string(errorCode) + " errorDetails = " + toString(errorDetails));
    Return<void> ret = respCb->cancelPendingUssdResponse(token, errorCode, errorDetails);
    if (!ret.isOk()) {
      QCRIL_LOG_ERROR("Unable to send response. Exception : %s", ret.description().c_str());
    }
  }
...
std::shared_ptr<QcRilRequestImsDialMessage> ImsRadioImpl_1_6::makeQcRilRequestImsDialMessage(
    int32_t token, const ::vendor::qti::hardware::radio::ims::V1_6::DialRequest& dialRequest) {
  std::shared_ptr<ImsRadioContext> ctx = getContext(token);
  // address is mandatory for normal call, but not mandatory for conference calls
  if (dialRequest.address.empty() &&
      !(dialRequest.hasIsConferenceUri && dialRequest.isConferenceUri)) {
  ...
  if (dialRequest.hasCallDetails) {
    msg->setCallType(V1_6::utils::convertCallType(dialRequest.callDetails.callType));
    ...
    if (dialRequest.callDetails.extras.size()) {
      std::string displayText = V1_0::utils::getExtra("DisplayText", dialRequest.callDetails.extras);
  ...
  if (dialRequest.redialInfo.callFailReason != V1_5::CallFailCause::CALL_FAIL_ERROR_UNSPECIFIED) {
    msg->setRetryCallFailReason(
        V1_5::utils::convertCallFailCause(dialRequest.redialInfo.callFailReason));
  }
  if (dialRequest.redialInfo.callFailRadioTech != V1_6::RadioTechType::RADIO_TECH_UNKNOWN) {
    msg->setRetryCallFailMode(
        V1_6::utils::convertRadioTechType(dialRequest.redialInfo.callFailRadioTech));
  }
  return msg;
}
...
Return<void> ImsRadioImpl_1_6::callComposerDial(int32_t token,
  ...
  QCRIL_LOG_FUNC_ENTRY();
  auto msg = makeQcRilRequestImsDialMessage(token, dialRequest);
  if (msg == nullptr) {
    QCRIL_LOG_ERROR("msg is nullptr");
...
Return<void> ImsRadioImpl_1_6::dial_1_6(
    int32_t token, const ::vendor::qti::hardware::radio::ims::V1_6::DialRequest& dialRequest) {
  imsRadiolog(
      ">", "dial_1_6: token = " + std::to_string(token) + " dialRequest: " + toString(dialRequest));
  QCRIL_LOG_FUNC_ENTRY();
  auto msg = makeQcRilRequestImsDialMessage(token, dialRequest);
  if (msg == nullptr) {
    QCRIL_LOG_ERROR("msg is nullptr");
...
Return<void> ImsRadioImpl_1_6::emergencyDial_1_6(
    int32_t token, const ::vendor::qti::hardware::radio::ims::V1_6::DialRequest& dialRequest,
    ::android::hardware::hidl_bitfield<
        ::vendor::qti::hardware::radio::ims::V1_5::EmergencyServiceCategory>
        categories,
  ...
  imsRadiolog(">", "emergencyDial_1_6: token = " + std::to_string(token) + " dialRequest = " +
                       toString(dialRequest) + " categories = " + std::to_string((int)categories) +
                       " route = " + std::to_string((int)route) + " hasKnownUserIntentEmergency = " +
