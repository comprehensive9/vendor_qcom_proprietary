...
// Initialize ImsRadioImplBase::mCallbackLock
qtimutex::QtiSharedMutex ImsRadioImplBase::mCallbackLock;
...
void ImsRadioImplBase::setUssdNotSupported() {
  if (property_set(QCRIL_USSD_IMS_RUNTIME_SUPPORT, "false") != E_SUCCESS) {
    QCRIL_LOG_ERROR("Failed to set property for %s as false",
                    "vendor.radio.ussd_support_at_runtime");
  }
}
...
sp<IImsRadioResponse> ImsRadioImpl::getResponseCallback() {
  std::shared_lock<qtimutex::QtiSharedMutex> lock(ImsRadioImplBase::mCallbackLock);
...
android::status_t ImsRadioImpl::registerService() {
