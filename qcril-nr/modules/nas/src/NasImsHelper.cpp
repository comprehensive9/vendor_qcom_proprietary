...
bool qcril_qmi_nas_handle_ims_wfc_status_change(const ImsWfcSettingsStatusInd::StatusInfo &wfcStatusInfo)
{
  ...
  if (!qcril_qmi_nas_get_apm_sim_not_pwdn_value())
  {
    auto state = qcril_get_current_radio_state();

    if ((state == RADIO_STATE_OFF) && qcril_qmi_nas_is_apm_enabled() &&
      (wifi_call != wfcStatusInfo.status))
    {
      std::shared_ptr<UimCardPowerReqMsg> uimPowerReq;

      if ((sImsWfcStatus.status == ImsWfcSettingsStatusInd::Status::WFC_ON) &&
        qcril_qmi_nas_get_card_state() == QCRIL_CARD_STATUS_DOWN)
      {
        uimPowerReq = std::make_shared<UimCardPowerReqMsg>(qmi_ril_get_sim_slot(),
          RIL_UIM_CARD_POWER_UP);
      }
      else if ((sImsWfcStatus.status == ImsWfcSettingsStatusInd::Status::WFC_OFF) &&
        qcril_qmi_nas_get_card_state() == QCRIL_CARD_STATUS_UP)
      {
        uimPowerReq = std::make_shared<UimCardPowerReqMsg>(qmi_ril_get_sim_slot(),
          RIL_UIM_CARD_POWER_DOWN);
      }

      if (uimPowerReq)
      {
        uimPowerReq->dispatch();
      }
    }
  }
