...
NasModule::NasModule() : AddPendingMessageList("NasModule") {
  ...
  mMessageHandler = {
    ...
    HANDLER(ClientConnectedMessage, NasModule::handleRilConnectedUnsolMessage),
...
#ifndef QMI_RIL_UTF
  std::vector<std::string> ring2{RilRequestSetInitialAttachApnRequestMessage::MESSAGE_NAME,
...
void NasModule::handleSetPreferredNetworkType(std::shared_ptr<RilRequestSetPrefNetworkTypeMessage> shared_msg) {
    ...
    if (mReady) {
      ...
      QCRIL_LOG_ERROR("NasModule is not ready, return RADIO_NOT_AVAILABLE");
      auto respData = std::make_shared<QcRilRequestMessageCallbackPayload>(
                RIL_E_RADIO_NOT_AVAILABLE, nullptr);
      shared_msg->sendResponse(shared_msg, Message::Callback::Status::SUCCESS, respData);
    }
}
...
void NasModule::handleRilConnectedUnsolMessage(std::shared_ptr<ClientConnectedMessage> msg) {
  Log::getInstance().d("[NasModule]: Handling msg = " + msg->dump());
