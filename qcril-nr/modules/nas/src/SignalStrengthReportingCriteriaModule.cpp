...
#ifdef RIL_VTS_ISSUES
        //VTS client will send a single RAT entry
        //check for NGRAN and return NOT_SUPPORTED error response.
        std::vector<SignalStrengthCriteriaEntry> &criteria = msg->getCriteria();
        if (criteria.size() == 1) {
          SignalStrengthCriteriaEntry &entry = criteria[0];
          QCRIL_LOG_DEBUG("RAN: %d ", entry.ran);
          if (entry.ran == NGRAN) {
            QCRIL_LOG_DEBUG("RAN: NGRAN is not supported ");
            rsp = std::make_shared<RIL_Errno>(RIL_E_REQUEST_NOT_SUPPORTED);
            msg->sendResponse(msg, status, rsp);
            return;
          } else if (entry.ran == UTRAN) {
            //in case of WCDMA, allow only rscp thresholds when sending
            //request to modem. modify the input struct to disable remaining
            //values
            entry.hysteresisMs = -1;
            entry.hysteresisDb = -1;
            msg->dump();
          }
        }
#endif
