...
sp<V2_5::IQtiRadioResponse> QtiRadioImpl::getResponseCallbackV2_5() {
    std::shared_lock<qtimutex::QtiSharedMutex> lock(mCallbackLock);
    return mResponseCbV2_5;
}

sp<V2_6::IQtiRadioResponse> QtiRadioImpl::getResponseCallbackV2_6() {
    std::shared_lock<qtimutex::QtiSharedMutex> lock(mCallbackLock);
    return mResponseCbV2_6;
}

sp<V2_7::IQtiRadioResponse> QtiRadioImpl::getResponseCallbackV2_7() {
    std::shared_lock<qtimutex::QtiSharedMutex> lock(mCallbackLock);
    return mResponseCbV2_7;
}

sp<V2_7::IQtiRadioIndication> QtiRadioImpl::getIndicationCallbackV2_7() {
    std::shared_lock<qtimutex::QtiSharedMutex> lock(mCallbackLock);
...
Return<void> QtiRadioImpl::getQtiRadioCapability(int32_t serial) {
    QCRIL_LOG_DEBUG("QtiRadioImpl::getQtiRadioCapability, serial=%d", serial);
    auto msg = std::make_shared<RilRequestGetEnhancedRadioCapabilityMessage>(getContext(serial));
    ...
    if(msg != nullptr) {
        ...
        if (respCb != nullptr) {
            ...
            if(resp != nullptr){
                ...
                payload = std::static_pointer_cast<qcril::interfaces::RilGetRadioAccessFamilyResult_t>(resp->data);
            }
            if(payload != nullptr) {
                ret = respCb->getQtiRadioCapabilityResponse(resp_info,
                        (hidl_bitfield<V2_6::RadioAccessFamily>)payload->radioAccessFamily);
                if (!ret.isOk()) {
                    QCRIL_LOG_ERROR("Unable to send response. Exception : %s", ret.description().c_str());
                }
            } else {
                ret = respCb->getQtiRadioCapabilityResponse(resp_info,
                        (hidl_bitfield<V2_6::RadioAccessFamily>)V2_6::RadioAccessFamily::UNKNOWN);
                if (!ret.isOk()) {
                    QCRIL_LOG_ERROR("Unable to send response. Exception : %s", ret.description().c_str());
                }
            }
        ...
        auto respCb = getResponseCallbackV2_6();
        if (respCb != nullptr) {
            Return<void> ret = respCb->getQtiRadioCapabilityResponse(respInfo,
                    (hidl_bitfield<V2_6::RadioAccessFamily>)V2_6::RadioAccessFamily::UNKNOWN);
            if (!ret.isOk()) {
                QCRIL_LOG_ERROR("Unable to send response. Exception : %s", ret.description().c_str());
            }
...
Return<void> QtiRadioImpl::setNrConfig(int32_t serial, const V2_5::NrConfig config) {
    QCRIL_LOG_DEBUG("QtiRadioImpl::setNrConfig, serial=%d", serial);

    auto msg = std::make_shared<RilRequestSetNrConfigMessage>(getContext(serial),
            convert_nr_config(config));
    if (msg != nullptr) {
        GenericCallback<QcRilRequestMessageCallbackPayload> cb(
            [this, serial](std::shared_ptr<Message> msg, Message::Callback::Status status,
                          std::shared_ptr<QcRilRequestMessageCallbackPayload> resp) -> void {
            ...
            if (respCb != nullptr) {
                RIL_Errno rilErr = (resp == nullptr) ? RIL_E_NO_MEMORY : resp->errorCode;
                Return<void> ret = respCb->setNrConfigResponse(serial,
                        static_cast<uint32_t>(rilErr), V2_5::Status::SUCCESS);
                if (!ret.isOk()) {
                    QCRIL_LOG_ERROR("Unable to send response. Exception : %s", ret.description().c_str());
                }
            }
        ...
        if (respCb != nullptr) {
            Return<void> ret = respCb->setNrConfigResponse(serial,
                static_cast<uint32_t>(RIL_E_NO_MEMORY), V2_5::Status::SUCCESS);
            if (!ret.isOk()) {
                QCRIL_LOG_ERROR("Unable to send response. Exception : %s", ret.description().c_str());
            }
        }
...
Return<void> QtiRadioImpl::queryNrConfig(int32_t serial) {
    QCRIL_LOG_DEBUG("QtiRadioImpl::queryNrConfig, serial=%d", serial);

    auto msg = std::make_shared<RilRequestQueryNrConfigMessage>(getContext(serial));
    if (msg != nullptr) {
        GenericCallback<QcRilRequestMessageCallbackPayload> cb(
            [this, serial](std::shared_ptr<Message> msg, Message::Callback::Status status,
                          std::shared_ptr<QcRilRequestMessageCallbackPayload> resp) -> void {
            ...
            if (respCb != nullptr) {
                ...
                if (resp != nullptr) {
                    rilErr = resp->errorCode;
                    payload = std::static_pointer_cast<qcril::interfaces::RilQueryNrConfigResult_t>(resp->data);
                } else {
                    rilErr = RIL_E_NO_MEMORY;
                }

                if (rilErr == RIL_E_SUCCESS && payload != nullptr) {
                    config = convert_nr_disable_mode(payload->mode);
                }

                Return<void> ret = respCb->onNrConfigResponse(serial,
                        static_cast<uint32_t>(rilErr), config);
                if (!ret.isOk()) {
                    QCRIL_LOG_ERROR("Unable to send response. Exception : %s", ret.description().c_str());
                }
            }
        ...
        if (respCb != nullptr) {
            Return<void> ret = respCb->onNrConfigResponse(serial,
                static_cast<uint32_t>(RIL_E_NO_MEMORY), V2_5::NrConfig::NR_CONFIG_INVALID);
            if (!ret.isOk()) {
                QCRIL_LOG_ERROR("Unable to send response. Exception : %s", ret.description().c_str());
            }
        }
...
void convertQosFilters(V2_7::QosFilter& dcQosFilter, const rildata::QosFilter_t& result) {
  ...
  dcQosFilter.protocol = (V2_7::QosProtocol)result.protocol;
  ...
  dcQosFilter.direction = (V2_7::QosFilterDirection )result.direction;
...
Return<void> QtiRadioImpl::getQosParameters(int32_t serial, int32_t cid) {
        ...
        GenericCallback<rildata::QosParamResult_t> cb(
            ...
            if(status != Message::Callback::Status::SUCCESS) {
                ...
                if (resp != nullptr) {
                    ...
                    error = RadioError::GENERIC_FAILURE;
                }
                RadioResponseInfo responseInfo = {RadioResponseType::SOLICITED,
                    serial, error};
                sp<V2_7::IQtiRadioResponse> respCb = getResponseCallbackV2_7();
                if(respCb != nullptr) {
                    Return<void> ret = respCb->getQosParametersResponse(responseInfo, result);
                    if (!ret.isOk()) {
                        QCRIL_LOG_ERROR("Unable to send response. Exception : %s",
                                ret.description().c_str());
                    }
...
void QtiRadioImpl::onQosDataChanged(rildata::QosParamResult_t &in) {
    V2_7::QosParametersResult result = {};
...
void QtiRadioImpl::registerService() {
  android::status_t ret = android::OK;
  std::string serviceName = "slot" + std::to_string(mInstanceId + 1);

  ret = V2_7::IQtiRadio::registerAsService(serviceName);
  if (ret == android::OK) {
    QCRIL_LOG_INFO("Registered V2_7::IQtiRadio");
    return;
  }

  ret = V2_6::IQtiRadio::registerAsService(serviceName);
  if (ret == android::OK) {
    QCRIL_LOG_INFO("Registered V2_6::IQtiRadio");
    return;
  }

  ret = V2_5::IQtiRadio::registerAsService(serviceName);
  if (ret == android::OK) {
    QCRIL_LOG_INFO("Registered V2_5::IQtiRadio");
    return;
  }
  ret = V2_4::IQtiRadio::registerAsService(serviceName);
  if (ret == android::OK) {
    QCRIL_LOG_INFO("Registered V2_4::IQtiRadio");
    return;
  }
  ret = V2_3::IQtiRadio::registerAsService(serviceName);
  if (ret == android::OK) {
    QCRIL_LOG_INFO("Registered V2_3::IQtiRadio");
    return;
  }
  ret = V2_2::IQtiRadio::registerAsService(serviceName);
  if (ret == android::OK) {
    QCRIL_LOG_INFO("Registered V2_2::IQtiRadio");
    return;
  }
  ret = V2_1::IQtiRadio::registerAsService(serviceName);
  if (ret == android::OK) {
    QCRIL_LOG_INFO("Registered V2_1::IQtiRadio");
    return;
  }
  ret = V2_0::IQtiRadio::registerAsService(serviceName);
  if (ret == android::OK) {
    QCRIL_LOG_INFO("Registered V2_0::IQtiRadio");
    return;
  }
  ret = V1_0::IQtiRadio::registerAsService(serviceName);
  if (ret == android::OK) {
    QCRIL_LOG_INFO("Registered V1_0::IQtiRadio");
    return;
  }
...
void qcril_qmi_qti_radio_service_init(int instanceId) {
  ...
  if (qtiRadioService) {
    qtiRadioService->registerService();
  }
}
