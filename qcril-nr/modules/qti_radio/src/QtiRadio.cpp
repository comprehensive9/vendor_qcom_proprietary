...
QtiRadio::QtiRadio() {
  ...
  mMessageHandler = {
    ...
    HANDLER(rildata::QosDataChangedIndMessage, QtiRadio::handleQosDataChangedIndMessage),
#ifndef QMI_RIL_UTF
...
void QtiRadio::handleQosDataChangedIndMessage(std::shared_ptr<rildata::QosDataChangedIndMessage> msg) {
    if (msg == nullptr) {
        return;
    }
    Log::getInstance().d("[" + mName + "]: Handling msg = " + msg->dump());
    using namespace vendor::qti::hardware::radio::qtiradio::V1_0::implementation;
    auto res = msg->getQosResult();
    onQosDataChanged(res);
}
