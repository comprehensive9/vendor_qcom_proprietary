...
)
{
  ...
  uint8_t                  rx_buffer[CCID_UIM_BULKOUT_BUFFER_SIZE];
...
)
{
  /* Retrieve global UIM struct pointer used in this thread */
  ccid_uim_struct_type   * ccid_uim_ptr = (ccid_uim_struct_type *)in_param;
  uint8_t                  rx_buffer[CCID_UIM_CTRL_BUFFER_SIZE];
...
{
  uint8_t   * current_ptr        = buffer_ptr;
  uint8_t   * end_ptr            = current_ptr + buffer_len;
  uint8_t   * subsystem_ptr      = NULL;
  uint8_t   * switch_name_ptr    = NULL;
  uint8_t   * switch_state_ptr   = NULL;
...
int main (int argc, char **argv)
{
  ...
  while(1)
  {
    int     usb_event_sd   = -1;
    int     bytes_read     = 0;
    uint8_t rx_buffer[CCID_UIM_USB_EVT_BUFFER_SIZE];
