...
void NasModemEndPoint::getRfBandInfo(PhysChanInfo &phyChanInfo) {
    ...
    if (!qmiError) {
        ...
        phyChanInfo.band_info_list_ext_valid = response.rf_band_info_list_ext_valid;
        if (phyChanInfo.band_info_list_ext_valid) {
            phyChanInfo.band_info_list_ext_len = response.rf_band_info_list_ext_len;
            for (unsigned int i = 0;
                    i < phyChanInfo.band_info_list_ext_len
                            && i < sizeof(phyChanInfo.band_info_list_ext) / sizeof(phyChanInfo.band_info_list_ext[0]);
                    i++) {
                phyChanInfo.band_info_list_ext[i] = response.rf_band_info_list_ext[i];
            }
        }
...
void NasModemEndPoint::getPhysicalCellId(PhysChanInfo &phyChanInfo) {
    ...
    if (!qmiError) {
        if (response.lte_intra_valid) {
            ...
            phyChanInfo.physical_cell_id_valid = true;
            phyChanInfo.physical_cell_id = response.umts_info.psc;
