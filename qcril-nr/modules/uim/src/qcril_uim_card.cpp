    ...
    case QMI_UIM_PERSO_STATE_PERM_BLOCKED:
      ...
      if (ret == 0)
      {
        ...
        if(perso_convert_status)
        {
          ...
          for(i = 0; i < rsp_data.rsp_data.get_configuration_rsp.perso_status_len && i < QMI_UIM_MAX_PERSO_FEATURES; i++)
          {
...
)
{
  ...
  if (slot == 0)
  {
    for (i = 0; i < status_change_ind_ptr->num_slots && i < QMI_UIM_MAX_CARD_COUNT; i++)
    {
          ...
          status_change_ind_ptr->card[i].card_state != QMI_UIM_CARD_STATE_UNKNOWN)
      {
        ...
        if (card_state != NULL)
        {
          Dispatcher::getInstance().dispatchSync(card_state);
        }
      }
    }
  }
  ...
  if (status_changed)
  {
         ...
         allow_indication )
    {
      ...
      if (sim_state != NULL)
      {
        Dispatcher::getInstance().dispatchSync(sim_state);
        /* Dispatch StatusChangeInd to clients synchronosuly to avoid any Dispatcher queue
           slow processing issues. Shall convert other broadcast() calls in this function
           to sync APIs so other clients also do not get out of sequence indications */
      }
    }
  }
  ...
  {
    Dispatcher::getInstance().dispatchSync(card_st_ch_ind_ptr);
  }
