...
)
{
  ...
  if (qcril_uim_sap_sim_state == RIL_UIM_SAP_CARD_STATE_REMOVED)
  {
    result = RIL_UIM_SAP_CARD_REMOVED;
    goto send_response;
  }
  else if(qcril_uim_sap_sim_state == RIL_UIM_SAP_CARD_STATE_NOT_ACCESSSIBLE)
  {
    result = RIL_UIM_SAP_CARD_NOT_ACCESSSIBLE;
    goto send_response;
  }
  else {
     QCRIL_LOG_INFO("Other cases process the request irrespective of card state");
  }
