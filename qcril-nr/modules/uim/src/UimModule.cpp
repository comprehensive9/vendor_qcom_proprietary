...
/* const uint8_t HLOS_PROV_PREF_AUTO   = 1; */

#define QCRIL_UIM_TOKEN_MASK     (1<<16)
...
UimModule::UimModule():uim_req_list("UimModule")
{
  ...
  mMessageHandler = {
       ...
       HANDLER(UimDeepSleepNotifyMsg, UimModule::handleNotifyDeepSleepMsg),
                    };
} /* UimModule::UimModule */
...
void UimModule::UimProcessQmiCommandCallback(std::shared_ptr<UimQmiUimResponseMsg> resp_msg_ptr)
{
  ...
  if (resp_msg_ptr != NULL)
  {
    ...
    QCRIL_LOG_INFO("Token value at response is %d", user_data.token);
  ...
  if (ind_ptr != NULL)
  {
    switch(ind_ptr->get_ind_id())
    {
      case QMI_UIM_SRVC_UP_IND_MSG:
        ...
        QCRIL_LOG_INFO("Recvd QMI_UIM_SRVC_UP_IND_MSG qcril_uim_hlos_prov_pref_auto - %d",
                        qcril_uim_hlos_prov_pref_auto);

        if( !qcril_uim_hlos_prov_pref_auto)
        {
...
)
{
  ...
  /* Masking User_data pointer with a constant  Mask value and original Token values
   * This ensure user_data pointer is always non null irrespective of token value
   */
  user_data.user_data = (const void *)(uintptr_t)(QCRIL_UIM_TOKEN_MASK | token.first);

  QCRIL_LOG_INFO( "Token value is  :%d", user_data.token);
...
/*=========================================================================
  ...
  Function:  UimModule::handleNotifyDeepSleepMsg

===========================================================================*/
void UimModule::handleNotifyDeepSleepMsg(std::shared_ptr<UimDeepSleepNotifyMsg> msgPtr)
{
  QCRIL_LOG_INFO( "%s\n", __FUNCTION__);
  ...
  if (mReady == false)
  {
    ...
    msgPtr->sendResponse(msgPtr, Message::Callback::Status::SUCCESS, rsp_ptr);
    return;
  }

  qcril_uim_handle_deep_sleep_notification(msgPtr);
} /* handleUimCardPowerReqMsg */
