                        ...
                        std::shared_ptr<RIL_UIM_SlotsStatusInfo> responseDataPtr) -> void
    {
      ...
      if(res_cb != nullptr) {
          ...
          if (!ret.isOk())
          {
            ...
            QCRIL_LOG_ERROR("RadioConfigImpl ResponseCb is null");
  ...
  if (msg)
  {
                        ...
                        std::shared_ptr<RIL_UIM_Errno> responseDataPtr) -> void
    {
      ...
      if(res_cb != nullptr) {
          ...
          QCRIL_LOG_ERROR("RadioConfigImpl ResponseCb null");
      }
    }));
    msg->setCallback(&cb);
    msg->dispatch();
  ...
  else
  {
    ...
    if (!ret.isOk())
    {
      QCRIL_LOG_ERROR("Unable to send response. Exception : %s", ret.description().c_str());
        ...
        QCRIL_LOG_ERROR("RadioConfigImpl response callback is null");
     }
  }
