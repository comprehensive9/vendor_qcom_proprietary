...
extern "C" {
  ...
  void sns_init_remote_handles(sns_remote_handles remote_handles);
}
      ...
      if(attr_event.attributes(i).attr_id() == SNS_STD_SENSOR_ATTRID_ADDITIONAL_LOW_LATENCY_RATES) {
        ...
        for (int j = 0; j< attr_value.values_size(); j++) {
          if (attr_value.values(j).has_flt()) {
            ALOGE("ll_rate %f", attr_value.values(j).flt());
        ...
        if(attr_event.attributes(i).attr_id() == SNS_STD_SENSOR_ATTRID_RATES) {
          ...
          for (int k = 0; k< attr_value.values_size(); k++) {
            if (attr_value.values(k).has_flt()) {
                ALOGE("rate %f", attr_value.values(k).flt());
    ...
    (JNIEnv *env, jobject obj, jint buffer_size)
{
    ...
    if (mutex_counter == 0) {
        if (0 != sns_rpc_get_handle(&remote_handles)) {
            LOGE("sns_rpc_get_handle failed");
            return -1;
        }
        LOGI("fastrpc is set-up");
   }
    mutex_counter++;
...
#endif
    output_buf = (uint8*)remote_handles.mem_alloc(heapid, RPCMEM_DEFAULT_FLAGS, buffer_size);
    if (NULL == output_buf) {
    ...
    (JNIEnv *env, jobject obj, jlongArray suids, jint sample_period_us, jint flags, jint stream_handle, jint sensor_handle)
{
    ...
    if (sample_period_us <= 0) {
        ...
        LOGE("%s: invalid sample_period_us", __FUNCTION__);
        input_exception = env->FindClass( "java/lang/IllegalArgumentException");
        if (input_exception != NULL) {
