...
static const auto RESPONSE_TIME_OUT_MS = 500;
...
void ssc_sensor::activate()
{
    ...
    if (!is_active()) {
        ...
        // update client id of sensors diag
        _diag.update_client_id();
...
void ssc_sensor::send_sync_sensor_request(string& pb_req_msg_encoded)
{
    using namespace std::chrono;
    ...
    _notify_resp = false;
    if (!is_active()) {
        sns_loge("ssc connection is reset");
        return;
    }
    ...
    if (_resp_received) {
        ...
        bool cv_ret = _resp_cv.wait_for(lk, std::chrono::milliseconds(RESPONSE_TIME_OUT_MS), [this]{ return _resp_received; });
        if (cv_ret) {
            ...
            sns_loge("response doesn't come within %u", RESPONSE_TIME_OUT_MS);
...
void ssc_sensor::send_sensor_config_request()
{
    ...
    if (_is_sync_request_enabled) {
        ...
        sns_logv("send async config request");
        if (is_active()) {
            _ssc_conn->send_request(pb_req_msg_encoded, false);
        } else {
            sns_loge("ssc connection is reset");
...
void ssc_sensor::flush()
{
    ...
    if (_is_sync_request_enabled) {
        ...
        sns_logv("send async flush request");
        _ssc_conn->send_request(pb_req_msg_encoded, false);
    }
}
...
{
    sns_logi("resp_value = %u", resp_value);
    if (_is_sync_request_enabled) {
        std::unique_lock<std::recursive_mutex> lk(_resp_mutex);
        _resp_received = true;
