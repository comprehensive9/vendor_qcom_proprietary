...
const char SENSORS_HAL_PROP_SSC_FIRMWARE_SUPPORT_PROPER_READING[] ="persist.vendor.sensors.firmware_property_read_support";
const char SENSORS_HAL_PROP_SSC_ALLOW_NON_DEFAULT_SENSOR_DISCOVERY[] = "persist.vendor.sensors.allow_non_default_discovery";
...
bool sensor_factory::check_property_reading_status(){
    char property[PROPERTY_VALUE_MAX];
    property_get(SENSORS_HAL_PROP_SSC_FIRMWARE_SUPPORT_PROPER_READING, property, "true");
    sns_logd("firmware_property_read_support: %s",property);
    if (!strncmp("false", property,5)) {
        sns_logi("firmware_property_read_support : %s",property);
        return false;
    }
    return true;
}

bool sensor_factory::non_default_dual_sensor_discovery_allowed(){
    char property[PROPERTY_VALUE_MAX];
    property_get(SENSORS_HAL_PROP_SSC_ALLOW_NON_DEFAULT_SENSOR_DISCOVERY, property, "false");
    sns_logd("allow_non_default_discovery: %s",property);
...
sensor_factory::sensor_factory()
{
    ...
    if (!(_settings & DISABLE_SENSORS_FLAG)) {
        //wait for registry sensor up
        if(check_property_reading_status() == false) {
...
{
    using namespace std::chrono;
    bool non_default_dual_sensor_discovery = non_default_dual_sensor_discovery_allowed();
    sns_logi("non_default_sensor_discovery is %s",
        non_default_dual_sensor_discovery ? "enabled":"disabled");
    ...
    for (const auto& dt : datatypes())  {
        if(non_default_dual_sensor_discovery == true) {
