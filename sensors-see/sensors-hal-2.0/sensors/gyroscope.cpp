    ...
    _gyro_temp_wakeup_type(wakeup)
{
    ...
    /* determin whether we use new unit or old unit */
    float max_range = get_sensor_info().maxRange;
    float resolution = get_sensor_info().resolution;

    bool use_new_units = max_range >= 100? false : true;

    if (use_new_units) {
        sns_logi("gyro: use new units: max_range = %f, resolution = %f", max_range, resolution);
    } else {
        max_range = max_range * RAD_PER_DEG;
        resolution = resolution * RAD_PER_DEG;
        sns_logi("gyro: use old units: max_range = %f, resolution = %f", max_range, resolution);
    }
...
void gyroscope::activate()
{
    ...
    if (!is_active()) {
        ...
        if(_gyro_temp_available) {
            ...
            _gyrotemp->register_event_callback(_event_cb);
            _gyrotemp->enable_sync_request(_is_sync_request_enabled);
