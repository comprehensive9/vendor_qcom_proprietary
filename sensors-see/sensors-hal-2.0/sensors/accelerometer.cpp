    ...
    _cal_type(cal_type)
{
    ...
    bool use_new_units = max_range >= 18? true : false;

    if (use_new_units) {
        sns_logi("accel: use new units: max_range = %f, resolution = %f", max_range, resolution);
    } else {
        max_range = max_range * ONE_G;
        resolution = resolution * ONE_G / 1000.0;
        sns_logi("accel: use old units: max_range = %f, resolution = %f", max_range, resolution);
    }
