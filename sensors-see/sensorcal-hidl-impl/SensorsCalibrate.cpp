...
std::string SensorsCalibrate::get_datatype(int32_t sensor_type, bool * primary) {
    *primary = true;
    ...
    case SENSOR_TYPE_HEART_BEAT:
         ...
         break;
    case QTI_DUAL_SENSOR_TYPE_BASE + SENSOR_TYPE_ACCELEROMETER:
         data_type = "accel";
         *primary = false;
         break;
    case QTI_DUAL_SENSOR_TYPE_BASE + SENSOR_TYPE_GYROSCOPE:
         data_type = "gyro";
         *primary = false;
         break;
    default:
...
Return<void> SensorsCalibrate::SensorsCal(int32_t sensor_type, int8_t test_type,
    ...
    bool primary = false;
    hidl_string data_type_tmp = get_datatype(sensor_type, &primary);
    if (!data_type_tmp.empty()) {
           str_result = SensorCalModule->methods->calibrate(data_type_tmp, test_type, primary);
           _hidl_cb(str_result);
        }
        else {
