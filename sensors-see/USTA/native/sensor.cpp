...
void Sensor::sensor_stream_resp_cb(uint32_t resp_value)
{
  ...
  if(sensors_diag_log_obj != NULL){
    sensors_diag_log_obj->log_response_msg(
        ...
        if("10" == current_request_msg_id){
          sensors_diag_log_obj->update_client_id();
        }
  }
    ...
    size_t encoded_string_size)
{
  ...
  {
    ERROR_LOG(log_instance, "(suid_low_field || suid_high_field) == NULL");
    return;
  }
  suid_low = sub_message->GetReflection()->GetUInt64(*sub_message,suid_low_field);
  suid_high = sub_message->GetReflection()->GetUInt64(*sub_message,suid_high_field);
...
//    DEBUG_LOG(log_instance , " unexpected number " + to_string(num_events) + " of events for attributes");
  ...
  {
    ERROR_LOG(log_instance, "msg_id_field is NULL");
    return;
  }
  ...
  {
    ERROR_LOG(log_instance, "payload_field is NULL");
    return;
  }
    ...
    string& in_logfile_name , string &usta_mode)
{
  ...
  current_request_msg_id = client_req_msg.msgid;
    ...
    send_req_nested_msg_info susp_config_msg;
    {
      ...
      send_req_field_info client_field;
      {
        ...
        if(!std_fields_data.client_type.empty()){
          ...
          proc_type = std_fields_data.client_type;
        }
      }
      send_req_field_info wakeup_field;
      {
        ...
        if(!std_fields_data.wakeup_type.empty()){
          ...
          wake_type = std_fields_data.wakeup_type;
        }
      }
...
void Sensor::send_disable_requst()
{
  ...
  send_req_field_info susp_config_field;
  {
    ...
    send_req_nested_msg_info susp_config_msg;
    {
      ...
      {
        client_field.name = "client_proc_type";
        client_field.value.push_back(proc_type);
      }
      send_req_field_info wakeup_field;
      {
        wakeup_field.name = "delivery_type";
        wakeup_field.value.push_back(wake_type);
      }
      susp_config_msg.fields.push_back(client_field);
      susp_config_msg.fields.push_back(wakeup_field);
    }
...
{
  usta_rc rc= USTA_RC_SUCCESS;
  INFO_LOG(log_instance, "get_payload_string Start @ sensor ");

  USTA_LOGI("sensor_payload_field.name %s " , sensor_payload_field.msg_name.c_str());
  USTA_LOGI("sensor_payload_field.msgid %s " , sensor_payload_field.msgid.c_str());
  USTA_LOGI("sensor_payload_field.fields size %d " , (int)sensor_payload_field.fields.size());

  USTA_LOGI("sensor_payload_field data verified till now.. Actual encoding starts from here ");
  string payload;
  if(USTA_RC_SUCCESS !=
      (rc=sensor_parser->formulate_request_msg(sensor_payload_field,
          payload)))
  {
    ERROR_LOG(log_instance , "sensor_payload_field request failed");
  }

  USTA_LOGI("get_payload_string - payload %s and size is %ld" , payload.c_str(), payload.size());

  encoded_data.assign(payload);
  USTA_LOGI("get_payload_string - encoded_data %s and size is %ld" , encoded_data.c_str(), encoded_data.size());

  INFO_LOG(log_instance, "get_payload_string End @ sensor ");
}


bool Sensor::is_direct_channel_supported_sensor()
{
  if(!sensor_attribute_obj)
  {
    ERROR_LOG(log_instance , " No attribute class created");
  ...
  {
    string data_type = sensor_attribute_obj->get_dataType();
    if((!data_type.compare("accel")) ||
        (!data_type.compare("gyro")) ||
        (!data_type.compare("mag")))
    {
      DEBUG_LOG(log_instance , " direct_channel_supported_sensor is " + data_type);
      return true;
    }
    else
    {
...
#endif
void Sensor::get_direct_channel_attributes_info(direct_channel_attributes_info &attributes_info) {
  string data_type = sensor_attribute_obj->get_dataType();
  if(!data_type.compare("direct_channel")) {
    DEBUG_LOG(log_instance , " Direct Channel Sensor It is ");
    if(sensor_attribute_obj != NULL) {

      if(sensor_attribute_obj->is_available(sns_std_sensor_attrid_drc_max_sampling_rate))
        attributes_info.max_sampling_rate = sensor_attribute_obj->get_drc_max_sample_rate();

      if(sensor_attribute_obj->is_available(sns_std_sensor_attrid_drc_max_report_rate))
          attributes_info.max_report_rate = sensor_attribute_obj->get_drc_max_report_rate();
