...
int64_t ssc_calibrate::get_rigid_body(sensor_uid suid)
{
    auto ints = get_ints(suid, SNS_STD_SENSOR_ATTRID_RIGID_BODY);
    if (ints.size() == 0) {
        sns_loge("rigid_body attribute not available");
        return -1;
    }
    return ints[0];
}
...
std::vector<sensor_uid> ssc_calibrate::get_all_available_suids(std::string data_type, uint8_t test_type)
{
    ...
    if((data_type == "pressure" || data_type == "humidity" || data_type == "mag")
        ...
        if(sensor_uids.size() == 2)
        {
            ...
            if (!get_rigid_body(*suid_1) == primary_sensor || get_stream_type(*suid_1) == primary_sensor)
...
calibrate(const string& datatype, const uint8_t &test_type, const bool primary)
{
    std::string test;
    ...
    lookup.request_suid(datatype, primary);
