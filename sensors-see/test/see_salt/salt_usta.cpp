...
static SensorCxt* usta_get_instance(see_salt *psalt)
{
   ...
   if ( salt_instance_num > (int)sns_cxts.size()) {
      SensorCxt *sns_cxt_instance = SensorCxt::getInstance((sensor_stream_path)psalt->get_stream_path());
...
/**
 * @brief call libUSTANative.so SensorCxt::direct_channel_open(...)
 * @param channel info
...
salt_rc usta_create_dc_channel(see_salt *psalt, see_create_dc_channel_info &dc_channel_info, unsigned long &channel_handle)
{
    ...
    create_info.shared_memory_ptr = dc_channel_info.shared_memory_ptr;
    create_info.shared_memory_size = dc_channel_info.shared_memory_size;
    create_info.channel_type_value = (sensor_channel_type)dc_channel_info.channel_type_value;
    create_info.client_type_value = (sensor_client_type)dc_channel_info.client_type_value;
    SensorCxt *sns_cxt_instance = usta_get_instance( psalt);
    usta_rc rc = sns_cxt_instance->direct_channel_open( create_info, channel_handle);
    if ( rc) {
       cout << "usta_create_dc_channel rc " << to_string(rc) << " - ";
       cout <<  usta_rc_to_string(rc) << endl << endl;
    }
    return (salt_rc)rc;
}
...
/**
 * @brief call libUSTANative.so
 *        SensorCxt::direct_channel_close(...)
 * @param channel handle
 * @return salt_rc
 */
...
{
    SensorCxt *sns_cxt_instance = usta_get_instance( psalt);
    usta_rc rc = sns_cxt_instance->direct_channel_close( channel_handle);
    if ( rc) {
       cout << "usta_close_dc_channel rc " << to_string(rc) << " - ";
       cout <<  usta_rc_to_string(rc) << endl << endl;
    }
    return (salt_rc)rc;
}
...
{
    usta_rc rc;
    if (is_delete_request_info)
    {
        direct_channel_delete_req_info delete_req_info;
        delete_req_info.sensor_handle = req_info.sensor_handle;
        delete_req_info.is_calibrated = req_info.is_calibrated;
        delete_req_info.is_fixed_rate = req_info.is_fixed_rate;

        SensorCxt *sns_cxt_instance = usta_get_instance( psalt);
        rc = sns_cxt_instance->direct_channel_delete_request(channel_handle, delete_req_info);
    }
    else
    {
        direct_channel_std_req_info std_req_info;
        std_req_info.sensor_handle = req_info.sensor_handle;
        std_req_info.is_calibrated = req_info.is_calibrated;
        std_req_info.is_fixed_rate = req_info.is_fixed_rate;
        std_req_info.sensor_type = req_info.sensor_type;

        if(!req_info.batch_period.empty()){
            std_req_info.batch_period = req_info.batch_period;
        }

        send_req_msg_info sensor_payload_field;
        send_req_field_info field_info;
        field_info.name = "sample_rate";
        field_info.value.push_back(to_string(req_info.sample_rate));
        sensor_payload_field.msg_name = "sns_std_sensor_config";
        sensor_payload_field.msgid = "513";
        sensor_payload_field.fields.push_back( field_info);

        SensorCxt *sns_cxt_instance = usta_get_instance( psalt);
        rc = sns_cxt_instance->direct_channel_send_request(channel_handle, std_req_info, sensor_payload_field);
    }

    if ( rc) {
       cout << "usta_send_request_dc_channel rc " << to_string(rc) << " - ";
       cout <<  usta_rc_to_string(rc) << endl << endl;
    }
    return (salt_rc)rc;
}
...
/**
 * @brief call libUSTANative.so
 *        SensorCxt::direct_channel_update_offset_ts(...)
 * @param channel handle
 * @param offset
 * @return salt_rc
 */
salt_rc usta_update_offset_ts_dc_channel(see_salt *psalt, unsigned long &channel_handle, int64_t offset)
{
    usta_rc rc;
    SensorCxt *sns_cxt_instance = usta_get_instance( psalt);
    rc = sns_cxt_instance->direct_channel_update_offset_ts(channel_handle, offset);

    if ( rc) {
       cout << "usta_update_offset_ts_dc_channel rc " << to_string(rc) << " - ";
       cout <<  usta_rc_to_string(rc) << endl << endl;
    }
    return (salt_rc)rc;
}
...
/**
 * @brief call libUSTANative.so
 *        SensorCxt::get_direct_channel_atttributes_info(...)
 * @param direct channel attributes
 */
void usta_get_direct_channel_attributes(see_salt *psalt, direct_channel_attributes &dc_attributes)
{
    direct_channel_attributes_info dc_attributes_info;
    SensorCxt *sns_cxt_instance = usta_get_instance( psalt);
    sns_cxt_instance->get_direct_channel_atttributes_info(dc_attributes_info);
    dc_attributes.max_sampling_rate = dc_attributes_info.max_sampling_rate;
    dc_attributes.max_report_rate = dc_attributes_info.max_report_rate;
}
