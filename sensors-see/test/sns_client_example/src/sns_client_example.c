...
get_encoded_suid_req(char const *data_type, void **encoded_req)
{
  ...
  *encoded_req = NULL;
  ...
  /* In order to query all sensors available for the given data type such as
     dual sensors use-cases, default_only should be set to false
     if default_only is true, only primary sensor will be published for the
     given data type */
...
/**
 * Get the attribute ID of the encoded event residing on stream.
 *
 * @return Attribute ID
 */
static uint32_t
get_attr_id(pb_istream_t *stream)
{
...
decode_attr(pb_istream_t *stream, const pb_field_t *field, void **arg)
{
  ...
  if(SNS_STD_SENSOR_ATTRID_RIGID_BODY == attr_id) {
    ...
    /* if multiple sensors are available for the given data type
       we can decide which one we want to start stream based on
       RIGID_BODY_TYPE information in the attributes
       if you want to start stream only for specific sensor,
       check the RIGID_BODY type and choose the right one */

    if (type == SNS_STD_SENSOR_RIGID_BODY_TYPE_DISPLAY ||
      type == SNS_STD_SENSOR_RIGID_BODY_TYPE_KEYBOARD) {
      __android_log_print(ANDROID_LOG_ERROR, "SNS_CLIENT_EXAMPLE",
        "start sensor for rigid_body type: %" PRIu64, type);
      send_enable_req(qmi_sensors_handle, &accel_suid);
    } else {
      __android_log_print(ANDROID_LOG_ERROR, "SNS_CLIENT_EXAMPLE",
        "DON'T start sensor for rigid_body type: %" PRIu64, type);
    }
  } else {
    if(!pb_decode(stream, sns_std_attr_fields, &attribute)) {
      __android_log_print(ANDROID_LOG_ERROR, "SNS_CLIENT_EXAMPLE",
