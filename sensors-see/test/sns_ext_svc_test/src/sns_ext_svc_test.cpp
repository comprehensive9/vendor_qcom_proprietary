...
// For compatability with older versions of ext_svc sensor
#define SNS_EXT_SVC_LOCATION 0
...
static uint32_t svc_id = SNS_EXT_SVC_LOCATION;
...
print_usage()
{
   ...
   cout << "[-duration=<seconds> -times=<number> -legacy=<1/0>] [-help]";
   cout << endl;
}
   ...
   for (int i = 1; i < argc; i++) {
       ...
       if (get_key_value(argv[i], key, value)) {
          if (key == "-duration") {
             ...
             legacy_mode = (value == "1" ? true : false);
          }
          else {
             valid_args = false;
...
main(int argc, char *argv[])
{
   ...
   if (legacy_mode == false) {
      qmi_idl_get_service_id(loc_get_service_object_v02(), &svc_id);
   }

   sns_logv("legacy_mode = %d, svc_id = %d", legacy_mode, svc_id);
