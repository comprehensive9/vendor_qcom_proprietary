...
static void parse_da_test_log(sns_da_test_log *da_test_log)
{
   ...
   if (da_test_log->has_recvd_phy_config_sample_rate()) {
...
void da_test_runner::handle_event(const uint8_t *data, size_t size)
{
    ...
    cout << unique_id << " handle_event " << endl;
    ...
    for ( int i = 0; i < event_msg.events_size(); i++) {
       ...
       if ( msg_id == SNS_DA_TEST_MSGID_SNS_DA_TEST_EVENT) {
          ...
          cout << unique_id << " event_cb attribute event for da_test" << endl;
          ...
          cout << unique_id << " handle event ignored msg_id: " << msg_id << endl;
       }
    }
}
...
void da_test_runner::send_memory_log_req(uint64_t cookie, sensor_uid suid)
{
    ...
    cout << unique_id << " enter send_memory_log_req cookie: " << to_string(cookie) << endl;
    ...
    cout << unique_id << " exit send_memory_log_req" << endl;
}
                            ...
                            int32_t msg_id)
{
    ...
    cout << unique_id << " enter da_test runner" << endl;
...
void crash_device()
{
    ...
    usleep( 4000000);   // 4 seconds: microseconds
}
...
void app_run(char *cmd_line_args, double duration)
{
    ...
    if (found_target) {
       cout << unique_id << " using da_test name="  << da_test_name
...
static void write_result_description( void)
{
   ...
   if ( fd >= 0) {
      if ( flock(fd, LOCK_EX) == 0) {
         ...
         cout << unique_id << " FAIL flock failed." << endl;
      }
      close(fd);
   }
   else {
      cout << unique_id <<  " FAIL open failed for " << results_fn << endl;
   }
}
...
{
   int identifier = 1;
   char *fn = (char *)"/data/drva_test_instance";
   char buffer[12];

   int fd = open( fn, O_RDWR | O_CREAT, 0666);
   if ( fd >= 0) {
      if ( flock(fd, LOCK_EX) == 0) {
         ssize_t len = read( fd, (void *)buffer, sizeof(buffer));
         if ( len > 0 ) {
...
int main(int argc, char* argv[])
{
    ...
    *args = '\0';
    ...
    my_exe_name = argv[0];
    cout << unique_id << " " << my_exe_name << " version 1.13" << endl;
