...
static void hal_pollFMQ( sensors_event_t buffer[],size_t maxNumEventsToRead, struct sensor_t const* sensors_list){
    ...
    while(1){
        ...
        for (int i = 0; i < eventsRead; i++) {
            ...
            find_sensor_by_type(buffer[i].type,'w', sensors_list, &ret_sensor );
...
int main( int argc, char * const argv[] )
{
  ...
  struct sensor_t const *list_p_temp = sensors_list;
  for(int i=0;i<sensor_count;i++){
    result  = sensors->activate(list_p_temp->handle, false );
    list_p_temp++;
  }

  std::thread t1(hal_pollFMQ, buffer, MAX_RECEIVE_BUFFER_EVENT_COUNT, sensors_list);
  if (list_sensors)
  {
