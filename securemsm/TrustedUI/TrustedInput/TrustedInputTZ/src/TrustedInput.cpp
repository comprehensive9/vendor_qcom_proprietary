...
std::mutex TrustedInput::sLock;
std::shared_ptr<TrustedInput> TrustedInput::sInstance = nullptr;
...
// singleton
std::shared_ptr<TrustedInput> TrustedInput::getInstance()
{
    ...
    sInstance = std::shared_ptr<TrustedInput>(new TrustedInput);
                                ...
                                init_cb _hidl_cb)
{
    ...
    LOGE("%s::%d ITrustedInput Version 1.0 init function is deprecated in TUI", __func__, __LINE__);
...
Return<void> TrustedInput::init2(const sp<ITrustedInputCallback> &cb,
