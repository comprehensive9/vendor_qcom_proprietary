...
typedef struct {
    ...
    SecureTouch_CompletionStatus_t status; /**< whether the user has pressed
                                              enter and completed the entry */
    uint32_t num_chars[MAX_INPUT_FIELDS];  /**< number of (possibly multibytes)
...
typedef struct {
    ...
    SecureTouch_CompletionStatus_t status; /**< will never be TL_SECURETOUCH_COMPLETION_STATUS_INPROGRESS
                   */
    uint32_t length[MAX_INPUT_FIELDS]; /**< length in bytes of the content of
                                          the input field */
