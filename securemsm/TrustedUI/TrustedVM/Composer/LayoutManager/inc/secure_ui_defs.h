...
/* We assume the following max values to avoid recurrent dynamic allocations of
 * small buffers */

/** @addtogroup qsee_core_layer
  @{ */
/** @name Maximum Values
  Qualcomm's Secure UI layout is subject to the limitations in the following
  constants:
  @{ */
...
typedef enum {
    ...
    SEC_UI_QSEE_SECURE_BUFFER_ERROR = -8, /**< Error when mapping the display
                                             buffer in the TrustZone software.
                                             */
    SEC_UI_NOT_SUPPORTED = -9,            /**< Not supported. */
    SEC_UI_INVALID_INPUT = -10, /**< Invalid input given to an API function; for example, a NULL
                pointer. */
    ...
    SEC_UI_NO_MEMORY = -17, /**< HLOS did not relinquish a buffer memory. This
                              error might
                              indicate out-of-memory issues in the multimedia
                              memory allocator. */
    ...
    SEC_UI_MORE_BUFFERS_AVAILABLE = -19, /**< More buffers than this QSEE
                                           application owns are tagged for
                                           secure display usage. This error
    ...
    SEC_UI_TOO_MANY_BUFFERS = -24, /**< HLOS is relinquishing more buffers than the QSEE service can
               handle. This error might indicate that different incompatible
               software
    ...
    SEC_UI_CALL_ACTIVE = -26, /**< Phone is off the hook for either an incoming
                                 or outgoing call. */
    ...
    SEC_UI_FRAME_AUTHENTICATION_FAILED = -32, /**< Error occurred while
                                                 authenticating the Secure UI
                                                 frame. @newpage*/
