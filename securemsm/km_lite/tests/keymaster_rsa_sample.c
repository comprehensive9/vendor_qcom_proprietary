...
/* GENERATE KEY */
/*************************************************************************************/
static keymaster_error_t create_rsa_key(keymaster_key_blob_t* rsa_key_blob) {
    keymaster_error_t err = KM_ERROR_UNKNOWN_ERROR;

    keymaster_key_param_t params[] = {
            keymaster_param_enum(KM_TAG_ALGORITHM, KM_ALGORITHM_RSA),
            keymaster_param_int(KM_TAG_KEY_SIZE, 2048),
            keymaster_param_int(KM_TAG_RSA_PUBLIC_EXPONENT, 65537),
    ...
    err = km_generate_key(&param_set, rsa_key_blob, &rsa_key_characteristics);
    if (err != KM_ERROR_OK)
        return err;

    printf("RSA key created successfully with key size of ");
...
/* SIGN */
/*************************************************************************************/
static keymaster_error_t sign_rsa(const keymaster_key_blob_t* key_blob,
...
/* VERIFY */
/*************************************************************************************/
static keymaster_error_t verify_rsa(const keymaster_key_blob_t* key_blob,
                              const uint8_t* text, size_t text_size,
...
//sample flow of RSA sign/verify
keymaster_error_t keymaster_rsa_sample(bool multi_thread) {
    keymaster_error_t err = KM_ERROR_UNKNOWN_ERROR;
    ...
    keymaster_key_blob_t rsa_key_blob = { 0 };
    ...
    do {
        ...
        err = create_rsa_key(&rsa_key_blob);
        if (err != KM_ERROR_OK)
            break;

        err = sign_rsa(&rsa_key_blob, text, sizeof(text),
        ...
        err = verify_rsa(&rsa_key_blob, text, sizeof(text), signature, signature_size);
        if (err != KM_ERROR_OK)
            break;

        printf("SUCCESS - RSA sign and verify completed!\n");
    ...
    //memory cleanup
    keymaster_free_key_blob(&rsa_key_blob);
