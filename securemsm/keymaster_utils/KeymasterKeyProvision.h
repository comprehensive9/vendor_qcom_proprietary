...
class KeymasterKeyProvision {
  public:
    ...
    int KeyMasterProvisionDeviceId(string Brand, string Device, string Product, string SerialNum,
                                   string Manufacturer, string Model, string Imei, string Meid);
  ...
  private:
    int ProvisionDeviceIdNew(const keymaster_key_param_set_t* in_params);
    int ProvisionDeviceIdOld(const keymaster_key_param_set_t* in_params);
    ...
    int CleanProvisioningStateOld(void);
    int SetProvisioniningSuccessOld(void);
