...
ASN1_SEQUENCE(KM_AUTH_LIST) = {
        ...
        ASN1_EXP_OPT(KM_AUTH_LIST, active_date_time, ASN1_INTEGER,
                     TAG_ACTIVE_DATETIME.masked_tag()),
        ...
        ASN1_EXP_OPT(KM_AUTH_LIST, application_id, ASN1_OCTET_STRING,
                     TAG_APPLICATION_ID.masked_tag()),
        ...
        ASN1_EXP_OPT(KM_AUTH_LIST, rollback_resistant, ASN1_NULL,
                     TAG_ROLLBACK_RESISTANT.masked_tag()),
        ...
        ASN1_EXP_OPT(KM_AUTH_LIST, vendor_patchlevel, ASN1_INTEGER,
                     TAG_VENDOR_PATCHLEVEL.masked_tag()),
...
static bool get_ulong(const ASN1_INTEGER* asn1_int, keymaster_tag_t tag,
    ...
    if (!BN_get_u64(bn.get(), &ulong)) return false;
...
static keymaster_error_t extract_auth_list(const KM_AUTH_LIST* record,
    ...
    if (record->trusted_confirmation_required &&
        !auth_list->push_back(TRUSTED_CONFIRMATION_REQUIRED))
        return KM_ERROR_MEMORY_ALLOCATION_FAILED;
    ...
    // Origin
    if (!get_enum(record->origin, TAG_ORIGIN, auth_list)) return KM_ERROR_MEMORY_ALLOCATION_FAILED;
...
keymaster_error_t parse_attestation_record(const uint8_t* asn1_key_desc, size_t asn1_key_desc_len,
    ...
    if (!record.get()) return KM_ERROR_UNKNOWN_ERROR;
