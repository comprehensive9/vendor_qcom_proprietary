 ...
 */
template <typename T>
inline T* dup_array(const T* a, size_t n) {
    T* dup = new (std::nothrow) T[n];
    if (dup)
        for (size_t i = 0; i < n; ++i) dup[i] = a[i];
    return dup;
}
 ...
 */
template <typename T, size_t N>
inline T* dup_array(const T (&a)[N]) {
 ...
 */
template <typename T, size_t N>
inline void copy_array(const T (&arr)[N], T* dest) {
    for (size_t i = 0; i < N; ++i) dest[i] = arr[i];
}
 ...
 */
template <typename T, size_t N>
inline bool array_contains(const T (&a)[N], T val) {
...
class Eraser {
  public:
    ...
    template <size_t N>
    explicit Eraser(uint8_t (&arr)[N]) : buf_(arr), size_(N) {}
...
template <typename T>
ArrayWrapper<T> array_range(T* begin, size_t length) {
...
struct KeymasterKeyBlob : public keymaster_key_blob_t {
    ...
    KeymasterKeyBlob(const uint8_t* data, size_t size) {
        ...
        if (key_material) key_material_size = size;
    }
    ...
    explicit KeymasterKeyBlob(const keymaster_key_blob_t& blob) {
        ...
        if (key_material) key_material_size = blob.key_material_size;
    }
    ...
    const uint8_t* Reset(size_t new_size) {
        ...
        if (key_material) key_material_size = new_size;
        return key_material;
    }
...
struct CertificateChainDelete {
    void operator()(keymaster_cert_chain_t* p) {
        if (!p) return;
        for (size_t i = 0; i < p->entry_count; ++i) delete[] p->entries[i].data;
        delete[] p->entries;
        delete p;
    }
};
