...
using ::vendor::qti::esepowermanager::V1_1::eseData;
...
void ioctlCallback(eseData outData) {
    memcpy(&outputEseData, &outData, sizeof(eseData));
}
...
static bool esepmInit() {
  ...
  ALOGD("Try 1.1 IEsePowerManager HIDL interface");
  mHal = mHalV1_1 = IEsePowerManagerV1_1::tryGetService();
  if(mHalV1_1 == nullptr) {
    ALOGE("Failure in getting 1.1 IEsePowerManager HIDL interface, try 1.0 IEsePowerManager");
    mHal = IEsePowerManager::tryGetService();
    if(mHal == nullptr) {
      ALOGE("Failure in getting 1.0 IEsePowerManager HIDL interface");
    }
  }
