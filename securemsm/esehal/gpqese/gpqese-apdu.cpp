...
static mChannelInfo_t channelsInfo[MAX_NUM_CHANNELS_SN220];
static int maxChannels = MAX_NUM_CHANNELS_DEFAULT;
...
TEEC_Result ese_init()
{
  TEEC_Result result = TEEC_ERROR_GENERIC;
  int32_t ret;
  char nfc_chipid[PROPERTY_VALUE_MAX] = {0};
  ...
  // Retrieve NFC HW chip-id
  ret = property_get("vendor.qti.nfc.chipid", nfc_chipid, "0x00");
  if (ret <= 0) {
    ALOGE("Failure in retrieving chip-id, ret = %d\n", ret);
  }
  else {
    ALOGD("Chip-id retrieved = %s\n", nfc_chipid);
  }

  // Transmit INTERFACE_RESET cmd in case NFC HW is SN1xx
  if ((!strncmp(nfc_chipid, SN1xx_CHIPID_A3, PROPERTY_VALUE_MAX)) ||
      (!strncmp(nfc_chipid, SN1xx_CHIPID_A4, PROPERTY_VALUE_MAX))) {
    ret = gpqese_generic(GPQESE_CMD_GENERIC_TAG_INTERFACE_RESET, &rx, &result);
    if (result != TEEC_SUCCESS || ret < 0) {
      ALOGE("interface reset failed");
      return result;
    }
    vendorId = rx.base[ATR_OS_VERSION_OFF];
  }
...
static int getMaxSupportedChannels()
{
  if (vendorId != 0xFF && vendorId >= SN220_OS_VERSION)
