...
#define IQwesService_OP_getAllInstalledFeatureIDs 21
#define IQwesService_OP_getAllInstalledSerialNumbers 22
#define IQwesService_OP_removeLicense 23
#define IQwesService_OP_checkInstalledLicense 24
#define IQwesService_OP_checkLicenseBuffer 25
...
static inline int32_t
IQwesService_getAllInstalledFeatureIDs(Object self, uint8_t *results_ptr, size_t results_len, size_t *results_lenout)
{
  ObjectArg a[1];
  a[0].b = (ObjectBuf) { results_ptr, results_len * sizeof(uint8_t) };

  int32_t result = Object_invoke(self, IQwesService_OP_getAllInstalledFeatureIDs, a, ObjectCounts_pack(0, 1, 0, 0));

  *results_lenout = a[0].b.size / sizeof(uint8_t);

  return result;
}

static inline int32_t
IQwesService_getAllInstalledSerialNumbers(Object self, uint8_t *results_ptr, size_t results_len, size_t *results_lenout)
{
  ObjectArg a[1];
  a[0].b = (ObjectBuf) { results_ptr, results_len * sizeof(uint8_t) };

  int32_t result = Object_invoke(self, IQwesService_OP_getAllInstalledSerialNumbers, a, ObjectCounts_pack(0, 1, 0, 0));
...
static inline int32_t
IQwesService_removeLicense(Object self, const uint8_t *serial_ptr, size_t serial_len)
{
  ObjectArg a[1];
  a[0].bi = (ObjectBufIn) { serial_ptr, serial_len * sizeof(uint8_t) };

  return Object_invoke(self, IQwesService_OP_removeLicense, a, ObjectCounts_pack(1, 0, 0, 0));
}

static inline int32_t
IQwesService_checkInstalledLicense(Object self, uint32_t featureID_val, const uint8_t *licenseeHash_ptr, size_t licenseeHash_len, uint8_t *licenseSerialNumber_ptr, size_t licenseSerialNumber_len, size_t *licenseSerialNumber_lenout)
{
  ObjectArg a[3];
  a[0].b = (ObjectBuf) { &featureID_val, sizeof(uint32_t) };
  a[1].bi = (ObjectBufIn) { licenseeHash_ptr, licenseeHash_len * sizeof(uint8_t) };
  a[2].b = (ObjectBuf) { licenseSerialNumber_ptr, licenseSerialNumber_len * sizeof(uint8_t) };

  int32_t result = Object_invoke(self, IQwesService_OP_checkInstalledLicense, a, ObjectCounts_pack(2, 1, 0, 0));

  *licenseSerialNumber_lenout = a[2].b.size / sizeof(uint8_t);

  return result;
}

static inline int32_t
IQwesService_checkLicenseBuffer(Object self, const uint8_t *license_ptr, size_t license_len, uint32_t featureID_val, const uint8_t *licenseeHash_ptr, size_t licenseeHash_len, uint8_t *licenseBufferCBOR_ptr, size_t licenseBufferCBOR_len, size_t *licenseBufferCBOR_lenout)
{
  ObjectArg a[4];
  a[0].bi = (ObjectBufIn) { license_ptr, license_len * sizeof(uint8_t) };
  a[1].b = (ObjectBuf) { &featureID_val, sizeof(uint32_t) };
  a[2].bi = (ObjectBufIn) { licenseeHash_ptr, licenseeHash_len * sizeof(uint8_t) };
  a[3].b = (ObjectBuf) { licenseBufferCBOR_ptr, licenseBufferCBOR_len * sizeof(uint8_t) };

  int32_t result = Object_invoke(self, IQwesService_OP_checkLicenseBuffer, a, ObjectCounts_pack(3, 1, 0, 0));

  *licenseBufferCBOR_lenout = a[3].b.size / sizeof(uint8_t);
