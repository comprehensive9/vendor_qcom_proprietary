...
class IIAppLoader {
   public:
    ...
    virtual int32_t connect(const void* appName_ptr, size_t appName_len, ProxyBase &appController_ref) = 0;
...
class IAppLoader : public IIAppLoader, public ProxyBase {
   public:
    ...
    virtual int32_t connect(const void* appName_ptr, size_t appName_len, ProxyBase &appController_ref) {
        ObjectArg a[2];
        a[0].bi = (ObjectBufIn) {appName_ptr, appName_len * 1};

        int32_t result = invoke(OP_connect, a, ObjectCounts_pack(1, 0, 0, 1));
        if (Object_OK != result) { return result; }
