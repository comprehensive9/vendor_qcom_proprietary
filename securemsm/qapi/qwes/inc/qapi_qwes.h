...
#define QAPI_ERR_FILE_NOT_FOUND                     QWES_ERR(15)
#define QAPI_ERR_PFMFILER_FILE_REMOVE_FAILED        QWES_ERR(16)
 ...
 *   "SubjectKeyID" : bstr
 *   "SubjectDN" : tstr
 *   "Expiration" : bstr as <range count,exp start,exp end>
 *   OR
 *   "ExpirationWithGracePeriod": bstr as <range count,exp start,exp end,grace period,grace count,grace period end time, grace count remaining>
 *   "LicenseeHashes" : bstr
 *   "FIDs" : [1 * uint ]
...
/**
 * @brief Retrieve FeatureIDs from licenses installed in license store.
 *
 * @param [out] fids_cbor          Buffer to hold FeatureID list in CBOR format
 * @param [out] fids_cbor_len      length of the Buffer fids_cbor
 * @param [out] fids_cbor_len_out  length of the data filled in fids_cbor
 *
 * Data written to the fids_cbor buffer is a CBOR structure.  The structure is
 * @code
 * fids_cbor = {
 *   "FIDs" : [1 * uint ]
 *   "FIDRanges" : [1 * (uint, uint)]
 ...
 * @retval QAPI_ERROR if info retrieval failed for any reason.
 */
qapi_Status_t qapi_QWES_Agent_Get_All_Installed_FeatureIDs(uint8_t *fids_cbor,
                                                           size_t   fids_cbor_len,
                                                           size_t  *fids_cbor_len_out);

/**
 * @brief Retrieve Serial numbers corresponding to licenses installed in license store.
 *
 * @param [out] serial_num_cbor          Buffer to hold serial numbers in CBOR format
 * @param [out] serial_num_cbor_len      length of the Buffer serial_num_cbor
 * @param [out] serial_num_cbor_len_out  length of the data filled in serial_num_cbor
 *
 * Data written to the serial_num_cbor buffer is a CBOR structure.  The structure is
 * @code
 * serial_num_cbor = {
 *    [*buffer]
 * }
 ...
 * @retval QAPI_ERROR if info retrieval failed for any reason.
 */
qapi_Status_t qapi_QWES_Agent_Get_All_Installed_SerialNumbers(uint8_t *serial_num_cbor,
                                                              size_t   serial_num_cbor_len,
                                                              size_t  *serial_num_cbor_len_out);

/**
 * @brief Delete the license corresponding to given Serial number from license store.
 *
 * @param [in] serial_num          Buffer with serial number of the license to be deleted
 * @param [in] serial_num_len      length of the Buffer serial_num
 *
 * @retval QAPI_OK if the license was installed successfully.
 * @retval QAPI_ERROR if info retrieval failed for any reason.
 */
qapi_Status_t qapi_QWES_Agent_Remove_License(const uint8_t *serial_num,
                                             size_t         serial_num_len);

/**
 * @brief Check license status for given FID and retrieve corresponding serial number
 * if valid license is found
 *
 * @param [in] featureID_val               Feature ID value
 * @param [in] licenseeHash_ptr            Buffer to with licensee hash
 * @param [in] licenseeHash_len            Length of licensee hash
 * @param [out] licenseSerialNumber_ptr    buffer to hold serial number of the license
 * @param [out] licenseSerialNumber_len    buffer length
 * @param [out] licenseSerialNumber_lenout length of serial number returned
 *
 * @retval QAPI_OK if the license was installed successfully.
 * @retval QAPI_ERROR if info retrieval failed for any reason.
 */
qapi_Status_t qapi_QWES_Agent_Check_Installed_License(uint32_t        featureID_val,
                                                      const uint8_t  *licenseeHash_ptr,
                                                      size_t          licenseeHash_len,
                                                      uint8_t        *licenseSerialNumber_ptr,
                                                      size_t          licenseSerialNumber_len,
                                                      size_t         *licenseSerialNumber_lenout);

/**
 * @brief Check license status for given FID and retrieve corresponding serial number
 * if valid license is found
 *
 * @param [in] pPFMLicense_ptr             License Buffer
 * @param [in] PFMLicense_len              Length of license
 * @param [in] featureID_val               Feature ID value
 * @param [in] licenseeHash_ptr            Buffer to with licensee hash
 * @param [in] licenseeHash_len            Length of licensee hash
 * @param [out] pLicenseInfoCBOR_ptr       buffer to hold license info in CBOR format
 * @param [out] LicenseInfoCBOR_len        buffer length
 * @param [out] LicenseInfoCBOR_lenout     length of license info returned
 *
 * @retval QAPI_OK if the license was installed successfully.
 * @retval QAPI_ERROR if info retrieval failed for any reason.
 */
qapi_Status_t qapi_QWES_Agent_Check_License_Buffer(const uint8_t  *pfmLicense_ptr,
                                                   size_t          pfmLicense_len,
                                                   uint32_t        featureID_val,
                                                   const uint8_t  *licenseeHash_ptr,
                                                   size_t          licenseeHash_len,
                                                   uint8_t        *licenseInfoCBOR_ptr,
                                                   size_t          licenseInfoCBOR_len,
                                                   size_t         *licenseInfoCBOR_lenout);
