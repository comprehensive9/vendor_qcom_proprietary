...
NONDETERMINISTIC_TESTS = ["RSA-KEYGEN", "RSA-SIGGEN", "ECDSA-KEYGEN", "ECDSA-SIGGEN", "KAS-ECC"]

ENGINES_USING_FIPSTESTAPP = ["TZSW32", "TZ_KERNEL", "GPCE"]
ENGINES_USING_FIPSTESTAPP64 = ["TZSW64", "TZ_KERNEL", "GPCE"]

CHIPSET_64BIT_TAS_ONLY = ["taro"]
...
def find_acvp_test_binary():
    ...
    target = ""
    if os.path.join("vendor", "qcom", "proprietary", "securemsm", DEVICE_BINARY_NAME) in script_dir:
...
def parse_engines_list(engines):
    ...
    result = []
    global chipset_name
    global ENGINES_USING_FIPSTESTAPP

    chipset_name = get_chipset_name().decode("utf-8")
    if chipset_name in CHIPSET_64BIT_TAS_ONLY:
        ENGINES_USING_FIPSTESTAPP = []

    if "ALL" in engines:
        result = ENGINES
        if chipset_name in CHIPSET_64BIT_TAS_ONLY:
            result.remove("TZSW32")
        return result
...
def generate_diff_report(engine, options, cryptos, consolidated_report, ignore_monte_carlo):
    ...
    for folder in folders:
        ...
        deterministic = not any([folder.startswith(t) for t in NONDETERMINISTIC_TESTS])
...
def sanity_check_device(options, engines):
    ...
    check_for_binary(DEVICE_BINARY_NAME, DEVICE_BINARY_PATH)
