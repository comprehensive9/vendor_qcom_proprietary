...
typedef enum {
    ...
    TUI_SI_GET_APP_OBJ_ERR                = -15, /**< Error occured while getting Trusted App Object. */
/** @cond */
...
/* Currently, supporting secure indicator image size upto 62KB  */
constexpr int MAX_SEC_IND_IMAGE_SIZE = 63488;
...
/**
  ...
  This function copies the image from the specified FS path and saves it
  securely in TrustZone. After the image is successfully saved into TrustZone,
  it is removed from the FS. The image size must not exceed MAX_SEC_IND_IMAGE_SIZE.
  ...
  On success, the image is deleted from the FS. @newpage
*/
tui_si_status_t SecureIndicator2_StoreSecureIndicator(const std::string imagePath);
