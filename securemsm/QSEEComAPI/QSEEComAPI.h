...
Copyright (c) 2010-2017, 2021-2022 Qualcomm Technologies, Inc.
All Rights Reserved.
...
enum QSEECom_command_id	{
  ...
  QSEECOM_DIAG_FUSE_REQ = 0x501,
  QSEECOM_DIAG_FUSE_REQ_RSP = 0x502,
...
#if defined(ICE_IOCTL_MAP_SLOT_PARTITION_NAME_REQ)
/**
 * @brief create key for given partition.
 *
 * This API creates the key for given parititon name.
 *
 * @param[in] usage, to specify which user application will use this key management function.
 *
 * @param[in] a 32bit hash as password (optional).
 *
 * @param[in] partition name for which key needs to be created.
 *
 * @return Zero on success, negative on failure. errno will be set on error.
 *
 **/
int QSEECom_create_key_for_partition(enum QSEECom_key_management_usage_type usage, unsigned char *hash32, unsigned char *partition_name);
#endif

#if defined(ICE_IOCTL_UMAP_SLOT_PARTITION_NAME_REQ)
/**
 * @brief wipe key for the given partition.
 *
 * This API wipes the key for given partition name.
 *
 * @param[in] usage, to specify which user application will use this key management function.
 *
 * @param[in] partition name for which key needs to be wiped.
 *
 * @return Zero on success, negative on failure. errno will be set on error.
 *
 **/

int QSEECom_wipe_key_for_partition(enum QSEECom_key_management_usage_type usage, unsigned char *partition_name);

/**
 * @brief clear key.
 *
 * This API clears key from PIPE.
 *
 * @param[in] usage, to specify which user application will use this key management function.
 *
 * @param[in] partition name for which key needs to be cleared.
 *
 * @return Zero on success, negative on failure. errno will be set on error.
 *
 **/

int QSEECom_clear_key_for_partition(enum QSEECom_key_management_usage_type usage, unsigned char *partition_name);
#endif


#if defined(ICE_IOCTL_UPDATE_PWD)
/**
 * @brief update key user info for given partition.
 *
 * This API updates the key user info for given partition name.
 *
 * @param[in] usage, to specify which user application will use this key management function.
 *
 * @param[in] current_hash32, to specify the current password hash
 ...
 * @param[in] new_hash32, to specify the new password hash.
 *
 * @param[in] partition name for which password  needs to be updated
 *
 * @return Zero on success, negative on failure. errno will be set on error.
 *
 **/

int QSEECom_update_key_user_info_partition(enum QSEECom_key_management_usage_type usage,
                                           unsigned char *current_hash32,
                                           unsigned char *new_hash32,
                                           unsigned char *partition_name);
#endif

#ifdef ENABLE_UC_SECURITY
/**
 * @brief enable ice FDE
 *
 * This API enables the ice FDE encryption.
 *
 * @param[in] flag, specify what to enable,
 *                  encryption : 0x1,
 *                  decryption : 0x2,
 *                  Both       : 0x3
 *
 * Recommended to pass flag as 0x3.
 *
 * @return Zero on success, negative on failure. errno will be set on error.
 *
**/
int QSEECom_enable_ice_fde(int flag);
#endif
/**
 * @brief QSEECom_set_deepsleep_state
 *
 * Notifies qseecom for deepsleep entry and exit events.
 *
 * @param[in] state: deep sleep state, DS_ENTERED = 0x1, DS_EXITED = 0x0
 * @return Zero on success, negative on failure. errno will be set on error.
 */
int QSEECom_set_deepsleep_state(int state);
