...
bool VerifyAttestation::verify_attestation_record(
    ...
    if (provision_device_id) {
        ...
        expected_tee_enforced.push_back(TAG_ATTESTATION_ID_SERIAL, SerialNum.c_str(),
                                        SerialNum.size());
        if (Imei.size())
            expected_tee_enforced.push_back(TAG_ATTESTATION_ID_IMEI, Imei.c_str(), Imei.size());
        if (Meid.size())
            expected_tee_enforced.push_back(TAG_ATTESTATION_ID_MEID, Meid.c_str(), Meid.size());
...
keymaster_error_t VerifyAttestation::AttestKey(const string& attest_challenge,
    ...
    if (provision_device_id) {
        ...
        if (Imei.size())
            attest_params.push_back(TAG_ATTESTATION_ID_IMEI, Imei.c_str(), Imei.size());
        if (Meid.size())
            attest_params.push_back(TAG_ATTESTATION_ID_MEID, Meid.c_str(), Meid.size());
...
keymaster_error_t VerifyAttestation::Attestation() {
    ...
    uint32_t expected_keymaster_api_minor_version = device->get_keymaster_api_minor_version();
    if (expected_keymaster_version == 4 && expected_keymaster_api_minor_version == 1 && level == 1)
    ...
    if (!verify_attestation_record("challenge", sw_enforced(), hw_enforced(),
                                   expected_keymaster_version, level, expected_attestation_version,
