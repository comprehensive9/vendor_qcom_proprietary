...
#include "android_keymaster_test.h"
...
int InstallKeybox::ProcessDeviceID(ifstream& inFile, keymaster_blob_t* device_id) {
    string input;
...
int InstallKeybox::ValidateProvisionedKeys() {
    VerifyAttestation dev(mProvisionDeviceId, Brand, Device, Product, SerialNum, Manufacturer,
                          Model, Imei, Meid, mSecurityLevel);
...
int main(int argc, char** argv) {
    ...
    if (argc == 7) {
        ...
        PROV_EXIT(installer_sb.ParseFile());
