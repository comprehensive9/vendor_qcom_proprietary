...
class InstallKeybox {
  public:
    InstallKeybox(std::string filename, std::string deviceId, bool provisionDeviceId,
                  keymaster_security_level_t security_level)
        : status(-1),
          mFilename(filename),
          mDeviceId(deviceId),
          provisioningClient(new KeymasterKeyProvision(security_level)),
          mProvisionDeviceId(provisionDeviceId),
  ...
  private:
    ...
    std::string ConsumeElement(std::ifstream& inFile, const std::string& startTag);
    ...
    int ProcessKeyAlgo(std::ifstream& inFile, keymaster_algorithm_t* algorithm);
    ...
    int ProcessCertificate(std::ifstream& inFile, keymaster_blob_t* certificate);
    int cleanKeyBox();
