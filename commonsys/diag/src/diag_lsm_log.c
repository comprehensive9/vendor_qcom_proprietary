...
Copyright (c) 2007-2011, 2014-2015, 2016, 2020, 2022 Qualcomm Technologies, Inc.
All Rights Reserved.
  ...
  if (ptr)
  {
    ...
    /* Set pointer to begining of diag pkt, not the log */
      ...
      if(-1 != diag_fd)
      {
	   ...
	   if((NumberOfBytesWritten = diag_lsm_comm_write(diag_fd, (unsigned char*) ptr, DIAG_REST_OF_DATA_POS+LOG_DIAGPKT_OFFSET + phdr_ptr->len)) != 0) /*TODO: Check the Numberofbyteswritten against number of bytes we wanted to write?*/
       {
	...
	if (length > sizeof(log_header_type))
	{
		if ((enabled_mask = log_status_mask(code)) != 0) {
			...
			if (pdiag_data != NULL) {
                ...
                int NumberOfBytesWritten = 0;
				if((NumberOfBytesWritten = diag_lsm_comm_write(diag_fd, (unsigned char*) pdiag_data, DIAG_REST_OF_DATA_POS +  FPOS (diag_log_rsp_type, log) + length)) != 0) {
