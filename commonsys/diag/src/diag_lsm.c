...
{
	return(diag_lsm_comm_ioctl(diag_fd, DIAG_IOCTL_REMOTE_DEV, remote_mask, sizeof(uint16)));
}
...
int delete_log(int type)
{
	...
	stat_flag = stat(file_name_del, &file_stat);
	if (stat_flag) {
		DIAG_LOGE("diag: %s: File %s doesn't exist, errno:%d\n",
...
int delete_qdss_log(int type)
{
	...
	stat_flag = stat(qdss_file_name_del, &file_stat);
	if (stat_flag) {
		DIAG_LOGE("diag: %s: File %s doesn't exist, errno:%d\n",
	...
	if (fd_dev < 0) {
		if(logging_mode == MEMORY_DEVICE_MODE) {
			...
			if(max_file_num > 1 && (file_count[type] >= max_file_num)) {
				...
				rc = -1;
				if (buffer_init[type] == READY) {
					if (!delete_log(type) || errno == ENOENT) {
						file_count[type]--;
						rc = 0;
					}
				}
