/******************************************************************************
 * ---------------------------------------------------------------------------
 *  Copyright (c) 2015-2017, 2020, 2022 Qualcomm Technologies, Inc.
 *  All Rights Reserved.
...
public class OsAgent
{
            ...
            switch (dataItems[index])
            {
                ...
                case MODEL_DATA_ITEM_ID:
                    if (! Build.MODEL.equals(Build.UNKNOWN)) {
                        if (mHidlClient != null) {
                            mHidlClient.string_dataitem_update(MODEL_DATA_ITEM_ID, Build.MODEL);
                        }
                    }
                break;
                case MANUFACTURER_DATA_ITEM_ID:
                    if (! Build.MANUFACTURER.equals(Build.UNKNOWN)) {
                        if (mHidlClient != null) {
                            mHidlClient.string_dataitem_update(MANUFACTURER_DATA_ITEM_ID,
                                    Build.MANUFACTURER);
                        }
                    }
                break;
                ...
                case POWER_CONNECTED_STATE_DATA_ITEM_ID:
                {
                    ...
                    if (currentBatteryCharging) {
                        if (mHidlClient != null) {
                            mHidlClient.power_connect_status_update(true);
                        }
                    }
                }
                break;
    ...
    private void updateContentData(List<Integer> updateDataItemList)
    {
            ...
            switch (newdataItem)
            {
                ...
                case ENH_DATA_ITEM_ID:
                    ...
                    if ( mEulaState != eula_state) {
                        ...
                        if (consentChanged && (null != IzatProvider.mProviderContext)) {
                            IzatProvider.mProviderContext.setUserConsent(consentAccepted);
                        }
        ...
        if (idxDataItem > 0) {
            ...
            if (mHidlClient != null) {
                mHidlClient.bool_dataitem_update(dataItemArray, dataItemValueArray);
            }
        }
    }
    ...
    private void sendTimeInfo()
    {
        ...
        if (mHidlClient != null) {
            mHidlClient.time_change_update(currentTimeMillis, rawOffset, dstSavings);
        }
    }
    ...
    private void sendTimeZoneInfo()
    {
        ...
        if (mHidlClient != null) {
            mHidlClient.timezone_change_update(currentTimeMillis, rawOffset,
                    dstSavings);
        }
    }
    ...
    private void updateConnectivityStatus()
    {
        ...
        if (mHidlClient != null) {
            mHidlClient.networkinfo_update(is_connected, type, type_name, subtype_name,
    ...
    private void updateWiFiSupplicantState(Intent intent) {
        ...
        if (mHidlClient != null) {
            mHidlClient.wifi_supplicant_status_update(supplicantState.ordinal(),
    ...
    private int isDeviceInRegulatedArea()
    {
        ...
        if (mTelephonyMgr.getServiceState() != null &&
                mTelephonyMgr.getServiceState().getState() == ServiceState.STATE_IN_SERVICE) {
        ...
        if (mBluetoothAdapter != null && (mBtleScanStarted != enable)) {
            ...
            if (enable) {
                ...
                if (!retVal) {
                    ...
                    if (mHidlClient != null) {
                        mHidlClient.btle_scan_data_inject(false, 0, dummyBtApMacAddress, 0, 0, 0,
                                ERR_BTLE_DISABLED);
                    }
    ...
    private BluetoothAdapter.LeScanCallback mLeScanCallback =new BluetoothAdapter.LeScanCallback() {
        public void onLeScan(final BluetoothDevice device, final int rssi, byte[] scanRecord) {
            ...
            if (mHidlClient != null) {
                mHidlClient.btle_scan_data_inject(true, rssi, btApMacAddress, btleDevScanRcvTime,
    ...
    private void sendServiceStateInfo()
    {
        ...
        if (mDataItemList.contains(SERVICESTATUS_DATA_ITEM_ID)) {
            ...
            if (mHidlClient != null) {
                mHidlClient.service_state_update(networkStatus);
            }
        }
    }
            ...
            if ((mCurrentServiceState == ServiceState.STATE_OUT_OF_SERVICE) ||
                ...
                if (phoneType == PhoneConstants.PHONE_TYPE_CDMA) {
                    ...
                    if (!mCDMAHomeCarrier.isEmpty()) {
                        ...
                        if (mHidlClient != null) {
                            mHidlClient.serviceinfo_update(LOC_RILAIRIF_CDMA,
                    ...
                    logd("Home carrier is G/W/L");
                      ...
                      logv("Operator = " + strSimOperator +" Carrier Mcc = " + iMcc +
                                  " Carrier Mnc = " + iMnc);
                      if (mHidlClient != null) {
                          mHidlClient.serviceinfo_update(LOC_RILAIRIF_GSM, "", iMcc, iMnc);
                      }
                }
                else {
    ...
    private void sendCellUpdateInfo()
    {
        try
        {
            ...
            if ((mCurrentServiceState== ServiceState.STATE_OUT_OF_SERVICE) ||
                ...
                if (mHidlClient != null) {
                    mHidlClient.cell_update_ooo(networkStatus, valid_mask);
                }
            }
            else {
                // get network status
                    ...
                    for (CellInfo ci: cellInfoValue) {
                        if (ci instanceof CellInfoLte && ci.isRegistered()) {
                            ...
                            if(cellIdentityLte != null) {
                                ...
                                if (mHidlClient != null) {
                                    mHidlClient.cell_update_lte(air_interface_type,
                            ...
                            if (mHidlClient != null) {
                                mHidlClient.cell_update_gw(air_interface_type, networkStatus,
                            ...
                            if (mHidlClient != null) {
                                mHidlClient.cell_update_cdma(air_interface_type, networkStatus,
    ...
    private void sendTac() {
        ...
        if (mHidlClient != null) {
            mHidlClient.string_dataitem_update(TAC_DATA_ITEM_ID, tac);
        }
    }
    ...
    private void updateMccmnc() {
        ...
        if (mccmncStr != null && !mccmncStr.isEmpty()) {
            if (mHidlClient != null) {
                mHidlClient.string_dataitem_update(MCCMNC_DATA_ITEM_ID, mccmncStr);
            }
        }
        ...
        public boolean handleMessage(Message msg) {
            ...
            switch(msgID)
            {
                case MSG_OSAGENT_INIT:
                    mHidlClient = new OsAgentHidlClient();
                    initAutoEula();
                ...
                case MSG_SCREEN_STATE_CHANGED:
                    if (mHidlClient != null) {
                        mHidlClient.screen_status_update((boolean)msg.obj);
                    }
                break;
                case MSG_POWER_STATE_CHANGED:
                    if (mHidlClient != null) {
                        mHidlClient.power_connect_status_update((boolean)msg.obj);
                    }
                break;
                ...
                case MSG_BTSCAN_DATA_INJECT:
                    if (mHidlClient != null) {
                        mHidlClient.bt_classic_scan_data_inject(true, msg.arg1, (byte[])msg.obj,
                                0, 0, 0, 0);
                    }
                break;
                case MSG_IDL_SERVICE_DIED:
                    if (mHidlClient != null) {
                        mHidlClient.resetIDLService();
                    }
                break;
                default:
    ...
    private void notifyBatteryPct(int level, int scale)
    {
        if ((scale > 0) && (level >= 0)) {
            ...
            if (mHidlClient != null) {
                mHidlClient.battery_level_update(batteryPct);
            }
        }
    }
    ...
    private OsAgentHidlClient mHidlClient;
    ...
    private class OsAgentHidlClient extends BaseHidlClient
            implements BaseHidlClient.IServiceDeathCb{

        private vendor.qti.gnss.V1_0.ILocHidlIzatSubscription mOsAgentIface;
        private HidlServiceVersion mVer = HidlServiceVersion.V0_0;
        ...
        private void getOsAgentIface() {
            if (null == mOsAgentIface) {
                ...
                mVer = getHidlServiceVersion();
                if (null != service) {
                    try {
                        if (mVer.compareTo(HidlServiceVersion.V1_1) >= 0) {
        ...
        public void serviceinfo_update(int air_interface_type, String carrierName, int Mcc, int Mnc) {
        ...
        public void cell_update_cdma(int air_interface_type, int nwStatus, int sid, int nid,
                int bsid, int bslat, int bslong, boolean inDST, int UTCTimeOffset, int valid_mask) {
