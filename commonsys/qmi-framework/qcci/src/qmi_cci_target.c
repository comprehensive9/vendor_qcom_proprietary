...
#ifdef QMI_CCI_MDMDETECT_ENABLE
static void qmi_cci_is_mdm_present(void)
{
  int ret = 0;
...
static void qmi_cci_identify_mdm_type(void)
{
...
static void qmi_cci_start_def_qmux_xports_internal(void)
{
  if (internal_mdm)
    qmi_cci_xport_start(&qmuxd_ops, (void *)QMI_CLIENT_QMUX_RMNET_INSTANCE_0);
}

static void qmi_cci_start_def_qmux_xports_external(void)
{
  if (external_mdm)
  {
...
static void qmi_cci_stop_def_qmux_xports(void)
{
  if(internal_mdm)
  {
