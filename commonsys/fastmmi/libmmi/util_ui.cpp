...
bool init_surface(draw_control_t * draw) {
    ...
    status_t status = SurfaceComposerClient::getActiveDisplayConfig(dtoken, &draw->dconfig);
    ...
    if(orient == eOrientation90 || orient == eOrientation270) {
        int temp = draw->dconfig.resolution.getHeight();

        draw->dconfig.resolution.setHeight(draw->dconfig.resolution.getWidth());
        draw->dconfig.resolution.setWidth(temp);
    }

    draw->pixels_percent_x = (float)draw->dconfig.resolution.getWidth() / (float)PERCENT_MAX;
    draw->pixels_percent_y = (float)draw->dconfig.resolution.getHeight() / (float)PERCENT_MAX;

    Rect destRect(draw->dconfig.resolution.getWidth(), draw->dconfig.resolution.getHeight());

    //draw->client->setDisplayProjection(dtoken, orient, destRect, destRect);

    // create the native surface
    draw->control =
        draw->client->createSurface(String8(draw->name), draw->dconfig.resolution.getWidth(),
                                    draw->dconfig.resolution.getHeight(), PIXEL_FORMAT_RGBA_8888);
    ...
    MMI_ALOGD("surface init success: draw name=%s, draw layer=%d, draw control=%p, "
              ...
              draw->name, draw->layer, draw->control.get(), draw->dconfig.resolution.getWidth(),
              draw->dconfig.resolution.getHeight());
    return true;
}
