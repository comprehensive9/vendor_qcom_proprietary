/*
 * Copyright (c) 2012-2014, 2016-2017,2020 Qualcomm Technologies, Inc.
 * All Rights Reserved.
...
public class DeviceListFragment extends ListFragment {
    ...
    public void handleStartClientClick(View v) {
        ...
        final NetDeviceIF localDevice = peerDevice.getNetTypeManager().getLocalDevice();
        Log.d(TAG, "handleStartClientClick: peer macAddress= " + peerDevice.getAddress());
        if (!localDevice.supportWfd()){
            Log.d(TAG, "handleStartClientClick: LocalDevice not available for session");
            AlertDialog.Builder builder = new AlertDialog.Builder(getActivity());
            builder.setTitle(localDevice.getAddress() + " is not currently available for a WFD session")
                .setMessage(
                    "Disconnect and try again after resetting local device"
                        +"\n"+"Select source/sink after turning on wlan")
                .setNeutralButton("Ok",
            ...
            alert.show();
            return;
        } else if (!peerDevice.supportWfd()){// || peerDevice.wfdInfo.isSessionAvailable() == false){
