...
public class QtiRadioHidlClient implements IHidlConnectionInterface {
    ...
    public class QtiRadioResponse extends
            vendor.qti.hardware.radio.qtiradio.V2_5.IQtiRadioResponse.Stub {
        ...
        // This fucntion is never called as per the current design,
        // as its client directly registers to QTIRIL.java and gets
        // the hidl response
        @Override
        ...
        public void setNrConfigResponse(int serial, int errorCode, int status) {
            ...
            if (mInflightRequests.containsKey(serial)) {
                ...
                mCallback.setNrConfigStatus(mSlotId, token, convertHidl2Aidl(errorCode));
    ...
    @Override
    public void setNrConfig(int slotId, NrConfig config, Token token) throws RemoteException {
        Log.d(TAG, "setNrConfig: slotId = " + slotId + " config = " + config + " token = " + token);
        int serial = token.get();
        mInflightRequests.put(serial, token);
        try {
            getQtiTelephonyComponentFactory().getRil(slotId).setNrConfig(serial, config);
        } catch (RemoteException | NullPointerException e) {
            mInflightRequests.remove(serial, token);
            Log.d(TAG, "setNrConfig: slotId = " + slotId + " Exception = " + e);
            e.printStackTrace();
            //Rethrow same exception.
            throw e;
        }
    ...
    public void queryNrConfig(int slotId, Token token) throws RemoteException {
        Log.d(TAG, "queryNrConfig: slotId = " + slotId + " token = " + token);
        int serial = token.get();
        mInflightRequests.put(serial, token);
        try {
            getQtiTelephonyComponentFactory().getRil(slotId).queryNrConfig(serial);
        } catch (RemoteException | NullPointerException  e) {
            mInflightRequests.remove(serial, token);
            Log.d(TAG, "queryNrConfig: slotId = " + slotId + " Exception = " + e);
            e.printStackTrace();
            //Rethrow same exception.
            throw e;
        }
