...
public class QtiPrimaryCardPriorityHandler {
    ...
    private int getMaxPrioritySlot() {
        ...
        for (int i = 0; i < QtiPrimaryCardUtils.PHONE_COUNT; i++) {
            if (mCurrPriorityConfigs[i] == null) {
                logd("has null priority config, return " + QtiPrimaryCardUtils.SLOT_INVALID);
                return QtiPrimaryCardUtils.SLOT_INVALID;
            }

            if (tempMaxPriority < mCurrPriorityConfigs[i].priority) {
                slotId = i;
