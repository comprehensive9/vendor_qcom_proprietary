...
public class QtiRadioCapabilityController extends Handler {
    ...
    private static final String ALLOW_FLEX_MAPPING_ON_INACTIVE_SUB_PROPERTY =
            "persist.radio.flex_map_inactive";
    private static final String FLEX_MAPPING_TYPE =
            "persist.vendor.radio.flexmap_type";
    ...
    private void handleUpdateBindingDone(int result) {
        if (bothPhonesMappedToSameStack && (result == SUCCESS)) {
            ...
            String flexMapSupportType =
                    SystemProperties.get(FLEX_MAPPING_TYPE, "dds");
        ...
        if (result == SUCCESS) {
            ...
            // 3. Query IMEI.
            for (int i = 0; i < mNumPhones; i++) {
                if (isSetNWModeInProgress()) updateNewNwModeToDB(i, mPrefNwMode[i]);
    ...
    */
    ...
    private void updateNewNwModeToDB(int phoneId, int nwMode) {
        int[] subId = mQtiSubscriptionController.getSubId(phoneId);
        if (subId != null && subId.length > 0) {
            logi("updateNewNwModeToDB: subId[" + phoneId + "] = "
                    + subId[0] + " n/w mode = " + nwMode);
            if (mSubscriptionManager.isActiveSubscriptionId(subId[0])) {
                android.provider.Settings.Global.putInt(mContext.getContentResolver(),
                        android.provider.Settings.Global.PREFERRED_NETWORK_MODE + subId[0], nwMode);
            }
            QtiPhoneUtils.putIntAtIndex(mContext.getContentResolver(),
                    android.provider.Settings.Global.PREFERRED_NETWORK_MODE, phoneId, nwMode);
        }
    }
    ...
    synchronized public void setPreferredNetworkType(int phoneId,
        ...
        if ((isFlexMappingAllowedOnInactiveSub() || isSubActive)
            ...
            internalSetNetworkMode(phoneId, networkType, response);
            setNWModeInProgressFlag(false);
        }
    }
    ...
    private void sendSubscriptionSettings() {
        for (int i = 0; i < mNumPhones; i++) {
            int[] subId = mQtiSubscriptionController.getSubId(i);
            if ((subId != null) && (subId.length > 0) &&
                    mSubscriptionManager.isActiveSubscriptionId(subId[0])) {
                int type = getNetworkModeFromDB(i);
                internalSetNetworkMode(i, type, null);
                // TODO (b/122357917) the below block was added without complete context;
                // review and modify as necessary
                DataEnabledSettings dataEnabledSettings = mPhone[i].getDataEnabledSettings();
    ...
    public boolean isFlexMappingRequired(int phoneId) {
        final String flexMapSupportType =
                SystemProperties.get(FLEX_MAPPING_TYPE, "dds");
        if (!flexMapSupportType.equalsIgnoreCase("dds")) return true;
        return false;
    }

    private void notifyNetworkTypeChanged(int phoneId) {
        try {
            Field f = Phone.class.getDeclaredField("mPreferredNetworkTypeRegistrants");
            f.setAccessible(true);
            RegistrantList registrants= (RegistrantList) f.get(mPhone[phoneId]);
            registrants.notifyRegistrants();
        } catch (Exception e) {
            loge("notifyNetworkTypeChanged failed");
        }
    }

    private void internalSetNetworkMode(int phoneId, int networkType, Message response) {
        final int modemRaf = mPhone[phoneId].getRadioAccessFamily();
        if (modemRaf == RadioAccessFamily.RAF_UNKNOWN) {
            logi("internalSetNetworkMode: modem rat is unknown");
            mCi[phoneId].setPreferredNetworkType(networkType, response);
            updateNewNwModeToDB(phoneId, networkType);
            notifyNetworkTypeChanged(phoneId);
            return;
        }
        final int rafFromType = RadioAccessFamily.getRafFromNetworkType(networkType);
        final int filteredRaf = (int) (rafFromType & modemRaf
                 & mPhone[phoneId].getEffectiveAllowedNetworkTypes());
        final int filteredType = RadioAccessFamily.getNetworkTypeFromRaf(filteredRaf);

        final int[] subId = mQtiSubscriptionController.getSubId(phoneId);

        logi("internalSetNetworkMode: subId[" + phoneId + "] = " + subId + " new n/w mode = "
                + filteredType + " old n/w mode = " + getNetworkModeFromDB(phoneId)
                + " requested n/w mode = " + networkType);
        updateNewNwModeToDB(phoneId, networkType);
        mCi[phoneId].setPreferredNetworkType(filteredType, response);
        notifyNetworkTypeChanged(phoneId);
    }
