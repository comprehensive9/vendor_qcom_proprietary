...
public class ImsSmsImpl extends ImsSmsImplBase {
    ...
    private int maybeAdjustSendStatus(int status) {
        if(status != ImsSmsImplBase.SEND_STATUS_ERROR_FALLBACK) {
            return status;
        }
        int subId = getSubId();
        if(Config.isCarrierConfigEnabled(mContext, subId, "config_retry_sms_over_ims")) {
            Log.d(this,"maybeAdjustSendStatus: retry SMS over IMS");
            return ImsSmsImplBase.SEND_STATUS_ERROR_RETRY;
        }
        if(isNetworkRoaming() && Config.isCarrierConfigEnabled(mContext, subId,
    ...
    private void sendResponseToFramework(AsyncResult ar) {
        ...
        if (smsResponse == null || !mSmsFwkReady) {
            Log.w(this, "smsResponse =" + smsResponse +
                    " isFrameworkReady=" + mSmsFwkReady);
            return;
        }
        final int ref = smsResponse.getMsgRef();
        int result = maybeAdjustSendStatus(smsResponse.getResult());
        ...
        try {
            if (result == ImsSmsImplBase.SEND_STATUS_OK) {
               ...
               onSendSmsResultError(token, ref, result, reason,
