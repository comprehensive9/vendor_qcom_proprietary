...
public class ImsCallSessionImpl extends QImsSessionBase {
    ...
    private QImsCallProfile mLocalCallProfile = new QImsCallProfile(
                ImsCallProfile.SERVICE_TYPE_NORMAL, ImsCallProfile.CALL_TYPE_VT_NODIR);
    private QImsCallProfile mRemoteCallProfile = new QImsCallProfile(
                ImsCallProfile.SERVICE_TYPE_NORMAL, ImsCallProfile.CALL_TYPE_VT_NODIR);
    private QImsCallProfile mCallProfile = new QImsCallProfile();
    ...
    public ImsCallSessionImpl(ImsCallProfile profile, ImsSenderRxr senderRxr,
        ...
        mCallProfile.updateProfile(profile);
    ...
    public void unMuteStateReporting() {
        ...
        if (isImsCallSessionAlive()) {
            mCallbackHandler.callSessionUpdated(mCallProfile.newImsCallProfile());
        }
    }
        ...
        switch (dcUpdate.state) {
            case ACTIVE:
                if (mDc == null) {
                    ...
                    if (mStateChangeReportingAllowed) {
                        mCallbackHandler.callSessionInitiated(mCallProfile.newImsCallProfile());
                    }
                ...
                // Check if the call is being resumed from a HOLDING state.
                    ...
                    if (mStateChangeReportingAllowed) {
                        mCallbackHandler.callSessionResumed(mCallProfile.newImsCallProfile());
                    }
                } else {
            ...
            case HOLDING:
                if (mDc.state != DriverCallIms.State.HOLDING) {
                    ...
                    if (mStateChangeReportingAllowed) {
                        mCallbackHandler.callSessionHeld(mCallProfile.newImsCallProfile());
                    }
                }
                break;
            case DIALING:
    ...
    private void setCapabilitiesInProfiles(DriverCallIms dcUpdate) {
        if (mLocalCallProfile != null) {
            ImsStreamMediaProfile localUpdate = mLocalCallProfile.getMediaProfile();
            localUpdate.copyFrom(
            ...
            localUpdate.setRttMode(getRttMode(
                    dcUpdate.callDetails.call_type, dcUpdate.callDetails.localAbility));
            mLocalCallProfile.updateMediaProfile(localUpdate);
        }
        if (mRemoteCallProfile != null) {
            setRemoteProfileCallType(dcUpdate);
            if (dcUpdate.callDetails.peerAbility != null) {
                ImsStreamMediaProfile remoteUpdate = mRemoteCallProfile.getMediaProfile();
                remoteUpdate.copyFrom(
                        ImsMediaUtils.newImsStreamMediaProfile());
                ...
                remoteUpdate.setRttMode(getRttMode(
                    dcUpdate.callDetails.call_type, dcUpdate.callDetails.peerAbility));
                mRemoteCallProfile.updateMediaProfile(remoteUpdate);
            }
        }
    }
            ...
            switch (suppSvcNotification.code) {
                case SUPP_SVC_CODE_MT_HOLD:
                    ...
                    if (startOnHoldLocalTone) {
                        mediaProfile.copyFrom(ImsMediaUtils.
                                newImsStreamMediaProfile(
                                mediaProfile.getAudioQuality(),
                                ImsStreamMediaProfile.DIRECTION_INACTIVE,
                                mediaProfile.getVideoQuality(),
                                mediaProfile.getVideoDirection(),
                                mediaProfile.getRttMode()));
                    }
                    mCallProfile.updateMediaProfile(mediaProfile);
                    mCallbackHandler.callSessionHoldReceived(mCallProfile.newImsCallProfile());
                ...
                case SUPP_SVC_CODE_MT_RESUME:
                    ...
                    mCallProfile.updateMediaProfile(mediaProfile);
                    mCallbackHandler.callSessionResumeReceived(mCallProfile.newImsCallProfile());
    ...
    public ImsCallProfile getCallProfile() {
        if (!isSessionValid()) return null;
        return mCallProfile.newImsCallProfile();
    }
    ...
    public ImsCallProfile getLocalCallProfile() {
        if (!isSessionValid()) return null;
        return mLocalCallProfile.newImsCallProfile();
    }
    ...
    public ImsCallProfile getRemoteCallProfile() {
        if (!isSessionValid()) return null;
        return mRemoteCallProfile.newImsCallProfile();
    }
    ...
    public int getRadioTechFromCallMode(int callMode) {
        ...
        switch (callMode) {
            ...
            case RadioTech.RADIO_TECH_LTE:
                radioTech = TelephonyManager.NETWORK_TYPE_LTE;
            break;
            case RadioTech.RADIO_TECH_WIFI:
            case RadioTech.RADIO_TECH_IWLAN:
                radioTech = TelephonyManager.NETWORK_TYPE_IWLAN;
            break;
            default:
    ...
    private void updateImsCallProfile(DriverCallIms dc) {
        ...
        if(mCallProfile == null) {
            mCallProfile = new QImsCallProfile();
        }
        ...
        mediaProfile.copyFrom(ImsMediaUtils.newImsStreamMediaProfile(
                mediaProfile.getAudioQuality(),
        ...
        switch (dc.callDetails.rttMode) {
            case RttMode.RTT_MODE_DISABLED:
                mediaProfile.setRttMode(ImsStreamMediaProfile.
                        RTT_MODE_DISABLED);
                break;
         ...
         */
        if (getRadioTechFromCallMode(callMode) == TelephonyManager.NETWORK_TYPE_IWLAN) {
    ...
    private void maybeUpdateCrsExtras(DriverCallIms dc) {
        if (dc == null ||  mDc == null ||
                (dc.crsData == mDc.crsData && dc.isPreparatory == mDc.isPreparatory)) {
            return;
        }
        // If CRS Type is CRS_TYPE_INVALID for old and new DriverCallIms
        //then there is nothing to be done and hence we exit the function.
        ...
        if (dc.crsData.getCrsType() == QtiCallConstants.CRS_TYPE_INVALID) {
            ...
            mCallProfile.removeExtras(extrasToRemove);
            return;
        }
    ...
    private void maybeUpdateCallProgressInfoExtras(DriverCallIms dc) {
        ...
        if (isTypeInvalid) {
            //Remove call progress info extras
            List<String> extrasToRemove = new ArrayList<>();
    ...
    static public QImsCallProfile toImsCallProfile(MultiIdentityLineInfo line,
    ...
    public void resumePendingCall(int videoState) {
        ...
        start(mCallee, mCallProfile.newImsCallProfile());
    }
    ...
    public void startConference(String[] participants, ImsCallProfile profile) {
        ...
        mCi.dial(mCallee, null, ImsPhoneCommandsInterface.CLIR_DEFAULT, details, false,
    ...
    public void transfer(ImsCallSessionImplBase otherSession) {
        ...
        try {
            if (otherSession != null && otherSession.getServiceImpl() != null) {
                int otherCallId = Integer.parseInt(otherSession.getServiceImpl().getCallId());
                mCi.explicitCallTransfer(mCallId, EctTypeInfo.CONSULTATIVE_TRANSFER, null,
    ...
    public void sendDtmf(char c, Message result) {
        if (!isSessionValid()) return;
        mCi.sendDtmf(c, mHandler.obtainMessage(EVENT_SEND_DTMF, result));
    }
    ...
    public void setCrsCrbtListener(ICrsCrbtListener listener) {
        ...
        synchronized(mCrsCrbtListeners) {
