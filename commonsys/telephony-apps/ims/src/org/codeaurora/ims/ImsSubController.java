...
public class ImsSubController implements ImsSenderRxr.ImsRadioServiceListener {
    ...
    public ImsSubController(Context context, ImsSenderRxr[] senderRxrs) {
        this(context, senderRxrs, Looper.getMainLooper());
    }
    ...
    public void dispose() {
        ...
        for (int i = 0; i < mSenderRxrs.length; i++) {
            mSenderRxrs[i].unregisterForAvailable(mHandler);
            mSenderRxrs[i].unregisterForNotAvailable(mHandler);
            mSenderRxrs[i].unregisterListener(this);
            mSenderRxrs[i].deregisterForImsSubConfigChanged(mHandler);
            mSenderRxrs[i] = null;
        }
        if (mIsReceiverRegistered) {
            mContext.unregisterReceiver(mBroadcastReceiver);
