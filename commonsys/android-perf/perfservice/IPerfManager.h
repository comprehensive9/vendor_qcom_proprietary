...
public:
    ...
    virtual int perfLockAcqAndRelease(int handle, int duration, int len, int reservelen , int* boostsList) = 0;
    virtual void perfEvent(int eventId, String16& pkg_name, int tid, int len, int* list) = 0;
    ...
    enum {
        ...
        PERF_LOCK_ACQUIRE_RELEASE = IBinder::FIRST_CALL_TRANSACTION + 6,
        PERF_EVENT = IBinder::FIRST_CALL_TRANSACTION + 8,
    };
};
