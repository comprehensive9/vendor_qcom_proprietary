...
public class WigigManager {
    ...
    public void connect(WifiConfiguration config, ActionListener listener) {
        ...
        if (listener != null) {
            listenerProxy = new ActionListenerProxy("connect", sLooper, listener);
            binder = new Binder();
        }
        try {
            // Use INVALID_NETWORK_ID for networkId when passing a config object
            mService.connect(config, WifiConfiguration.INVALID_NETWORK_ID, binder, listenerProxy);
        } catch (RemoteException e) {
    ...
    public void connect(int networkId, ActionListener listener) {
        ...
        try {
            mService.connect(null, networkId, binder, listenerProxy);
        } catch (RemoteException e) {
            if (listenerProxy != null) listenerProxy.onFailure(ERROR);
        } catch (SecurityException e) {
            if (listenerProxy != null) listenerProxy.onFailure(NOT_AUTHORIZED);
    ...
    public void save(WifiConfiguration config, ActionListener listener) {
        ...
        if (config == null) throw new IllegalArgumentException("config cannot be null");
        if (listener != null) {
            listenerProxy = new ActionListenerProxy("save", sLooper, listener);
            binder = new Binder();
        }
        try {
    ...
    public void forget(int netId, ActionListener listener) {
        ...
        if (listener != null) {
            listenerProxy = new ActionListenerProxy("forget", sLooper, listener);
            binder = new Binder();
        }
        try {
    ...
    /**
     * Callback proxy for WpsCallback objects.
     */
    private class WpsCallbackProxy extends IWpsCallback.Stub {
        private final String mActionTag;
        private final Handler mHandler;
        private final WpsCallback mCallback;

        WpsCallbackProxy(String actionTag, Looper looper, WpsCallback callback) {
            mActionTag = actionTag;
            mHandler = new Handler(looper);
            mCallback = callback;
        }
        ...
        public void onStarted(String pin) {
            Log.v(TAG, "WpsCallbackProxy:" + mActionTag + ": onStarted=" + pin);
            mHandler.post(() -> {
                mCallback.onStarted(pin);
            });
        }

        @Override
        public void onSucceeded() {
            Log.v(TAG, "WpsCallbackProxy:" + mActionTag + ": onSucceeded");
            mHandler.post(() -> {
        ...
        public void onFailed(int reason) {
            Log.v(TAG, "WpsCallbackProxy:" + mActionTag + ": onFailed=" + reason);
            mHandler.post(() -> {
    ...
    public void startWps(WpsInfo config, WpsCallback listener) {
        WpsCallbackProxy listenerProxy = null;
        Binder binder = null;

        if (config == null) throw new IllegalArgumentException("config cannot be null");
        if (listener != null) {
            listenerProxy = new WpsCallbackProxy("startWps", sLooper, listener);
            binder = new Binder();
        }
        try {
            mService.startWps(config, binder, listenerProxy);
        } catch (RemoteException e) {
            if (listenerProxy != null) listenerProxy.onFailed(ERROR);
        } catch (SecurityException e) {
            if (listenerProxy != null) listenerProxy.onFailed(NOT_AUTHORIZED);
        }
    }
    ...
    public void cancelWps(WpsCallback listener) {
        ...
        if (listener != null) {
            listenerProxy = new WpsCallbackProxy("cancelWps", sLooper, listener);
            binder = new Binder();
        }
        try {
            mService.cancelWps(binder, listenerProxy);
        } catch (RemoteException e) {
            if (listenerProxy != null) listenerProxy.onFailed(ERROR);
        } catch (SecurityException e) {
            if (listenerProxy != null) listenerProxy.onFailed(NOT_AUTHORIZED);
