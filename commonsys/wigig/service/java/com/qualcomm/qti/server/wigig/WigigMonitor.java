...
public class WigigMonitor {
    ...
    private static final ISuppTunnelCallback mSuppTunnelCallback = new ISuppTunnelCallback.Stub() {
        public void onEvent(String evt) {
            if (WigigMonitorSingleton.sInstance.dispatchEvent(evt)) {
                if (DBG) WLog.v(TAG, "Disconnecting from the supplicant, no more events??");
            }
        }
    };
   ...
   /* Network connection completed */
    ...
    public WigigMonitor(StateMachine stateMachine, WigigNative wigigNative) {
        if (DBG) WLog.v(TAG, "Creating WigigMonitor");
    ...
    private static class WigigMonitorSingleton {
        ...
        public synchronized boolean startMonitoring(String iface) {
            ...
            WLog.v(TAG, "startMonitoring(" + iface + ") with mCallbackRegistered = " + mCallbackRegistered);
        ...
        public synchronized void stopMonitoring(String iface) {
            WigigMonitor m = mIfaceMap.get(iface);
            if (DBG) WLog.v(TAG, "stopMonitoring(" + iface + ") = " + m.mStateMachine);
            m.mMonitoring = false;
        ...
        public synchronized void unregisterInterfaceMonitor(String iface) {
            ...
            if (DBG) WLog.v(TAG, "unregisterInterface(" + iface + "+" + m.mStateMachine + ")");
        }
        ...
        private synchronized boolean dispatchEvent(String eventStr) {
            ...
            if (DBG) WLog.v(TAG, "Dispatching event to interface: " + iface);
            ...
            if (m != null) {
                if (m.mMonitoring) {
                    ...
                    if (DBG) WLog.v(TAG, "Dropping event because (" + iface + ") is stopped");
                    return false;
                }
            }

            if (DBG) WLog.v(TAG, "Monitor not found for iface (" + iface + ")");
            return false;
        }
    }
    ...
    private boolean dispatchEvent(String eventStr, String iface) {
        ...
        if (DBG) {
            if (eventStr != null)
                WLog.v(TAG, iface + " dispatchEvent: " + eventStr);
        }
        ...
        if (eventName.equals(CONNECTED_STR)) {
            ...
            int status = -1;
            if (!match.find()) {
                if (DBG) WLog.v(TAG, "Assoc Reject: Could not parse assoc reject string");
            } else {
                BSSID = match.group(1);
                try {
            ...
            if (DBG) WLog.v(TAG, "event ignored");
        }
    ...
    private void handleP2pEvents(String dataString) {
        if (dataString.startsWith(P2P_DEVICE_FOUND_STR)) {
            ...
            WLog.v(TAG, "Received event= " + dataString);
    ...
    private void handleNetworkStateChange(NetworkInfo.DetailedState newState, String data) {
        ...
        if (newState == NetworkInfo.DetailedState.CONNECTED) {
            ...
            if (!match.find()) {
               if (DBG) WLog.v(TAG, "handleNetworkStateChange: Couldnt find BSSID in event string");
            } else {
                BSSID = match.group(1);
                try {
            ...
            if (!match.find()) {
               if (DBG) WLog.v(TAG, "handleNetworkStateChange: Could not parse disconnect string");
            } else {
                BSSID = match.group(1);
                try {
