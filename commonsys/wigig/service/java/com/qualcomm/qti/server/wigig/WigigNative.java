...
public class WigigNative {
    ...
    private boolean doBooleanCommand(String command) {
        if (DBG) WLog.v(mTAG, "doBoolean: " + command);
    ...
    private int doIntCommand(String command) {
        if (DBG) WLog.v(mTAG, "doInt: " + command);
        String result = doStringCommand(command);
        int resultInt = -1;
        try {
    ...
    private String doStringCommand(String command) {
        ...
        synchronized (mLock) {
            ...
            if (result == null) {
                if (DBG) WLog.v(mTAG, "doStringCommandNative no result");
                result = "";
            } else {
                if (!command.startsWith("STATUS-")) {
                    localLog(toLog + " -> " + result);
                }
                if (DBG) WLog.v(mTAG, command + ": returned " + result);
            }
            return result;
        }
    }
    ...
    private String doHostapdStringCommand(String command) {
        synchronized (mLock) {
            ...
            if (result == null) {
                if (DBG) WLog.v(mTAG, "doHostapdStringCommand no result");
                result = "";
            } else {
                if (!command.startsWith("STATUS-")) {
                    localLog(toLog + " -> " + result);
    ...
    private boolean doHostapdBooleanCommand(String command) {
        if (DBG) WLog.v(mTAG, "doHostapdBoolean: " + command);
    ...
    public boolean p2pSetChannel(int lc, int oc) {
        if (DBG) WLog.v(mTAG, "p2pSetChannel: lc="+lc+", oc="+oc);
    ...
    public String p2pGetDeviceAddress() {
        ...
        WLog.v(TAG, "p2pGetDeviceAddress");
        ...
        WLog.v(TAG, "p2pGetDeviceAddress returning " + result);
        return result;
    }
