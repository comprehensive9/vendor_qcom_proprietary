...
/* mask to reset the bit for ACTION SHUTDOWN event in current event status*/
#define ACTION_SHUTDOWN_MASK (0xFFFFFFDF)
...
Java_com_qualcomm_qti_services_systemhelper_SysHelperService_sendEventNotification(
    ...
    /* Reset the bit for ACTION SHUTDOWN event once it is sent to the clients
       who registered for it*/
    eventStatus = (eventStatus & ACTION_SHUTDOWN_MASK);
    return id;
}
