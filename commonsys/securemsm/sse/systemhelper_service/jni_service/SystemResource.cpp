...
// Orientation
#define ORIENTATION_270 "ORIENTATION_270"
#define ORIENTATION_90 "ORIENTATION_90"
#define SF_ORIENTATION "ro.surface_flinger.primary_display_orientation"
...
uint32_t SystemResource::getResourceId(void) {
  ...
  for (i = 0; i < MAX_RETRY; i++) {
    if ((resourceMap.find(rId) != resourceMap.end()) ||
        (surfaceMap.find(rId) != surfaceMap.end())) {
      rId = rand();
    } else {
      break;
    }
...
Return<void> SystemResource::acquireSurface(uint32_t width, uint32_t height,
  ...
  if (property_get(SF_ORIENTATION, property, NULL) > 0) {
    if (!strncmp(property, ORIENTATION_270, strlen(ORIENTATION_270))
        || !strncmp(property, ORIENTATION_90, strlen(ORIENTATION_90))) {
      t.setSize(sData->mControl, height, width);
    }
  }
