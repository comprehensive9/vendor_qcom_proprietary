...
public class IWlanNetworkService extends NetworkService {
    ...
    HashSet<IWlanNetworkServiceProvider> providerSet = new HashSet<>();
    ...
    private class IWlanNetworkServiceProvider extends NetworkServiceProvider {
        ...
        private int getNetworkTypeBasedOnRegState(int halRegState) {
            switch (halRegState) {
                case RegState.NOT_REG_MT_NOT_SEARCHING_OP_EM:
                case RegState.NOT_REG_MT_SEARCHING_OP_EM:
                case RegState.REG_DENIED_EM:
        ...
        private NetworkRegistrationInfo getRegistrationStateFromResult(IWlanDataRegStateResult result) {
            ...
            int accessNetworkTechnology = getNetworkTypeBasedOnRegState(result.regState);
        ...
        public void close() {
            ...
            if(isActive) {
                Log.d(TAG, "unregister with proxy");
                ...
                if(providerSet.contains(this)) {
                    ...
                    Log.d(TAG, "removing entry from providerSet");
                }
            }
        }
    ...
    public NetworkServiceProvider onCreateNetworkServiceProvider(int slotId) {
        ...
        IWlanNetworkServiceProvider provider = new IWlanNetworkServiceProvider(slotId);
    ...
    protected void dump(FileDescriptor fd, PrintWriter writer, String[] args){
        ...
        for(IWlanNetworkServiceProvider provider: providerSet) {
            if (provider != null)
    ...
    public void onDestroy() {
        Log.d(TAG, "IWlanNetworkService onDestroy called");
        super.onDestroy();
        HashSet<IWlanNetworkServiceProvider> tempSet = new HashSet<>();
        tempSet.addAll(providerSet);
        for(IWlanNetworkServiceProvider provider: tempSet) {
            provider.close();
        }
        providerSet.clear();
    }
