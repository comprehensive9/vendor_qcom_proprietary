...
public class QualifiedNetworksServiceImpl extends QualifiedNetworksService {
    ...
    HashSet<NetworkAvailabilityProviderImpl> providerSet = new HashSet<>();
    ...
    protected void dump(FileDescriptor fd, PrintWriter writer, String[] args){
        ...
        for(NetworkAvailabilityProviderImpl p: providerSet) {
        if (p != null)
           p.dump(writer);
        }
    }
    ...
    public void onDestroy() {
        Log.d(TAG, "QualifiedNetworksService onDestroy called");
        super.onDestroy();
        HashSet<NetworkAvailabilityProviderImpl> tempSet = new HashSet<>();
        tempSet.addAll(providerSet);
        for(NetworkAvailabilityProviderImpl p: tempSet) {
            p.close();
        }
        providerSet.clear();
    }
