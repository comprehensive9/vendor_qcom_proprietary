...
static int add_camera_metadata_entry_raw(camera_metadata_t *dst,
    ...
    assert(validate_camera_metadata_structure(dst, nullptr) == OK);
    return OK;
}
...
camera_metadata_t *clone_camera_metadata(const camera_metadata_t *src) {
    ...
    assert(validate_camera_metadata_structure(clone, nullptr) == OK);
    return clone;
}
...
camera_metadata_t *place_camera_metadata(void *dst,
    ...
    assert(validate_camera_metadata_structure(metadata, nullptr) == OK);
    return metadata;
}
...
namespace haltests {
    ...
    */
    ...
    void Camera3Metadata:: ResizeIfNeeded (size_t extraEntries, size_t extraData)
    {
        if (mBuffer == nullptr)
        {
        ...
        else
        {
            ...
            if (newEntryCount > currentEntryCap || newDataCount > currentDataCap)
            {
                ...
                NT_ASSERT(mBuffer != nullptr,"Can't allocate larger metadata buffer");
