...
public:
    ...
    static NativeTest* GetHalTestObject() { return m_pTestObj; };
...
extern bool Check(NativeTest *funcObj, bool passed, const char* fileName, int lineNumber, const char* condition,
    const char* errorFmt, ...);
extern int catchRunErrorSignal(int(*func)());
#define NT_ASSERT(_condition, _errorfmt, ...)\
    if (false == Check(NativeTest::GetHalTestObject(), _condition, __FILENAME__, __LINE__, #_condition, _errorfmt, ##__VA_ARGS__))\
        return
...
#define NT_EXPECT(_condition, _errorfmt, ...)\
    Check(NativeTest::GetHalTestObject(), _condition, __FILENAME__, __LINE__, #_condition, _errorfmt, ##__VA_ARGS__)
