    ...
    ChiFeature2Identifier* pKey)
{
    ...
    if ((NULL != pPipelineData) && (NULL != pPipelineData->pPipeline))
    {
        ...
        if (CDKResultSuccess == result)
        {
            ...
            std::vector<CHILINKNODEDESCRIPTOR> inputNodeList;
            CHINODELINK* pLinks   = m_pUsecaseDesc->pPipelineTargetCreateDesc[pKey->pipeline].pipelineCreateDesc.pLinks;
            UINT32       numLinks = m_pUsecaseDesc->pPipelineTargetCreateDesc[pKey->pipeline].pipelineCreateDesc.numLinks;
            for (UINT32 linkIdx = 0; linkIdx < numLinks; ++linkIdx)
            {
                if (pLinks[linkIdx].pDestNodes[0].nodeId == 255)
                {
                    inputNodeList.push_back(pLinks[linkIdx].pDestNodes[0]);
                }
            }

            // if snapshot sat node input port is NOT from source targets
            if (inputNodeList.size() >= m_pCameraInfo->numPhysicalCameras)
            {
                for (UINT32 configIdx = 0; configIdx < m_pCameraInfo->numPhysicalCameras; configIdx++)
                {
                    ...
                    pInputConfig->nodeDescriptor                   = inputNodeList[configIdx];
                    CHX_LOG_VERBOSE("map cameraid:%d, port:%d from internal link",
            ...
            else
            {
                for (UINT32 configIdx = 0; configIdx < m_pCameraInfo->numPhysicalCameras; configIdx++)
                {
                    ...
                    CHX_LOG_VERBOSE("map cameraid:%d, port:%d from source target",
                        pInputConfig->physicalCameraId, pInputConfig->nodeDescriptor.nodePortId);
                }
            }
