    ...
    ChiFeature2Identifier* pKey)
{
    ...
    if (CDKResultSuccess == result)
    {
        ...
        if ((NULL != pPipelineData) && (NULL != pInstanceProps))
        {
            ...
            if (NULL != pPipeline)
            {
                ...
                if (NULL != pMetadata)
                {
                    if (NULL != pSessionSettings)
                    {
                        ...
                        ChxUtils::AndroidMetadata::GetVendorTagValue(pSessionSettings, VendorTag::NumPCRsBeforeStreamOn,
                            reinterpret_cast<VOID**>(&pNumPCRsBeforeStreamOn));

                        if (NULL != pNumPCRsBeforeStreamOn)
                        {
                            result = pMetadata->SetTag("org.codeaurora.qcamera3.sessionParameters",
                                "numPCRsBeforeStreamOn", pNumPCRsBeforeStreamOn, 1);

                            if (CDKResultSuccess != result)
                            {
                                CHX_LOG_ERROR("Unable to set numPCRsBeforeStreamOn vendor tag in Usecase Pool for camera %d."
                                              " Result: %d",
                                              pInstanceProps->cameraId, result);
                            }
                        }

                        UINT32* pHDRvideoMode = NULL;

                        ChxUtils::AndroidMetadata::GetVendorTagValue(pSessionSettings, VendorTag::HDRVideoModeSession,
                            reinterpret_cast<VOID**>(&pHDRvideoMode));

                        if (NULL != pHDRvideoMode)
                        {
                            result = pMetadata->SetTag("org.codeaurora.qcamera3.sessionParameters",
                                "HDRVideoMode", pHDRvideoMode, 1);

                            if (CDKResultSuccess != result)
                            {
                                CHX_LOG_ERROR("Unable to set pHDRvideoMode tag in Usecase Pool for camera %d."
                                              " Result: %d",
                                              pInstanceProps->cameraId, result);
                            }
                        }
                    ...
                    if ((TRUE == pInstanceProps->instanceFlags.isBPSCamera))
                    {
                        BOOL cameraRunningOnBPS = TRUE;

                        result = pMetadata->SetTag("com.qti.chi.bpsrealtimecam", "cameraRunningOnBPS", &cameraRunningOnBPS,
                            sizeof(BOOL));

                        if (CDKResultSuccess != result)
                        {
                            CHX_LOG_ERROR("Unable to set BPS Vendor Tag in Usecase Pool for camera %d. Result: %d",
                                          pInstanceProps->cameraId, result);
                        }
                    }
                }
    ...
    StreamNegotiationOutput*        pNegotiationOutput)
{
    ...
    else
    {
        ...
        pNegotiationOutput->pDesiredStreamConfig->pSessionSettings = pNegotiationInfo->pFwkStreamConfig->pSessionSettings;
