    ...
    std::vector<FGDKeysForTable>* FeatureGraphDesc)
{
    ...
    for (auto &it : *FeatureGraphDesc)
    {
        ...
        {
            std::set<UINT> cameraIdSet = cameraMapIt.second;

            if ((cameraIdSet.find(pKeysForRequest->cameraId) != cameraIdSet.end()) && (it.cameraId == cameraMapIt.first))
            {
                cameraIdFound = TRUE;
            }
        }
    ...
    if (CDKResultSuccess == result)
    {
        ...
        if ((CDKResultSuccess == result) && (0 != CdkUtils::StrLen(pruneIDString)))
        {
            ...
            else
            {
                ...
                if (CDKResultSuccess != result || TRUE == pruneRules.empty())
                {
                    FreeGraphDescResources(pFinalGraphDesc);
                    pFinalGraphDesc = NULL;
                }
    ...
    ChiFeature2Hint*                 pCustomHints)
{
    ...
    if (NULL != pCustomHints)
    {
        ...
        UINT numberOfPCRsBeforeStreamOn = 0;
        if (NULL != m_pCameraInfo)
        {
            numberOfPCRsBeforeStreamOn = ExtensionModule::GetInstance()->GetNumPCRsBeforeStreamOn(m_pCameraInfo->cameraId);
        }

        pCustomHints->numEarlyPCRs = numberOfPCRsBeforeStreamOn;
    ...
    {
        CHX_LOG_ERROR("pCustomHints is NULL");
    }
}
    ...
    GraphDescriptorTables*           pGraphDescriptorTables)
{
    ...
    if (ANDROID_CONTROL_CAPTURE_INTENT_STILL_CAPTURE == keysForRequest.captureIntent)
    {
        ...
        keysForRequest.flags = GetNZSLSnapshotFlag(pAppSettings, pResultMeta, pChiRequest);
    }
    ...
    ChiFeature2NameMap&                    rFeatureNameToOpsMap)
{
    ...
    for (auto &it : *pGraphDescriptorTables->pCameraIdDescriptorNameSet)
    {
        for (auto &cameraMapIt : m_cameraIdMap)
        {
            ...
            if ((cameraIdSet.find(cameraId) != cameraIdSet.end()) && (it.first == cameraMapIt.first))
            {
    ...
    if (TRUE == m_isQCFAUsecase)
    {
        if (NULL != pResultMeta)
        {
            ...
            if (NULL != pFrameControl)
            {
                ...
                // If RAW stream is enabled from the framework and the requested size is greater
                // than the quadcfa binning size then irrespective of AEC gain threshold
                // NZSL pipeline needs to be enabled to meet the RAW framework request
                BOOL    isNZSLPipelineNeeded = FALSE;
                CHIRECT binning_size = { 0 };

                for (UINT i = 0; i < m_pCameraInfo->m_cameraCaps.numSensorModes; i++)
                {
                    if (1 == m_pCameraInfo->pSensorModeInfo[i].sensorModeCaps.u.QuadCFA)
                    {
                        binning_size.width  = m_pCameraInfo->pSensorModeInfo[i].frameDimension.width  >> 1;
                        binning_size.height = m_pCameraInfo->pSensorModeInfo[i].frameDimension.height >> 1;
                        break;
                    }
                }
                ...
                for (UINT i = 0; i < pChiRequest->numOutputs; i++)
                {
                    if ((ChiStreamFormatRaw10 == pChiRequest->pOutputBuffers[i].pStream->format) ||
                        (ChiStreamFormatRaw16 == pChiRequest->pOutputBuffers[i].pStream->format))
                    {
                        if ((binning_size.width < pChiRequest->pOutputBuffers[i].pStream->width) &&
                            (binning_size.height < pChiRequest->pOutputBuffers[i].pStream->height))
                        {
                            isNZSLPipelineNeeded = TRUE;
                            break;
                        }
                    }
                }

                CHX_LOG_VERBOSE("QCFA NZSL Pipeline needed - %d", isNZSLPipelineNeeded);

                if ((AECGainThreshold > curAecGain) ||
                    (TRUE == isNZSLPipelineNeeded))
                {
...
VOID ChiFeature2GraphSelector::BuildCameraIdSet()
{
    ...
    m_cameraIdMap.insert({ SINGLE_CAMERA,   { cameraIdSetSingle }  });
    m_cameraIdMap.insert({ MULTI_CAMERA,    { cameraIdSetMulti }   });
    m_cameraIdMap.insert({ BOKEH_CAMERA,    { cameraIdSetBokeh }   });
    m_cameraIdMap.insert({ FUSION_CAMERA,   { cameraIdSetFusion }  });

    for (auto &cameraMapIt : m_cameraIdMap)
    {
        UINT setType = cameraMapIt.first;
        for (auto &cameraId : cameraMapIt.second)
        {
            CHX_LOG_INFO("cameraId %u, set %u", cameraId, setType);
        }
    }
