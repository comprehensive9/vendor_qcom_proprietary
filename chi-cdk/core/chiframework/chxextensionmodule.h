...
public:
    ...
    /// IsFrameSyncEnabled
    ///
    /// @brief  Is frame sync enabled or not.
    ///
    /// @param  logicalCameraType The type of this logical camera
    ///
    ...
    /// IsFPSMatchEnabled
    ///
    /// @brief  Is FPS match enabled or not.
    ///
    /// @param  logicalCameraType The type of this logical camera
    ///
    /// @return True if FPS match enabled, False if not.
    ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    BOOL IsFPSMatchEnabled(
        LogicalCameraType logicalCameraType);
    ...
    CHX_INLINE UINT32 GetLogicalCameraId(UINT32 cameraId) const
    {
        return m_logicalCameraInfo[cameraId].cameraId;
    }
    ...
    UINT32 GetNumPCRsBeforeStreamOn(
            UINT32 logicalCameraId) const;
    ...
    /// Setting to get frame Sync scheme
    CHX_INLINE UINT32 GetMultiCameraFrameSyncMask() const
    {
        if (NULL != m_pMultiCameraFrameSyncMask)
        {
            return static_cast<UINT32>(*m_pMultiCameraFrameSyncMask);
        }
        return 0;
    }

    /// Setting to get FPS match scheme
    CHX_INLINE UINT32 GetMultiCameraFPSMatchMask() const
    {
        if (NULL != m_pMultiCameraFPSMatchMask)
        {
            return static_cast<UINT32>(*m_pMultiCameraFPSMatchMask);
        }
        return 0;
    }
    ...
    CHX_INLINE BOOL IsLongExpInProgress(UINT32 logicalCameraId)
    {
        if (logicalCameraId > (MaxNumImageSensors - 1))
        {
            return FALSE;
        }
        return static_cast<BOOL>(ChxUtils::AtomicLoadU32(&m_aLongExposureInProgress[logicalCameraId]));
    }
    ...
    CHX_INLINE VOID SetOriginalMetadata(const VOID* pMetadata, UINT32 cameraId)
    {
        m_pOriginalMetadata[cameraId] = pMetadata;
    }

    CHX_INLINE const VOID* GetOriginalMetadata(UINT32 cameraId)
    {
        return m_pOriginalMetadata[cameraId];
    }
...
private:
    ...
    const VOID*             m_pOriginalMetadata[MaxNumImageSensors];         ///< Original pointer to the metadata
    ...
    UINT32                  m_longExposureFrame[MaxNumImageSensors];         ///< Long exposure snapshot frameNumber
    volatile UINT32         m_aFlushInProgress[MaxNumImageSensors];          ///< Is flush in progress
    volatile UINT32         m_aLongExposureInProgress[MaxNumImageSensors];   ///< Is long exposure in progress
    ...
    UINT32*                         m_pMultiCameraFrameSyncMask;             ///< Indicates camera frame sync mask
    UINT32*                         m_pMultiCameraFPSMatchMask;              ///< Indicates camera FPS match mask
