...
UINT32           MultiCamControllerManager::m_vendorTagFrameSyncInfo;
UINT32           MultiCamControllerManager::m_vendorTagFPSMatchInfo;
...
MultiCamControllerManager::MultiCamControllerManager()
{
    ...
    s_vendorTagOps.pQueryVendorTagLocation("com.qti.chi.multicamerainfo",
                                           "SyncMode",
                                           &m_vendorTagFrameSyncInfo);

    s_vendorTagOps.pQueryVendorTagLocation("com.qti.chi.multicamerainfo",
                                           "FPSMatch",
                                           &m_vendorTagFPSMatchInfo);
    ...
    MulticamReqSettings* pMultiCamSettings)
{
    ...
    // Enable FPS match if override settings is enabled and if its in in DUAL ZONE
    if ((TRUE == pMultiCamSettings->sensorFPSMatchEnable) && (TRUE == isDualZoneActive))
    {
        isFPSMatchEnabled = TRUE;
    }

    result  = pMetadata->SetTag(MultiCamControllerManager::m_vendorTagFrameSyncInfo, &isSyncActive, sizeof(SyncModeInfo));
    result |= pTranslatedMetadata->SetTag(MultiCamControllerManager::m_vendorTagFrameSyncInfo, &isSyncActive, sizeof(SyncModeInfo));
    result |= pMetadata->SetTag(MultiCamControllerManager::m_vendorTagFPSMatchInfo, &isFPSMatchEnabled, sizeof(BOOL));
    result |= pTranslatedMetadata->SetTag(MultiCamControllerManager::m_vendorTagFPSMatchInfo, &isFPSMatchEnabled, sizeof(BOOL));

    CHX_LOG("isSyncActive %d isFPSMatchEnabled %d", isSyncActive, isFPSMatchEnabled);
    ...
    {
        CHX_LOG_ERROR("Failed to set metadata for frame sync and FPS match");
    }
}
    ...
    if (NULL != pMultiCamSettings)
    {
        ...
        if ((NULL != pMetadata) && (NULL != pTranslatedMetadata))
        {
             if (0 == pMultiCamSettings->frameNum)
            {
                ...
                {
                    CHX_LOG_ERROR("Failed to set tag for frame sync and fps match");
                }
    ...
    MulticamReqSettings* pMultiCamSettings)
{
    ...
    if (NULL != pMultiCamSettings)
    {
        ...
        CHX_LOG_INFO("isSyncActive %d isFPSMatchEnabled %d", isSyncActive, isFPSMatchEnabled);
        ...
        for (UINT32 i = 0; i < pMultiCamSettings->numSettingsBuffers; i++)
        {
            ...
            result  = SetMetadata(pMetadata, MultiCamControllerManager::m_vendorTagFrameSyncInfo,
                                 &isSyncActive, sizeof(SyncModeInfo));
            result |= SetMetadata(pMetadata, MultiCamControllerManager::m_vendorTagFPSMatchInfo,
                                 &isFPSMatchEnabled, sizeof(BOOL));

            result |= SetMetadata(pMetadata, MultiCamControllerManager::m_vendorTagLPMInfo,
                                 &lowPowerMode, sizeof(LowPowerModeInfo));
            if (CDKResultSuccess == result)
            {
                CHX_LOG_INFO("Update frame sync %d FPS match %d LPM %d",
                              isSyncActive, isFPSMatchEnabled,
                              lowPowerMode.lowPowerMode[i].isEnabled);
            }
            else
            {
                CHX_LOG_ERROR("Failed to set metadata for frame sync/FPS match/LPM mode");
            }
        }
    }
    return;
    ...
    MulticamReqSettings* pMultiCamSettings)
{
    ...
    if (NULL != pMultiCamSettings)
    {
        ...
        for (UINT32 i = 0; i < pMultiCamSettings->numSettingsBuffers; i++)
        {
            ...
            if (CDKResultSuccess == result)
            {
                CHX_LOG_INFO("Update frame sync %d FPS match %d",
                              isSyncActive, isFPSMatchEnabled);
            }
            else
            {
                CHX_LOG_ERROR("Failed to set metadata for frame sync/FPS match");
            }
        }
    }
    return;
