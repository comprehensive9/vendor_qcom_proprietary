    ...
    bool initializeMetadataManager)
{
    ...
    m_numPCRsBeforeStreamOn = 0; // Derived classes need to initialize this value using a valid cameraId
...
VOID Usecase::RestoreRequestMetadata(
    ...
    UINT32                     cameraId)
{
    pRequest->settings = static_cast<const camera_metadata_t*>(ExtensionModule::GetInstance()->GetOriginalMetadata(cameraId));
    ExtensionModule::GetInstance()->SetOriginalMetadata(NULL, cameraId);
}
    ...
    camera_metadata_t* pResultAndroidMeta)
{
    ...
    if (0 == find_camera_metadata_entry(pResultAndroidMeta, ANDROID_STATISTICS_FACE_LANDMARKS, &entry))
    {
    ...
    if (NULL != vendorTagOps.pQueryVendorTagLocation)
    {
        ...
        result = vendorTagOps.pQueryVendorTagLocation("org.codeaurora.qcamera3.stats", "contours", &metaTag);

        if (CDKResultSuccess == result)
        {
            if (0 == find_camera_metadata_entry(pResultAndroidMeta, metaTag, &entry))
            {
                for (UINT32 i = 0; i < entry.count; ++i)
                {
                    entry.data.i32[i] = entry.data.i32[i] >> 1;
