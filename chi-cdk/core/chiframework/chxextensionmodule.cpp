...
static LogicalCameraConfiguration logicalCameraConfiguration[] =
{
    ...
    {1,        LogicalCameraType_Default, TRUE,      1,    {{1,                    0.0, 0.0,   FALSE,    TRUE,     RealtimeEngineType_IFE}},  1   }, ///< Front camera
    {2,        LogicalCameraType_Default, TRUE,      1,    {{5,                    0.0, 0.0,   FALSE,    TRUE,     RealtimeEngineType_IFE}},  5   }, ///< Tele camera
    {3,        LogicalCameraType_Default, TRUE,      1,    {{4,                    0.0, 0.0,   FALSE,    TRUE,     RealtimeEngineType_IFE}},  4   }, ///< SAT
    {4,        LogicalCameraType_Default, TRUE,      1,    {{3,                    0.0, 0.0,   FALSE,    TRUE,     RealtimeEngineType_IFE}},  3   }, ///< RTB
    {5,        LogicalCameraType_Default, TRUE,      1,    {{2,                    0.0, 0.0,   FALSE,    TRUE,     RealtimeEngineType_IFE}},  2   }, ///< Extra Camera
    {6,        LogicalCameraType_Default, TRUE,      1,    {{7,                    0.0, 0.0,   FALSE,    TRUE,     RealtimeEngineType_IFE}},  7   },   ///< U+W+T FOV transition
    {7,        LogicalCameraType_Default, TRUE,      1,    {{8,                    0.0, 0.0,   FALSE,    TRUE,     RealtimeEngineType_IFE}},  8   }, //skyworthVR eye track left
    ...
    , m_terminateOfflineLogThread(FALSE)
{
    ...
    m_pMultiCameraFrameSyncMask       = NULL;
    m_pMultiCameraFPSMatchMask        = NULL;
    m_pBLMClient                      = NULL;
    ...
    for (UINT idx = 0; idx < MaxNumImageSensors; idx++)
    {
        m_longExposureFrame[idx]    = static_cast<UINT32>(InvalidFrameNumber);
    }
            ...
            if (NULL != m_hCHIContext)
            {
                ...
                if (NULL != vendorTagOps.pQueryVendorTagLocation)
                {
                    ...
                    m_pvtVendorTags[static_cast<UINT>(VendorTag::T2TResultROI)].pTagName
                        = "ResultROI";
                    m_pvtVendorTags[static_cast<UINT>(VendorTag::NumPCRsBeforeStreamOn)].pSectionName
                        = "org.codeaurora.qcamera3.sessionParameters";
                    m_pvtVendorTags[static_cast<UINT>(VendorTag::NumPCRsBeforeStreamOn)].pTagName
                        = "numPCRsBeforeStreamOn";
                    m_pvtVendorTags[static_cast<UINT>(VendorTag::HDRVideoModeSession)].pSectionName
                        = "org.codeaurora.qcamera3.sessionParameters";
                    m_pvtVendorTags[static_cast<UINT>(VendorTag::HDRVideoModeSession)].pTagName
                        = "HDRVideoMode";
                    m_pvtVendorTags[static_cast<UINT>(VendorTag::NumRawBatchedFramesTag)].pSectionName
                        = "org.codeaurora.qcamera3.sessionParameters";
                    m_pvtVendorTags[static_cast<UINT>(VendorTag::NumRawBatchedFramesTag)].pTagName
                        = "numRawBatchedFramesTag";
        ...
        if ((1 < pLogicalCam->numPhysicalCameras) && (LogicalCameraType_Default != logicalCameraType))
        {
            ...
            if (NULL != pLogicalCamInfo->conflicting_devices)
            {
                ...
                if (CDKResultSuccess == result)
                {
                    ...
                    memset(logicalMultiCamPhysicalIds, '\0', sizeof(CHAR) * MaxPhyIdsTagSize);
                    ...
                    for (UINT phyIndex = 0; phyIndex < pLogicalCam->numPhysicalCameras; ++phyIndex)
                    {
                        if (MaxPhyIdsTagSize > logicalMultiCamPhysicalIdSize + physicalIdString[phyIndex].size())
                        {
                            ...
                            if (phyIndex != (pLogicalCam->numPhysicalCameras - 1))
                            {
...
CDKResult ExtensionModule::UpdateStaticSettings(UINT32 logicalCameraId)
{
    ...
    if (CDKResultSuccess == result)
    {
        ...
        const CHAR* pTagNames[] = { "overrideResourceCostValidation", "availableStreamMap" ,
                                    "enableMCTFwithReferenceFrame", "numPCRsBeforeStreamOn",
                                    "HDRVideoMode", "numRawBatchedFramesTag"};
    ...
    for (UINT i = 0; i < numTokens; i++)
    {
        switch (pTokens[i].id)
        {
            ...
            case static_cast<UINT32>(ChxSettingsToken::MultiCameraHWSyncMask):
                ...
                break;
            case static_cast<UINT32>(ChxSettingsToken::MultiCameraFrameSyncMask):
                m_pMultiCameraFrameSyncMask          = &m_pConfigSettings[i];
                break;
            case static_cast<UINT32>(ChxSettingsToken::MultiCameraFPSMatchMask):
                m_pMultiCameraFPSMatchMask           = &m_pConfigSettings[i];
                break;
    ...
    const CHICAMERAINFO* pDeviceCaps)
{
    ...
    switch (logicalCameraType)
    {
        ...
        case LogicalCameraType_RTB:
            if (syncMask & MultiCameraSyncRTB)
            {
                enableHWSync = TRUE;
            }
            break;
        case LogicalCameraType_SAT:
            if (syncMask & MultiCameraSyncSAT)
            {
                enableHWSync = TRUE;
            }
            break;
        case LogicalCameraType_VR:
            if (syncMask & MultiCameraSyncVR)
            {
                enableHWSync = TRUE;
            }
            break;
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
UINT32 ExtensionModule::GetNumPCRsBeforeStreamOn(
    UINT32 logicalCameraId) const
{
    ...
    // If batch mode is disabled and the camxsetting is not enabled, check to see if vendor tag is set
    if ((0 == numPCRsBeforeStreamOn) && (1 >= m_usecaseNumBatchedFrames))
    {
        CHITAGSOPS tagOps      = { 0 };
        UINT32     tagLocation = 0;
        ...
        CDKResult tagOpResult = tagOps.pQueryVendorTagLocation("org.codeaurora.qcamera3.sessionParameters",
                                                               "numPCRsBeforeStreamOn",
                                                               &tagLocation);
        if (CDKResultSuccess == tagOpResult)
        {
            UINT32 numPCRsBeforeStreamOnTagVal = 0;

            camera_metadata_t* pSessionMetadata = const_cast<camera_metadata_t*>(m_pStreamConfig[logicalCameraId]->session_parameters);

            if (NULL != pSessionMetadata)
            {
                tagOpResult = tagOps.pGetMetaData(pSessionMetadata, tagLocation, &numPCRsBeforeStreamOnTagVal, sizeof(UINT32));
                if (CDKResultSuccess == tagOpResult)
                {
                    numPCRsBeforeStreamOn = numPCRsBeforeStreamOnTagVal;
                }
                else
                {
                    CHX_LOG_VERBOSE("Could not get numPCRsBeforeStreamOn vendor tag in metadata. result: %d", tagOpResult);
                }
            }
        }
    }
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// ExtensionModule::IsFrameSyncEnabled
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
BOOL ExtensionModule::IsFrameSyncEnabled(
    LogicalCameraType    logicalCameraType)
{
    // Here is the frame sync configuration for MTP multi camera solution. Customer need to
    // configure frame sync as their hardware design.

    BOOL           enableFrameSync = FALSE;
    UINT32         syncMask        = GetMultiCameraFrameSyncMask();
    ...
    switch (logicalCameraType)
    {
        ...
        case LogicalCameraType_MAX:
        default:
            CHX_LOG_ERROR("invalid logical camera type %u", logicalCameraType);
            break;
    };

    CHX_LOG_INFO("logical camera type %d enable frame sync %d.",
        logicalCameraType, enableFrameSync);
...
// ExtensionModule::IsFrameSyncEnabled
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
BOOL ExtensionModule::IsFPSMatchEnabled(
    LogicalCameraType    logicalCameraType)
{
    // Here is the sensor FPS match configuration for MTP multi camera solution. Customer need to
    // configure sensor FPS match mode as their hardware design.

    BOOL           enableFPSMatch = FALSE;
    UINT32         syncMask       = GetMultiCameraFPSMatchMask();
    ...
    CHX_LOG_INFO("logical camera type %d enable fps match %d.",
        logicalCameraType, enableFPSMatch);

    return enableFPSMatch;
}
