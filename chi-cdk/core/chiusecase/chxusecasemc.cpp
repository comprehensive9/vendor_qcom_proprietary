...
CDKResult UsecaseMultiCamera::ExecuteFlush()
{
    ...
    // If ExecuteCaptureRequest is running then wait
    m_pExecutionDoneMutex->Lock();
...
VOID UsecaseMultiCamera::Destroy(BOOL isForced)
{
    ...
    if (NULL != m_pExecutionDoneMutex)
    {
        m_pExecutionDoneMutex->Destroy();
        m_pExecutionDoneMutex = NULL;
    }
}
    ...
    ChiCaptureResult* pResult)
{
    ...
    // Releasing reference of output RDI metadata of request picked from ZSL queue for current snapshot request
    for (UINT32 rtIndex = 0 ; rtIndex < m_numOfPhysicalDevices; rtIndex++)
    {
        RequestMapInfo* pRequestInfo      = &(m_requestMapInfo[applicationFrameNumIndex]);
        ...
        if (INVALIDSEQUENCEID != snapshotRequestID)
        {
            ...
            for (UINT32 bufferIndex = 0; bufferIndex < pInputInfo->numOfBuffers; bufferIndex++)
            {
                if (NULL != pInputInfo->pRDIMetadata[rtIndex][bufferIndex])
                {
                    m_pMetadataManager->Release(pInputInfo->pRDIMetadata[rtIndex][bufferIndex]);
                    pInputInfo->pRDIMetadata[rtIndex][bufferIndex] = NULL;
                }
            }
        }
    }
        ...
        for (UINT32 pipelineIndex = 0; pipelineIndex < m_numOfPhysicalDevices; pipelineIndex++)
        {
                ...
                (FALSE == m_sessionInfo[REALTIME_SESSION].m_pSession->IsPipelineActive(pipelineIndex)))
            {
                ...
                CHX_LOG_INFO("NumPCR:%d reqFrameno:%d base frame num:%d",
                             m_numPCRsBeforeStreamOn,
                             pRequest->frame_number,
                             m_multiCamResource[pipelineIndex].m_baseFrameNumber);
                if ((pRequest->frame_number >= m_multiCamResource[pipelineIndex].m_baseFrameNumber) &&
                    (m_numPCRsBeforeStreamOn == (pRequest->frame_number - m_multiCamResource[pipelineIndex].m_baseFrameNumber)))
                {
    ...
    camera3_stream_configuration_t* pStreamConfig)
{
    ...
    for (UINT32 i = 0; i < m_numOfPhysicalDevices; i++)
    {
        ...
        if (FALSE == m_isSnapshotYUVNeeded)
        {
            ...
            // Assign target snapshot dimensions to YUV output of master camera in Bokeh so that Bokeh
            // processes two YUV inputs of different dimensions, one based on RDI and other based on
            // target snapshot dimensions, instead of processing inputs scaled to max RDI W X H.
            if (FALSE == ExtensionModule::GetInstance()->EnableMultiCameraJPEG())
            {
                pObj->m_pYUVOutputSnapshotStream->width = RDIMaxWidth;
                pObj->m_pYUVOutputSnapshotStream->height = RDIMaxHeight;
            }
            else if ((UsecaseRTB == m_usecaseMode) && (i == m_primaryCameraIndex) && (NULL != m_pTargetSnapshotStream))
            {
        ...
        if (NULL != m_pTargetSnapshotStream)
        {
            // Configure JPEG input same as Target snapshot dimension which is same as master camera
            // YUV output/Bokeh output dimension.
            if ((UsecaseRTB == m_usecaseMode) && (TRUE == ExtensionModule::GetInstance()->EnableMultiCameraJPEG()))
            {
        ...
        ChxUtils::Memcpy(pObj->m_pFusionInputStream, pObj->m_pYUVOutputSnapshotStream, sizeof(CHISTREAM));
        pObj->m_pFusionInputStream->streamType = ChiStreamTypeInput;
    }
    ...
    camera3_stream_configuration_t* pStreamConfig) ///< Stream configuration
{
    ...
    m_pExecutionDoneMutex      = Mutex::Create();
    ...
    m_kernelFrameSyncEnable = ExtensionModule::GetInstance()->IsFrameSyncEnabled(m_pLogicalCameraInfo->logicalCameraType);
    m_sensorFPSMatchEnable  = ExtensionModule::GetInstance()->IsFPSMatchEnabled(m_pLogicalCameraInfo->logicalCameraType);
    ...
    ControllerResult mccResult)
{
    ...
    for (UINT32 i = 0 ; i < nFrames; i++)
    {
        ...
        result = GenerateRealtimeRequest(&request, NULL, &RecoveryTriggered);
    }
}
    ...
    camera3_capture_request_t* pRequest)
{
    ...
    result  = m_pExecutionDoneMutex->TryLock();
    ...
    if (CDKResultSuccess == result)
    {
            ...
            (NULL != pFeature && FeatureType::Feature2 == pFeature->GetFeatureType()))
        {
                                                                     ...
                                                                     (FALSE == m_isB2YSnapshot)))
            {
                ...
                // We need to skip current request execute and return sucess here to make Framework go on.
                if (FALSE == RecoveryTriggered)
                {
            ...
            else
            {
                ...
                if (TRUE == m_isB2YSnapshot)
                {
                    ...
                    if (CDKResultSuccess == result)
                    {
                        ...
                        for (UINT32 i = 0; i < requestForFeature2.num_output_buffers; i++)
                        {
                            ...
                            for (UINT32 j = 0; j < m_numOfPhysicalDevices; j++)
                            {
                                if (pRequestOutputStream == m_multiCamResource[j].m_pTargetYUVStream)
                                {
                                    pRequestOutputStream->physicalCameraId =
                                        physicalCameraIdName[m_pLogicalCameraInfo->ppDeviceInfo[j]->cameraId];
                ...
                if (CDKResultSuccess == result)
                {
                    for (UINT32 i = 0; i < requestInfo.numOfRequest; i++)
                    {
                        ...
                        result = GenerateRealtimeRequest(&request, requestInfo.metadataInfo[i].pInputMetadata, &RecoveryTriggered);
                ...
                if ((NULL != pFeature) && canEnableAdvanceFeature())
                {
                       ...
                       ((TRUE == m_isLLSSnapshot) || (TRUE == m_isInSensorHDR3ExpCapture))))
                    {
                        ...
                        for (UINT32 i = 0; i < requestInfo.numOfRequest; i++)
                        {
                            ...
                            result = GenerateRealtimeRequest(&requestInfo.request[i], requestInfo.metadataInfo[i].pInputMetadata, &RecoveryTriggered);
    ...
    {
        CHX_LOG_WARN("UseCase is in Flush/Destroy State");
   ...
   BOOL*                      pRecoveryTriggered)
{
    ...
    for (UINT32 rtIndex = 0 ; rtIndex < m_numOfPhysicalDevices ; rtIndex++)
    {
        ...
        // Always set video usecaseMode, if Framework video stream is present.
        // This will make sure preview IQ during camcorder preview and camcroder recording will be same.
        if (TRUE == m_isVideoNeeded)
        {
            usecaseMode = ChiModeUsecaseSubModeType::Video;
        }
    ...
    if ((NULL != pRequest->settings) || (TRUE == LPMDecisionChanged))
    {
        ...
        multiCamSettings.sensorFPSMatchEnable     = m_sensorFPSMatchEnable;
    ...
    ChiMetadata*               pFeatureMetadata)
{
    ...
    if (FALSE == IsUsecaseInBadState())
    {
        ...
        {
            UINT32 isZSLMode                                = 0;
            ChiTuningModeParameter *pCHITuningModeParameter = NULL;
            ...
            if (NULL != m_snapshotInputInfo[snapshotReqIdIndex].pAppMetadata)
            {
                ...
                ChxUtils::GetVendorTagValue(m_snapshotInputInfo[snapshotReqIdIndex].pAppMetadata,
                          VendorTag::TuningMode, reinterpret_cast<VOID**>(&pCHITuningModeParameter));

                ChiModeUsecaseSubModeType usecaseMode = ChxUtils::GetUsecaseMode(pRequest);

                if (NULL != pCHITuningModeParameter)
                {
                    (*pCHITuningModeParameter).TuningMode[2].subMode.usecase    = usecaseMode;

                    ChxUtils::SetVendorTagValue(m_snapshotInputInfo[snapshotReqIdIndex].pAppMetadata,
                              VendorTag::TuningMode, sizeof(*pCHITuningModeParameter),
                              reinterpret_cast<VOID*>(pCHITuningModeParameter));

                }
                else
                {
                    CHX_LOG_ERROR("pCHITuningModeParameter is NUll");
                }
            }
            else
            {
                CHX_LOG_ERROR("pAppMetadata is NUll");
            }
        ...
        m_requestMapInfo[internalFrameNumIndex].snapshotRequestID  = m_maxSnapshotReqId;
        ...
        ChiCaptureResult* pResult)
{
    ...
    for (UINT32 i = 0; i < pResult->numOutputBuffers; i++)
    {
        if ((MaxDevicePerLogicalCamera > pipelineIndex) && (NULL != pResult->pOutputBuffers))
        {
            ...
            else if (IsRdiStream(pResult->pOutputBuffers[i].pStream))
            {
                rtBuffer     = const_cast<CHISTREAMBUFFER*>(&pResult->pOutputBuffers[i]);
                if(NULL != rtBuffer)
                {
                    CHIBufferManager* pBufferManager = GetBufferManager(pipelineIndex);
                    if (NULL != pBufferManager)
                    {
                        pBufferManager->ReleaseReference(&(rtBuffer->bufferInfo));

                        UINT32          bufferIndex   = internalFrameNum % RDIBufferQueueDepth;
                        TargetBuffer*   pTargetBuffer = GetTargetBufferPointer(m_RDIBufferID[pipelineIndex]);
                        if (NULL != pTargetBuffer)
                        {
                            pTargetBuffer->pMutex->Lock();
                            if (internalFrameNum == pTargetBuffer->bufferQueue[bufferIndex].frameNumber)
                            {
                            ...
                            CHX_LOG_INFO("releasing output RDI buffer");
                        }
                    }
                }
            }
