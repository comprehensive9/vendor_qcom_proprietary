    ...
    camera3_capture_request_t* pRequest)
{
    ...
    if (pRequest->settings != NULL)
    {
        ...
        // Set SyncInfo
        //TODO: Kernel frame sync and sensor fps match aren't verified with LPM yet.
        BOOL isFrameSyncActive = FALSE;
        BOOL isFPSMatchEnabled = FALSE;

        result = MultiCamControllerManager::s_vendorTagOps.pSetMetaData(const_cast<camera_metadata_t*>(pRequest->settings),
                                                                        MultiCamControllerManager::m_vendorTagFrameSyncInfo,
                                                                        &isFrameSyncActive,
                                                                        sizeof(SyncModeInfo));
        if (CDKResultSuccess != result)
        {
            CHX_LOG_ERROR("Request: frameNum :%d failed to set SyncActive tag %d",
                    pRequest->frame_number, isFrameSyncActive);
            return result;
        }

        result = MultiCamControllerManager::s_vendorTagOps.pSetMetaData(const_cast<camera_metadata_t*>(pRequest->settings),
                                                                        MultiCamControllerManager::m_vendorTagFPSMatchInfo,
                                                                        &isFPSMatchEnabled,
                                                                        sizeof(BOOL));
        if (CDKResultSuccess != result)
        {
            CHX_LOG_ERROR("Request: frameNum :%d failed to set SyncActive tag %d",
                    pRequest->frame_number, isFPSMatchEnabled);
            return result;
        }
