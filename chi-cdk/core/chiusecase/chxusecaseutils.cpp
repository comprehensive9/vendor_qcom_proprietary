    ...
    camera3_stream_configuration_t* pStreamConfig)
{
    ...
    if (pStreamConfig->num_streams <= ChiMaxNumTargets)
    {
        ...
        else if (((pStreamConfig->operation_mode & StreamConfigModeQTIMinLatencyYUV)
                == StreamConfigModeQTIMinLatencyYUV)
                && (NULL == pSelectedUsecase))
        {
            CHX_LOG("Only selecting YUV MinLatency usecases");
            switch (pStreamConfig->num_streams)
            {
            case 1:
                if (TRUE == UsecaseMatches(&Usecases1Target[UsecaseMinLatencyPreviewId]))
                {
                    pSelectedUsecase = &Usecases1Target[UsecaseMinLatencyPreviewId];
                }
                break;
            case 2:
                if (TRUE == UsecaseMatches(&Usecases2Target[UsecaseMinLatencyPreviewVideoId]))
                {
                    pSelectedUsecase = &Usecases2Target[UsecaseMinLatencyPreviewVideoId];
                }
                break;
            default:
                break;
            ...
            if (NULL == pSelectedUsecase)
            {
                CHX_LOG_ERROR("Failed to match with a YUV MinLatency usecase");
            }
        }
    ...
    for (/*Initialized outside*/; numTargetsExpected < ChiMaxNumTargets; numTargetsExpected++)
    {
        ...
        for (UINT i = 0; i < pChiTargetUsecases->numUsecases; i++)
        {
            ...
            if ((pChiUsecaseDescriptor == g_pUsecaseSecurePreview) ||
                (pChiUsecaseDescriptor == g_pUsecaseSecurePreviewVideo))
            {
                if (FALSE == secureMode)
                {
                    CHX_LOG_VERBOSE("Will not select Secure Usecase: %s", pChiUsecaseDescriptor->pUsecaseName);
                    continue; // Prevent selecting Secure usecase if above flag is disabled
                }
            }
    ...
    const ChiBufferFormat* pMatchingFormats)
{
    ...
    for (UINT32 i = 0; i < numFormats; i++)
    {
        ...
        else if ((ChiStreamFormatRaw8 == streamFormat) && (ChiFormatRawMIPI8 == pMatchingFormats[i]))
        {
            isMatching = TRUE;
        }
