    ...
    camera3_capture_request_t* pRequest)
{
    ...
    if (NULL != pUsecaseRequestObject)
    {
        ...
        else
        {
            ...
            CHX_LOG_WARN("FGM ExecuteProcessRequest cancelled as flash in progress:%d frameNumber=%d",
                          result,
                          pRequest->frame_number);
        }
        m_pFGMLock->Unlock();

        // We dont want to alter if the result is cancelled as we would like to send that back to Framework
