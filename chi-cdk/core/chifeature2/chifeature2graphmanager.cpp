    ...
    ChiFeature2UsecaseRequestObject* pRequestObject)
{
    ...
    else if ((TRUE == m_isFlush) || (TRUE == m_isTeardown))
    {
        CHX_LOG_WARN("Flush:%d teardown:%d", m_isFlush, m_isTeardown);
...
CDKResult ChiFeature2GraphManager::Flush()
{
    ...
    if (FALSE == m_isTeardown)
    {
             ...
             featureGraphIdx++)
        {
            ...
            {
                pFeautureGraph->Flush();
            }
        }

        result = m_pFeaturePoolManager->Flush(isSynchronousFlush);
        m_isFlush = FALSE;
    }
