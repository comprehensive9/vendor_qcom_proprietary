    ...
    if (NULL != pFeatureGraphCreateInputInfo)
    {
        ...
        if (NULL != pFeatureGraph)
        {
            ...
            if (TRUE == isValid)
            {
                ...
                if (CDKResultSuccess == result)
                {
                    pFeatureGraph->m_pShutterTimestampLock = Mutex::Create();

                    if (NULL == pFeatureGraph->m_pShutterTimestampLock)
                    {
                        result = CDKResultENoMemory;
                        CHX_LOG_ERROR("Cannot allocate ShutterTimestamp lock %p",
                                      pFeatureGraph->m_pShutterTimestampLock);
                    }
                }
            }
            else
...
VOID ChiFeature2Graph::Destroy()
{
    ...
    if (NULL != m_pShutterTimestampLock)
    {
        m_pShutterTimestampLock->Destroy();
        m_pShutterTimestampLock = NULL;
    }
...
{
    ChxUtils::AtomicStoreU32(&m_flushed, 1);

    CDKResult result       = CDKResultSuccess;
    INT32    totalNumNodes = m_pFeatureGraphNodes.size()-1;

    // Call Flush on Sink nodes first and then move upstream to the Src nodes at the end, so that the upstream nodes dont
    // submit request while the downstream nodes arent flushed.
    for (INT32 graphNodeIndex = totalNumNodes; graphNodeIndex >= 0; graphNodeIndex--)
    {
        ChiFeature2GraphNode* pGraphNode = (FALSE == m_pFeatureGraphNodes.empty()) ?
            m_pFeatureGraphNodes[graphNodeIndex] : NULL;

        if (NULL != pGraphNode)
        {

            for (UINT32 FROIndex = 0; FROIndex < pGraphNode->pFeatureRequestObjList.size(); ++FROIndex)
            {
                ChiFeature2RequestObject* pFRObj = pGraphNode->pFeatureRequestObjList[FROIndex];

                if (NULL != pFRObj)
                {
                    if (NULL != pGraphNode->pFeatureBaseObj)
                    {
                        pGraphNode->pFeatureBaseObj->ProcessFlush(pFRObj);
                    }

                    pFRObj->SetOutputPortsProcessingCompleteNotified();
            ...
            if (((ChiFeature2UsecaseRequestObjectState::Complete != m_pUsecaseRequestObj->GetRequestState()) &&
                 (ChiFeature2UsecaseRequestObjectState::Invalid != m_pUsecaseRequestObj->GetRequestState())) &&
                (TRUE == allFROsProcessingComplete))
            {
        ...
        else
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    if ((TRUE == static_cast<BOOL>(ChxUtils::AtomicLoadU32(&m_flushed))) ||
                        (CDKResultECancelledRequest == result))
                    {
                        result = CDKResultSuccess;
                        CHX_LOG_INFO("Flush in progress so moving the URO to complete state");
                        pUsecaseRequestObj->SetRequestState(ChiFeature2UsecaseRequestObjectState::Complete);
                    }
                }
            }
        }
            ...
            if (CDKResultSuccess == result)
            {
                switch (pMessages->chiNotification.pChiMessages->messageType)
                {
                    ...
                    case ChiMessageTypeShutter:
                        pFeatureGraph->m_pShutterTimestampLock->Lock();
                        ...
                        if (0 == pUsecaseRequestObject->GetShutterTimestamp())
                        {
                            ...
                            pFeatureGraph->m_pShutterTimestampLock->Unlock();
