////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// Copyright (c) 2019-20222 Qualcomm Technologies, Inc.
// All Rights Reserved.
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// ChiFeature2Base::ProcessFlush
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
CDKResult ChiFeature2Base::ProcessFlush (
    ChiFeature2RequestObject* pRequestObject)
{
    ...
    for (requestId = 0; requestId < numRequest; requestId++)
    {
        // Clear all the jobs (if any) and then move the FRO to complete and release all TBMs
        if (0 != m_hFeatureJob)
        {
            ChxUtils::AtomicStoreU32(&m_isFlushInProgress, 1);
            result = m_pThreadManager->FlushJob(m_hFeatureJob, FALSE);
            ChxUtils::AtomicStoreU32(&m_isFlushInProgress, 0);
        }

        if (CDKResultSuccess != result)
        {
            CHX_LOG_WARN("%s FRO jobs sync timed out with requestID %d error %d",
                pRequestObject->IdentifierString(), requestId, result);
            return result;
        }
        ...
        switch (requestState)
        {
            ...
            case ChiFeature2RequestState::ReadyToExecute:
                pRequestObject->SetCurRequestState(ChiFeature2RequestState::Flush, requestId);
            ...
            case ChiFeature2RequestState::OutputErrorNotificationPending:
                result = TriggerHardReleaseFeature(pRequestObject, requestId);
                if (CDKResultSuccess != result)
                {
                    CHX_LOG_WARN("TriggerHardReleaseFeature failed with result: %d", result);
                }
                break;
            default:
                break;
    ...
    {
        RemoveFeatureRequestObj(pRequestObject);
    }
        ...
        do
        {
            ...
            switch (requestState)
            {
                case ChiFeature2RequestState::Initialized:
                {
                    ...
                    {
                        AddFeatureRequestObj(pRequestObject);
                ...
                case ChiFeature2RequestState::Executing:
                {
                    ...
                    {
                        m_pExecuteProcessSem->WaitForSemaphore();
                        ...
                        m_pExecuteProcessSem->NotifyAllThreads();

                        m_pExecuteThreadDoneMutex->Lock();
                        m_pExecuteThreadDone->Signal();
                        m_pExecuteThreadDoneMutex->Unlock();
                    }
                    break;
                }
    ...
    BOOL isSynchronousFlush)
{
    ...
    if (TRUE == isSynchronousFlush)
    {
        ...
        BOOL isLongExposureInProgress =
             ExtensionModule::GetInstance()->IsLongExpInProgress(m_pCameraInfo->cameraId);

        // do not block flush from executing when long exposure capture is in progress
        // Reason : Suppose FrameRequest 1(longExposure 32 sec),2,3,4,5,6,7
        // Now Request 7 wait to acquireResource( in chifeature2realtime) before submitting
        // to session. If Flush came before any result is send back to framework, then flush
        // will wait. So in LongExposure case no need to wait.
        if (FALSE == isLongExposureInProgress)
        {
            // Calling DoFlush here inorder to reset Semaphore use by RealTimeFeature
            // Suppose PCR 'x' is waiting to AcquireResource in RealTimeFeature
            // During wait if Flush is called, No Need to wait for Resource. For that
            // we will call DoFlush which will reset semaphore.
            DoFlush();

            m_pExecuteThreadDoneMutex->Lock();

            while (m_pExecuteProcessSem->GetSemaphoreCount() <  m_pMaxHalReqCount)
            {
                CHX_LOG_INFO("SemaphoreCount Before Wait : %d", m_pExecuteProcessSem->GetSemaphoreCount());
                result = m_pExecuteThreadDone->TimedWait(m_pExecuteThreadDoneMutex->GetNativeHandle(), 100);
                if (CamxResultSuccess != result)
                {
                    CHX_LOG_WARN("TimedWait for HandleExecute time out at 100 ms");
                }
                CHX_LOG_INFO("SemaphoreCount After Wait : %d", m_pExecuteProcessSem->GetSemaphoreCount());
            }
        ...
        for (UINT8 curRequestId = 0; curRequestId < pRequestObject->GetNumRequests(); ++curRequestId)
        {
            ...
            if (ChiFeature2RequestState::InputResourcePendingScheduled == curRequestState)
            {
                    ...
                    ChiFeature2InputDependencyStatus::inputDependenciesErroredOut         == dependencyStatus)
                {
                    ...
                    if (NULL != pFeature)
                    {
                        if ((CDKResultSuccess == result) && (FALSE == pFeature->IsFeatureRequestObjActive(pRequestObject)))
                        {
                            result = CDKResultEFailed;
                            CHX_LOG_ERROR("%s: Failed: FRO Already destroyed request:%d",
                                            pFeature->GetFeatureName(), curRequestId);
                        }
    ...
    VOID*                      pCallbackdata)
{
    ...
    if (NULL != pCallbackdata)
    {
        ...
        ChiFeature2RequestObject*        pRequestObject    = pInflightCallback->pRequestObject;
        ...
        if ((NULL != pFeatureInstance) && (FALSE == pFeatureInstance->IsFeatureRequestObjActive(pRequestObject)))
        {
            CHX_LOG_ERROR("%s: FRO not available. Ignore Callback: :",
                pFeatureInstance->GetFeatureName());
            result = CDKResultEFailed;
        }

        if ((NULL != pPipelineData) && (NULL != pPortData) && (CDKResultSuccess == result))
        {
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// ChiFeature2Base::TriggerHardReleaseFeature
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
CDKResult ChiFeature2Base::TriggerHardReleaseFeature(
    ChiFeature2RequestObject* pRequestObject,
    UINT8                     requestId)
{
    CDKResult                           result             = CDKResultSuccess;
    ...
    {
        CHX_LOG_ERROR("%s No output ports requested numOutput %d pRequestOutputInfo %p", pRequestObject->IdentifierString(),
                      numRequestOutputs, pRequestOutputInfo);
        result = CDKResultEFailed;
    }
    ...
    if (CDKResultSuccess == result)
    {
        ...
        for (UINT8 portIndex = 0; portIndex < numRequestOutputs; ++portIndex)
        {
            ...
            if (NULL != pPortDescriptor)
            {
                pRequestObject->SetOutputNotifiedForPort(pPortDescriptor->globalId, requestId);
            }
        }
    ...
    if (CDKResultSuccess != result)
    {
        CHX_LOG_WARN("HandleOutputNotificationPending failed with result: %d", result);
    }
    ...
    UINT8                     requestId)
{
    ...
    if ((ChiFeature2RequestState::OutputNotificationPending      == pRequestObject->GetCurRequestState(requestId)) ||
        ...
        (ChiFeature2RequestState::Flush                          == pRequestObject->GetCurRequestState(requestId)))
    {
    ...
    ChiFeature2CreateInputInfo* pCreateInputInfo)
{
    ...
    m_pActivationMutex                       = Mutex::Create();
    ...
    if (CDKResultSuccess == result)
    {
        m_pMaxHalReqCount = ExtensionModule::GetInstance()->GetMaxHalRequests();
        if (60 <= ExtensionModule::GetInstance()->GetUsecaseMaxFPS())
        {
            m_pMaxHalReqCount += 1;
        }

        CHX_LOG_INFO("maxHalRequests : %d", m_pMaxHalReqCount);

        if (0 == m_pMaxHalReqCount)
        {
            m_pMaxHalReqCount = 1;
        }
        m_pExecuteProcessSem        = Semaphore::Create(m_pMaxHalReqCount);
        m_pExecuteThreadDone        = Condition::Create();
        m_pExecuteThreadDoneMutex   = Mutex::Create();
    }
...
VOID ChiFeature2Base::DestroyFeatureData()
{
    ...
    m_activeFROMapLock.lock();
    if (FALSE == m_activeFeatureRequestObjs.empty())
    {
        CHX_LOG_WARN("%s Few requests are still pending to be released: %zu",
            GetFeatureName(), m_activeFeatureRequestObjs.size());
    }
    m_activeFeatureRequestObjs.clear();
    m_activeFROMapLock.unlock();
    ...
    if (NULL != m_pActivationMutex)
    {
        m_pActivationMutex->Destroy();
        m_pActivationMutex = NULL;
    }

    if (NULL != m_pExecuteProcessSem)
    {
        m_pExecuteProcessSem->Destroy();
        m_pExecuteProcessSem = NULL;
    }

    if (NULL != m_pExecuteThreadDone)
    {
        m_pExecuteThreadDone->Destroy();
        m_pExecuteThreadDone = NULL;
    }

    if (NULL != m_pExecuteThreadDoneMutex)
    {
        m_pExecuteThreadDoneMutex->Destroy();
        m_pExecuteThreadDoneMutex = NULL;
    }
}
    ...
    VOID*               pPrivateCallbackData)
{
    ...
    if (CDKResultSuccess == result)
    {
        ...
        if ((NULL == pCombinedCbData) || (TRUE == m_destroyInProgress))
        {
        ...
        if (CDKResultSuccess == result)
        {
            std::vector<BOOL> isFROActive;
            isFROActive.reserve(pCombinedCbData->numCallbackData);

            for (UINT8 resultIndex = 0; resultIndex < pCombinedCbData->numCallbackData; ++resultIndex)
            {
                ChiFeature2RequestObject* pReqObj =
                    static_cast<ChiFeature2RequestObject*>(pCombinedCbData->pCombinedCallbackData[resultIndex]->pRequestObj);
                if (FALSE == IsFeatureRequestObjActive(pReqObj))
                {
                    CHX_LOG_ERROR("%s: FRO not available. Ignore Result: :%d Seq:%d stage:%d stageSeq:%d",
                        GetFeatureName(),
                        pCombinedCbData->pCombinedCallbackData[resultIndex]->requestId,
                        pCombinedCbData->pCombinedCallbackData[resultIndex]->sequenceId,
                        pCombinedCbData->pCombinedCallbackData[resultIndex]->stageId,
                        pCombinedCbData->pCombinedCallbackData[resultIndex]->stagesequenceId);
                    isFROActive[resultIndex] = FALSE;
                }
                else if (TRUE == pCombinedCbData->pCombinedCallbackData[resultIndex]->pOutputPorts.empty())
                {
                    CHX_LOG_ERROR("%s: Outputs already processed. Ignore Result: :%d Seq:%d stage:%d stageSeq:%d",
                        pReqObj->IdentifierString(),
                        pCombinedCbData->pCombinedCallbackData[resultIndex]->requestId,
                        pCombinedCbData->pCombinedCallbackData[resultIndex]->sequenceId,
                        pCombinedCbData->pCombinedCallbackData[resultIndex]->stageId,
                        pCombinedCbData->pCombinedCallbackData[resultIndex]->stagesequenceId);
                ...
                {
                    isFROActive[resultIndex] = TRUE;
                }
            }

            INT numCallbacks = pCombinedCbData->numCallbackData;
            for (UINT8 resultIndex = 0; ((resultIndex < numCallbacks) && (TRUE == isFROActive[resultIndex])); ++resultIndex)
            {
...
VOID ChiFeature2Base::OnProcessingDependenciesComplete(
    ...
    UINT8                     requestId)
{
    BOOL                            isAllRequestsCompleted  = TRUE;
    const ChiFeature2InstanceProps* pInstanceProps          = GetInstanceProps();
    ...
    if (FALSE == pRequestObject->GetOutputPortsProcessingCompleteNotified())
    {
        ...
        {
            if (ChiFeature2RequestState::Complete != pRequestObject->GetCurRequestState(reqId))
            {
        ...
        // and all the Requests FRO states are Complete then send the notification

        if ((TRUE == pRequestObject->AllOutputPortsProcessingComplete()) &&
            ((pRequestObject->GetNumRequests() - 1) == requestId) && (TRUE == isAllRequestsCompleted))
        {
            ...
            if ((TRUE == pInstanceProps->instanceFlags.isNZSLSnapshot) &&
                ((TRUE == pInstanceProps->instanceFlags.isSWRemosaicSnapshot) ||
                 (TRUE == pInstanceProps->instanceFlags.isHWRemosaicSnapshot)) &&
                (pRequestObject->GetFeature()->GetFeatureId() == static_cast<UINT32>(ChiFeature2Type::REALTIME)))
            {
                RecycleTargetBuffers(pRequestObject, requestId);
            }
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// ChiFeature2Base::RecycleTargetBuffers()
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
VOID ChiFeature2Base::RecycleTargetBuffers(
    ChiFeature2RequestObject* pRequestObject,
    UINT8                     requestId)
{
    CDKResult                               result                    = CDKResultSuccess;
    ...
    if (CDKResultSuccess == result)
    {
        for (UINT8 portIndex = 0; portIndex < numRequestOutputs; ++portIndex)
        {
            ...
            if (NULL != pPortDescriptor && NULL != pBufferMetaInfo)
            {
                ...
                if (TRUE == isPortNotified && TRUE == isPortReleased)
                {
                    // Release buffers
                    CHITargetBufferManager* pManager = CHITargetBufferManager::GetTargetBufferManager(
                                                                                pBufferMetaInfo->hBuffer);
