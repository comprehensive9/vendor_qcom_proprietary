        ...
        switch (portIdentifier.portType)
        {
            case ChiFeature2PortType::ImageBuffer:
            {
                    ...
                    ((0 != m_GPURotation) && (0 != portIdentifier.port)))
                {
                    ...
                    if (NULL == m_hInputImageBuffer)
                    {
                        CHX_LOG_ERROR("Setup target buffer for seqId %d on TBM %p failed!",
                                m_frameNumber, m_hInputImageBuffer);
                        result = CDKResultEFailed;
                    }
                    else
                    {
                        // Update status to ready
                        m_pInputImageTBM->UpdateTarget(seqId, (UINT64)(&m_yuv2JpegStreamsInput),
                                m_inputBuffer.bufferInfo.phBuffer, ChiTargetStatus::Ready, &m_hInputImageBuffer);
                    }
            ...
            case ChiFeature2PortType::MetaData:
            {
                ...
                if (NULL == m_hMetaBuffer)
                {
                    CHX_LOG_ERROR("Setup target buffer for seqId %d on TBM %p failed!",
                            m_frameNumber, m_hMetaBuffer);
                    result = CDKResultEFailed;
                }
                else
                {
                    // Update status to ready
                    m_pMetadataTBM->UpdateTarget(seqId, static_cast<UINT64>(clientId),
                            pMetadata, ChiTargetStatus::Ready, &m_hMetaBuffer);
                }
    ...
    for (UINT i = 0; i < pMessages->pFeatureMessages->message.result.numPorts; ++i)
    {
        ...
        if (ChiFeature2PortType::ImageBuffer == portIdentifier.portType)
        {
            if (NULL != pTargetBufferManager)
            {
                ...
                m_pOutputImageTBM->UpdateTarget(m_frameNumber, (UINT64)(&m_yuv2JpegStreamsOutput),
                    m_outputBuffer.bufferInfo.phBuffer, ChiTargetStatus::Ready, NULL);
