...
#if (TARGET_SUPPORTS_WEARABLES == TRUE)
#undef WLAN_BT_PROFILE_PATH
#define WLAN_BT_PROFILE_PATH "/sys/devices/platform/soc/c800000.qcom,icnss/bt_profile"
#endif
...
#if (TARGET_SUPPORTS_WEARABLES == TRUE)
    if (open(WLAN_BT_PROFILE_PATH, O_WRONLY) >= 0){
      ret = property_set("persist.vendor.qcom.bluetooth.soc", "cherokee");
      if (ret < 0) {
        printf("%s: set soc name property failed error = %d\n",__func__, ret);
      }
    }
#endif
