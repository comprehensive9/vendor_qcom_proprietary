...
Copyright (c) 2007-2019, 2021 Qualcomm Technologies, Inc.
All Rights Reserved.
...
#define MAX_PCIE_SWITCH_RETRY	2
...
retry_mode_switch:
		if (pcie_test) {
			...
			DIAG_LOGE("test_diag: Request to switch %s mode\n",
			(pcie_test == PCIE_MODE ? "PCIE": "USB"));

			diag_switch_logging(pcie_test, NULL);

			DIAG_LOGE("test_diag: present logging mode: %s\n",
			(logging_mode == PCIE_MODE ? "PCIE": "USB"));

			if (logging_mode != pcie_test) {
				if (pcie_retry_count < MAX_PCIE_SWITCH_RETRY) {
					DIAG_LOGE("test_diag: waiting for 45s\n");
					sleep(45);
					pcie_retry_count++;
					goto retry_mode_switch;
				} else {
					DIAG_LOGE("test_diag: Exiting with logging mode: %d, pcie_retry_count: %d\n",
							logging_mode, pcie_retry_count);
					Diag_LSM_DeInit();
					free(test_info);
					return 0;
				}
