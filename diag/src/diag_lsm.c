...
void diag_print_debug_info(unsigned char *ptr)
{
	...
	for (i = 0; i < (NUM_PERIPHERALS + 1); i++) {
		printf("stm_support[%d]: %d\t", i, debug_all_params.stm_support[i]);
		printf("stm_state_requested[%d]: %d\t", i, debug_all_params.stm_state_requested[i]);
		printf("stm_state[%d]: %d\n", i, debug_all_params.stm_state[i]);
	}
	...
	for (i = 0; i < NUM_MD_SESSIONS; i++) {
		printf("buffering_mode[%d].peripheral: %d\t", i, debug_all_params.buffering_mode[i].peripheral);
		printf("buffering_mode[%d].mode: %d\t", i, debug_all_params.buffering_mode[i].mode);
		printf("buffering_mode[%d].high_wm_val: %d\t", i, debug_all_params.buffering_mode[i].high_wm_val);
		printf("buffering_mode[%d].low_wm_val: %d\t", i, debug_all_params.buffering_mode[i].low_wm_val);
		printf("buffering_flag[%d]: %d\n", i, debug_all_params.buffering_flag[i]);
	}
...
static void query_reg_table_mask(char *ptr)
{
	...
	if (reg_tbl_cnt == 0) {
		...
		printf("Client IDs: Modem: %d, LPASS: %d, WCNSS: %d, SLPI: %d, WDSP: %d, CDSP: %d, NPU: %d, NSP1: %d, GPDSP0: %d, GPDSP1: %d, RESERVED: %d\n",
...
void diag_register_callback(int (*client_cb_func_ptr)(unsigned char *ptr, int len, void *context_data), void *context_data)
{
	...
	err = diag_lsm_comm_ioctl(diag_fd, DIAG_IOCTL_REGISTER_CALLBACK, &reg, 0);
	if (err)
...
int diag_vote_md_real_time_proc(int proc, int real_time)
{
	...
	ret = diag_lsm_comm_ioctl(diag_fd, DIAG_IOCTL_VOTE_REAL_TIME, &vote, 0);
	return ret;
}
...
int diag_get_real_time_status_proc(int proc, int *real_time)
{
	...
	query.proc = proc;
	err = diag_lsm_comm_ioctl(diag_fd, DIAG_IOCTL_GET_REAL_TIME, &query, 0);
	if (err != 0) {
...
boolean Diag_LSM_DeInit(void)
{
	...
	time.tv_nsec = (now.tv_usec * 1000) + (200 * 1000 * 1000);
	...
	if (diag_fd != DIAG_INVALID_HANDLE) {
		...
		DIAG_LOGE("diag: %s: Read status in CreateWaitThread, in_read: %d\n", __func__, in_read);
		...
		DIAG_LOGE("diag: %s: status of deinit_rcvd: %d, sig_rcvd: %d\n",
			__func__, deinit_rcvd, sig_rcvd);
		...
		DIAG_LOGE("diag: %s: Joining the read thread, deinit_rcvd: %d, sig_rcvd: %d\n", __func__, deinit_rcvd, sig_rcvd);
		deinit_rcvd = 0;
		sig_rcvd = 0;
		ret = pthread_join(read_thread_hdl, NULL);
		if (ret != 0) {
			DIAG_LOGE("diag: %s: Error trying to join with thread: %d\n",
					  __func__, ret);
			bReturn = FALSE;
		}
		...
		if (flush_log) {
			...
			if (!num_bytes_read) {
				if (diag_use_dev_node) {
					...
					DIAG_LOGE("diag: %s: Invalid read of %d bytes\n",
			...
			if (num_bytes_read < 0) {
				if (diag_use_dev_node)	{
					...
					if (flush_log) {
						...
						DIAG_LOGE("diag: %s: Invalid read with error: %d\n",
							__func__, errno);
						break;
					}
				}
