/*====*====*====*====*====*====*====*====*====*====*====*====*====*====*====*
# Copyright (c) 2012-2014,2016,2018,2020,2022 Qualcomm Technologies, Inc.
# All Rights Reserved.
...
int diag_get_dci_support_list_proc(int proc, diag_dci_peripherals *list)
{
	...
	p_list.proc = proc;
	err = diag_lsm_comm_ioctl(diag_fd, DIAG_IOCTL_DCI_SUPPORT, &p_list, 0);
	if (err == DIAG_DCI_NO_ERROR)
		*list = p_list.list;
...
int diag_get_health_stats_proc(int client_id, struct diag_dci_health_stats *dci_health, int proc)
{
	...
	err = diag_lsm_comm_ioctl(diag_fd, DIAG_IOCTL_DCI_HEALTH_STATS, &health_proc, 0);
	if (err == DIAG_DCI_NO_ERROR) {
...
int diag_get_log_status(int client_id, uint16 log_code, boolean *value)
{
	...
	stats.code = log_code;
	stats.is_set = 0;
	err = diag_lsm_comm_ioctl(diag_fd, DIAG_IOCTL_DCI_LOG_STATUS, &stats, sizeof(struct diag_log_event_stats));
	if (err != DIAG_DCI_NO_ERROR)
		return DIAG_DCI_SEND_DATA_FAIL;
	else
...
int diag_get_event_status(int client_id, uint16 event_id, boolean *value)
{
	...
	stats.code = event_id;
	stats.is_set = 0;
	err = diag_lsm_comm_ioctl(diag_fd, DIAG_IOCTL_DCI_EVENT_STATUS, &stats, sizeof(struct diag_log_event_stats));
	if (err != DIAG_DCI_NO_ERROR)
		return DIAG_DCI_SEND_DATA_FAIL;
	else
		*value = (stats.is_set == 1) ? TRUE : FALSE;
...
int diag_disable_all_logs(int client_id)
{
	...
	if (!IS_VALID_DCI_PROC(proc))
		return DIAG_DCI_NOT_SUPPORTED;
	ret = diag_lsm_comm_ioctl(diag_fd, DIAG_IOCTL_DCI_CLEAR_LOGS, &client_id, sizeof(int));
	if (ret != DIAG_DCI_NO_ERROR) {
...
int diag_disable_all_events(int client_id)
{
	...
	if (!IS_VALID_DCI_PROC(proc))
		return DIAG_DCI_NOT_SUPPORTED;
	ret = diag_lsm_comm_ioctl(diag_fd, DIAG_IOCTL_DCI_CLEAR_EVENTS, &client_id, sizeof(int));
	if (ret != DIAG_DCI_NO_ERROR) {
...
int diag_dci_vote_real_time(int client_id, int real_time)
{
	...
	err = diag_lsm_comm_ioctl(diag_fd, DIAG_IOCTL_VOTE_REAL_TIME, &vote, 0);
	if (err == -1) {
