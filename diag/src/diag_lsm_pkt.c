...
Copyright (c) 2007-2011, 2013-2016, 2018, 2020, 2022 Qualcomm Technologies, Inc.
All Rights Reserved.
...
static uint16 diagpkt_get_delayed_rsp_id_lsm(void)
{
	...
	err = diag_lsm_comm_ioctl(diag_fd, DIAG_IOCTL_GET_DELAYED_RSP_ID, &delayed_rsp_id, 0);
	if (err) {
		delayed_rsp_id = 0;
...
boolean Diag_LSM_Pkt_DeInit(void)
{
	...
	for (walk_ptr = head_ptr->next; next = walk_ptr->next,
	     walk_ptr && walk_ptr != head_ptr; walk_ptr = next)
...
void diagpkt_delay_commit (void *pkt)
{
	...
	if (pkt_len > 0 && -1 != diag_fd) {
		...
		if (is_dci_pkt) {
			...
			num_bytes = diag_lsm_comm_write(diag_fd, temp, rsp_len, 0);
			...
			num_bytes = diag_lsm_comm_write(diag_fd, (unsigned char*)pdiag_data, pkt_len + DIAG_REST_OF_DATA_POS, 0);
		}
		if(num_bytes != 0) {
...
void diagpkt_commit (void *pkt)
{
	...
	if ((ret = diag_lsm_comm_write(diag_fd, temp, DIAG_REST_OF_DATA_POS + rsp_length, 0)) != 0) {
		DIAG_LOGE("Diag_LSM_Pkt: Write failed in %s, bytes written: %d, error: %d\n", __func__, ret, errno);
		gPkt_commit_fail++;
	}
	DiagSvc_Free(temp, PKT_SVC_ID);
