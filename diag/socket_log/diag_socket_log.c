...
Copyright (c) 2012-2014, 2016-2022 Qualcomm Technologies, Inc.
All Rights Reserved.
...
static struct diag_con_all_param_t g_params_con;
static int mask_clear = 0;
...
void usage (char *progname)
{
	...
	printf("\n-m, --mask_clear:\t mask clear with sleep in sec before falling back to USB mode\n");
	printf("\n-h, --help:\t usage help\n");
...
void parse_args(int argc, char **argv)
{
	...
	while ((command = getopt_long(argc, argv, "a:c:f:p:r:m:kh", longopts,
					NULL)) != -1) {
		switch (command) {
		case 'a':
...
void clear_diag_mask(void)
{
	...
	if (mask_clear) {
		...
		sleep(sleep_cnt);
		DIAG_LOGE("diag: Clear mask command done\n");
	}
}
...
void close_connection(struct diag_socket *socket)
{
	...
	if (keep_logging) {
		...
		params.peripheral_mask = g_params_con.diag_con_all;
		params.pd_mask = 0;

		status = diag_switch_logging_proc(&params);
		if (status) {
			DIAG_LOGE("diag_socket_log:%s: Switch to USB mode fail with error %d\n",
			__func__, status);
		}
	}
...
int main(int argc, char *argv[])
{
	...
	params_con.num_peripherals = NUM_PERIPHERALS;
	...
	g_params_con.diag_con_all = params_con.diag_con_all;
	g_params_con.num_peripherals = params_con.num_peripherals;
	g_params_con.upd_map_supported = params_con.upd_map_supported;

	z = 1;
	diag_lsm_comm_ioctl(diag_fd_temp, DIAG_IOCTL_REMOTE_DEV, &remote_mask, 0);
	while(remote_mask) {
		if(remote_mask & 1 ) {
			proc_type = z;
	...
	query_pid.pd_mask = 0;
	query_pid.pid = 0;
	err = diag_lsm_comm_ioctl(diag_fd_temp, DIAG_IOCTL_QUERY_MD_PID, &query_pid, sizeof(struct diag_query_pid_t));
	if(err)
		DIAG_LOGE("diag: Kernel does not support PID query, err: %d, errno: %d\n",
			 err, errno);
	DIAG_LOGE("diag_socket_log: Closing diag_fd_temp\n");
