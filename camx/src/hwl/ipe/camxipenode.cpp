    ...
    UINT64          requestId)
{
        ...
        (0    != (IPEStabilizationType::IPEStabilizationTypeEIS3 & m_instanceProperty.stabilizationType))))
    {
        ...
        FLOAT cropFactorW           = static_cast<FLOAT>(pCropInfo->width) / fullInputWidth;
        FLOAT cropFactorH           = static_cast<FLOAT>(pCropInfo->height) / fullInputHeight;
        ...
        pCropInfo->width  = static_cast<INT32>((adjustedAdditionalCropWidth  * cropFactorW) + 0.5);
        pCropInfo->height = static_cast<INT32>((adjustedAdditionalCropHeight * cropFactorH) + 0.5);
        pCropInfo->left   = static_cast<INT32>((adjustedAdditionalCropWidth  * cropFactorOffsetLeft) +
                                               (m_additionalCropOffset.widthPixels / 2) + 0.5);
        pCropInfo->top    = static_cast<INT32>((adjustedAdditionalCropHeight * cropFactorOffsetTop) +
                                               (m_additionalCropOffset.heightLines / 2) + 0.5);

        CAMX_LOG_VERBOSE(CamxLogGroupPProc, "ZDBG: After IPE ICA1 Zoom Window [%d, %d, %d, %d] full %d x %d "
                         "crop_factor [%f %f], leftOffsetCropFactor %f, topOffsetCropFactor %f, Stabilization type %d,"
    ...
    ISPInputData* pModuleInput)
{
    ...
    CAMX_LOG_VERBOSE(CamxLogGroupPProc, "IPE:%d partialMwbMode %d", InstanceID(), pModuleInput->pHALTagsData->partialMwbMode);
    ...
    ExecuteProcessRequestData* pExecuteProcessRequestData)
{
    ...
    if (1 == sequenceNumber)
    {
        ...
        if (CamxResultSuccess == result)
        {
            ...
            moduleInput.pCalculatedData->toneMapData.tonemapMode = moduleInput.pHALTagsData->tonemapCurves.tonemapMode;
        }
...
CamxResult IPENode::AcquireDevice()
{
    ...
    if (CamxResultSuccess == result)
    {
        ...
        {
            CAMX_LOG_VERBOSE(CamxLogGroupPProc, "Avoided SetupDeviceResource since pVirtualAddr:%p hHandle%p",
                m_configIOMem.pVirtualAddr, m_configIOMem.hHandle);
        }
        ...
        if (CamxResultSuccess == result)
        {
            m_IPEClockEfficiency = GetIPEClockEfficiency(
                m_realTimeIPE,
                m_instanceProperty.profileId,
                m_instanceProperty.processingType,
                m_instanceProperty.stabilizationType);
            CAMX_LOG_INFO(CamxLogGroupPProc, "%s: profile %d, stab %d, processType %d, IPEClockEfficiency %f numIPE %d",
                NodeIdentifierString(),
                m_instanceProperty.profileId,
                m_instanceProperty.stabilizationType,
                m_instanceProperty.processingType,
                m_IPEClockEfficiency,
                m_capability.numIPE);
        }
    }
    else
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
/// IPENode::GetIPEClockEfficiency
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
FLOAT IPENode::GetIPEClockEfficiency(
    UINT32 realtime,
    UINT32 profileID,
    UINT32 processingType,
    UINT32 stabilizationType)
{
    CAMX_UNREFERENCED_PARAM(stabilizationType);
    CAMX_UNREFERENCED_PARAM(processingType);

    FLOAT efficiency = 1.0f;
    if (TRUE == realtime)
    {
        efficiency = (0 != ((static_cast<Titan17xContext*>(GetHwContext())->GetTitan17xSettingsManager()->
        GetTitan17xStaticSettings()->perPassClockBWCalculationEnable) & 0x1)) ?
            m_capability.realtimeClockEfficiencyNoOverhead :
            m_capability.realtimeClockEfficiencyOverhead;
    }
    else
    {
        switch (profileID)
        {
            case IPEProfileIdDefault:
            case IPEProfileIdPPS:
            case IPEProfileIdNoZoomCrop:
            case IPEProfileIdHDR10:
                efficiency = (0 != ((static_cast<Titan17xContext*>(GetHwContext())->GetTitan17xSettingsManager()->
                    GetTitan17xStaticSettings()->perPassClockBWCalculationEnable) & 0x1)) ?
                    m_capability.nonrealtimeClockEfficiencyNoOverhead :
                    m_capability.nonrealtimeClockEfficiencyOverhead;
                break;
            default:
                // for any profile without HNR/LENR we can use RT efficiency.
                efficiency = (0 != ((static_cast<Titan17xContext*>(GetHwContext())->GetTitan17xSettingsManager()->
                    GetTitan17xStaticSettings()->perPassClockBWCalculationEnable) & 0x1)) ?
                    m_capability.realtimeClockEfficiencyNoOverhead :
                    m_capability.realtimeClockEfficiencyOverhead;
    ...
    UINT numPasses)
{
    ...
    CAMX_LOG_INFO(CamxLogGroupPProc, "[%s]: m_numPasses=%d, processingtype %d, profile %d, stabilizationtype %d"
                     "node with bps %d stream type %d, decided Realtime %d",
                     NodeIdentifierString(),
                     numPasses,
                     m_instanceProperty.processingType,
                     m_instanceProperty.profileId,
                     m_instanceProperty.stabilizationType,
                     IsNodeInPipeline(BPS),
                     GetStreamProcessType(),
                     isRealTime);
    return isRealTime;
}
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// IPENode::DumpDebugInternalNodeInfo
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
VOID IPENode::DumpDebugInternalNodeInfo(
    UINT64  requestId,
    BOOL    isPerFrameDump)
{
