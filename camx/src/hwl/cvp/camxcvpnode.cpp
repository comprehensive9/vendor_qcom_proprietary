...
VOID CVPNode::Cleanup()
{
    ...
    if ((NULL != m_pSourceContextBufferManager) && (NULL != m_ppSourceContextBuffers))
    {
        ...
        DeleteBufferManager(m_pSourceContextBufferManager, m_ppSourceContextBuffers, m_srcContextBufferCount);
    ...
    if ((NULL != m_pGridBufferManager) && (NULL != m_ppGridBuffers))
    {
        ...
        DeleteBufferManager(m_pGridBufferManager, m_ppGridBuffers, m_gridBufferCount);
        ...
        else
        {
            ...
            if (TRUE == m_bICAEnabled)
            {
                if (CamxResultSuccess == result)
                {
                    ...
                    UINT32 requestIdIndex = static_cast<UINT32>(requestId) % m_gridBufferCount;
                ...
                for (UINT inputIndex = 0; inputIndex < pPerRequestPorts->numInputPorts; inputIndex++)
                {
                    ...
                    switch(pInputPort->portId)
                    {
                        case CVPInputPortTARIFEDS4:
                            ...
                            {
                                CAMX_LOG_ERROR(CamxLogGroupCVP, "Input: TARIFEDS4, numBatchedFrames is %u, <= 0",
                                               numBatchedFrames);
                                result = CamxResultEFailed;
                                break;
                            }
                        ...
                        case CVPInputPortREFIFEDS4:
                            ...
                            {
                                CAMX_LOG_ERROR(CamxLogGroupCVP, "Input: REFIFEDS4, numBatchedFrames is %u, <= 0",
                                               numBatchedFrames);
                                result = CamxResultEFailed;
                                break;
                            }
            ...
            if (CamxResultSuccess == result)
            {
                ...
                if (TRUE == IsProfileIdRegistration())
                {
                    ...
                    {
                        m_prefilterSrcContextIndex =  static_cast<UINT32>(requestId) % m_srcContextBufferCount;
                        srcFrameCtx.nFD            = m_ppSourceContextBuffers[m_prefilterSrcContextIndex]->
                                                         GetCSLBufferInfo()->fd;
                    }
                    else if (TRUE == IsCVPInBlend())
                    {
                        srcFrameCtx.nFD = m_ppSourceContextBuffers[m_srcContextBufferCount - 1]->GetCSLBufferInfo()->fd;
                ...
                else
                {
                    ...
                    srcFrameCtx.nFD = m_ppSourceContextBuffers[requestId % m_srcContextBufferCount]->GetCSLBufferInfo()->fd;
                    refFrameCtx.nFD = m_ppSourceContextBuffers[(requestId - 1) % m_srcContextBufferCount]->
                                          GetCSLBufferInfo()->fd;
                }
...
CamxResult CVPNode::PostPipelineCreate()
{
    ...
    CHAR        bufferName[MaxStringLength256];
    ImageFormat format;
    ...
    if ((CamxResultSuccess == result) && (FALSE == m_disableCVPDriver))
    {
        ...
        if (CamxResultSuccess == result)
        {
            ...
            format.height = 1;
            OsUtils::SNPrintF(bufferName, sizeof(bufferName), "%s_CVP_SrcCtxtBuffMgr",
                 NodeIdentifierString());
            m_srcContextBufferCount = 2;

            CAMX_LOG_VERBOSE(CamxLogGroupCVP, "Source buffer manager wXh %dx%d", format.width, format.height);
            result = CreateBufferManager(&m_pSourceContextBufferManager, format, m_srcContextBufferCount);
            if (CamxResultSuccess != result)
            {
            ...
            {
                m_ppSourceContextBuffers = static_cast<ImageBuffer**>(CAMX_CALLOC(sizeof(ImageBuffer*) *
                                                m_srcContextBufferCount));
                if (NULL != m_ppSourceContextBuffers)
                {
                    result = AllocateBuffers(m_pSourceContextBufferManager, m_ppSourceContextBuffers, m_srcContextBufferCount);
                    if (CamxResultSuccess != result)
                    {
        ...
        if (CamxResultSuccess == result)
        {
            ...
            m_gridBufferCount = ((TRUE == IsProfileIdRegistration()) && (TRUE == IsCVPInBlend())) ? m_requestQueueDepth : 2;
            result = CreateBufferManager(&m_pGridBufferManager, format,  m_gridBufferCount);
            if (CamxResultSuccess != result)
            {
            ...
            {
                m_ppGridBuffers = static_cast<ImageBuffer**>(CAMX_CALLOC(sizeof(ImageBuffer*) * m_gridBufferCount));
                if (NULL != m_ppGridBuffers)
                {
                    result = AllocateBuffers(m_pGridBufferManager, m_ppGridBuffers, m_gridBufferCount);
                    if (CamxResultSuccess != result)
                    {
    ...
    UINT32               numBuffers)
{
    ...
    createData.maxBufferCount               = numBuffers;
    createData.immediateAllocBufferCount    = numBuffers;
    ...
    createData.bufferProperties.immediateAllocImageBuffers = numBuffers;
    createData.bufferProperties.maxImageBuffers            = numBuffers;
