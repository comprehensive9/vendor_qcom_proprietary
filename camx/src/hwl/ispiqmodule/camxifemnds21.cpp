    ...
    ISPInputData* pInputData)
{
    ...
    // 6. we keep three decimal precison for scale factor, and make sure output widht/height is even
    if (outAspectRatio > inAspectRatio)
    {
        downScalingFactor = m_pState->inputWidth * 1000 / m_pState->MNDSOutput.dimension.width;
    }
    else
    {
        downScalingFactor = m_pState->inputHeight * 1000 / m_pState->MNDSOutput.dimension.height;
    }
    m_pState->MNDSOutput.scalingFactor    = static_cast<FLOAT>(downScalingFactor) / 1000.0f;
    m_pState->MNDSOutput.dimension.width  =
        Utils::EvenFloorUINT32(m_pState->inputWidth * 1000 / downScalingFactor);
    m_pState->MNDSOutput.dimension.height =
        Utils::EvenFloorUINT32(m_pState->inputHeight * 1000 / downScalingFactor);
