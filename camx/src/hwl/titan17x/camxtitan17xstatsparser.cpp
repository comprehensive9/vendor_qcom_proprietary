    ...
    PropertyISPHDRBEStats*  pISPConfig)
{
    ...
    UINT32 verticalNum_metaTag   = 0;
    UINT32 horizontalNum_metaTag = 0;
    ...
    ParsedAWBBGStatsOutput* pAWBBGStatsOutput)
{
    ...
    const BOOL              leftStripeValid      = pLeftConfig->isStripeValid;
    const BOOL              rightStripeValid     = pRightConfig->isStripeValid;
    ...
    if (CamxResultSuccess == result)
    {
        const UINT32 leftHorizNum      = (leftStripeValid) ? pLeftConfig->horizontalNum : 0;
        const UINT32 rightHorizNum     = (rightStripeValid) ? pRightConfig->horizontalNum : 0;
        ...
        CAMX_LOG_VERBOSE(CamxLogGroupISP,
                         ...
                         pAWBBGStatsOutput->flags.usesY,
                         pAWBBGStatsOutput->flags.hasSatInfo,
                         leftHorizNum,
                         pStatsConfig[0].AWBBGConfig.verticalNum,
                         pLeftConfig->regionWidth,
                         pLeftConfig->regionHeight,
                         pLeftConfig->ROI.left,
                         pLeftConfig->ROI.top,
                         pLeftConfig->ROI.width,
                         pLeftConfig->ROI.height);
        CAMX_LOG_VERBOSE(CamxLogGroupISP,
                         "Right Stripe: isvalid:%d numROI:%d Hnum:%d Vnum:%d Region:%dX%d ROI(%d,%d,%d,%d)",
                         rightStripeValid,
                         totalRightRegions,
                         rightHorizNum,
                         pStatsConfig[1].AWBBGConfig.verticalNum,
                         pRightConfig->regionWidth,
                         pRightConfig->regionHeight,
                         pRightConfig->ROI.left,
                         pRightConfig->ROI.top,
                         pRightConfig->ROI.width,
                         pRightConfig->ROI.height);
    ...
    PropertyISPBHistStats*  pISPConfig)
{
    ...
    if (CamxResultSuccess == result)
    {
        ...
        UINT              pDataCount[CAMX_ARRAY_SIZE(WriteProps)]  = { sizeof(pStatsOutput->BHistogramStats) / sizeof(UINT32) };
