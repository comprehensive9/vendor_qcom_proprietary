    ...
    CropType    cropType)
{
    ...
    if ((cropLeft * 2 + cropWidth) > m_pState->scalerOutput.dimension.width)
    {
        if (cropWidth < m_pState->scalerOutput.dimension.width)
        {
            cropLeft = Utils::EvenFloorUINT32((m_pState->scalerOutput.dimension.width - cropWidth) / 2);
        }
        else
        {
    ...
    if ((cropTop * 2 + cropHeight) > m_pState->scalerOutput.dimension.height)
    {
        if (cropHeight < m_pState->scalerOutput.dimension.height)
        {
            cropTop = Utils::EvenFloorUINT32((m_pState->scalerOutput.dimension.height - cropHeight) / 2);
        }
        else
        {
    ...
    if (widthPixel > m_pState->streamDimension.width)
    {
        ...
        {
            m_pState->cropInfo.left = Utils::EvenFloorUINT32((m_pState->streamDimension.width - m_pState->cropInfo.width) / 2);
        }
    ...
    if (heightLine > m_pState->streamDimension.height)
    {
        ...
        {
            m_pState->cropInfo.top = Utils::EvenFloorUINT32((m_pState->streamDimension.height - m_pState->cropInfo.height) / 2);
        }
    ...
    if (cropType == CropTypeCentered)
    {
        if ((((m_previousAppliedCrop[m_output].firstPixel == pCrop->firstPixel)                  &&
            ...
            (m_previousAppliedCrop[m_output].lastPixel < m_pState->scalerOutput.dimension.width) &&
            (m_previousAppliedCrop[m_output].lastLine < m_pState->scalerOutput.dimension.height))
        {
