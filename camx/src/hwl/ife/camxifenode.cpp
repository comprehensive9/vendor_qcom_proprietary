    ...
    NodeCreateOutputData*      pCreateOutputData)
{
    ...
    if (TRUE == GetStaticSettings()->enableFormatMeasure)
    {
        m_genericBlobCmdBufferSizeBytes += sizeof(IFESensorDimConfig);
    }
    // For RDI only use case too we need to allocate cmd buffer for kernel use
    m_totalIQCmdSizeDWord += IFEKMDCmdBufferMaxSize;
    ...
    ExecuteProcessRequestData* pExecuteProcessRequestData)
{
    ...
    for (UINT i = 0; (i < pPerRequestPorts->numOutputPorts) && (CamxResultSuccess == result); i++)
    {
        ...
        if (m_usecaseRawBatchedFrames > 1)
        {
            numBatchedFrames = m_usecaseRawBatchedFrames;
        }
                    ...
                    if (NULL != pImageBuffer)
                    {
                        ...
                        if ((TRUE == isPortIOConfig) && (NULL != pOutputImageFormat))
                        {
                            CAMX_LOG_INFO(CamxLogGroupISP,
                                          ...
                                          "memHandle=0x%X WxH=%dx%d request=%llu, format %d, numBatchedFrames: %d",
                                          InstanceID(),
                                          pOutputPort->portId,
                                          *pOutputPort->phFence,
                                          pImageBuffer,
                            ...
                            m_IFEPerFrameData[index].ioConfig[rPortIndex].width       = pOutputImageFormat->width;
                            m_IFEPerFrameData[index].ioConfig[rPortIndex].height      = pOutputImageFormat->height;
                            m_IFEPerFrameData[index].ioConfig[rPortIndex].format      = pOutputImageFormat->format;
...
CamxResult IFENode::PostPipelineCreate()
{
    ...
    if (CamxResultSuccess == result)
    {
        ...
        if (CamxResultSuccess == result)
        {
            ...
            if (TRUE == IsRawBatchingEnabled())
            {
                m_usecaseNumBatchedFrames = m_usecaseRawBatchedFrames;
            }

            CAMX_LOG_INFO(CamxLogGroupISP,
                "%s, m_usecaseNumBatchedFrames: %d, m_usecaseRawBatchedFrames: %d, pData[0]: %p",
                NodeIdentifierString(), m_usecaseNumBatchedFrames, m_usecaseRawBatchedFrames, pData[0]);
    ...
    ISPInputData* pInputData)
{
    ...
    if ((CamxResultSuccess == result) && (TRUE == GetStaticSettings()->enableFormatMeasure))
    {
        SetupSensorDimInitialConfig();
    }
    ...
    IFEConfigInfo*  pIFEDebugData)
{
    ...
    CAMX_LOG_TO_FILE(fd, 0,
        ...
        "minHorizontalBlanking %u minVerticalBlanking %u",
        ...
        m_pSensorModeData->minHorizontalBlanking, m_pSensorModeData->minVerticalBlanking);
    ...
    for (UINT index = 0; index < numIOConfig; index++)
    {
        ...
        CAMX_LOG_TO_FILE(fd, 0, "I/O cfg portId=%d, hFence=%d, hMemHandle=%d "
            ...
            pIOConfig->width, pIOConfig->height, pIOConfig->format);
    }
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// IFENode::GetOffCenteredZoomPixelMargin
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
UINT32 IFENode::GetOffCenteredZoomPixelMargin()
{
    UINT32                   offCenteredPixelMargin = 0;
    const  StaticSettings*   pSettings              = HwEnvironment::GetInstance()->GetStaticSettings();

    if ((TRUE == m_pSensorCaps->isQuadCFASensor) && (NULL != pSettings) && (FALSE == pSettings->exposeFullSizeForQCFA))
    {
        offCenteredPixelMargin = ISPOffCenteredZoomQCFAPixelMargin;
    }
    else
    {
        offCenteredPixelMargin = ISPOffCenteredZoomPixelMargin;
    }

    CAMX_LOG_VERBOSE(CamxLogGroupISP, "Update Off-centered pixel margin %u", offCenteredPixelMargin);
    return offCenteredPixelMargin;
}
    ...
    BOOL    isPerFrameDump)
{
    ...
    if ((NULL != m_pIFEPipeline) && (TRUE == dumpEnable) && (NULL != m_pHangDumpBufferAddr))
    {
        ...
        if ((TRUE == pSettings->enableIFERegDump) && (TRUE == isPerFrameDump))
        {
            ...
            {
                CAMX_LOG_ERROR(CamxLogGroupISP, "pRegBuffer is NULL for request ID = %d", requestId);
                return;
            }
        }
        else
        ...
        if (CamxResultSuccess == result)
        {
            ...
            if (CamxResultSuccess == result)
            {
                ...
                if (IFEModuleMode::DualIFENormal == m_mode)
                {
                    ...
                    UINT32 pixelMargin = GetOffCenteredZoomPixelMargin();

                    if ((Utils::AbsoluteINT32(static_cast<INT32>(m_sensorActiveArrayWidth) - expectedWidth) > pixelMargin) ||
                        (Utils::AbsoluteINT32(static_cast<INT32>(m_sensorActiveArrayHeight) - expectedHeight) > pixelMargin))
                    {
...
CamxResult IFENode::InitialCAMIFConfig()
{
    ...
    for ( UINT32 i = 0; i < m_IFEAcquiredHWInfo.valid_acquired_hw; i++)
    {
        ...
        if (m_IFEAcquiredHWInfo.acquired_hw_id[i] &
    ...
    ISPInputData* pModuleInput)
{
    ...
    else
    {
        ...
        CAMX_LOG_ERROR(CamxLogGroupISP, "IFE:%d Invalid sensor active array size : %dx%d, input %dx%d",
                       InstanceID(), m_sensorActiveArrayWidth, m_sensorActiveArrayHeight,
                       sensorWidth, sensorHeight);
    }
    ...
    GetMetadataPartialMwbMode(&pModuleInput->pHALTagsData->partialMwbMode);
    CAMX_LOG_VERBOSE(CamxLogGroupISP, "IFE:%d partialMwbMode %d", InstanceID(), pModuleInput->pHALTagsData->partialMwbMode);
    ...
    BufferNegotiationData* pBufferNegotiationData)
{
    ...
    for (UINT index = 0; index < pBufferNegotiationData->numOutputPortsNotified; index++)
    {
        ...
        if (IFEOutputPortDisplayFull == outputPortId)
        {
            ...
            // Consider HVX binning, IFE output should be less than HVX output
            if (TRUE == m_HVXInputData.enableHVX)
            {
                CAMX_LOG_INFO(CamxLogGroupISP, "outputPortId = %d, HVX out %d %d, Full port %d %d Display %d %d",
                    outputPortId, m_IFEinputResolution.resolution.width,
                    m_IFEinputResolution.resolution.height,
                    fullPortOutputWidth,
                    fullPortOutputHeight,
                    displayFullPortOutputWidth,
                    displayFullPortOutputHeight);

                /// Clip if output greater than HVX output, since IFE cannot do upscale.
                if (displayFullPortOutputWidth > m_IFEinputResolution.resolution.width)
                {
                    displayFullPortOutputWidth = m_IFEinputResolution.resolution.width;
                    CAMX_LOG_INFO(CamxLogGroupISP, "outputPortId = %d, HVX display Output buffer clipped width = %d",
                        outputPortId, displayFullPortOutputWidth);
                }

                /// Clip if output greater than than HVX output, since IFE cannot do upscale.
                if (displayFullPortOutputHeight > m_IFEinputResolution.resolution.height)
                {
                    displayFullPortOutputHeight = m_IFEinputResolution.resolution.height;
                    CAMX_LOG_INFO(CamxLogGroupISP, "outputPortId = %d, HVX display Output buffer clipped height = %d",
                        outputPortId, displayFullPortOutputHeight);
                }

                /// Clip if output greater than HVX output, since IFE cannot do upscale.
                if (fullPortOutputWidth > m_IFEinputResolution.resolution.width)
                {
                    fullPortOutputWidth = m_IFEinputResolution.resolution.width;
                    CAMX_LOG_INFO(CamxLogGroupISP, "outputPortId = %d, HVX full Output buffer clipped width = %d",
                        outputPortId, fullPortOutputWidth);
                }

                /// Clip if output greater than than HVX output, since IFE cannot do upscale.
                if (fullPortOutputHeight > m_IFEinputResolution.resolution.height)
                {
                    fullPortOutputHeight = m_IFEinputResolution.resolution.height;
                    CAMX_LOG_INFO(CamxLogGroupISP, "outputPortId = %d, HVX full Output buffer clipped height = %d",
                        outputPortId, fullPortOutputHeight);
                }
            }
            /// If downscale ratio is beyond ife limits, cap the output dimension.
    ...
    ISPStripeConfig*    pStripeConfig)
{
    ...
    if (TRUE == pStripeOut->BGAWBOut.BGEnabled)
    {
        ...
        pStripeConfig->AWBStatsUpdateData.statsConfig.BGConfig.isStripeValid        = TRUE;
    }
    ...
    ISPStripeConfig*    pStripeConfigs)
{
    ...
    /* Reset Left & Right Stripe config for AWB BG stats ROI */
    pStripeConfigs[0].AWBStatsUpdateData.statsConfig.BGConfig.isStripeValid = FALSE;
    pStripeConfigs[1].AWBStatsUpdateData.statsConfig.BGConfig.isStripeValid = FALSE;
    ...
    if (CamxResultSuccess == result)
    {
        ...
        for (UINT portIndex = 0; portIndex < totalOutputPorts; portIndex++)
        {
            ...
            if (CamxResultSuccess == result)
            {
                ...
                CAMX_LOG_INFO(CamxLogGroupISP, "%s, framedropPattern: %d, framedropPeriod: %d,"
                    " subsamplePattern: %d, subSamplePeriod: %d, port Id: %d, isBatchEnabledPort: %d,"
                    "numBatchedFrames: %d",
                    NodeIdentifierString(),
                    pPortHFRConfig->framedropPattern,
                    pPortHFRConfig->framedropPeriod,
                    pPortHFRConfig->subsamplePattern,
                    pPortHFRConfig->subsamplePeriod,
                    outputPortId[portIndex],
                    isBatchEnabledPort,
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// IFENode::SetupSensorDimInitialConfig
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
CamxResult IFENode::SetupSensorDimInitialConfig()
{
    CamxResult         result            = CamxResultSuccess;
    IFESensorDimConfig sensorDimCfg;
    UINT               inputResourcePortSourceType;
    UINT               outputSourceType;
    UINT               streamIndex;

    memset(&sensorDimCfg, 0, sizeof(sensorDimCfg));

    if (FALSE == m_RDIOnlyUseCase)
    {
        sensorDimCfg.pix.width         = m_pSensorModeData->resolution.outputWidth;
        sensorDimCfg.pix.height        = m_pSensorModeData->resolution.outputHeight;
        sensorDimCfg.pix.measureEnable = TRUE;
    }
    // Filter the output port that matches with the current input port source type
    for (UINT index = 0; index < GetNumOutputPorts(); index++)
    {
        UINT outputPortIndex            = GetOutputPortId(index);
        const ImageFormat* pImageFormat = GetOutputPortImageFormat(index);
        UINT nodeIndex                  = OutputPortIndex(outputPortIndex);
        if (NULL != pImageFormat)
        {
            // CheckOutputPortIndexIfUnsupported
            // Check port are RDI first, so that in non RDI case
            if ( (IFEOutputPortRDI0 <= outputPortIndex && IFEOutputPortRDI3 >= outputPortIndex) &&
                 (FALSE == CheckOutputPortIndexIfUnsupported(nodeIndex)) )
            {
                sensorDimCfg.rdi[outputPortIndex - IFEOutputPortRDI0].width         = pImageFormat->width;
                sensorDimCfg.rdi[outputPortIndex - IFEOutputPortRDI0].height        = pImageFormat->height;
                sensorDimCfg.rdi[outputPortIndex - IFEOutputPortRDI0].measureEnable = TRUE;
            }
            else if (IFEOutputPortStatsDualPD == outputPortIndex && PortSrcTypePDAF == GetOutputPortSourceType(nodeIndex))
            {
                if (TRUE == FindSensorStreamConfigIndex(StreamType::PDAF, &streamIndex))
                {
                    sensorDimCfg.ppp.width         = m_pSensorModeData->streamConfig[streamIndex].frameDimension.width;
                    sensorDimCfg.ppp.height        = m_pSensorModeData->streamConfig[streamIndex].frameDimension.height;
                    sensorDimCfg.ppp.measureEnable = TRUE;
                }
            }
        }
    }
    sensorDimCfg.hbi = m_pSensorModeData->minHorizontalBlanking;
    sensorDimCfg.vbi = m_pSensorModeData->minVerticalBlanking;

    CAMX_LOG_VERBOSE(CamxLogGroupISP,
                         "pix (%dx%d) ppp(%d %d) rdi (%dx%d, %dx%d, %dx%d, %dx%d),"
                         " hbi: %d, vbi:%d m_pGenericBlobCmdBuffer:%p",
                         sensorDimCfg.pix.width, sensorDimCfg.pix.height,
                         sensorDimCfg.ppp.width, sensorDimCfg.ppp.height,
                         sensorDimCfg.rdi[0].width, sensorDimCfg.rdi[0].height,
                         sensorDimCfg.rdi[1].width, sensorDimCfg.rdi[1].height,
                         sensorDimCfg.rdi[2].width, sensorDimCfg.rdi[2].height,
                         sensorDimCfg.rdi[3].width, sensorDimCfg.rdi[3].height,
                         sensorDimCfg.hbi, sensorDimCfg.vbi,
                         m_pGenericBlobCmdBuffer);

    result = PacketBuilder::WriteGenericBlobData(m_pGenericBlobCmdBuffer,
                                                 IFEGenericBlobTypeSensorDimConfig,
                                                 sizeof(IFESensorDimConfig),
                                                 reinterpret_cast<BYTE*>(&sensorDimCfg));
    return result;
}
    ...
    else if (2 == m_ICAGridDomain)
    {
        if (NULL != pResidualCropInfo)
        {
            if (TRUE == isFullOutPortEnabled)
            {
                ...
                m_ifeZoomWindowInDomain[FullPath].fullWidth     = m_IFEOutputResolution.fullPortDimension.width;
                m_ifeZoomWindowInDomain[FullPath].fullHeight    = m_IFEOutputResolution.fullPortDimension.height;
            }
            ...
            if (TRUE == isDisplayOutPortEnabled)
            {
                ...
                m_ifeZoomWindowInDomain[DisplayFullPath].fullWidth      = m_IFEOutputResolution.displayPortDimension.width;
                m_ifeZoomWindowInDomain[DisplayFullPath].fullHeight     = m_IFEOutputResolution.displayPortDimension.height;;
            }
        }
        else
        {
