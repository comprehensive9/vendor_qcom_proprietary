...
/// @brief IFE Sensor Dimension
struct IFESensorDim
{
    UINT32 width;          ///< width
    UINT32 height;         ///< height
    UINT32 measureEnable;  ///< enable
} CAMX_PACKED;

/// @brief IFE Sensor Dimension configuration
struct IFESensorDimConfig
{
    IFESensorDim ppp;         ///< pdaf path dimenstion
    IFESensorDim pix;         ///< pix path dimenstion
    IFESensorDim rdi[4];      ///< rdi path dimenstion
    UINT32       hbi;         ///< hbi
    UINT32       vbi;         ///< vbi
} CAMX_PACKED;
...
CAMX_STATIC_ASSERT_MESSAGE((sizeof(IFESensorDimConfig) % 8) == 0,     "IFESensorDimConfig 64b-aligned");
...
private:
    ...
    ///
    /// @brief  Setup IFE sensor dimension configuration initial configuration in Generic Blob Command Buffer
    ///
    /// @return CamxResult True on successful execution else False on failure
    ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    CamxResult SetupSensorDimInitialConfig();
    ...
    ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    /// IsRawBatchingEnabled
    ///
    /// @brief  Check if Raw Batching is enabled for IFE
    ///
    /// @return TRUE if Raw batching is enabled else return FALSE
    ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    CAMX_INLINE BOOL IsRawBatchingEnabled()
    {
        BOOL        enabled         = FALSE;
        CamxResult  result          = CamxResultSuccess;
        UINT32      metaTag         = 0;
        m_usecaseRawBatchedFrames   = 1;

        if (CamxResultSuccess == VendorTagManager::QueryVendorTagLocation("org.codeaurora.qcamera3.sessionParameters",
                                   "numRawBatchedFramesTag", &metaTag))
        {
            static const UINT32 PropertiesNumRawBatchedFrames[] =
            {
                metaTag | UsecaseMetadataSectionMask
            };

            UINT    length                   = CAMX_ARRAY_SIZE(PropertiesNumRawBatchedFrames);
            VOID*   pData[1]                 = { 0 };
            UINT64  propertyDataOPEOffset[1] = { 0 };

            result = GetDataList(PropertiesNumRawBatchedFrames, pData, propertyDataOPEOffset, length);
            if ((CamxResultSuccess == result) && (NULL != pData[0]))
            {
                m_usecaseRawBatchedFrames = *reinterpret_cast<UINT *>(pData[0]);
    ...
    ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    /// GetOffCenteredZoomPixelMargin
    ///
    /// @brief  Get Off Centered Zoom Pixel Margin for QCFA and Non QCFA sensor
    ///
    /// @return UINT32 Off Centered Zoom Pixel Margin
    ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    UINT32 GetOffCenteredZoomPixelMargin();
    ...
    UINT                    m_usecaseRawBatchedFrames;              ///< Number of batched frames in raw use case
