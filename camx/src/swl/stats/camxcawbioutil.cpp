    ...
    VOID*                           pPeerInfo)
{
    ...
    if (TRUE == m_inputs.appSyncModeInfo.appSyncMode)
    {
        result = StatsUtil::WriteVendorTag(m_pNode,
                                           "org.codeaurora.qcamera3.multicamsyncdata",
                                           "AWBPeerInfoSize",
                                           &AWBPeerInfoSize,
                                           sizeof(AWBPeerInfoSize));
        if (CamxResultSuccess != result)
        {
            CAMX_LOG_ERROR(CamxLogGroupAWB, "cannot write to AWBPeerInfoSize vendor tag");
        }
        result = StatsUtil::WriteVendorTag(m_pNode,
                                           "org.codeaurora.qcamera3.multicamsyncdata",
                                           "AWBPeerInfo",
        ...
        if (CamxResultSuccess != result)
        {
            CAMX_LOG_ERROR(CamxLogGroupAWB, "cannot write to AWBPeerInfo vendor tag");
        }
    }
    return result;
}
    ...
    const StatsProcessRequestData* pStatsProcessRequestDataInfo)
{
    ...
    if (TRUE == m_inputs.appSyncModeInfo.appSyncMode)
    {
        if (TRUE == m_inputs.appSyncModeInfo.appSyncModeMaster)
        {
            m_inputs.cameraInfo.algoRole = StatsAlgoRoleMaster;
        }
        else
        {
            m_inputs.cameraInfo.algoRole = StatsAlgoRoleSlave;
        }
    }

    CAMX_LOG_VERBOSE(CamxLogGroupAWB, "camera info: algo role: %d, cam id: %d appSyncMode %d appSyncModeMaster %d",
        m_inputs.cameraInfo.algoRole,
        m_inputs.cameraInfo.cameraId,
        m_inputs.appSyncModeInfo.appSyncMode,
        m_inputs.appSyncModeInfo.appSyncModeMaster);
}
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// CAWBIOUtil::SetAppSyncModeInfo
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
VOID CAWBIOUtil::SetAppSyncModeInfo(
    AppSyncModeInfo syncModeInfo)
{
    m_inputs.appSyncModeInfo.appSyncMode       = syncModeInfo.appSyncMode;
    m_inputs.appSyncModeInfo.appSyncModeMaster = syncModeInfo.appSyncModeMaster;
}
    ...
    AWBAlgoSetParamList*            pSetInputList)
{
    ...
    else if (TRUE == m_inputs.appSyncModeInfo.appSyncMode && FALSE == m_inputs.appSyncModeInfo.appSyncModeMaster)
    {
        VOID* pData = NULL;

        // Read peer info from application for slave camera only
        result = ReadVendorTag("org.codeaurora.qcamera3.multicamsyncdata", "AWBPeerInfo", &pData);

        if (CamxResultSuccess != result)
        {
            CAMX_LOG_ERROR(CamxLogGroupAWB, "AWBPeerInfo tag read failed");
        }
        else
        {
            m_setInputs.pPeerInfo = pData;
            LoadSetInputParamList(pSetInputList, AWBSetParamTypePeerInfo, m_setInputs.pPeerInfo, sizeof(VOID*));
        }
        // return success even if peer info is not sent by application as we don't want to stop slave processing
    ...
    AWBAlgoGetParam*                pGetParam)
{
    ...
    else if (AWBGetParamTypePeerInfoSize == pGetParam->type)
    {
        m_getParamOutputArray[AWBGetParamOutputTypePeerInfoSize].getParamOutputType          =
            AWBGetParamOutputTypePeerInfoSize;
        m_getParamOutputArray[AWBGetParamOutputTypePeerInfoSize].pGetParamOutput             = &m_outputs.peerInfoSize;
        m_getParamOutputArray[AWBGetParamOutputTypePeerInfoSize].sizeOfGetParamOutput        = sizeof(UINT32);
        m_getParamOutputArray[AWBGetParamOutputTypePeerInfoSize].sizeOfWrittenGetParamOutput = 0;
    }
