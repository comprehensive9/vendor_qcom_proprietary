    ...
    const StatsInitializeData* pInitializeData)
{
    ...
    CamX::Utils::Memset(&m_HALParam, 0, sizeof(AECEngineHALParam));
    ...
    AECEngineAlgorithmInput*       pInput)
{
    ...
    if (CamxResultSuccess == result)
    {
        if (TRUE == m_HALParam.appSyncMode)
        {
            if (TRUE == m_HALParam.appSyncModeMaster)
            {
                pInput->cameraInfo.algoRole = StatsAlgoRoleMaster;
            }
            else
            {
                pInput->cameraInfo.algoRole = StatsAlgoRoleSlave;
            }
        }
        else
        {
    ...
    StatsVendorTagList*             pVendorTagOutput)
{
    ...
    /// Publish Peer info tag only when app sync mode is enabled
    if (TRUE == m_HALParam.appSyncMode)
    {
        StatsUtil::WriteVendorTag(m_pNode,
...
#define READ(sec, tag, val) \
    ...
    // Get app sync mode from application
    READ("org.codeaurora.qcamera3.multicamsync", "appSyncMode", pHALParam->appSyncMode);

    if (TRUE == pHALParam->appSyncMode)
    {
        READ("org.codeaurora.qcamera3.multicamsync", "isMaster", pHALParam->appSyncModeMaster);
    }
    ...
    ) const
{
    ...
    // Get peer info from application for slave camera only
    if (TRUE == pHALParam->appSyncMode && FALSE == pHALParam->appSyncModeMaster)
    {
        GetAppPeerInfo(&pHALParam->pPeerInfo);
    }
    ...
    CAMX_LOG_INFO(CamxLogGroupAEC, "ReqId:%llu, camID:%d, AECompensation:%d, AELock:%d, AEMode:%d, AEMeteringMode:%d, "
                  ...
                  "videoHDRType:%d, ZSLEnable %d, FrameDuration:%lld UnStabilized Face (Cnt:%d x:%d y:%d dx:%d dy:%d), "
                  ...
                  pHALParam->faceROI.unstabilizedROI[0].faceRect.left,
                  pHALParam->faceROI.unstabilizedROI[0].faceRect.top,
                  pHALParam->faceROI.unstabilizedROI[0].faceRect.width,
                  pHALParam->faceROI.unstabilizedROI[0].faceRect.height,
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// CAECStatsProcessor::GetAppPeerInfo
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
CamxResult CAECStatsProcessor::GetAppPeerInfo(
    VOID** ppPeerInfo
    ) const
{
    UINT32          tag;
    ...
    result = VendorTagManager::QueryVendorTagLocation("org.codeaurora.qcamera3.multicamsyncdata", "AECPeerInfo", &tag);
    if (CamxResultSuccess != result)
    {
        CAMX_LOG_ERROR(CamxLogGroupAEC, "AECPeerInfo tag not defined");
        result = CamxResultEFailed;
    }
    else
    {
        ...
        m_pNode->GetDataList(props, pData, offsets, 1);

        if (NULL == pData[0])
        {
            CAMX_LOG_ERROR(CamxLogGroupAEC, "AECPeerInfo tag not published");
            result = CamxResultEFailed;
        }
        else
        {
