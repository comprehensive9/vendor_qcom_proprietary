    ...
    AECEngineFrameControl*   pFrameControl)
{
    ...
    if (CamxResultSuccess == result)
    {
        result = SetSingleParamToAlgorithm(AECAlgoSetParamPeerInfo,
                                           &m_HALParam.pPeerInfo,
                                           sizeof(m_HALParam.pPeerInfo));
        if (CamxResultSuccess != result)
        {
            CAMX_LOG_ERROR(CamxLogGroupAEC, "Failed to set peer info result: %d", Utils::CamxResultToString(result));
        }
    }
    ...
    if (CamxResultSuccess == result)
    {
        ...
        outputList[0].outputType                = AECAlgoGetParamOutputPeerInfoSize;
        outputList[0].pOutputData               = &peerInfoSize;
        outputList[0].sizeOfOutputData          = sizeof(peerInfoSize);
        outputList[0].sizeOfWrittenOutputData   = 0;

        // Query AEC peer info size
        result = GetParamFromAlgorithm(AECAlgoGetParamPeerInfoSize, NULL, &outputList[0], 1);

        if ((CamxResultSuccess != result) || (0 == outputList[0].sizeOfWrittenOutputData))
        {
            CAMX_LOG_VERBOSE(CamxLogGroupAEC, "Invalid AEC peer info size from Algo Result:%d Size:%d",
                result, outputList[0].sizeOfWrittenOutputData);
        }
        else
        {
            CAMX_LOG_VERBOSE(CamxLogGroupAEC, "AEC peer info size from Algo %u", peerInfoSize);
            if (AECPeerInfoSize != peerInfoSize)
            {
                CAMX_LOG_ERROR(CamxLogGroupAEC, "AEC peer info size changed at algo. Update AECPeerInfoSize to %u",
                    peerInfoSize);
            }
        }
    }
    ...
    StatsSensorInfo*                pStatsSensorInfo)
{
    ...
    if (CamxResultSuccess == result)
    {
        ...
        for (UINT32 index = 0; index < pHALParam->faceROI.ROICount; index++)
        {
            ...
            pUnstabilizedFaceROIInformation->roi[index].roiID       = pHALParam->faceROI.unstabilizedROI[index].id;
            pUnstabilizedFaceROIInformation->roi[index].rect.left   = pRect->left;
            pUnstabilizedFaceROIInformation->roi[index].rect.top    = pRect->top;
            pUnstabilizedFaceROIInformation->roi[index].rect.width  = pRect->width;
            pUnstabilizedFaceROIInformation->roi[index].rect.height = pRect->height;
            pUnstabilizedFaceROIInformation->roi[index].weight      = 0;

            pUnstabilizedFaceROIInformation->roiCount++;
            CAMX_LOG_VERBOSE(CamxLogGroupAEC, "UnstabilizedROI: index:%d SensorDim [%ux%u] ROI[l:%u t:%u w:%u h:%u]"
                    "roiID:%d CONV: [l:%u t:%u w:%u h:%u Cnt:%d]",
                    index,
                    pStatsSensorInfo->sensorResWidth,
                    pStatsSensorInfo->sensorResHeight,
                    pRect->left,
                    ...
                    pRect->width,
                    pRect->height,
                    pUnstabilizedFaceROIInformation->roi[index].roiID,
                    pUnstabilizedFaceROIInformation->roi[index].rect.left,
                    pUnstabilizedFaceROIInformation->roi[index].rect.top,
                    pUnstabilizedFaceROIInformation->roi[index].rect.width,
                    pUnstabilizedFaceROIInformation->roi[index].rect.height,
                    pUnstabilizedFaceROIInformation->roiCount);
        }
