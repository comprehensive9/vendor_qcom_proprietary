...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// CAWBStatsProcessor::CheckPeerInfoSize
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
CamxResult CAWBStatsProcessor::CheckPeerInfoSize()
{
    CAMX_ENTRYEXIT(CamxLogGroupAWB);

    CamxResult      result       = CamxResultSuccess;
    UINT32          peerInfoSize = 0;
    AWBAlgoGetParam algoGetParam = {};

    algoGetParam.type = AWBGetParamTypePeerInfoSize;

    result = AlgoGetParam(NULL, &algoGetParam);

    if (CamxResultSuccess == result)
    {
        peerInfoSize = *static_cast<UINT*>(
            algoGetParam.outputInfoList.pGetParamOutputs[AWBGetParamOutputTypePeerInfoSize].pGetParamOutput);
    }
    else
    {
        CAMX_LOG_ERROR(CamxLogGroupAWB, "Cannot get AWB peer info size");
    }

    if (AWBPeerInfoSize != peerInfoSize)
    {
        CAMX_LOG_ERROR(CamxLogGroupAWB, "AWB peer info size changed at algo. Update AWBPeerInfoSize to %u", peerInfoSize);
    }

    CAMX_LOG_INFO(CamxLogGroupAWB, "AWB peer info size from algo: %u", peerInfoSize);
    ...
    const StatsProcessRequestData* pStatsProcessRequestDataInfo)
{
    ...
    // get app sync mode info from application
    GetAppSyncModeinfo();

    if (TRUE == m_appSyncModeInfo.appSyncMode)
    {
        if (TRUE == m_appSyncModeInfo.appSyncModeMaster)
        {
    ...
    else
    {
        ...
        if (CamxResultSuccess == result)
        {
            ...
            if (((StatsAlgoRoleDefault != m_cameraInfo.algoRole) &&
                ...
                (TRUE == m_appSyncModeInfo.appSyncMode && FALSE == m_appSyncModeInfo.appSyncModeMaster))
            {
    ...
    // Get peer info from algorithm if 1 OR 2
    // 1. if algo role is not Default AND 3A sync is enabled AND is MultiCamera.
    // 2. 2A sync enabled from application AND camera is Master.
    if ((CamxResultSuccess == result) &&
        (((StatsAlgoRoleDefault != m_cameraInfo.algoRole) &&
        (MultiCamera3ASyncDisabled != m_pStaticSettings->multiCamera3ASync) && (TRUE == m_pNode->IsMultiCamera())) ||
        (TRUE == m_appSyncModeInfo.appSyncMode && TRUE == m_appSyncModeInfo.appSyncModeMaster)))
    {
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// CAWBStatsProcessor::GetAppSyncModeinfo
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
VOID CAWBStatsProcessor::GetAppSyncModeinfo()
{
    VOID* pData = NULL;

    m_appSyncModeInfo.appSyncMode       = FALSE;
    m_appSyncModeInfo.appSyncModeMaster = FALSE;

    StatsUtil::ReadVendorTag(m_pNode, "org.codeaurora.qcamera3.multicamsync", "appSyncMode", &pData);

    if (NULL != pData)
    {
        Utils::Memcpy(&m_appSyncModeInfo.appSyncMode, pData, sizeof(BOOL));
    }

    if (TRUE == m_appSyncModeInfo.appSyncMode)
    {
        StatsUtil::ReadVendorTag(m_pNode, "org.codeaurora.qcamera3.multicamsync", "isMaster", &pData);

        if (NULL != pData)
        {
            Utils::Memcpy(&m_appSyncModeInfo.appSyncModeMaster, pData, sizeof(BOOL));
        }
    }

    CAMX_LOG_VERBOSE(CamxLogGroupAWB, "appSyncMode: %d appSyncModeMaster: %d",
                     m_appSyncModeInfo.appSyncMode,
                     m_appSyncModeInfo.appSyncModeMaster);

    m_pAWBIOUtil->SetAppSyncModeInfo(m_appSyncModeInfo);
}
