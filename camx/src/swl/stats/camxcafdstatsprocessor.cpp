    ...
    const StatsInitializeData* pInitializeData)
{
        ...
        NULL != pInitializeData->pTuningDataManager)
    {
        ...
        if (NULL != pInitializeData->initializecallback.pAFDCallback)
        {
            ...
            result = pCreate(&createParamList, &m_pAFDAlgorithm);
        }
    ...
    const StatsProcessRequestData* pStatsProcessRequestDataInfo)
{
    ...
    else if (m_pNode->GetMaximumPipelineDelay() >= requestIdOffsetFromLastFlush)
    {
        ...
        CDKResult cdkResult = m_pAFDAlgorithm->AFDSetParam(m_pAFDAlgorithm, &setParamList);
    ...
    else
    {
        ...
        if (CamxResultSuccess == result)
        {
            if (NULL != m_pAFDAlgorithm)
            {
                result = m_pAFDAlgorithm->AFDProcess(m_pAFDAlgorithm, &input, pOutput);
                if (CamxResultSuccess != result)
                {
                    CAMX_LOG_ERROR(CamxLogGroupAFD,
...
VOID CAFDStatsProcessor::GetPrevConfigFromAlgo()
{
    ...
    if (NULL != m_pAFDAlgorithm)
    {
        m_pAFDAlgorithm->AFDGetParam(m_pAFDAlgorithm, &getParamList);
    }
}
...
CAFDStatsProcessor::~CAFDStatsProcessor()
{
    ...
    if (NULL != m_pAFDAlgorithm)
    {
        m_pAFDAlgorithm->AFDDestroy(m_pAFDAlgorithm);
        m_pAFDAlgorithm = NULL;
        CAMX_LOG_INFO(CamxLogGroupStats, "CAFDStatsProcessor::Destroy");
    }
