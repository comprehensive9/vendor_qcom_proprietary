...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// ChiEISV3Node::ValidateZoomWindow
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
VOID ChiEISV3Node::ValidateZoomWindow(
    CHIRectangle*   pCropInfo,
    CHIRectangle*   pAdjCropInfo,
    UINT32          fullWidth,
    UINT32          fullHeight,
    UINT32          adjFullWidth,
    UINT32          adjFullHeight)
{
    // Validation for centered zooom
    if ((static_cast<UINT32>(2*pCropInfo->left + pCropInfo->width) == fullWidth) &&
        (static_cast<UINT32>(2*pAdjCropInfo->left + pAdjCropInfo->width) != adjFullWidth))
    {
        // Centre has moved with respect to Width
        LOG_VERBOSE(CamxLogGroupPProc, "fullInW %d AdjInW %d OrigCropW (%d %d) AdjCropW (%d %d)",
            fullWidth, adjFullWidth,
            pCropInfo->left, pCropInfo->width,
            pAdjCropInfo->left, pAdjCropInfo->width);
    }

    if ((static_cast<UINT32>(2*pCropInfo->top + pCropInfo->height) == fullHeight) &&
        (static_cast<UINT32>(2*pAdjCropInfo->top + pAdjCropInfo->height) != adjFullHeight))
    {
        // Centre has moved with respect to Height
        LOG_VERBOSE(CamxLogGroupPProc, "fullInH %d AdjInH %d OrigCropH (%d %d) AdjCropH (%d %d)",
            fullHeight, adjFullHeight,
            pCropInfo->top, pCropInfo->height,
            pAdjCropInfo->top, pAdjCropInfo->height);
    }

    // Validation for non-centred zoom
    if (static_cast<UINT32>(pAdjCropInfo->left + pAdjCropInfo->width) >  adjFullWidth)
    {
        pAdjCropInfo->width = adjFullWidth - pAdjCropInfo->left;

        LOG_WARN(CamxLogGroupPProc, "adjCrop (%d %d %d %d) AdjImageSize (%d %d)",
            pAdjCropInfo->left, pAdjCropInfo->top, pAdjCropInfo->width, pAdjCropInfo->height,
            adjFullWidth, adjFullHeight);
    }

    if (static_cast<UINT32>(pAdjCropInfo->top + pAdjCropInfo->height) >  adjFullHeight)
    {
        pAdjCropInfo->height = adjFullHeight - pAdjCropInfo->top;
    ...
    UINT64        requestId)
{
    ...
    CHIRectangle origCropInfo;
    FLOAT  cropFactorW          = 1.0f;
    FLOAT  cropFactorH          = 1.0f;
    FLOAT  cropFactorOffsetLeft = 1.0f;
    FLOAT  cropFactorOffsetTop  = 1.0f;
    FLOAT  applyFOVCFactor      = 0.0f;

    origCropInfo.width  = pCropRect->width;
    origCropInfo.height = pCropRect->height;
    origCropInfo.left   = pCropRect->left;
    origCropInfo.top    = pCropRect->top;
    ...
    cropFactorW          = static_cast<FLOAT>(pCropRect->width)  / m_perSensorData[sensorIndex].inputSize.width;
    cropFactorH          = static_cast<FLOAT>(pCropRect->height) / m_perSensorData[sensorIndex].inputSize.height;
    ...
    pCropRect->width  = static_cast<INT32>((adjustedFullWidth  * cropFactorW) + 0.5);
    pCropRect->height = static_cast<INT32>((adjustedFullHeight * cropFactorH) + 0.5);
    pCropRect->left   = static_cast<INT32>((adjustedFullWidth  * cropFactorOffsetLeft) + 0.5);
    pCropRect->top    = static_cast<INT32>((adjustedFullHeight * cropFactorOffsetTop) + 0.5);

    // Validate zoom window after adjustment
    ValidateZoomWindow(pCropRect, &origCropInfo, m_perSensorData[sensorIndex].inputSize.width,
        m_perSensorData[sensorIndex].inputSize.height, adjustedFullWidth, adjustedFullHeight);
    ...
    LOG_VERBOSE(CamxLogGroupChi,
                ...
                "cropFactor [%f %f], left cropFactor %f, top cropFactor %f, fovc factor applied %f request %"
                PRIu64,
                pCropRect->left,
                pCropRect->top,
                pCropRect->width,
    ...
    CHINODEPROCESSREQUESTINFO* pProcessRequestInfo)
{
    ...
    if (pProcessRequestInfo->frameNum <= endOfStreamReqId)
    {
       ...
       // Avoid by-passing node if input buffer dependency is not met as this will result CVP timeout and use-case failure.
        for (UINT32 inputIdx = 0; inputIdx < pProcessRequestInfo->inputNum; inputIdx++)
        {
            if ((pProcessRequestInfo->phInputBuffer[inputIdx]->pfenceHandle                      != NULL) &&
                (pProcessRequestInfo->phInputBuffer[inputIdx]->pIsFenceSignaled                  != NULL) &&
                (*(static_cast<BOOL*> (pProcessRequestInfo->phInputBuffer[0]->pIsFenceSignaled)) == TRUE))
            {
                sequenceNumber = -1;
                LOG_VERBOSE(CamxLogGroupChi, "Triggering Req id %" PRIu64 ", seq %d fence signaled %d",
                    pProcessRequestInfo->frameNum,
                    sequenceNumber,
                    *(static_cast<BOOL*> (pProcessRequestInfo->phInputBuffer[0]->pIsFenceSignaled)));
            }
            // Avoid RequestIdDone until unless IFE's RequestIdDone happened.
            else
            {
                //This code change can create deadlock if ife is unable to complete the request.
                sequenceNumber = 0;
                LOG_INFO(CamxLogGroupChi, " Triggering Request id %" PRIu64 ", endOfStreamReqId %" PRIu64 ", seq num %d",
                    pProcessRequestInfo->frameNum,
                    endOfStreamReqId,
                    sequenceNumber);
            }
