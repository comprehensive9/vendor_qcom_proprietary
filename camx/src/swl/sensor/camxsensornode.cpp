...
SensorSubDevicesCache::SensorSubDevicesCache()
{
    ...
    {
        CHAR mutexLockName[256] = { 0 };

        OsUtils::SNPrintF(mutexLockName, sizeof(mutexLockName), "SubDeviceCache-cam%d", cameraId);

        m_pCacheLock[cameraId] = Mutex::Create(mutexLockName);

        if (NULL == m_pCacheLock[cameraId])
        {
            CAMX_LOG_ERROR(CamxLogGroupSensor, "Fail to create mutex lock: %s", mutexLockName);
            break;
        }
    }
}
...
SensorSubDevicesCache::~SensorSubDevicesCache()
{
    for (UINT32 cameraId = 0; cameraId < MaxNumImageSensors; ++cameraId)
    {
        if (NULL != m_pCacheLock[cameraId])
        {
            m_pCacheLock[cameraId]->Destroy();
            m_pCacheLock[cameraId] = NULL;
        }
    }
}
    ...
    SubDevice       type)
{
    ...
    if ((MaxSubDevices <= type) || !AcquireCacheLock(cameraId))
    {
        CAMX_LOG_ERROR(CamxLogGroupSensor,
                      "Mutex object NULL or Invalid params: cameraId=%u Subdevice=%u",
                       cameraId, type);
    ...
    else
    {
        ...
        if (m_sensorSubDevices[cameraId].hCSLSession[0] == hCSLSession)
        {
            if (FALSE == pSubDevice->isAcquired)
            {
                ...
                CAMX_LOG_INFO(CamxLogGroupSensor,
                             "CameraId=%u: Cached subdevice=%u hDevice=0x%x refCount=%u",
                              cameraId,
                              type,
                              handle,
                              m_sensorSubDevices[cameraId].refCount);
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
SensorNode::SensorNode()
    : m_pReadPacketManager(NULL)
    , m_pReadCmdBufferManager(NULL)
...
SensorNode::~SensorNode()
{
    ...
    if (NULL != m_pReadPacketManager)
    {
        m_pReadPacketManager->RecycleAllRequests();
    }

    if (NULL != m_pReadCmdBufferManager)
    {
        m_pReadCmdBufferManager->RecycleAllRequests();
    }
    ...
    if (NULL != m_pStrobeInfo)
    {
        CAMX_FREE(m_pStrobeInfo);
        m_pStrobeInfo = NULL;
    }
    ...
    UINT64 requestId)
{
    ...
    for (UINT32 index = 0; index < pResolutionData->streamInfo.streamConfigurationCount; index++)
    {
        if (StreamType::IMAGE == pResolutionData->streamInfo.streamConfiguration[index].type)
        {
            ...
            sensorModeCrop.left     = pResolutionData->streamInfo.streamConfiguration[index].frameDimension.xStart;
            sensorModeCrop.top      = pResolutionData->streamInfo.streamConfiguration[index].frameDimension.yStart;
            sensorModeCrop.right    = pResolutionData->streamInfo.streamConfiguration[index].frameDimension.xStart +
                                      (pResolutionData->streamInfo.streamConfiguration[index].frameDimension.width *
                                      pResolutionData->horizontalBinning);
            sensorModeCrop.bottom   = pResolutionData->streamInfo.streamConfiguration[index].frameDimension.yStart +
                                      (pResolutionData->streamInfo.streamConfiguration[index].frameDimension.height *
                                      pResolutionData->verticalBinning);
        }
    }
    ...
    UINT            sensorModeTag[5]     = { 0 };
    const VOID*     pSensorModeData[5]   = { 0 };
    UINT            pSensorModeCount[5]  = { 0 };
    UINT            tagIndex             = 0;

    CHIDIMENSION    sensorOutputDimension;
    sensorOutputDimension.width     = metaData.width;
    sensorOutputDimension.height    = metaData.height;

    UINT32  binning[2];
    binning[0]  = pResolutionData->horizontalBinning;
    binning[1]  = pResolutionData->verticalBinning;

    UINT32 lineReadoutTimeNs = GetSensorDataObject()->GetLineReadoutTime(m_currentResolutionIndex,
                                                                         currentSeamlessType);

    UINT32 adcReadoutTimeNs = pResolutionData->ADCReadoutTime * 1000000;

    VendorTagManager::QueryVendorTagLocation("org.quic.camera2.sensormode.info",
                                             "SensorModeDimension",
                                             &metaTag);
    sensorModeTag[tagIndex]       = metaTag;
    pSensorModeData[tagIndex]     = &sensorOutputDimension;
    pSensorModeCount[tagIndex]    = sizeof(sensorOutputDimension);
    tagIndex                     += 1;
    ...
    VendorTagManager::QueryVendorTagLocation("org.quic.camera2.sensormode.info",
                                             ...
                                             &metaTag);
    sensorModeTag[tagIndex]       = metaTag;
    pSensorModeData[tagIndex]     = &sensorModeCrop;
    pSensorModeCount[tagIndex]    = sizeof(sensorModeCrop);
    tagIndex                     += 1;
    ...
    VendorTagManager::QueryVendorTagLocation("org.quic.camera2.sensormode.info",
                                             ...
                                             &metaTag);
    sensorModeTag[tagIndex]       = metaTag;
    pSensorModeData[tagIndex]     = &binning[0];
    pSensorModeCount[tagIndex]    = sizeof(UINT32) * 2;
    tagIndex                     += 1;

    VendorTagManager::QueryVendorTagLocation("org.quic.camera2.sensormode.info",
                                             "SensorModeLineTimeNs",
                                             &metaTag);
    sensorModeTag[tagIndex]       = metaTag;
    pSensorModeData[tagIndex]     = &lineReadoutTimeNs;
    pSensorModeCount[tagIndex]    = 1;
    tagIndex                     += 1;

    VendorTagManager::QueryVendorTagLocation("org.quic.camera2.sensormode.info",
                                             "SensorModeADCReadoutTimeNs",
                                             &metaTag);
    sensorModeTag[tagIndex]       = metaTag;
    pSensorModeData[tagIndex]     = &adcReadoutTimeNs;
    pSensorModeCount[tagIndex]    = 1;
    tagIndex                     += 1;

    WriteDataList(sensorModeTag, pSensorModeData, pSensorModeCount, tagIndex);
    ...
    UINT64                  requestIdOffsetFromLastFlush)
{
    ...
    if ((TRUE == hasExplicitDependencies) && (TRUE == IsStatsNodeEnabled()))
    {
        ...
        if ((TRUE == m_isFPSMatchEnabled) && (TRUE == m_isMultiCameraUsecase) &&
        ...
        else if (CamxResultSuccess == result)
        {
            if ((FALSE == IsStatsNodeEnabled()) || (TRUE == isFSCaptureRequest))
            {
                ...
                if (CamxResultSuccess == result)
                {
                    ...
                    m_appliedSensorParam.previousStrobeOff        = m_appliedSensorParam.currentStrobeOff;
                    m_appliedSensorParam.previousStrobeWidthNs    = m_appliedSensorParam.currentStrobeWidthNs;
...
CamxResult SensorNode::InitializeMultiCameraInfo()
{
    ...
    m_isFPSMatchEnabled = IsFPSMatchEnabled();
    ...
    {
        CAMX_LOG_INFO(CamxLogGroupSensor,
                      "Sensor[%d] m_isMultiCameraUsecase=%d isMasterCamera=%d currentCameraId=%d m_isFPSMatchEnabled=%d",
                      currentCameraId,
                      m_isMultiCameraUsecase,
                      isMasterCamera,
                      m_isFPSMatchEnabled);
    }
    else
    ...
    ExecuteProcessRequestData*  pNodeRequestData)
{
    ...
    BOOL                  isFPSMatchEnabled = IsFPSMatchEnabled();

    if (m_isFPSMatchEnabled != isFPSMatchEnabled)
    {
        CAMX_LOG_CONFIG(CamxLogGroupSensor, "m_isFPSMatchEnabled is changing from %d to %d",
                        m_isFPSMatchEnabled, isFPSMatchEnabled);
        m_isFPSMatchEnabled = isFPSMatchEnabled;
    }
...
CamxResult SensorNode::CreatePerFrameUpdateResources()
{
    ...
    if (NULL == m_pStrobeInfo)
    {
        m_pStrobeInfo = static_cast<VOID*>(GetSensorDataObject()->Allocate(SensorAllocateType::StrobeInfo));
    }
    ...
    NodeProcessRequestData* pNodeProcessRequestData)
{
    ...
    VendorTagManager::QueryVendorTagLocation("org.quic.camera.strobeControl",
                                             "strobeControl",
                                             &metaTagStrobeControl);
    ...
    UINT metaTagFpsOverrideData[] =
    {
        ...
        metaTagStrobeControl               | InputMetadataSectionMask,  // 2
    };
    ...
    SensorStrobeInfo *strobeInfo = (reinterpret_cast<SensorStrobeInfo*>(m_pStrobeInfo));
    if (NULL != pInputData[2])
    {
        strobeInfo = (reinterpret_cast<SensorStrobeInfo*> (pInputData[2]));
    }
    ...
    m_appliedSensorParam.currentStrobeOff       = strobeInfo->strobeOff;
    m_appliedSensorParam.currentStrobeWidthNs   = strobeInfo->strobeWidthNs;
    GetSensorDataObject()->FillExposureArray(reinterpret_cast<VOID*>(&appliedExposureInfo),
                                             ...
                                             applyMiddleExposure,
                                             m_currentResolutionIndex,
                                             reinterpret_cast<VOID*>(strobeInfo));
}
    ...
    if (CamxResultSuccess == result)
    {
        ...
        if (CamxResultSuccess == result)
        {
            ...
            if (CamxResultSuccess == result)
            {
                ...
                // Packet for sensor read request
                pSensorReadPacket = GetPacket(m_pReadPacketManager);

                // Cmd buffer for sensor read request
                pReadCmd = GetCmdBuffer(m_pReadCmdBufferManager);
                ...
                if ((NULL != m_pReadPacketManager) && (NULL != pSensorReadPacket))
                {
                    m_pReadPacketManager->Recycle(pSensorReadPacket);
                    pSensorReadPacket = NULL;
                }

                if ((NULL != m_pReadCmdBufferManager) && (NULL != pReadCmd))
                {
                    m_pReadCmdBufferManager->Recycle(pReadCmd);
                    pReadCmd = NULL;
                }
            }
        }
    ...
    for (UINT i = 0; i < static_cast<UINT>(I2CRegSettingType::Max); i++)
    {
        ...
        switch (type)
        {
            ...
            case I2CRegSettingType::LSC:
                if ((NULL != m_pOTPData) && (0 != m_pOTPData->LSCCalibration[0].settings.regSettingCount))
                {
                    cmdSize = pSensorData->GetI2CCmdSize(I2CRegSettingType::LSC,
                                                         static_cast<const VOID*>(&m_pOTPData->LSCCalibration[0].settings),
                                                         0, &regSettingIdx);
                    if (cmdSize > maxCmdSize)
                    {
...
CamxResult SensorNode::CreateCmdBuffers()
{
    ...
    {
        CAMX_LOG_VERBOSE(CamxLogGroupSensor, "PacketManager created successfully");
    }
    else
    {
    ...
    if (CamxResultSuccess == result)
    {
        // Create Packet Manager for read request
        ResourceParams packetResourceParams                   = {0};
        packetResourceParams.usageFlags.packet                = 1;
        packetResourceParams.packetParams.maxNumCmdBuffers    = 1;
        ...
        result = CreateCmdBufferManager("ReadPacketMgr", &packetResourceParams, &m_pReadPacketManager);
        if ((CamxResultSuccess == result) && (NULL != m_pReadPacketManager))
        {
            CAMX_LOG_VERBOSE(CamxLogGroupSensor, "ReadPacketMgr created successfully");
        }
        else
        {
            CAMX_LOG_ERROR(CamxLogGroupSensor, "ReadPacketMgr not created rc=%d", result);
        }
    }

    if (CamxResultSuccess == result)
    {
        // Create command buffer to process read request
        ResourceParams resourceParams       = {0};
        ...
        result = CreateCmdBufferManager("ReadCmdBufferMgr", &resourceParams, &m_pReadCmdBufferManager);
        if ((CamxResultSuccess == result) && (NULL != m_pReadCmdBufferManager))
        {
            CAMX_LOG_VERBOSE(CamxLogGroupSensor, "ReadCmdBufferMgr created successfully");
        }
        else
        {
            CAMX_LOG_ERROR(CamxLogGroupSensor, "ReadCmdBufferMgr not created rc=%d", result);
        }
    }
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// SensorNode::LogStaticData
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
void SensorNode::LogStaticData()
{
    const StaticSettings* pStaticSettings = GetStaticSettings();

    CAMX_LOG_VERBOSE(CamxLogGroupSensor,
                    "Static: caching:%d, FPSMatchMask:%d, perframemode:%d, OISOptimization:%d, disableFlash:%d, disablePDAF:%d,"
                    "debugExposure:%d, dumpEEPROMData:%d, acquireOptimization:%d, multicameraEnable:%d, multicamera3ASync:%d"
                    "ForceMode:%d, HWSyncMask:0x%X, frameSyncMask:%d",
                    pStaticSettings->enableSensorCaching,
                    pStaticSettings->multiCameraFPSMatchMask,
                    pStaticSettings->perFrameSensorMode,
                    pStaticSettings->enableOISOptimization,
                    pStaticSettings->disableFlash,
                    pStaticSettings->disablePDAF,
                    pStaticSettings->enableDebugSensorExposure,
                    pStaticSettings->dumpSensorEEPROMData,
                    pStaticSettings->enableSensorAcquireOptimization,
                    pStaticSettings->multiCameraEnable,
                    pStaticSettings->multiCamera3ASync,
                    pStaticSettings->overrideForceSensorMode,
                    pStaticSettings->multiCameraHWSyncMask,
                    pStaticSettings->multiCameraFrameSyncMask);
}
