...

{
    ...
    if ((BT2020 == rGrallocProperties.colorSpace) || (BT2020_PQ == rGrallocProperties.colorSpace) ||
        (BT2020_HLG == rGrallocProperties.colorSpace))
    {
              ...
              (FALSE == rGrallocProperties.isInternalBuffer)))
    {
        ...
        else if (TRUE == Utils::IsBitMaskSet64(rGrallocProperties.grallocUsage, GrallocUsageUBWC))
        {
            ...
            {
                if (TRUE == Utils::IsBitMaskSet64(rGrallocProperties.grallocUsage, GrallocUsageUBWCNV124R))
                {
                    format        = Format::UBWCNV124R;
                    grallocFormat = GrallocFormatUBWCNV124R;
                }
                else
                {
    ...
    else if (TRUE == Utils::IsBitMaskSet64(rGrallocProperties.grallocUsage, GrallocUsageUBWC|GrallocUsage10Bit))
    {
        format        = Format::UBWCTP10;
        grallocFormat = GrallocFormatUBWCTP10;;
    }
    else if (TRUE == Utils::IsBitMaskSet64(rGrallocProperties.grallocUsage, GrallocUsageUBWCNV124R))
    {
        format        = Format::UBWCNV124R;
        grallocFormat = GrallocFormatUBWCNV124R;
    }
