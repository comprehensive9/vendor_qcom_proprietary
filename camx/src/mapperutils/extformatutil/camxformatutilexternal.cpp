...
static const UINT  CameraTitanSocSM7250P    = 476;                        ///< SDM7250P SOC Id
...
static const UINT  CameraTitanSocQCS8250    = 481;                        ///< QCS8250 SOC Id
    ...
    CamxExternalFormatCaps& rSOCCaps)
{
    ...
    switch (rSOCCaps.m_socId)
    {
        ...
        case CameraTitanSocQCS8250:
            rSOCCaps.m_isUbwcFormatSupported   = true;
            rSOCCaps.m_ubwcFormatCount         = 4;
            ...
            pUbwcFormatCaps                   = &rSOCCaps.m_ubwcFormatCaps[3];
            pUbwcFormatCaps->ubwcFormat       = CAMERA_PIXEL_FORMAT_YCbCr_420_UBWC4R;
            pUbwcFormatCaps->version          = 4;
            pUbwcFormatCaps->isLossySupported = true;
            pUbwcFormatCaps->isSupported      = true;
            break;
    ...
    int*            pPlanetypeCount)
{
    ...
    if (NULL != pPlanetypesArray)
    {
        switch (pixelFormat)
        {
            ...
            case CAMERA_PIXEL_FORMAT_YCbCr_420_UBWC4R:
            {
                pPlanetypesArray[0] = CAMERA_PLANE_TYPE_Y;
                pPlanetypesArray[1] = CAMERA_PLANE_TYPE_UV;
                pPlanetypesArray[2] = CAMERA_PLANE_TYPE_META_Y;
    ...
    if ((NULL != pPlaneInfo) && (0 < width))
    {
        ...
        switch (pixelFormat)
        {
            ...
            case CAMERA_PIXEL_FORMAT_YCbCr_420_UBWC4R:
            {
                if (CAMERA_PLANE_TYPE_Y == planeType)
                {
                    pPlaneInfo->stride   = MSM_MEDIA_ALIGN(width, 256);
                    pPlaneInfo->scanline = MSM_MEDIA_ALIGN(height, 16);
                }
                else if (CAMERA_PLANE_TYPE_UV == planeType)
                {
                    pPlaneInfo->stride   = MSM_MEDIA_ALIGN(width, 256);
                    pPlaneInfo->scanline = MSM_MEDIA_ALIGN((height + 1) >> 1, 16);
                }
                else if (CAMERA_PLANE_TYPE_META_Y == planeType)
                {
                    int y_tile_width     = 64;
                    int y_tile_height    = 4;
                    int y_meta_stride    = 0;
                    int y_meta_scanlines = 0;

                    y_meta_stride        = MSM_MEDIA_ROUNDUP(width, y_tile_width);
                    pPlaneInfo->stride   = MSM_MEDIA_ALIGN(y_meta_stride, 64);

                    y_meta_scanlines     = MSM_MEDIA_ROUNDUP(height, y_tile_height);
                    pPlaneInfo->scanline = MSM_MEDIA_ALIGN(y_meta_scanlines, 16);
                }
                else if (CAMERA_PLANE_TYPE_META_VU == planeType)
                {
                    int uv_tile_width     = 32;
                    int uv_tile_height    = 4;
                    int uv_meta_stride    = 0;
                    int uv_meta_scanlines = 0;

                    uv_meta_stride       = MSM_MEDIA_ROUNDUP((width + 1) >> 1, uv_tile_width);
                    pPlaneInfo->stride   = MSM_MEDIA_ALIGN(uv_meta_stride, 64);

                    uv_meta_scanlines    = MSM_MEDIA_ROUNDUP((height + 1) >> 1, uv_tile_height);
                    pPlaneInfo->scanline = MSM_MEDIA_ALIGN(uv_meta_scanlines, 16);
                }
        ...
        switch (pixelFormat)
        {
            ...
            case CAMERA_PIXEL_FORMAT_YCbCr_420_UBWC4R:
            {
                if (CAMERA_PLANE_TYPE_Y == planeType)
                {
                    ...
                    *pStride = MSM_MEDIA_ALIGN(width, strideAlign);
                }
                else if (CAMERA_PLANE_TYPE_UV == planeType)
                {
                    strideAlign = 256;
                ...
                else if (CAMERA_PLANE_TYPE_META_Y == planeType)
                {
                    ...
                    local     = MSM_MEDIA_ROUNDUP(width, y_tile_width);
                    *pStride  = MSM_MEDIA_ALIGN(local, 64);
                }
                else if (CAMERA_PLANE_TYPE_META_VU == planeType)
                {
                    int uv_tile_width  = 32;
                    ...
                    local = MSM_MEDIA_ROUNDUP((width + 1) >> 1, uv_tile_width);
                    *pStride       = MSM_MEDIA_ALIGN(local, 64);
                }
           ...
           case CAMERA_PIXEL_FORMAT_RAW12:
            ...
            case CAMERA_PIXEL_FORMAT_YCbCr_420_UBWC4R:
            {
                if (CAMERA_PLANE_TYPE_Y == planeType)
                {
                    ...
                    *pScanline = MSM_MEDIA_ALIGN(height, scanLineAlign);
                }
                else if (CAMERA_PLANE_TYPE_UV == planeType)
                {
                    scanLineAlign = 16;

                    *pScanline = MSM_MEDIA_ALIGN((height + 1) >> 1, scanLineAlign);
                }
                else if (CAMERA_PLANE_TYPE_META_Y == planeType)
                {
                    scanLineAlign = 16;
                    ...
                    local      = MSM_MEDIA_ROUNDUP(height, y_tile_height);
                    *pScanline = MSM_MEDIA_ALIGN(local, 16);
                }
                else if (CAMERA_PLANE_TYPE_META_VU == planeType)
                {
                    scanLineAlign = 16;
                    ...
                    local      = MSM_MEDIA_ROUNDUP((height + 1) >> 1, uv_tile_height);
                    *pScanline = MSM_MEDIA_ALIGN(local, 16);
                }
                result = CamxFormatResultSuccess;
            }
    ...
    if (NULL != pSubsamplingFactor)
    {
        ...
        switch (pixelFormat)
        {
            ...
            case CAMERA_PIXEL_FORMAT_MULTIPLANAR_FLEX:
            {
                ...
                {
                    *pSubsamplingFactor = 1; // H2V2 formats
                    result              = CamxFormatResultSuccess;
                }
            }
            break;
    ...
    if ((NULL != pFlexFormatInfo) && (sizeof(CamxFlexFormatInfo) == pFlexFormatInfo->size))
    {
        ...
        if (count + 1 < CamxFormatUtilMaxFlexibleFormats)
        {
            if ((CameraTitanSocSDM865  == rSOCCaps.m_socId) || (CameraTitanSocSM7250  == rSOCCaps.m_socId) ||
                ...
                (CameraTitanSocSM7225  == rSOCCaps.m_socId) || (CameraTitanSocSM7250P  == rSOCCaps.m_socId) ||
                (CameraTitanSocQCS8250  == rSOCCaps.m_socId))
            {
