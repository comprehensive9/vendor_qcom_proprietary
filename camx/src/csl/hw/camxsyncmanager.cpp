    ...
    VOID* pPollData)
{
    ...
    while (1)
    {
        ...
        if (rc > 0)
        {
            if (fds[0].revents & POLLPRI)
            {
                ...
                if (NULL == pEv)
                {
                    CAMX_LOG_ERROR(CamxLogGroupSync, "No memory");
                    break;
                }
            ...
            if (fds[1].revents & POLLIN)
            {
                ...
                {
                    CAMX_LOG_ERROR(CamxLogGroupSync, "Exiting!");
                    break;
                }
                else
                {
        ...
        else
        {
            ...
            CAMX_LOG_ERROR(CamxLogGroupSync, "Poll break: %d: %s", rc, errnoStr);
        }
    }
    ...
    close(pCtrl->syncFd);
    CAMX_LOG_ERROR(CamxLogGroupSync, "Thread killed Closed Sync FD = %d", pCtrl->syncFd);
    pthread_exit(NULL);
}
    ...
    INT32* pSyncid)
{
    ...
    if (rc < 0)
    {
        ...
        CAMX_LOG_ERROR(CamxLogGroupSync,
                       "Create syncObj failed FD:%d with err reason %s rc %d",
                       s_ctrl.syncFd, errnoStr, errno);
        result = CamxResultEFailed;
    }
