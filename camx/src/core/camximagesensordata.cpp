    ...
    VOID * pRegisterAddressInfo)
{
    ...
    pRegAddressInfo->strobeStartAddr            = m_pSensorData->regAddrInfo.strobeStartAddr;
    pRegAddressInfo->strobeWidthAddr            = m_pSensorData->regAddrInfo.strobeWidthAddr;
    ...
    VOID*  pStrobeControlParams)
{
    ...
    SensorStrobeInfo*       pStrobeControl      = reinterpret_cast<SensorStrobeInfo*>(pStrobeControlParams);
    ...
    if (NULL != m_sensorLibraryAPI.pFillExposureSettings)
    {
        ...
        exposureData.pStrobeInfo                 = pStrobeControl;
    ...
    UINT    resolutionIndex)
{
    ...
    if (0 != sofTimeStamp)
    {
        ...
        retVal = static_cast<UINT64>(
            ...
            (pResolutionInfo->resolutionData[resolutionIndex].ADCReadoutTime * 1000000));
        CAMX_LOG_VERBOSE(CamxLogGroupSensor, "sofTimeStamp %llu LineReadoutTime %llu currentExposure %llu ADCReadoutTime %f",
                                              sofTimeStamp, lineReadoutTime, currentExposure,
                                              (pResolutionInfo->resolutionData[resolutionIndex].ADCReadoutTime *
                                              1000000));
    }
    ...
    UINT32                  cameraID)
{
    ...
    if ((NULL != pCapability) && (NULL != m_pSensorData))
    {
        ...
        ResolutionData resolutionData = m_pSensorData->resolutionInfo->resolutionData[0];
        DOUBLE maxFrameRateNormal     = resolutionData.frameRate;
        for (UINT resolutionIndex = 0; resolutionIndex < pResolutionInfo->resolutionDataCount; resolutionIndex++)
        {
            DOUBLE resolutionFPS = 1.0;
            resolutionData       = m_pSensorData->resolutionInfo->resolutionData[resolutionIndex];
            for (UINT j = 0; j < resolutionData.capabilityCount; j++)
            {
                if (SensorCapability::NORMAL == resolutionData.capability[j])
                {
                    resolutionFPS = resolutionData.frameRate;
                    break;
                }

            }
            if (resolutionFPS > maxFrameRateNormal)
            {
                maxFrameRateNormal = resolutionFPS;
            }
        }

        pCapability->defaultExposureTime = static_cast<UINT64>(NanoSecondsPerSecond / maxFrameRateNormal);
        CAMX_LOG_VERBOSE(CamxLogGroupSensor, "Default Exposure Time %llu for %s",
                pCapability->defaultExposureTime, pCapability->pSensorName);
        for (UINT resolutionIndex = 0; resolutionIndex < pResolutionInfo->resolutionDataCount; resolutionIndex++)
        {
    ...
    SensorAllocateType type)
{
    ...
    switch (type)
    {
        ...
        case SensorAllocateType::StrobeInfo:
            pAllocatedPtr = static_cast<VOID*>(CAMX_CALLOC(sizeof(SensorStrobeInfo)));
            break;
