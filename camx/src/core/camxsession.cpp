...

{
    ...
    const StaticSettings* pStaticSettings = HwEnvironment::GetInstance()->GetStaticSettings();
    if ((MultiCameraSyncDisabled != pStaticSettings->multiCameraHWSyncMask) && (m_numInputSensors > 1))
    {
        UINT32  metaTag                           = 0;
        VOID*   pSyncMode                         = NULL;
    ...
    SessionCreateData* pCreateData)
{
    ...
    if (CamxResultSuccess == result)
    {
        ...
        if (CamxResultSuccess == result)
        {
            ...
            m_notifyMessages.freeIndex          = 0;
            m_PartialDataMessages.freeIndex     = 0;
            m_resultStreamBuffers.freeIndex     = 0;
    ...
    UINT*         pNumResults)
{
    ...
    if (NULL != pResultHolder)
    {
        ...
        {
            CAMX_LOG_INFO(CamxLogGroupCore,
            "RequestId: %u SequenceId: %u Framenumber: %u - All results were injected with buffer error,"
    ...
    VOID*           pPrivData)
{
    ...
    switch (pError->code)
    {
        ...
        case MessageCodeRequest:
            ...
            pPayloads[0] = pNotify;
            InjectResult(ResultType::RequestError, pPayloads[0], pError->sequenceId, NULL, pipelineIndex);
            break;
    ...
    UINT32*                   pPipelineIndexes)
{
    ...
    if (numOfRealtimePipelines > 1)
    {
        result = GetMultiCameraSync(pCaptureRequest, &isFrameSyncEnabled);
    }

    if ((CamxResultSuccess == result) && (TRUE == IsRealtimeSession()) && (TRUE == isFrameSyncEnabled))
    {
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// Session::GetMultiCameraSync
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
CamxResult Session::GetMultiCameraSync(
    const ChiCaptureRequest* pCaptureRequest,
    BOOL*                    pEnable)
{
    CamxResult      result                      = CamxResultSuccess;
    MetaBuffer*     pInputMetabuffer            = reinterpret_cast<MetaBuffer*>(pCaptureRequest->pInputMetadata);
    SyncModeInfo*   pSyncModeInfo               = NULL;


    pSyncModeInfo = static_cast<SyncModeInfo*>(pInputMetabuffer->GetTag(m_sessionSync.syncModeTagID, TRUE));
    if (NULL != pSyncModeInfo)
    {
        *pEnable = pSyncModeInfo->isSyncModeEnabled;
    }
    else
    {
        *pEnable = FALSE;
        result   = CamxResultEFailed;
        CAMX_LOG_ERROR(CamxLogGroupCore, "Get Multi Camera frame sync metadata failed %p", pSyncModeInfo);
    }
