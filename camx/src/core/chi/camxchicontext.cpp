    ...
    UINT32 cameraId)
{
    ...
    {
        isLiveshotSizeSameAsVideoSize = FALSE;
    }
    ...
    CHIPIPELINEINPUTOPTIONS*           pPipelineInputOptions)
{
    ...
    if (NULL != pPipelineDescriptor)
    {
        ...
        // Check HDR-video mode for HLG or HDR10
        MetaBuffer* pMetaBuffer = pPipelineDescriptor->pSessionMetadata;
        INT32 HDRModeValue = 1;
        VOID* pSlotData = NULL;
        UINT32 metaDataTag = 0;

        result = VendorTagManager::QueryVendorTagLocation("org.codeaurora.qcamera3.sessionParameters", "HDRVideoMode",
                                                          &metaDataTag);
        if ((NULL != pMetaBuffer) && (CamxResultSuccess == result))
        {
            pSlotData = pMetaBuffer->GetTag(metaDataTag, TRUE);
            if (NULL != pSlotData)
            {
                HDRModeValue = *static_cast<INT32*>(pSlotData);
            }
        }
        ...
        for (UINT streamId = 0; streamId < numOutputs; streamId++)
        {
                ...
                (TRUE == overrideImpDefinedFormat.isHDR))
            {
                ...
                {
                    pChiStream->dataspace = DataspaceStandardBT2020_HLG;
                }
            }
            ...
            // when the output block is the IFE, use UBWCNV124R
            if ((ChiStreamFormatImplDefined                            == grallocProperties.pixelFormat) &&
                (selectedFormat                                        == Format::UBWCNV12) &&
                (pOutputBufferDescriptor[streamId].pNodePort[0].nodeId == IFE))
            {
                selectedFormat = Format::UBWCNV124R;
            }
    ...
    OverrideOutputFormat  overrideImpDefinedFormat)
{
    ...
    if (NULL != pStream)
    {
        ...
        else if (ChiStreamFormatRaw8 == pixelFormat)
        {
            format = Format::RawMIPI8;
        }
                ...
                if (ChiStreamFormatImplDefined == pStream->format)
                {
                    ...
                    switch (pChiStreamWrapper->GetInternalFormat())
                    {
                        ...
                        case Format::UBWCNV124R:
                            grallocUsage  |= (GrallocUsageUBWC  | GrallocUsageUBWCNV124R);
                            overrideFormat = ChiStreamFormatUBWCNV124R;
                            break;
