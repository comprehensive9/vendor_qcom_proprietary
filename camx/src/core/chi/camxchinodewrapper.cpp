                ...
                for (UINT i = 0; i < info.pDependency->count; i++)
                {
                    ...
                    else
                    {
                        ...
                        if (FALSE == GetPipeline()->IsPSMPublished(tag))
                        {
                            ...
                            CAMX_LOG_VERBOSE(CamxLogGroupChi, "Node::%s vendor tag value %x, mapped property value %x,"
                                             " offset %d", NodeIdentifierString(), tag,
                ...
                if (0 < fenceCount)
                {
                    ...
                    if (NULL != pFenceCallbackInfo)
                    {
                        CAMX_LOG_VERBOSE(CamxLogGroupChi,
                                         ...
                                         NodeIdentifierString(), fenceCount, info.frameNum,
                                         info.pDependency->processSequenceId);
                    ...
                    pNodeRequestData->dependencyInfo[0].dependencyFlags.hasFenceDependency   =
                        TRUE;
                    pNodeRequestData->numDependencyLists                                     =
                        1;
                    pNodeRequestData->dependencyInfo[0].chiFenceDependency.chiFenceCount     =
                        fenceCount;
                    pNodeRequestData->dependencyInfo[0].chiFenceDependency.pUserData         =
                        pFenceUserdata;
                    pNodeRequestData->dependencyInfo[0].chiFenceDependency.pChiFenceCallback =
                        ...
                        for (UINT i = 0; i < pPerRequestPorts->numOutputPorts; i++)
                        {
                            ...
                            if ((NULL != pOutputPort) && (TRUE == pOutputPort->flags.isDelayedBuffer))
                            {
                                ...
                                if (TRUE == isBypassNeeded)
                                {
                                    ...
                                    if (bypassIndex < pPerRequestPorts->numInputPorts)
                                    {
                                        ...
                                        const PerRequestInputPortInfo* pInputInfo   =
                                            &pPerRequestPorts->pInputPorts[bypassIndex];
                                        ImageBuffer* const             pImageBuffer =
                                        ...
                                        imageBufferReleaseCount[numBuffersToRelease]   = (pImageBuffer->GetReferenceCount()
                                            - 1);
                                    ...
                                    else
                                    {
                                        ...
                                        CAMX_LOG_ERROR(CamxLogGroupChi,
                                                       "Node::%s Cannot bypass request: %llu - Input Idx: %u OOB |"
                                                       " NumInputs: %u Output Port Id: %u",
                            ...
                            for (UINT idx = 0; idx < numBuffersToRelease; idx++)
                            {
                                ...
                                for (UINT j = 0; j < imageBufferReleaseCount[idx]; j++)
                                {
                                    ...
                                    {
                                        CAMX_LOG_WARN(CamxLogGroupChi, "Node: %s Cannot release last image buffer"
                                             " reference here!", NodeIdentifierString());
                                    }
                                }
                            }
