    ...
    if (NULL == GetInstance(cameraId)->m_pMetadataTemplates[requestTemplateIndex])
    {
            ...
            (requestTemplate <= RequestTemplateManual))
        {
            ...
            if ((NULL != pMetadata) && (NULL != cameraInfo.pPlatformCaps) && (NULL != cameraInfo.pSensorCaps))
            {
                ...
                // Add strobeControl setting
                UINT32 strobeControlTag;
                StrobeControl strobeControlValue { 1, 0, 0 };

                result = VendorTagManager::QueryVendorTagLocation("org.quic.camera.strobeControl",
                                                                  "strobeControl",
                                                                  &strobeControlTag);
                CAMX_ASSERT_MESSAGE(CamxResultSuccess == result, "Failed to get vendor tag location for strobeControl");
                if (CamxResultSuccess == result)
                {
                    result = HAL3MetadataUtil::SetMetadata(pMetadata,
                                                           strobeControlTag,
                                                           reinterpret_cast<VOID*>(&strobeControlValue),
                                                           sizeof(StrobeControl));
                    CAMX_ASSERT_MESSAGE(CamxResultSuccess == result,
                                        "Failed to write vendor tag strobeControl to default metadata.");
                }
                ...
                // exposure metering update
                UINT32 exposureMeteringModeTag;
                INT32  defaultExposureMeteringModeValue = 0;

                result = VendorTagManager::QueryVendorTagLocation("org.codeaurora.qcamera3.exposure_metering",
                                                                  "exposure_metering_mode",
                                                                  &exposureMeteringModeTag);
                CAMX_ASSERT_MESSAGE(CamxResultSuccess == result,
                                    "Failed to get vendor tag location for exposureMeteringMode");
                if (CamxResultSuccess == result)
                {
                    result = HAL3MetadataUtil::SetMetadata(pMetadata, exposureMeteringModeTag,
                                                           reinterpret_cast<VOID*>(&defaultExposureMeteringModeValue),
                                                           1);
                    CAMX_ASSERT_MESSAGE(CamxResultSuccess == result,
                                        "Failed to write vendor tag exposureMeteringMode to default metadata.");
                }
