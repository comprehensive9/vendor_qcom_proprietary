    ...
    ) const
{
    ...
    if (CamxResultSuccess == result)
    {
        ...
        for (UINT32 stream = 0; stream < pStreamConfigs->numStreams; stream++)
        {
           ...
           if ((FALSE == isAvailableStream) &&
                (StreamTypeOutput       == pStreamConfigs->ppStreams[stream]->streamType) &&
                ((HALPixelFormatRaw16   == pStreamConfigs->ppStreams[stream]->format) ||
                (HALPixelFormatY16      == pStreamConfigs->ppStreams[stream]->format)))
            {
                isAvailableStream = TRUE;
                CAMX_LOG_INFO(CamxLogGroupHAL,
                    "Output(Raw16,Y16) Valid streamStype: %d, format: %d, Accept size (%u x %u) ",
                    pStreamConfigs->ppStreams[stream]->streamType,
                    pStreamConfigs->ppStreams[stream]->format,
                    pStreamConfigs->ppStreams[stream]->width,
                    pStreamConfigs->ppStreams[stream]->height);
    ...
    Camera3StreamConfig* pStreamConfigs)
{
    ...
    if (CamxResultSuccess == result)
    {
        ...
        {
            if (TRUE == HwEnvironment::GetInstance()->GetStaticSettings()->EnableDisplaySmomo)
            {
                DisplayConfigInterface::SetCameraSmoothInfo(FALSE, 0);
            }
        ...
        // Config smomo feature for display hal
        if ((TRUE == HwEnvironment::GetInstance()->GetStaticSettings()->EnableDisplaySmomo) &&
            (TRUE == m_bCHIModuleInitialized))
        {
            BOOL  isVideoMode = FALSE;
            for (UINT32 stream = 0; stream < pStreamConfigs->numStreams; stream++)
            {
                Camera3Stream* pStream = pStreamConfigs->ppStreams[stream];
                if (0 != (pStream->grallocUsage & GrallocUsageHwVideoEncoder))
                {
                    isVideoMode = TRUE;
                    break;
                }
            ...
            if (TRUE == isVideoMode)
            {
                ...
                pMetadata = reinterpret_cast<camera_metadata*>(pStreamConfigs->pSessionParameters);
                ...
                if (pMetadata != NULL)
                {
                    ...
                    INT32 ret = find_camera_metadata_entry(pMetadata, entry.tag, &entry);

                    if (CamxResultSuccess == ret)
                    {
...
#endif // ((CAMX_ANDROID_API) && (CAMX_ANDROID_API >= 28))
                if (((30 == minSessionFps || 60 == minSessionFps)) &&
                    (minSessionFps == maxSessionFps))
                {
                    DisplayConfigInterface::SetCameraSmoothInfo(TRUE, minSessionFps);
                }
            }
        }
