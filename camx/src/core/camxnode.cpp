    ...
    BOOL*                pIsNodeEnabled)
{
    ...
    m_perRequestInfo[requestIdIndex].activePorts.numOutputPorts = 0;
    m_perRequestInfo[requestIdIndex].activePorts.numInputPorts  = 0;
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// Node::GetMetadataPartialMwbMode
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
VOID Node::GetMetadataPartialMwbMode(
    ISPPartialMWBMode* pPartialMwbMode)
{
    UINT32     metaTag = 0;
    CamxResult result  = VendorTagManager::QueryVendorTagLocation("org.codeaurora.qcamera3.manualWB",
                                                                    "partial_mwb_mode",
                                                                    &metaTag);
    if (CamxResultSuccess == result)
    {
        static const UINT VendorTag[] =
        {
            metaTag | InputMetadataSectionMask,
        };

        const static UINT length            = CAMX_ARRAY_SIZE(VendorTag);
        VOID* pData[length]                 = { 0 };
        UINT64 vendorTagsIFEOffset[length]  = { 0 };

        GetDataList(VendorTag, pData, vendorTagsIFEOffset, length);
        if (NULL != pData[0])
        {
            *pPartialMwbMode = *(static_cast<CamX::ISPPartialMWBMode*>(pData[0]));
            CAMX_LOG_VERBOSE(CamxLogGroupCore, "Node:: %s partialMwbMode = %d", NodeIdentifierString(), *pPartialMwbMode);
        }
        else
        {
            CAMX_LOG_ERROR(CamxLogGroupCore, "Node:: %s Cannot obtain partial MWB mode. Set default to Disable",
                NodeIdentifierString());
            *pPartialMwbMode = MWBDisable;
        }
    }
    else
    {
        CAMX_LOG_WARN(CamxLogGroupCore, "Node:: %s No partial MWB mode available. Set default to Disable",
            NodeIdentifierString());
        *pPartialMwbMode = MWBDisable;
    }
}
    ...
    CSLFenceResult  fenceResult)
{
    ...
    pNode-> m_pFenceCreateReleaseLock-> Lock();
    ...
    pNode-> m_pFenceCreateReleaseLock-> Unlock();
    ...
    NodeFenceHandlerData* pFenceHandlerData)
{
    ...
    if (TRUE == CamxAtomicCompareExchangeU(&pFenceHandlerData->isFenceSignaled, 0, 1))
    {
        ...
        else if (TRUE == pOutputPort->flags.isNonSinkHALBufferOutput)
        {
            NotifyAndReleaseFence(pOutputPort, pFenceHandlerData->requestId);
        }
        m_pBufferRequestLock->Unlock();
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// Node::IsFPSMatchEnabled
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
BOOL Node::IsFPSMatchEnabled()
{
    CamxResult result            = CamxResultSuccess;
    BOOL       isFPSMatchEnabled = FALSE;
    UINT32     FPSMatchTag       = 0;

    result = VendorTagManager::QueryVendorTagLocation("com.qti.chi.multicamerainfo", "FPSMatch", &FPSMatchTag);

    if (CamxResultSuccess == result)
    {
        FPSMatchTag |= InputMetadataSectionMask;

        const UINT          propertyTag[]          = { FPSMatchTag };
        ...
        if (NULL != pData[0])
        {
            UINT32* pIsFPSMatchEnable = static_cast<UINT32*>(pData[0]);

            isFPSMatchEnabled = static_cast<BOOL>(*pIsFPSMatchEnable);
        }
        else
        {
            CAMX_LOG_ERROR(CamxLogGroupCore, "Node::%s : read FPSMatch vendor tag failed",
                NodeIdentifierString());
        }
    }
    else
    {
        CAMX_LOG_ERROR(CamxLogGroupCore,
                       "Node::%s : FPSMatch vendor tag location not available %d",
                       NodeIdentifierString(), FPSMatchTag);
    }

    return isFPSMatchEnabled;
}
...
CamxResult Node::IsEarlyPCREnabled(
    BOOL& rIsEarlyPCREnabled)
{
    CamxResult result             = CamxResultSuccess;
    BOOL       isBatchModeEnabled = ((1 < m_pPipeline->GetBatchedHALOutputNum()) ? TRUE : FALSE);

    rIsEarlyPCREnabled = FALSE;

    if (FALSE == isBatchModeEnabled)
    {
        if (0 < GetStaticSettings()->numPCRsBeforeStreamOn)
        {
            rIsEarlyPCREnabled = TRUE;
        }
        else
        {
            MetadataSlot*  pCurrentSlot           = m_pUsecasePool->GetSlot(0);
            UINT32*        pNumPCRsBeforeStreamOn = NULL;
            ...
            if (CamxResultSuccess == result)
            {
                pNumPCRsBeforeStreamOn =
                    static_cast<UINT32*>(pCurrentSlot->GetMetadataByTag(tagLocation, NodeIdentifierString()));
                if (NULL != pNumPCRsBeforeStreamOn)
                {
                    if (0 < *pNumPCRsBeforeStreamOn)
                    {
                        rIsEarlyPCREnabled = TRUE;
                    }
                }
                ...
                {
                    CAMX_LOG_WARN(CamxLogGroupCore, "Disabling earlyPCR, numPCRsBeforeStreamOn vendor tag is not initialized.");
                }
            }
            else
            {
                CAMX_LOG_ERROR(CamxLogGroupCore, "Disabling earlyPCR, could not query numPCRsBeforeStreamOn vendor tag."
                                                 " result: %d.", result);
            }
        }
    }
    ...
    UINT64 lastCompletedRequestId)
{
    ...
    for (UINT32 portIndex = 0; portIndex < m_inputPortsData.numPorts; portIndex++)
    {
        ...
        if (TRUE == pInputPort->flags.isSourceBuffer)
        {
            if (0 != bufferDelta)
            {
                CAMX_LOG_INFO(CamxLogGroupCore, "Node %s Input Port id %u is source port with buffer delta %llu ",
                              NodeIdentifierString(), pInputPort->portId, bufferDelta);
            }
            continue;
        }
        ...
        for (bufferDelay = 0; bufferDelay < bufferDelta; bufferDelay++)
        {
            ...
            {
                CAMX_LOG_WARN(CamxLogGroupCore, "For node %s input port id %u Parent node is Null, cannot clear dependency",
                              NodeIdentifierString(), pInputPort->portId);
            }
        }
    }
}
