    ...
    JobRegistry*    pJobRegistry)
{
    ...
    if ((m_head != m_tail))
    {
        ...
        while (localHead != m_tail)
        {
            if ((JobStatus::Submitted == m_pJobs[localHead]->status) &&
                 ((FALSE == pJobRegistry->IsSerial(m_pJobs[localHead]->hJob)) ||
                  (0 == pJobRegistry->GetInflightCount(m_pJobs[localHead]->hJob))))
            {
