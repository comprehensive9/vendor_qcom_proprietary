...
TEST_F(BluetoothHidlTest, BTSarNewTest) {
  bluetooth_init();
  if(bluetooth_sar)
...
TEST_F(BluetoothHidlTest, BTSarNewTestOutOfRange) {
  bluetooth_init();
  if(bluetooth_sar) {
...
TEST_F(BluetoothHidlTest, BTSarNewTestNegativePowerLevel) {
  bluetooth_init();
  if(bluetooth_sar) {
...
TEST_F(BluetoothHidlTest, BTSarNewTestAfterBToff) {
  if(bluetooth_sar) {
   bluetooth_sar->setBluetoothTechBasedTxPowerCap(3, 4, 5);
...
TEST_F(BluetoothHidlTest, BTSarNewTestAfterDebugInfo) {
  hidl_vec<uint8_t> cmd = COMMAND_HCI_DEBUG_INFO;
  bluetooth_init();
  bluetooth->sendHciCommand(cmd);
  ALOGI("VTS:: sending debug info cmd");
  bluetooth_sar =
          ::testing::VtsHalHidlTargetTestBase::getService<IBluetoothSar>(BluetoothHidlEnvironment::Instance()->getServiceName<IBluetoothSar>());
  if(bluetooth_sar) {
    bluetooth_sar->setBluetoothTechBasedTxPowerCap(2, 3, 4);
  }
  bluetooth_sar =
          ::testing::VtsHalHidlTargetTestBase::getService<IBluetoothSar>(BluetoothHidlEnvironment::Instance()->getServiceName<IBluetoothSar>());
  if(bluetooth_sar) {
