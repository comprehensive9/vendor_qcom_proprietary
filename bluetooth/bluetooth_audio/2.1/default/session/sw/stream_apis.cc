...
#define LOG_TAG "BTAudioHalStream_2_1"
...
using ::vendor::qti::bluetooth_audio::V2_1::BluetoothAudioPortOut;
...
static uint32_t out_get_sample_rate_2_1(const struct audio_stream* stream) {
...
static int out_set_sample_rate_2_1(struct audio_stream* stream, uint32_t rate) {
...
static size_t out_get_buffer_size_2_1(const struct audio_stream* stream) {
  const auto* out = reinterpret_cast<const BluetoothStreamOut*>(stream);
  size_t buffer_size =
...
static audio_channel_mask_t out_get_channels_2_1(
...
static audio_format_t out_get_format_2_1(const struct audio_stream* stream) {
  const auto* out = reinterpret_cast<const BluetoothStreamOut*>(stream);
  audio_config_t audio_cfg;
  if (out->bluetooth_output_.LoadAudioConfig(&audio_cfg)) {
    LOG(VERBOSE) << __func__ << ": state=" << out->bluetooth_output_.GetState()
...
static int out_set_format_2_1(struct audio_stream* stream, audio_format_t format) {
  auto* out = reinterpret_cast<BluetoothStreamOut*>(stream);
  LOG(VERBOSE) << __func__ << ": state=" << out->bluetooth_output_.GetState()
               << ", format=" << out->format_;
...
static int out_standby_2_1(struct audio_stream* stream) {
  auto* out = reinterpret_cast<BluetoothStreamOut*>(stream);
...
static int out_dump_2_1(const struct audio_stream* stream, int fd __unused) {
  const auto* out = reinterpret_cast<const BluetoothStreamOut*>(stream);
  LOG(VERBOSE) << __func__ << ": state=" << out->bluetooth_output_.GetState();
  return 0;
}

static int out_set_parameters_2_1(struct audio_stream* stream,
                              const char* kvpairs) {
  auto* out = reinterpret_cast<BluetoothStreamOut*>(stream);
  std::unique_lock<std::mutex> lock(out->mutex_);
  int retval = 0;
...
static char* out_get_parameters_2_1(const struct audio_stream* stream,
                                const char* keys) {
  const auto* out = reinterpret_cast<const BluetoothStreamOut*>(stream);
  std::unique_lock<std::mutex> lock(out->mutex_);
...
static uint32_t out_get_latency_ms_2_1(const struct audio_stream_out* stream) {
  const auto* out = reinterpret_cast<const BluetoothStreamOut*>(stream);
  std::unique_lock<std::mutex> lock(out->mutex_);
  /***
   * audio_a2dp_hw:
...
static uint32_t out_get_latency_2_1(const struct audio_stream_out* stream,
...
static int out_set_volume_2_1(struct audio_stream_out* stream, float left,
                          float right) {
  auto* out = reinterpret_cast<BluetoothStreamOut*>(stream);
  LOG(VERBOSE) << __func__ << ": state=" << out->bluetooth_output_.GetState()
...
static ssize_t out_write_2_1(struct audio_stream_out* stream, const void* buffer,
                         size_t bytes) {
  auto* out = reinterpret_cast<BluetoothStreamOut*>(stream);
  std::unique_lock<std::mutex> lock(out->mutex_);
  int sent = -1;
...
finish:
  ...
  if (sent == -1) {
    ...
    int64_t sleep_time = bytes * 1000000LL /
                             ...
                             out_get_sample_rate_2_1(&stream->common) -
...
static int out_get_render_position_2_1(const struct audio_stream_out* stream,
                                   uint32_t* dsp_frames) {
  const auto* out = reinterpret_cast<const BluetoothStreamOut*>(stream);
  std::unique_lock<std::mutex> lock(out->mutex_);
  ...
  uint64_t latency_frames =
      (uint64_t)out_get_latency_ms_2_1(stream) * out->sample_rate_ / 1000;
...
static int out_add_audio_effect_2_1(const struct audio_stream* stream,
...
static int out_remove_audio_effect_2_1(const struct audio_stream* stream,
                                   effect_handle_t effect) {
  const auto* out = reinterpret_cast<const BluetoothStreamOut*>(stream);
  LOG(VERBOSE) << __func__ << ": state=" << out->bluetooth_output_.GetState()
               << ", effect=" << effect;
...
static int out_get_next_write_timestamp_2_1(const struct audio_stream_out* stream,
                                        int64_t* timestamp) {
  const auto* out = reinterpret_cast<const BluetoothStreamOut*>(stream);
  *timestamp = 0;
  LOG(VERBOSE) << __func__ << ": state=" << out->bluetooth_output_.GetState()
...
static int out_pause_2_1(struct audio_stream_out* stream) {
  auto* out = reinterpret_cast<BluetoothStreamOut*>(stream);
  std::unique_lock<std::mutex> lock(out->mutex_);
  int retval = 0;
  LOG(VERBOSE) << __func__ << ": state=" << out->bluetooth_output_.GetState()
...
static int out_resume_2_1(struct audio_stream_out* stream) {
  auto* out = reinterpret_cast<BluetoothStreamOut*>(stream);
  std::unique_lock<std::mutex> lock(out->mutex_);
  int retval = 0;
...
static int out_get_presentation_position_2_1(const struct audio_stream_out* stream,
                                         uint64_t* frames,
                                         struct timespec* timestamp) {
  ...
  uint64_t latency_frames = ((uint64_t)out_get_latency_2_1(stream, timestamp) * out->sample_rate_) / 1000;
...
static void out_update_source_metadata_2_1(
...
static int out_set_event_callback_2_1(struct audio_stream_out *stream,
                                   stream_event_callback_t callback __unused,
                                   void *cookie __unused){
  auto* out = reinterpret_cast<BluetoothStreamOut*>(stream);
  LOG(VERBOSE) << __func__ << ": state=" << out->bluetooth_output_.GetState();
  return 0;
}
static size_t samples_per_ticks_2_1(size_t milliseconds, uint32_t sample_rate,
...
int open_output_stream_2_1(struct audio_hw_device* dev,
  ...
  *stream_out = nullptr;
  ...
  out->stream_out_.common.get_sample_rate = out_get_sample_rate_2_1;
  out->stream_out_.common.set_sample_rate = out_set_sample_rate_2_1;
  out->stream_out_.common.get_buffer_size = out_get_buffer_size_2_1;
  out->stream_out_.common.get_channels = out_get_channels_2_1;
  out->stream_out_.common.get_format = out_get_format_2_1;
  out->stream_out_.common.set_format = out_set_format_2_1;
  out->stream_out_.common.standby = out_standby_2_1;
  out->stream_out_.common.dump = out_dump_2_1;
  out->stream_out_.common.set_parameters = out_set_parameters_2_1;
  out->stream_out_.common.get_parameters = out_get_parameters_2_1;
  out->stream_out_.common.add_audio_effect = out_add_audio_effect_2_1;
  out->stream_out_.common.remove_audio_effect = out_remove_audio_effect_2_1;
  out->stream_out_.get_latency = out_get_latency_ms_2_1;
  out->stream_out_.set_volume = out_set_volume_2_1;
  out->stream_out_.write = out_write_2_1;
  out->stream_out_.get_render_position = out_get_render_position_2_1;
  out->stream_out_.get_next_write_timestamp = out_get_next_write_timestamp_2_1;
  out->stream_out_.pause = out_pause_2_1;
  out->stream_out_.resume = out_resume_2_1;
  out->stream_out_.get_presentation_position = out_get_presentation_position_2_1;
  out->stream_out_.update_source_metadata = out_update_source_metadata_2_1;
  out->stream_out_.set_event_callback = out_set_event_callback_2_1;
  ...
  out->frames_count_ =
      samples_per_ticks_2_1(kBluetoothDefaultOutputBufferMs, out->sample_rate_, 1);
  out->frames_rendered_ = 0;
...
size_t stream_input_buffer_size_2_1(const struct audio_hw_device* dev __unused,
                                  const struct audio_config* config __unused) {
  return 320;
}

int open_input_stream_2_1(struct audio_hw_device* dev __unused,
...
void close_input_stream_2_1(struct audio_hw_device* dev __unused,
                             struct audio_stream_in* stream_in __unused) {}
