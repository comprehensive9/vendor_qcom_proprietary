...
ifeq ($(USESECTOOLV2), true)
	SECTOOLSV2_BIN=$(SECIMAGE_BASE)/Linux/sectools
	define sec-image-generate
		@echo "Generating signed appsbl using secimagev2 tool"
		@cp $(PRODUCT_OUT)/abl.elf $(PRODUCT_OUT)/unsigned_abl.elf
		...
		@echo "Completed sectool v2 signed appsbl (ABL) (logs in $(PRODUCT_OUT)/secimage.log)"
	endef
else
	ifeq ($(USESECIMAGETOOL), true)
		ifeq ($(USE_SOC_HW_VERSION), true)
