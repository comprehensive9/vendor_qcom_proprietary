...
/*===========================================================================
  ...
  Copyright (c) 2008-2021 Qualcomm Technologies, Inc.
  All Rights Reserved.
...
typedef enum
{
  QCRIL_DATA_RIL_UNKNOWN = -1,
  QCRIL_DATA_RIL_SLAVE = 0,
  QCRIL_DATA_RIL_MASTER
} is_master_ril;

is_master_ril master_ril = QCRIL_DATA_RIL_UNKNOWN;

static pthread_mutex_t update_master_ril_mutex = PTHREAD_MUTEX_INITIALIZER;
...
#if !defined(RIL_FOR_LOW_RAM)
  global_dsd_info.roaming = qcril_data_is_roaming();
#else
  global_dsd_info.roaming = FALSE;
#endif
...
)
{
  ...
  if( qmi_wds_clat_query_not_supported ) {
    pthread_mutex_lock(&update_master_ril_mutex);
    if( master_ril == QCRIL_DATA_RIL_MASTER ) {
      pthread_mutex_unlock(&update_master_ril_mutex);
...
int qcril_data_get_force_limited_sys_ind()
{
    char* args = NULL;
    int ret = 0;
    args = readProperty("persist.vendor.radio.force_ltd_sys_ind", args, "0");
    if (args != NULL) {
       ret = ds_atoi(args);
       QCRIL_LOG_DEBUG(" persist.vendor.radio.force_ltd_sys_ind = %d", ret);
       free(args);
       args = NULL;
    }
    return ret;
...
static void qcril_data_qmi_dsd_init(void)
{
  int rc;
  int ret = DSI_ERROR;
  boolean profile_update_property_value = FALSE;
  char* args = NULL;
  ...
  do {
    ...
    args = readProperty("persist.vendor.data.profile_update", args, "false");
    if (args != NULL && 0 == strcmp(args, "true"))
    {
      profile_update_property_value = TRUE;
      free(args);
      args = NULL;
    }
    qdp_init3(default_qmi_port, isAPAssistMode, profile_update_property_value);
...
void qcril_data_init_dpm_client_for_legacy_modem( boolean isMasterRIL )
{
  ...
  if( qmi_wds_clat_query_not_supported ) {
    pthread_mutex_lock(&update_master_ril_mutex);
    if( isMasterRIL )
    master_ril = QCRIL_DATA_RIL_MASTER;
    else
    master_ril = QCRIL_DATA_RIL_SLAVE;
    pthread_mutex_unlock(&update_master_ril_mutex);
    if( master_ril == QCRIL_DATA_RIL_MASTER ) {
      (void)qcril_data_qmi_dpm_init();
    }
