...
{
  qcril_instance_id_e_type instance_id = QCRIL_DATA_INVALID_INSTANCE_ID;
  qcril_data_lqe_event_data_t *evt_info_ptr = NULL;
  ...
  if(evt_info_ptr == NULL) {
    QCRIL_LOG_ERROR("evt_info_ptr is NULL");
    goto bail;
  }
  ...
  switch(evt_info_ptr->event)
  {
    case LQE_DOWNLINK_THROUGHPUT_IND_EV:
    {
      ...
      if(dl_info == NULL) {
        QCRIL_LOG_ERROR("dl_info is NULL");
        goto bail;
      }
    ...
    case LQE_UPLINK_THROUGHPUT_IND_EV:
    {
      ...
      if(ul_info == NULL) {
        QCRIL_LOG_ERROR("ul_info is NULL");
        goto bail;
      }
