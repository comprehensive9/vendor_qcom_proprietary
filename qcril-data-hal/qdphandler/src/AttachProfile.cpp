...
unsigned int AttachProfile::findMatch(const std::list<uint16_t>& profile_list, bool &attachRequired) const
{
  ...
  {
    int ret = qdp_match_lte_attach_profile_params(*it, qdp_params);
    if (QDP_SUCCESS == ret)
    {
      dataprofile_release_legacy_params((char**)qdp_params);
      return *it;
    } else if (QDP_ATTACH_REQUIRED == ret) {
      attachRequired = TRUE;
    }
  }
  dataprofile_release_legacy_params((char**)qdp_params);
