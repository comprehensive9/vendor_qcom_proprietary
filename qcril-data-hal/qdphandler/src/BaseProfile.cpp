...
unsigned int BaseProfile::findMatch(const std::list<uint16_t>& profile_list, bool &attachRequired) const
{
  (void) profile_list;
  (void) attachRequired;
  Log::getInstance().d(mName + "Not yet implemented");
...
void BaseProfile::params_t::dump(std::string padding, std::ostream& os) const
{
  os << padding << "profileId=" << profileId << std::boolalpha <<
        ", tech=" << static_cast<int>(radioTech) <<
        ", apn=" << apn << std::endl <<
        padding << "username=" << username <<
        ", password=" << password <<
        ", authType=" << authType << std::endl <<
        padding << "protocol=" << protocol <<
        ", roamingProtocol=" << roamingProtocol << std::endl << std::boolalpha <<
        padding << "apnTypeBitmask=" << apnTypeBitmask <<
        ", bearerBitmask=" << bearerBitmask <<
        ", mtu=" << static_cast<int>(mtu) << std::endl << std::boolalpha <<
        padding << "roamingAllowed=" << roamingAllowed <<
        ", preferred=" << preferred <<
        ", persistent=" << persistent <<
        ", enabled=" << enableProfile << std::endl <<
        padding << "maxConnsTime=" << maxConnsTime <<
        ", maxConns=" << maxConns <<
        ", waitTime=" << waitTime << std::endl;
}

void BaseProfile::dump(std::string padding, std::ostream& os) const
{
  params.dump(padding, os);
}
