    ...
    if (qualNetwork.network.empty() ||
      ...
      if (lastRepIt != mLastReportedNetworkAvailability.end()) {
        ...
        if (newran != oldran) {
          ...
          Log::getInstance().d("There is no change in pref RAT, Not adding this entry in qualnetworks");
        }
      } else {
        Log::getInstance().d("[NetworkAvailabilityHandler] Pref RAT for APN type " +
           convertToString(static_cast<ApnTypes_t>(qualNetwork.apnType)) +
           " changed to " + convertToString(qualNetwork.network.size() > 0 ?
           static_cast<AccessNetwork_t>(qualNetwork.network.front()): AccessNetwork_t::UNKNOWN));
...
void NetworkAvailabilityHandler::updateNetworkAvailabilityCache(std::shared_ptr<Message> msg) {
  ...
  if (m != nullptr) {
    ...
    for (auto profile : profiles) {
      ...
      {
        Log::getInstance().d("Duplicate entry for apn " + apnLower);
        ConsolidatedNetwork_t consolidateNet = mNetworkAvailabilityCache.at(apnLower);
        consolidateNet.apnTypes.insert(consolidateNet.apnTypes.end(), apnTypes.begin(), apnTypes.end());
        //Adding all the vector elements into set to remove duplicate
        std::unordered_set<int32_t> set(consolidateNet.apnTypes.begin(), consolidateNet.apnTypes.end());
        std::vector<int32_t> type(set.begin(), set.end());
        cacheEntry = {false, type, std::vector<int32_t>()};
      }
