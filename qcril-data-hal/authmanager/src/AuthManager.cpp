...
int AuthManager::updateModemWithCarrierImsiKeyCache()
{
...
void AuthManager::updateCarrierImsiKeyCache(RIL_CarrierInfoForImsiEncryption *in_data_ptr)
{
  ...
  size = MIN(strlen(in_data_ptr->mcc), QMI_AUTH_MCC_MNC_SIZE_V01);
  memset(carrierImsiKeyCache.mcc, 0, size+1);
  memcpy(carrierImsiKeyCache.mcc, in_data_ptr->mcc, size);
  size = MIN(strlen(in_data_ptr->mnc), QMI_AUTH_MCC_MNC_SIZE_V01);
  memset(carrierImsiKeyCache.mnc, 0, size+1);
...
bool AuthManager::updateCarrierImsiKeyCache(const ImsiEncryptionInfo_t *imsiEncryptionInfo)
{
  bool rc = false;
  if(!imsiEncryptionInfo)
    return rc;
  ...
  if ( updateModemWithCarrierImsiKeyCache() == QMI_NO_ERR )
    return true;
  else
    return false;
}
...
bool AuthManager::setCarrierInfoImsiEncryption(const ImsiEncryptionInfo_t *imsiEncryptionInfo)
{
  Log::getInstance().d("[AuthManager]: setCarrierInfoImsiEncryption");
  isKeyTypeAvailable = true;
  return updateCarrierImsiKeyCache(imsiEncryptionInfo);
}
