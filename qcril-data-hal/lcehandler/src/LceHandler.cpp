...
RIL_Errno LceHandler::toggleReporting(ReportFilter_t reporting_state) {
...
LinkCapCriteriaResult_t LceHandler::setCriteria(LinkCapCriteria_t criteria) {
   ...
   Log::getInstance().d("[" + mName + "]: result = "+ std::to_string(result));
   return result;
}
