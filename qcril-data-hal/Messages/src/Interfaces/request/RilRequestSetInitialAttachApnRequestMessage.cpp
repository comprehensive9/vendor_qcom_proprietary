...
bool RilRequestSetInitialAttachApnRequestMessage::allocate_and_copy_data(const char *src, char **des)
{
    if (!des)
      return false;
...
RilRequestSetInitialAttachApnRequestMessage::~RilRequestSetInitialAttachApnRequestMessage() {
  if (params != nullptr) {
...
void RilRequestSetInitialAttachApnRequestMessage::deepCopy(const ril_request_type &request){
#if (QCRIL_RIL_VERSION < 15)
  ...
  do {
    ...
    if (!apninfo)
    {
      Log::getInstance().d("[RilRequestSetInitialAttachApnRequestMessage]::apninfo is NULL. Exiting");
      break;
    }
    ...
    if (!payload)
    {
      Log::getInstance().d("[RilRequestSetInitialAttachApnRequestMessage]::"
...
string RilRequestSetInitialAttachApnRequestMessage::dump()
{
  std::stringstream ss;
  ss << "[" << mName << "] ";
