...
)
{
  ...
  {
    Log::getInstance().d("apn type = " + std::to_string(apn_type)+" was enabled");
    return TRUE;
  }
  return FALSE;
}
  ...
  const uint32_t apn_type_mask)
{
  if (getState() == ModemEndPoint::State::OPERATIONAL)
  {
    ...
    Log::getInstance().d("[DSDModemEndPoint]: apn_name= " +apn_name+" apn_types= " +std::to_string(apn_type_mask));
    do
    {
      if (apn_type_mask <= 0)
      {
...
bool DSDModemEndPoint::UIChangeInfoRegistrationRequest(bool RegBit)
{
  dsd_register_ui_info_change_req_msg_v01 qmiReq;
  memset(&qmiReq, 0, sizeof(qmiReq));
  Log::getInstance().d("[DSDModemEndPoint]:: Register for UI indication Bit : " + std::to_string(RegBit));
  if( RegBit ) {
    Log::getInstance().d("[DSDModemEndPoint]::register for UI indication");
    qmiReq.report_ui_changes_valid = true;
    qmiReq.report_ui_changes = 1;
