...
void dsiGetMtuByFamily(dsi_hndl_t dsiHandle, dsi_v4_v6_mtu_t &v4_v6_mtu) {
  ...
  if (qcril_data_mtu > 0) {
    ...
    // Fix specifically for R update to take care of customer use case.
    // Todo: during next Android update check that this can be removed.
    char cPropValue[PROPERTY_VALUE_MAX] = {'\0'};
    property_get("persist.vendor.radio.min_mtu", cPropValue, "false");
    string propValue = string(cPropValue);
    if(propValue == "true") {
      if(v4_v6_mtu.v4_mtu != 0) {
        v4_v6_mtu.v4_mtu = v4_v6_mtu.v6_mtu != 0 ?
            MAX(MIN_V6_MTU, MIN(MIN(v4_v6_mtu.v4_mtu, MAX_V4_PROP_MTU),MIN(v4_v6_mtu.v6_mtu,
                                                                           MAX_V4_PROP_MTU))) :
            MAX(MIN_V6_MTU, MIN(v4_v6_mtu.v4_mtu, MAX_V4_PROP_MTU));
      }
      if(v4_v6_mtu.v6_mtu != 0) {
        v4_v6_mtu.v6_mtu = v4_v6_mtu.v6_mtu != 0 && v4_v6_mtu.v4_mtu != 0 ? v4_v6_mtu.v4_mtu :
            MAX(MIN_V6_MTU, MIN(v4_v6_mtu.v6_mtu, MAX_V4_PROP_MTU));
      }
      Log::getInstance().d("[DsiWrapper]: using min MTU's: v4 mtu = " +
                           std::to_string((int)v4_v6_mtu.v4_mtu) + ", v6 mtu = " +
                           std::to_string((int)v4_v6_mtu.v6_mtu));
    }
    else if ((v4_v6_mtu.v4_mtu < 1280 && v4_v6_mtu.v6_mtu == 0) ||
             (v4_v6_mtu.v6_mtu < 1280 && v4_v6_mtu.v6_mtu != 0)) {
      // Interface or v6 MTU can never be below MIN_V6_MTU.
      // MIN MTU PROP above handles this in a different way.
      v4_v6_mtu.v6_mtu = MIN_V6_MTU;
    }
  }
