...
NativeWindow::NativeWindow(OMX_BOOL bRotate,
    ...
    status_t ret = m_pSurfaceComposer->getDisplayState(
        primaryDisplay, &displayState);
    if (ret == NO_ERROR) {
        ret = m_pSurfaceComposer->getActiveDisplayConfig(
            primaryDisplay, &displayConfig);
    }
    if (ret == NO_ERROR) {
        if (displayState.orientation == android::ui::ROTATION_90
            || displayState.orientation == android::ui::ROTATION_270) {
            m_nDisplayHeight = displayConfig.resolution.getWidth();
            m_nDisplayWidth = displayConfig.resolution.getHeight();
        } else {
            m_nDisplayWidth = displayConfig.resolution.getWidth();
            m_nDisplayHeight = displayConfig.resolution.getHeight();
        }
    }
