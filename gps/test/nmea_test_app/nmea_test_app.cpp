...
#include <stdlib.h>  /* LNIX standard library */
...
void registerMasterClient()
{
    ...
    status = loc_sync_send_req(handle, QMI_LOC_REGISTER_MASTER_CLIENT_REQ_V02,
    ...
    if (eLOC_CLIENT_SUCCESS != status ||
        ...
        printf("error status = %s\n, reg_master_client_ind.status = %s \n",
        ...
        if (eLOC_CLIENT_FAILURE_INVALID_MESSAGE_ID == status ||
            ...
            printf("MasterRegisterNotSupported !!\n");
        }
    }
}

bool registerEventMask(
...
void printHelp(char **arg)
{
    ...
    printf("    -m:  nmea types subscription mask\n"
           ...
           "         NMEA_MASK_DEBUG      (0x00020000) Enable NMEA type\n"
           "         NMEA_MASK_GPDTM      (0x00040000) Enable GPDTM type\n"
           "         NMEA_MASK_GNGGA      (0x00080000) Enable GNGGA type\n"
           "         NMEA_MASK_GNRMC      (0x00100000) Enable GNRMC type\n"
           "         NMEA_MASK_GNVTG      (0x00200000) Enable GNVTG type\n"
           "         NMEA_MASK_GAGNS      (0x00400000) Enable GAGNS type\n"
           "         NMEA_MASK_ALL        (0x007FFFFF) Enable ALL types \n" );
...
int main(int argc, char** argv)
{
        ...
        handle == LOC_CLIENT_INVALID_HANDLE_VALUE )
    {
        ...
        registerEventMask(handle, QMI_LOC_EVENT_MASK_NMEA_V02);
        setNMEATypes();
