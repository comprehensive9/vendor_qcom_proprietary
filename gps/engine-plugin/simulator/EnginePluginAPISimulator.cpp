...
//License buffer Info (char array)
static unsigned char licenseBuff[] = {0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09};
// Lenght of license buffer
static unsigned int licenseBuffLen = 10;
...
{
    log_msg("%s \n", __func__);
    /* When connStatus is set to EP_FEATURE_ACTIVATE_CONNECTION, call epRequestFeatureStatusCb
       from separate thread context
    */
    if (EP_FEATURE_ACTIVATE_CONNECTION == connStatus) {
        if ((NULL != pEpCbs) && (NULL != pEpCbs->epRequestFeatureStatusCb)) {
            pEpCbs->epRequestFeatureStatusCb();
        }
    }
}
...
{
    log_msg("%s \n", __func__);
    /* When status is set to EP_FEATURE_STATUS_NONE, install license using
       epRequestInstallLicenseCb() from separate thread context
    */
    if (EP_FEATURE_STATUS_NONE == status) {
        if ((NULL != pEpCbs) && (NULL != pEpCbs->epRequestFeatureStatusCb)) {
            pEpCbs->epRequestInstallLicenseCb(&licenseBuff[0], licenseBuffLen);
        }
    }
}
...
{
    log_msg("%s License Installation status: %d \n", __func__, status);
...
static void ep_init()
{
    ...
    epInterface.epProvideConnectionStatus = ePProvideConnectionStatus;
    epInterface.epProvideFeatureStatus = ePProvideFeatureStatus;
    epInterface.epProvideInstallLicenseStatus = ePProvideInstallLicenseStatus;
