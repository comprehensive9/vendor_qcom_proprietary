/*
Copyright (c) 2011-2012, 2014-2015, 2017, 2020 Qualcomm Technologies, Inc.
All Rights Reserved.
...
)
{
    ...
    zSample.sampleTimeOffsetMs = (uint16_t)calcMsTimeDiffTick(m_zSensorData.timeBase, tTimestampMs);
...
sp<NDKProvider> NDKProvider::getInstance()
{
    ...
    if (0 == s_pzInstance.get())
    {
        ...
        android::sp<ISensorManager> sm = ISensorManager::getService();
        if (nullptr != sm)
        {
            Return<bool> linked = sm->linkToDeath(s_pzInstance, /*cookie*/ 0);
            if (!linked.isOk()) {
