    ...
    else
    {
      ...
      if (ev[0].type == EV_SYN)
      {
        SLIM_LOGD("Sensor Type %d x=%f,y=%f,z=%f,timestamp=%d, CurrentTime - %lld sec= %d count:%d\n",
                 ...
                 zEventBuffer->sample[2], zEventBuffer->sampleTimeOffsetMs,
        ...
        if (ev[0].code == ABS_X) {
          ...
          if (eSLIM_SERVICE_SENSOR_ACCEL == eSensorType) {
            zEventBuffer->sampleTimeOffsetMs = seca * 1000 + (ev[0].value) / 1000000;
            zEventBuffer->sampleTimeOffsetSubMilliNs = ev[0].value % 1000000;
          } else {
            zEventBuffer->sampleTimeOffsetMs = secb * 1000 + (ev[0].value) / 1000000;
            zEventBuffer->sampleTimeOffsetSubMilliNs = ev[0].value % 1000000;
          }
        }
      }
  ...
  while ( mBufferTxStatus && (!mSecAccBufferList.empty() || !mSecGyroBufferList.empty()) )
  {
    ...
    if (!mSecAccBufferList.empty()){
      ...
      if (zSensorEvent.sampleTimeOffsetMs < samples[0].sampleTimeOffsetMs ) {
        SLIM_LOGD("ACCEL Igone this duplicate sample, time stamp %d",
    ...
    if (!mSecGyroBufferList.empty()) {
      ...
      if (zSensorEvent.sampleTimeOffsetMs < samples[1].sampleTimeOffsetMs ) {
        SLIM_LOGD("GYRO Igone this duplicate sample, time stamp %d",
             zSensorEvent.sampleTimeOffsetMs);
        gyroNumRead -= 1;
...
bail:
  slim_TaskReadyAck();
  if (NULL != mfdBuffAccelE)
  {
    if ((fwrite("0", 1, 1, mfdBuffAccelE) != 1)){
      SLIM_LOGE("failed to write data into %s, errno = %d (%s)",
          acc_boot_sample, errno, strerror(errno));
    } else {
      SLIM_LOGD("flushing Accel");
      fflush(mfdBuffAccelE);
    }
  }

  if (NULL != mfdBuffGyroE)
  {
    if (fwrite("0", 1, 1, mfdBuffGyroE) != 1){
      SLIM_LOGE("failed to write data into %s, errno = %d (%s)",
          gyr_boot_sample, errno, strerror(errno));
    } else {
      SLIM_LOGD("flushing Gyro");
      fflush(mfdBuffGyroE);
    }
  }
...
bool SensorHalProvider::initialize()
{
  ...
  SLIM_LOGD("waiting for slim_TaskReadyWait THREAD_ID_SENSOR_HAL \n");
  ...
  if (memsBufferDataInit()) {
    ...
    slim_TaskReadyWait(THREAD_ID_SENSOR_HAL_BUFF);
  } else {
     memsBufferDataDeInit();
  }
  ...
  routeConfigurationChange(s_qSupportedServices);
...
void SensorHalProvider::runEventLoop()
{
   ...
   while (1)
   {
      ...
      if(VARIABLE_COUNT_BASED_BATCHING == mCountBasedBatchingCfg) {
        mAcc100HzBatchCount = 0;
        mGyro100HzBatchCount = 0;
        for (int i=0 ; i<n ; i++) {
          const sensors_event_t& data = buffer[i];
          if (SENSOR_TYPE_ACCELEROMETER == data.type) {
            mAcc100HzBatchCount++;
          } else if (SENSOR_TYPE_GYROSCOPE == data.type
                       || SENSOR_TYPE_GYROSCOPE_UNCALIBRATED == data.type) {
            mGyro100HzBatchCount++;
          }
        }
          SLIM_LOGD("Accel count %d and Gyro count:%d", mAcc100HzBatchCount, mGyro100HzBatchCount);
      }
               ...
               /* If buffer data is in progress, keep these samples in secondary buffer */
                       ...
                       mBufferTxStatus == SLIM_BUFFER_DATA_IN_PROGRESS)
                {
                  SLIM_LOGD("sensor=%s, time=%lld,current Time= %lld value=<%f,%f,%f>\n",
                          ...
                          data.data[1],
                          data.data[2]);
                  uint64_t sensorTimeNsec = data.timestamp;
                  ...
                  m_zSensorData.samples[0].sampleTimeOffsetMs = 0;
                  m_zSensorData.samples[0].sampleTimeOffsetSubMilliNs = sensorTimeNsec % 1000000;
                  m_zSensorData.samples_len = 1;
                  m_zSensorData.provider = eSLIM_SERVICE_PROVIDER_NATIVE;
                  m_zSensorData.timeBase = sensorTimeNsec / 1000000; //Convert it to msec;
                  m_zSensorData.timeSource = eSLIM_TIME_SOURCE_UNSPECIFIED;
                  m_zSensorData.flags = 0;
...
/**
 ...
 @param[in] aRates- ptr to array, aSR- sample rate value, aSC - Sample count value
 ,nearest-pointer to result, aLength -Length of array
 @return Latency
*/
long long CalculateLatency(const float *aRates, int aSR, int aSC, int aType, int aLength)
{
  int closest = -1;
  int aInputRate = aSR * aSC;
  if(aType < 1)
  {
    return 0;
  }
  ...
  SLIM_LOGD("nri - %f, aType-%d", aRates[closest], aType);
  long long lat = (((double)1/aRates[closest]) * constants[aType-1] * aSC);
...
)
{
      ...
      (eSLIM_SERVICE_SENSOR_ACCEL == eService || eSLIM_SERVICE_SENSOR_GYRO == eService) )
  {
    latency = CalculateLatency(
                         rateMapping[m_zLibInfo.sensorType-1][eService/2],
                         wReportRateHz, wSampleCount, m_zLibInfo.sensorType,
                         sizeof(rateMapping[m_zLibInfo.sensorType-1][eService/2])/sizeof(float)
                         );
  }
