...
public:
  ...
  */
  ...
  uint64_t m_tLastAPSSPktTime;
protected:
  void UpdateCanControllerTimeDelta(uint64_t tMCTime);
  uint64_t VnTimeToApTime(uint64_t tVnTime);
  uint8_t IsOptimizationEnabled(slimServiceEnumT eService);
  inline int32_t convertBytesToInt(uint8_t lowByte, uint8_t hiByte)
  {
    return (uint16_t)(lowByte | (hiByte << 8));
  }
  inline uint16_t combineTwoBytes(uint8_t lowByte, uint8_t hiByte)
  {
    uint16_t val = combineTwoBytes(lowByte, hiByte);
...
/* ------------------------------------------------------------------------- */
/**
@brief Default Message Processor for steering wheel sensor implementation

This class implements processing of messages for configuration
eSTEERINGWHEEL_CONFIG_DEFAULT.

Refer to @sa VehicleNetworkMessageProcessor class documentation for details.

*/
class VehicleNetworkDefaultSteeringWheelMP
  : public VehicleNetworkMessageProcessor
{
public:
  VehicleNetworkDefaultSteeringWheelMP(VehicleNetworkProvider* pzParent,
                               SteeringWheelConfig* pzCfg);
  virtual void OnNewTimerExpiryEvent(os_TimerExpiryType* pzEvent);
  virtual void ToString(char* pStr, size_t sLen);

protected:
  SteeringWheelConfig* m_pzCfg;
};

/* ------------------------------------------------------------------------- */
/**
@brief CONF1 Message Processor for steering wheel sensor implementation

This class implements processing of messages for configuration
eSTEERINGWHEEL_CONFIG_CONF1.

Refer to @sa VehicleNetworkMessageProcessor class documentation for details.

*/
class VehicleNetworkCONF1SteeringWheelMP
  : public VehicleNetworkMessageProcessor
{
public:
  slimVehicleMotionDataStructT mZData;
  VehicleNetworkCONF1SteeringWheelMP(VehicleNetworkProvider* pzParent,
                             SteeringWheelCONF1Config* pzCfg);
  virtual void OnNewCanFrameEvent(CanFrameEvent* pzEvent);
  virtual void ToString(char* pStr, size_t sLen);

protected:
  SteeringWheelCONF1Config* m_pzCfg;
};
