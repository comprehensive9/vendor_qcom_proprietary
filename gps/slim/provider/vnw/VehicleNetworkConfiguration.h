...
public:
  ...
  @sa VehicleNetworkCONF1DwsMP, @sa VehicleNetworkCONF1GearMP,
  abd @sa VehicleNetworkCONF1SteeringWheelMP
  ...
  */
  enum
  {
    ...
    eSTEERINGWHEEL_CONFIG_DEFAULT = 0, /**<  Steering wheel configuration #0 */
    ...
    eSTEERINGWHEEL_CONFIG_CONF1 = 1, /**< Steering wheel configuration #1 */
  ...
  */
  ...
  , where
   <SNS> is sensor name: 'ACCEL', 'GYRO' ,'ODOMETRY', 'SPEED' , 'DWS', 'GEAR',
   or 'STEERINGWHEEL'
   ...
   GetOdometryConfig(), GetSpeedConfig(),  GetDwsConfig(), GetGearConfig()
   and GetSteeringWheelConfig return non-null pointers for successfully read
   configuration .
  */
  void Load();
  ...
  */
  ...
  /**
  @brief Get configuration of steering wheel sensor(s) on CAN bus

  Function returns pointer to the object describing configuration of the
  Steering wheel sensor(s) connected to the CAN bus.

  @return pointer to the object of a type inherited from SensorConfig
  */
  SensorConfig* GetSteeringWheelConfig() const;
  ...
  */
  ...
  /**
  @brief Get user input about VNW Data Optimization

  Function returns a value specified in configuration file or 0
  if value is not set or cannot be parsed.

  @return value if VNW data optimization is required or not
  */
  uint64_t GetVnwEnableDataOptimizationValue() const;

  /**
  @brief Get user input about VNW Data routing time interval

  Function returns a value specified in configuration file or default value
  if value is not set or cannot be parsed.

  @return time interval value for VNW data optimization
  */
  uint64_t GetVnwDataRoutingTimeIntervalValue() const;
...
private:
  ...
  */
  ...
  /**
  @brief Load configuration of type eSTEERINGWHEEL_CONFIG_DEFAULT (0)

  This methods parses configuration string and extracts configuration
  assuming the configuration version is eSTEERINGWHEEL_CONFIG_DEFAULT (0).

  @return pointer to the object of SteeringWheelConfig class
  */
  SensorConfig* LoadSteeringWheelConfig(const char* pStr);
  ...
  */
  ...
  /**
  @brief Load configuration of type eSTEERINGWHEEL_CONFIG_CONF1 (1)

  This methods parses configuration string and extracts configuration
  assuming the configuration version is eSTEERINGWHEEL_CONFIG_CONF1 (1).

  @return pointer to the object of SteeringWheelCONF1Config class
  */
  SensorConfig* LoadSteeringWheelCONF1Config(const char* pStr);
  ...
  */
  ...
  SensorConfig*         m_pzSteeringWheelConfig;

  double                m_dCanApClockRatio;
  uint64_t              mVnwEnableDataOptimization;
  uint64_t              mVnwDataRoutingTimeInterval;
...
/* -------------------------------------------------------------------------*/
/**
@brief Class describing default configuration of SteeringWheel sensor.
...
/* -------------------------------------------------------------------------*/
/**
@brief Class describing reference configuration of Steering wheel sensor
       connected to CAN bus.
       ...
       This configuration assumes that CAN frame with the ID stored to
       @sa m_iFrameId carries gear information as follows.
        VN_STEERINGWHEEL_CFG=1:CAN_STEERINGWHEEL_PID,

        Where

*/
class SteeringWheelCONF1Config
    : public SensorConfig
{
public:
  SteeringWheelCONF1Config();
  void Load(const char* pStr);
  bool CfgValid() const;
  void ToString(char* pStr, size_t sLen) const;
  void loadBatchConfig(const char* pStrBatchType, const char* pStrBatchValue);
  ...
  float scale;
  float offset;
  uint8_t steeringWheelOffsetLowBits;
  uint8_t steeringWheelLengthLowBits;
  uint8_t steeringWheelOffsetUpBits;
  uint8_t steeringWheelLengthUpBits;
  uint8_t isSigned;
  float min;
  float max;
  uint8_t isMaxSwaValid;
  float maxSwa;
  int          m_iFrameId;
  int          m_iBatchSize;
  int          m_iBatchType;
