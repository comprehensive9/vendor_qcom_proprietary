...
)
{
  ...
  if (eSLIM_SERVICE_VEHICLE_STEERING_WHEEL == eService &&
      VehicleNetworkConfiguration::eSTEERINGWHEEL_CONFIG_DEFAULT == iVersion)
  {
    return new VehicleNetworkDefaultSteeringWheelMP(pzParent,
        static_cast<SteeringWheelConfig*>(pzCfg));
  }
  ...
  if (eSLIM_SERVICE_VEHICLE_STEERING_WHEEL == eService &&
      VehicleNetworkConfiguration::eSTEERINGWHEEL_CONFIG_CONF1 == iVersion)
  {
    return new VehicleNetworkCONF1SteeringWheelMP(pzParent,
        static_cast<SteeringWheelCONF1Config*>(pzCfg));
  }
...
void VehicleNetworkMessageProcessor::clearPrevData()
{
  SLIM_LOG_ENTRY();
  return;
}
...
/**
@brief Checks if Vehicle Data Optimization is enabled or not for any service

@param[in] eService     Vehicle Service Type
@return Value           1 if optimization is enabled else 0
*/
uint8_t VehicleNetworkMessageProcessor::IsOptimizationEnabled(slimServiceEnumT eService)
{
  uint8_t retVal = 0;
  uint64_t cfgVal = m_pzParent->m_pzVehicleNetworkCfg->GetVnwEnableDataOptimizationValue();

  switch(eService) {
    case eSLIM_SERVICE_VEHICLE_SPEED:
      if(SLIM_MASK_IS_SET(cfgVal, eSLIM_SERVICE_VEHICLE_SPEED)) {
        retVal = 1;
      }
      break;
    case eSLIM_SERVICE_VEHICLE_DWS:
      if(SLIM_MASK_IS_SET(cfgVal, eSLIM_SERVICE_VEHICLE_DWS)) {
        retVal = 1;
      }
      break;
    case eSLIM_SERVICE_VEHICLE_GEAR:
      if(SLIM_MASK_IS_SET(cfgVal, eSLIM_SERVICE_VEHICLE_GEAR)) {
        retVal = 1;
      }
      break;
    case eSLIM_SERVICE_VEHICLE_STEERING_WHEEL:
      if (SLIM_MASK_IS_SET(cfgVal, eSLIM_SERVICE_VEHICLE_STEERING_WHEEL)) {
        retVal = 1;
      }
      break;
    default:
...
)
{
  ...
  /* If optimization is enabled for service then skip batching */
  if (1 == IsOptimizationEnabled(eService)) {
    /* Batch count reached, send data to client */
    SLIM_LOGD("Optimization enabled eService = %d. \
    ...
    m_tLastAPSSPktTime = tCurrentTimestamp;
    return true;
  }
  ...
  {/* Time Based Batching configured */
    ...
    if (tFrameTime >= m_tLastCANPktTime+iBatchValue ||
        tCurrentTimestamp >= m_tLastAPSSPktTime+iBatchValue ||
       iSamplesLen >= SLIM_VEHICLE_MOTION_DATA_MAX_SAMPLE_SETS)
    {
      ...
      SLIM_LOGD("IsBatchCompleted eService = %d iBatchType = %d  iBatchValue = %d \
           m_tLastCANPktTime=%" PRIu64 " ms. tCurrentTimestamp = %" PRIu64 " ms. \
           samples_len = %d. FrameTime =%" PRIu64 " ms. m_tLastAPSSPktTime = %" PRIu64 " \n", \
           eService, iBatchType, iBatchValue, \
           m_tLastCANPktTime, tCurrentTimestamp, iSamplesLen, tFrameTime, m_tLastAPSSPktTime);
      m_tLastCANPktTime = tFrameTime;
      m_tLastAPSSPktTime = tCurrentTimestamp;
      batchComplete = true;
    }
            ...
            eSLIM_SERVICE_VEHICLE_ACCEL))
    {
      ...
      if (pzEvent->mIsBuffFrame) {
        if (mZData.samples_len == 0) {
          mZData.flags = SLIM_FLAGS_MASK_SENSOR_BUFFER_DATA;
      ...
      && pzFrame->getId() == (uint32_t) m_pzCfg->m_iFrameId)
  {
    if (0
        != SLIM_MASK_IS_SET(m_pzParent->m_qEnabledServices,
            eSLIM_SERVICE_VEHICLE_GEAR))
    {
      SLIM_LOGD("Gear CONF1 MP, event:%p", pzEvent);
      ...
      SLIM_LOGV("gear data %d samples_len %d, mIsBuffFrame %d FrameTime %d",
                 geardata,
                 mZData.samples_len,
                 pzEvent->mIsBuffFrame,
                 pzFrame->getTimestamp());
      ...
      if (pzEvent->mIsBuffFrame) {
        ...
        if (1 == IsOptimizationEnabled(eSLIM_SERVICE_VEHICLE_GEAR)) {
          if (m_pzParent->m_pzVehicleNetworkCfg->GetVnwDataRoutingTimeIntervalValue() >
               (pzFrame->getTimestamp() - mPrevGearDataTime)) {
            if (mPrevGearData == geardata) {
              SLIM_LOGV("No Change in Gear data:%d", geardata);
              return;
            }
          }
          mPrevGearData = geardata;
          mPrevGearDataTime = pzFrame->getTimestamp();
        }
...
void VehicleNetworkCONF1GearMP::clearPrevData()
{
  mPrevGearData = 0;
  mPrevGearDataTime = 0;
  return;
...
/* -------------------------------------------------------------------------- */
VehicleNetworkDefaultSteeringWheelMP::VehicleNetworkDefaultSteeringWheelMP(
    VehicleNetworkProvider* pzParent, SteeringWheelConfig* pzCfg) :
    VehicleNetworkMessageProcessor(pzParent), m_pzCfg(pzCfg)
{
}

void VehicleNetworkDefaultSteeringWheelMP::OnNewTimerExpiryEvent(
    os_TimerExpiryType* pzEvent)
{
  VehicleNetworkMessageProcessor::OnNewTimerExpiryEvent(pzEvent);
  if (0
          ...
          eSLIM_SERVICE_VEHICLE_STEERING_WHEEL))
  {
    ...
    zData.timeSource = eSLIM_TIME_SOURCE_COMMON;
    zData.provider = eSLIM_SERVICE_PROVIDER_NATIVE;
    zData.sensorType = eSLIM_VEHICLE_MOTION_SENSOR_TYPE_STEERING_WHEEL;
    zData.samples_len = 1;
    zData.samples[0].steering_wheel.sampleTimeOffset = 10;
    zData.samples[0].steering_wheel.data = 100;

    m_pzParent->routeIndication(eSLIM_SERVICE_VEHICLE_STEERING_WHEEL, zData);
  }
}

void VehicleNetworkDefaultSteeringWheelMP::ToString(char* pStr, size_t sLen)
{
  if(0 == pStr)
  {
    SLIM_LOGE("String empty...");
    return;
  }
  snprintf(pStr, sLen, "[VehicleNetworkDefaultSteeringWheelMP]");
}

/* -------------------------------------------------------------------------- */
VehicleNetworkCONF1SteeringWheelMP::VehicleNetworkCONF1SteeringWheelMP(
    VehicleNetworkProvider* pzParent, SteeringWheelCONF1Config* pzCfg) :
    VehicleNetworkMessageProcessor(pzParent), m_pzCfg(pzCfg)
{
  mZData.timeSource = eSLIM_TIME_SOURCE_UNSPECIFIED;
  mZData.provider = eSLIM_SERVICE_PROVIDER_NATIVE;
  mZData.sensorType = eSLIM_VEHICLE_MOTION_SENSOR_TYPE_STEERING_WHEEL;
  mZData.samples_len = 0;
  m_tLastCANPktTime = 0;
  m_tLastAPSSPktTime = 0;
}

void VehicleNetworkCONF1SteeringWheelMP::OnNewCanFrameEvent(CanFrameEvent* pzEvent)
{
  CwFrame* pzFrame = pzEvent->GetFrame();
  float conversionFactor2degrees = 1.0;
  if (0 == pzFrame)
  {
    return;
  }
      ...
      && pzFrame->getId() == (uint32_t) m_pzCfg->m_iFrameId)
  {
    if (0
        != SLIM_MASK_IS_SET(m_pzParent->m_qEnabledServices,
            eSLIM_SERVICE_VEHICLE_STEERING_WHEEL))
    {
      SLIM_LOGD("SteeringWheel CONF1 MP, event:%p", pzEvent);

      /* Update difference between AP and CAN bus mictocontroller clock.
       * TODO: This code has to be moved to processing of DR_SYNC CAN frame */
      UpdateCanControllerTimeDelta(pzFrame->getTimestamp());

      /* bit stream process to fetch steering_wheel data */
      BitDivider bitStream(pzFrame->getDataPtr(), pzFrame->getDataLen());

      bitStream.SetPos(m_pzCfg->steeringWheelOffsetLowBits);
      if (bitStream.GetStatus() != 0)
        return;
      uint32_t steeringWheelLowBits  = bitStream.Get(m_pzCfg->steeringWheelLengthLowBits);

      bitStream.SetPos(m_pzCfg->steeringWheelOffsetUpBits);
      if (bitStream.GetStatus() != 0)
        return;
      uint32_t steeringWheelUpBits  = bitStream.Get(m_pzCfg->steeringWheelLengthUpBits);

      int32_t steeringWheelBytes = ((steeringWheelUpBits << m_pzCfg->steeringWheelLengthLowBits) |
                                     steeringWheelLowBits);

      /* Signed extension handled here for negative data */
      if (m_pzCfg->isSigned)
      {
        if (steeringWheelBytes &  (1 << ((m_pzCfg->steeringWheelLengthLowBits +
                                          m_pzCfg->steeringWheelLengthUpBits) - 1)))
        {
          steeringWheelBytes -= 1UL << (m_pzCfg->steeringWheelLengthLowBits +
                                        m_pzCfg->steeringWheelLengthUpBits);
        }
      }

      float steeringWheeldata = ((steeringWheelBytes / m_pzCfg->scale) + m_pzCfg->offset);

      if (m_pzCfg->isMaxSwaValid && steeringWheeldata > m_pzCfg->maxSwa) {
        steeringWheeldata -= 2.0 * m_pzCfg->maxSwa;
      }

      if (m_pzCfg->isMaxSwaValid && (std::fabs(steeringWheeldata) > m_pzCfg->maxSwa)) {
        SLIM_LOGE("Unlikely steering wheel data: %f", steeringWheeldata);
        return;
      }

      if (m_pzCfg->min != m_pzCfg->max &&
            (steeringWheeldata < m_pzCfg->min || steeringWheeldata > m_pzCfg->max)) {
        SLIM_LOGE("CAN value out of range: %f", steeringWheeldata);
        return;
      }

      SLIM_LOGV("steering_wheel data %f samples_len %d, mIsBuffFrame %d FrameTime %d",
                 steeringWheeldata,
                 mZData.samples_len,
                 pzEvent->mIsBuffFrame,
                 pzFrame->getTimestamp());
      ...
      if (pzEvent->mIsBuffFrame) {
        if (0 == mZData.samples_len) {
          mZData.flags = SLIM_FLAGS_MASK_SENSOR_BUFFER_DATA;
          mZData.timeBase = VnTimeToApTime(pzFrame->getTimestamp());
          mZData.samples[0].steering_wheel.data = steeringWheeldata;
          mZData.samples[0].steering_wheel.sampleTimeOffset = 0;
        } else {
          mZData.samples[mZData.samples_len].steering_wheel.data = steeringWheeldata;
          mZData.samples[mZData.samples_len].steering_wheel.sampleTimeOffset =
              SLIM_CONVERT_MSEC_TO_USEC(VnTimeToApTime(pzFrame->getTimestamp()) - mZData.timeBase);
        }
        mZData.samples_len += 1;
        if (mZData.samples_len == SLIM_VEHICLE_MOTION_DATA_MAX_SAMPLE_SETS) {
          m_pzParent->routeIndication(eSLIM_SERVICE_VEHICLE_STEERING_WHEEL, mZData);
          /* sleep for 2 ms to allow every batch of data sent to application */
          usleep(TIME_BW_BUFFERED_DATA_BATCHES);
          mZData.samples_len = 0;
        }
        ...
        mZData.samples[mZData.samples_len].steering_wheel.data = steeringWheeldata;
        mZData.samples_len++;
        if (true == IsBatchCompleted (eSLIM_SERVICE_VEHICLE_STEERING_WHEEL,
                                               mZData.samples_len,
                                               pzFrame->getTimestamp(),
                                               m_pzCfg->m_iBatchType,
                                               m_pzCfg->m_iBatchSize))
...
void VehicleNetworkCONF1SteeringWheelMP::ToString(char* pStr, size_t sLen)
{
  if(0 == pStr)
  {
    SLIM_LOGE("String empty...");
  ...
  snprintf(pStr, sLen, "[VehicleNetworkCONF1SteeringWheelMP]");
}
