...
const slimAvailableServiceMaskT VehicleNetworkProvider::s_qSupportedServices =
    (uint32_t(1) << eSLIM_SERVICE_VEHICLE_ACCEL)
        ...
        | (uint32_t(1) << eSLIM_SERVICE_VEHICLE_STEERING_WHEEL);
...
VehicleNetworkProvider::VehicleNetworkProvider()
    ...
    m_pzGearProc(0), m_pzSteeringWheelProc(0),
...
void VehicleNetworkProvider::runEventLoop()
{
  ...
  int earIdSteeringWheel = 0, mearIdSteeringWheel = 0;
  ...
  if (0 != m_pzVehicleNetworkCfg->GetSteeringWheelConfig())
  {
    m_pzVehicleNetworkCfg->GetSteeringWheelConfig()->ToString(pzTStr, sizeof(pzTStr));
    SLIM_LOGD("m_pzSteeringWheelConfig: %s", pzTStr);

    if (m_pzVehicleNetworkCfg->GetSteeringWheelConfig()->CfgValid())
    {
      m_pzSteeringWheelProc = VehicleNetworkMessageProcessor::CreateProcessor(
          this,
          m_pzVehicleNetworkCfg->GetSteeringWheelConfig(),
          eSLIM_SERVICE_VEHICLE_STEERING_WHEEL);
      if (0 != m_pzSteeringWheelProc)
      {
        m_pzSteeringWheelProc->ToString(pzTStr, sizeof(pzTStr));
        SLIM_LOGD("m_pzSteeringWheelConfig: %s", pzTStr);
        if (VehicleNetworkConfiguration::eSTEERINGWHEEL_CONFIG_CONF1 ==
               m_pzVehicleNetworkCfg->GetSteeringWheelConfig()->GetVersion())
        {
          SteeringWheelCONF1Config *steeringWheelConfig =
           static_cast<SteeringWheelCONF1Config*>(m_pzVehicleNetworkCfg->GetSteeringWheelConfig());
          /* Register callback to CanWrapper */
          /* Configure listener to get only this frame on all interfaces*/
          earIdSteeringWheel = m_pzCanWrapper->registerListener(steeringWheelConfig->m_iFrameId,
                                                       CwBase::MASK11,
                                                       cbOnNewCanFrame,
                                                       (void *)(this),
                                                       CwBase::IFACE_ANY);

          mearIdSteeringWheel = m_pzCanWrapper->registerBufferedDataListener(
                                                                   steeringWheelConfig->m_iFrameId,
                                                                   CwBase::MASK11,
                                                                   cbOnNewBufferedCanFrame,
                                                                   (void *)(this),
                                                                   CwBase::IFACE_ANY);
          ...
          if (0 == earIdSteeringWheel || 0 == mearIdSteeringWheel)
          {
              SLIM_LOGE("Error registering CanWrapper listener for SteeringWheel");
          }
        }
      }
      else
      {
        SLIM_LOGD("m_pzSteeringWheelProc was NOT created");
      }
    }
  }
  else
  {
    SLIM_LOGD("m_pzSteeringWheelConfig was NOT loaded");
  }
  ...
  while (1)
  {
    ...
    case eIPC_MSG_NEW_FRAME_EVENT:
    {
      ...
      if ((m_pzSteeringWheelProc) && (earIdSteeringWheel) && (mearIdSteeringWheel))
        m_pzSteeringWheelProc->OnNewCanFrameEvent(pzNewEvent);
    ...
    case eIPC_MSG_TIMER:
    {
      ...
      if (m_pzSteeringWheelProc)
        m_pzSteeringWheelProc->OnNewTimerExpiryEvent(&z_TimerMsg);
    ...
    default:
    {
      ...
      if (m_pzSteeringWheelProc)
        m_pzSteeringWheelProc->OnNewIpcMessage(pz_Msg);
    }
      break;
    }
