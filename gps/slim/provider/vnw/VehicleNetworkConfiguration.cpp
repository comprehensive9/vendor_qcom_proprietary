...
#define VN_STEERINGWHEEL_CFG_STR_ID  "VN_STEERINGWHEEL_CFG"
...
#define VN_STEERINGWHEEL_CFG_BATCH_VAL_STR_ID    "VN_STEERINGWHEEL_CFG_BATCH_VALUE"

#define VN_ENABLE_DATA_OPTIMIZATION_ID           "VN_ENABLE_DATA_OPTIMIZATION"
#define VN_DATA_ROUTING_TIME_INTERVAL_ID         "VN_DATA_ROUTING_TIME_INTERVAL_MSEC"
...
VehicleNetworkConfiguration::VehicleNetworkConfiguration()
  ...
  m_pzDwsConfig(0),
  m_pzGearConfig(0),
  m_pzSteeringWheelConfig(0),
  m_dCanApClockRatio(0.0),
  mVnwEnableDataOptimization(0),
  mVnwDataRoutingTimeInterval(5000)
{
}
...
VehicleNetworkConfiguration::~VehicleNetworkConfiguration()
{
  ...
  delete m_pzSteeringWheelConfig;
}
...
// See header file for function documentation
SensorConfig* VehicleNetworkConfiguration::GetSteeringWheelConfig() const
{
  return m_pzSteeringWheelConfig;
}
...
// See header file for function documentation
uint64_t VehicleNetworkConfiguration::GetVnwEnableDataOptimizationValue() const
{
  return mVnwEnableDataOptimization;
}

// See header file for function documentation
uint64_t VehicleNetworkConfiguration::GetVnwDataRoutingTimeIntervalValue() const
{
  return mVnwDataRoutingTimeInterval;
}
...
// See header file for function documentation
SensorConfig* VehicleNetworkConfiguration::LoadSteeringWheelConfig
(const char* pStr)
{
  SensorConfig* pzSWC = 0;
  int v = GetCfgVersion(pStr);
  SLIM_LOGD("Steering wheel configuration version v= %d", v);
  if (v < 0) return 0;
  switch (v)
  {
  case eSTEERINGWHEEL_CONFIG_DEFAULT:
    {
      pzSWC = new (std::nothrow) SteeringWheelConfig();
    }
    break;
  case eSTEERINGWHEEL_CONFIG_CONF1:
    {
      pzSWC = new (std::nothrow) SteeringWheelCONF1Config();
    }
    break;
  /* Other configuration versions to be added here */
  default:
...
void VehicleNetworkConfiguration::Load()
{
  ...
  char pzSteeringWheelCfgStr[LOC_MAX_PARAM_STRING] = {0};
  ...
  char pzSteeringWheelCfgBatchValStr[LOC_MAX_PARAM_STRING] = {0};
  ...
  uint8_t uSteeringWheelCfgBatchValStrSet = 0;
  ...
  uint8_t uSteeringWheelCfgStrSet = 0;
  ...
  { VN_STEERINGWHEEL_CFG_STR_ID, pzSteeringWheelCfgStr, &uSteeringWheelCfgStrSet, 's'},
  ...
  { VN_STEERINGWHEEL_CFG_BATCH_VAL_STR_ID, pzSteeringWheelCfgBatchValStr,
      &uSteeringWheelCfgBatchValStrSet, 's'},
  { VN_ENABLE_DATA_OPTIMIZATION_ID, &mVnwEnableDataOptimization, NULL, 'n' },
  { VN_DATA_ROUTING_TIME_INTERVAL_ID, &mVnwDataRoutingTimeInterval, NULL, 'n' }
  };
 ...
 /* Load parsing configuration for CAN packets*/
  ...
  m_pzSteeringWheelConfig = (0 != uSteeringWheelCfgStrSet) ?
      VehicleNetworkConfiguration::LoadSteeringWheelConfig(pzSteeringWheelCfgStr) : 0;
  ...
  if (NULL != m_pzSteeringWheelConfig && 0 != uSteeringWheelCfgBatchValStrSet)
  {
    if (eSTEERINGWHEEL_CONFIG_CONF1 == VehicleNetworkConfiguration::
                                               GetCfgVersion(pzSteeringWheelCfgStr))
    {
      m_pzSteeringWheelConfig->loadBatchConfig(pzCfgBatchTypeStr, pzSteeringWheelCfgBatchValStr);
    }
  }
}
...
void VehicleNetworkConfiguration::ToString(char* pStr, size_t sLen) const
{
  ...
  snprintf(pStr, sLen, "[file:'%s', m_dCanApClockRatio:%f, "
                       ...
                       "m_pzDwsConfig=%X, "
                       "m_pzGearConfig=%X, "
                       "m_pzSteeringWheelConfig=%X, "
                       "mVnwEnableDataOptimization:%X, "
                       "mVnwDataRoutingTimeInterval:%" PRIu64 "]",
           VN_CFG_FILE_NAME,
           m_dCanApClockRatio,
           ...
           (uintptr_t)m_pzSteeringWheelConfig,
           mVnwEnableDataOptimization,
           mVnwDataRoutingTimeInterval);
}
...
SteeringWheelConfig::SteeringWheelConfig()
  : SensorConfig(VehicleNetworkConfiguration::eSTEERINGWHEEL_CONFIG_DEFAULT),
  m_fPeriod(0)
{
}

// See header file for function documentation
void SteeringWheelConfig::Load(const char* pStr)
{
  if (0 == pStr || 0 == *pStr)
  {
    return;
...
// See header file for function documentation
bool SteeringWheelConfig::CfgValid() const
{
  return SensorConfig::CfgValid() &&
         VehicleNetworkConfiguration::eSTEERINGWHEEL_CONFIG_DEFAULT == m_qCfgVersion &&
         1.0 <= m_fPeriod && 10.0 >= m_fPeriod;
}

// See header file for function documentation
void SteeringWheelConfig::ToString(char* pStr, size_t sLen) const
{
  if (0 == pStr || 0 == sLen)
  {
    return;
...
/* -------------------------------------------------------------------------- */
// See header file for function documentation
SteeringWheelCONF1Config::SteeringWheelCONF1Config()
  : SensorConfig(VehicleNetworkConfiguration::eSTEERINGWHEEL_CONFIG_CONF1)
{
  m_iFrameId = -1;
  m_iBatchType = SLIM_VNW_BATCH_TYPE_DEFAULT;
  m_iBatchSize = SLIM_VNW_BATCH_VAL_DEFAULT;
...
// See header file for function documentation
void SteeringWheelCONF1Config::Load(const char* pStr)
{
  int tempInt;
  float tempFloat;
  ...
  if (',' == *pFieldbegin)
  {
    pFieldbegin++;
    GetNextInt(&tempInt, &pFieldbegin, 0);
    steeringWheelOffsetLowBits = static_cast<uint8_t>(tempInt);
  }
  ...
  if (',' == *pFieldbegin)
  {
    pFieldbegin++;
    GetNextInt(&tempInt, &pFieldbegin, 0);
    steeringWheelLengthLowBits = static_cast<uint8_t>(tempInt);
  }
  ...
  if (',' == *pFieldbegin)
  {
    pFieldbegin++;
    GetNextInt(&tempInt, &pFieldbegin, 0);
    steeringWheelOffsetUpBits = static_cast<uint8_t>(tempInt);
  }
  ...
  if (',' == *pFieldbegin)
  {
    pFieldbegin++;
    GetNextInt(&tempInt, &pFieldbegin, 0);
    steeringWheelLengthUpBits = static_cast<uint8_t>(tempInt);
  }
  ...
  if (',' == *pFieldbegin)
  {
    pFieldbegin++;
    GetNextInt(&tempInt, &pFieldbegin, 0);
    isSigned = static_cast<uint8_t>(tempInt);
  }

  if (',' == *pFieldbegin)
  {
    pFieldbegin++;
    GetNextFloat(&tempFloat, &pFieldbegin, 0);
    min = tempFloat;
  }
  ...
  if (',' == *pFieldbegin)
  {
    pFieldbegin++;
    GetNextInt(&tempInt, &pFieldbegin, 0);
    isMaxSwaValid = static_cast<uint8_t>(tempInt);
  }
...
// See header file for function documentation
bool SteeringWheelCONF1Config::CfgValid() const
{
  return SensorConfig::CfgValid() &&
         VehicleNetworkConfiguration::eSTEERINGWHEEL_CONFIG_CONF1 == m_qCfgVersion &&
         0 <= scale && 0 <= offset;
}

// See header file for function documentation
void SteeringWheelCONF1Config::ToString(char* pStr, size_t sLen) const
{
  if (0 == pStr || 0 == sLen)
  {
    return;
  ...
  char pzTStr[1024];
  SensorConfig::ToString(pzTStr, sizeof(pzTStr));
  snprintf(pStr, sLen, "[%s, %d, %.6E,  %.6E,"
             " %hhu, %hhu, %hhu, %hhu, %hhu, %.6E, %.6E, %hhu, %.6E]",
              pzTStr, m_iFrameId, scale, offset,
              steeringWheelOffsetLowBits, steeringWheelLengthLowBits,
              steeringWheelOffsetUpBits, steeringWheelLengthUpBits,
              isSigned, min, max, isMaxSwaValid, maxSwa);
}
...
@param[in] pStrBatchValue  configuration string for Batch Values
*/
void SteeringWheelCONF1Config::loadBatchConfig(
  const char* pStrBatchType,
  const char* pStrBatchValue)
{
  /* Batch type is default initilized to Time based batching */
  if ( 0 != pStrBatchType )
  {
