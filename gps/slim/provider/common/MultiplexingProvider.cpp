...
)
{
  ...
  if ((eSLIM_MESSAGE_ID_NONE != eMessageId) && pData)
  {
    slim_ServiceStatusDispatchDataToRegistered(
      ...
      pData);
    slimLogDiagPkt(
      eSLIM_LOG_SOURCE_OTHER,
      eSLIM_TIME_SOURCE_APSS_TIME,
      m_eProvider,
      (slim_TimeTickGetMilliseconds() * 1000),
      pData,
      qDataSize,
      zSlimDaemonMessage.eService);
  }
...
@param[in] sensorTimeNsec         Current sensor sample Timestamp in nano sec
...
)
{
  ...
  /* offset with Msec resolution */
  sensorData.samples[sensorData.samples_len].sampleTimeOffsetMs =
                          tCurrentTimestamp - sensorData.timeBase;
  /* sub milli offset with Nsec resolution */
  sensorData.samples[sensorData.samples_len].sampleTimeOffsetSubMilliNs =
