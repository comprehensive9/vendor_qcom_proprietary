...
)
{
...
#endif
    break;
  case SLIM_PROVIDER_SENSOR_PLAYBACK:
#ifdef FEATURE_LOCTECH_SLIM_SENSOR_PLAYBACK
    SLIM_LOGD("Initializing Sensor Playback provider");
    if (SLIM_MASK_IS_SET(pz_ConfData->q_EnabledMask, SLIM_PROVIDER_SENSOR_PLAYBACK))
    {
      SLIM_LOGD("Creating Sensor Playback provider");
      pz_Provider = slim::SensorPlayBackProvider::getProvider();
    }
#endif
    break;
  ...
  case SLIM_PROVIDER_QMISLIM_TEST:
    ...
    break;
#endif /* FEATURE_LOCTECH_QMISLIM_PROVIDER_AP */
  case SLIM_PROVIDER_RP:
#ifdef FEATURE_LOCTECH_SLIM_REMOTE_PROVIDER
    SLIM_LOGD("Initializing Slim Remote provider");
    if (SLIM_MASK_IS_SET(pz_ConfData->q_EnabledMask, SLIM_PROVIDER_RP))
    {
      SLIM_LOGD("Creating Slim Remote provider");
      pz_Provider = slim::remoteProv::getProvider();
    }
#endif
    break;
    default:
...
)
{
    ...
    if (SLIM_PROVIDER_NONE == e_Provider &&
        SLIM_MASK_IS_SET(q_AvailableProviders, SLIM_PROVIDER_RP) &&
        SLIM_MASK_IS_SET(q_AllowedProviders, eSLIM_SERVICE_PROVIDER_NATIVE))
    {
      e_Provider = SLIM_PROVIDER_RP;
    }
    ...
    if (SLIM_PROVIDER_NONE == e_Provider &&
        SLIM_MASK_IS_SET(q_AvailableProviders, SLIM_PROVIDER_SENSOR_PLAYBACK) &&
        SLIM_MASK_IS_SET(q_AllowedProviders, eSLIM_SERVICE_PROVIDER_NATIVE))
    {
      e_Provider = SLIM_PROVIDER_SENSOR_PLAYBACK;
    }
...
/**
@brief APSS functionality below:

Place holder for future implementation of service discovery in APSS side.
Service discovery feature is included in MPSS and not in APSS.
In service discovery feature, runtime prvoider status is checked and started
using below function (EX: SSC provider in MPSS)
More details please check MPSS code.

@return TRUE always in APSS
*/
boolean slim_ProviderConfStartProvider
(
    slim_ProviderEnumType e_Provider
)
{
...
{
  slim_ProviderConfDataT *pz_ConfData = &z_ProviderConfData;
  if (0 != (pz_ConfData->q_EnabledMask & q_DisabledProviders))
  {
    slimAvailableServiceMaskT q_ServiceUpdateMask = 0;
    int32 l_ServiceNo;

    pz_ConfData->q_EnabledMask &= ~q_DisabledProviders;

    for (l_ServiceNo = 0; l_ServiceNo < SLIM_SERVICE_COUNT; ++l_ServiceNo)
    {
      SLIM_MASK_SET(q_ServiceUpdateMask, l_ServiceNo);
    }
    slim_InternalRequestConfigurationChange(
      q_DisabledProviders,
      q_ServiceUpdateMask);
