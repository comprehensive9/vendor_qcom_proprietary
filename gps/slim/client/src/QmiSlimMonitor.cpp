...
class SLIM_CLIENT_LOCAL MonitorPrivate
{
  ...
  MonitorPrivate(Monitor *pzMonitor = nullptr);
  ~MonitorPrivate();
};
...
{
   size_t offset = 0;
   SLIM_QS_P2M_OFFSET(MonitorPrivate, m_pzMonitorCbVt, offset);
   return reinterpret_cast<MonitorPrivate*>(slim_QSMember2Container((void*)p_Iface, offset));
}
