...
/* Submilli second computation, common for all sensors */
/* return value is sample offset in nano sec relative to msec and base*/
#define GET_SUBMILLI_OFFSET_IN_NANO(sampleTs, base, offsetMsec)   \
            (sampleTs - ((base + offsetMsec) * 1000000));

/* return value is uint64 format and nano sec resolution */
#define GET_SENSORTIME_IN_NANO(base, offsetMsec, offsetNsec)     \
            (offsetNsec + ((base + offsetMsec) * 1000000));
...
typedef enum
{
  ...
  THREAD_ID_RP,
  /**< SLIM Remote provider thread */
  THREAD_ID_IC,
  /**< SLIM Internal Client thread */
  THREAD_ID_SENSOR_PLAYBACK,
  /**< SLIM Sensor playback thread */
  THREAD_ID_PLAYBACK_FILEREAD,
  /**< SLIM Sensor playback File Read thread */
