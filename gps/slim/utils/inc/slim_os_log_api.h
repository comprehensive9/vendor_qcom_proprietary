...
/**
@brief set sensor diag logging level

Set diag logging level configured by user

@param[in] cgfLevel configured logging level.

@ingroup slim_Log
*/
void slimSetSensorDiagLevel
(
  uint32_t cfgLevel
);

/**
@brief diag logging level

func to query sensor diag level

@return configured logging level.

@ingroup slim_Log
*/
uint32_t slimGetSensorDiagLevel();
...
#define SLIM_LOGV(...) if (slim_LogIsLoggable(SLIM_LOG_LOCAL_MODULE, SLIM_LOG_VERBOSE)) \
                                                { syslog(LOG_NOTICE,  "SLIM_LOGV: " __VA_ARGS__); }
//! @brief Logging macro for debug level.
#define SLIM_LOGD(...) if (slim_LogIsLoggable(SLIM_LOG_LOCAL_MODULE, SLIM_LOG_DEBUG)) \
                                                { syslog(LOG_DEBUG,   "SLIM_LOGD: " __VA_ARGS__); }
//! @brief Logging macro for info level.
#define SLIM_LOGI(...) if (slim_LogIsLoggable(SLIM_LOG_LOCAL_MODULE, SLIM_LOG_INFO)) \
                                                { syslog(LOG_NOTICE,  "SLIM_LOGI: " __VA_ARGS__); }
//! @brief Logging macro for warning level.
#define SLIM_LOGW(...) if (slim_LogIsLoggable(SLIM_LOG_LOCAL_MODULE, SLIM_LOG_WARNING)) \
                                                { syslog(LOG_WARNING, "SLIM_LOGW: " __VA_ARGS__); }
//! @brief Logging macro for error level.
#define SLIM_LOGE(...) if (slim_LogIsLoggable(SLIM_LOG_LOCAL_MODULE, SLIM_LOG_ERROR)) \
                                                { syslog(LOG_ERR,     "SLIM_LOGE: " __VA_ARGS__); }
//! @brief Logging macro for fatal error level.
#define SLIM_LOGF(...) if (slim_LogIsLoggable(SLIM_LOG_LOCAL_MODULE, SLIM_LOG_FATAL)) \
                                                { syslog(LOG_ERR,     "SLIM_LOGF: " __VA_ARGS__); }
