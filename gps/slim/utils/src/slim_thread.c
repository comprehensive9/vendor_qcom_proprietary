/*
Copyright (c) 2014-15, 2020 Qualcomm Technologies, Inc.
All Rights Reserved.
...
)
{
 ...
 {
    int err = -ENODEV;
    err = pthread_setname_np(pz_Thread->z_Thread, pz_Name);
    if (0 != err)
      SLIM_LOGE("pthread_setname_np failed for thread:%s", pz_Name);
  }
  ...
  SLIM_LOGD("pthread_setname_np create %s", pz_Name);
  return true;
...
)
{
 ...
 {
    int err = -ENODEV;
    err = pthread_setname_np(z_ThreadMain.z_Thread, pz_Name);
    SLIM_LOGD("pthread_setname_np main Init %s", pz_Name);
    if (0 != err)
      SLIM_LOGE("pthread_setname_np failed for thread: %s", pz_Name);
  }
