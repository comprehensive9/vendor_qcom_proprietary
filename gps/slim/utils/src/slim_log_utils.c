...
typedef struct
{
  ...
  //! @brief enable sensor diag for logging
  uint32_t sensorDiagLevel;
...
)
{
  ...
  recursive = 1;
#endif
#if (defined (__ANDROID__) || defined(USE_ANDROID_LOGGING)) && defined(FEATURE_LOCTECH_SLIM_LOG_LOGCAT)
...
{
  z_Config.sensorDiagLevel = cfgLevel;
}

uint32_t slimGetSensorDiagLevel()
{
  return z_Config.sensorDiagLevel;
}
