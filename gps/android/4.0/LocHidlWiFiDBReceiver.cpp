...
static void DBReceiverApListUpdateCb(APInfo_s *ap_list[], size_t ap_list_size,
    ...
    if (wifiDbRcvrCb != nullptr) {
        ...
        auto result = wifiDbRcvrCb->apListUpdateCallback_2_1(apInfoList, apListSize,
                (LocHidlWifiDBListStatus)apListStatus, ulpLoc, bUlpLocValid);
        if (result.isOk()) {
            LOC_LOGD("wifiDbRcvrCb->apListUpdateCallback_2_1 success.");
        } else {
            LOC_LOGE("wifiDbRcvrCb->apListUpdateCallback_2_1 failed.");
        }
    }
}
