...
using vendor::qti::gnss::V4_0::ILocHidlGnss;
using vendor::qti::gnss::V3_0::ILocHidlIzatConfig;
using vendor::qti::gnss::V4_0::ILocHidlFlpService;
using vendor::qti::gnss::V1_0::ILocHidlGeofenceService;
using vendor::qti::gnss::V1_0::ILocHidlGnssNi;
using vendor::qti::gnss::V4_0::ILocHidlFlpServiceCallback;
...
using LocHidlLocation_v4_0 = vendor::qti::gnss::V4_0::LocHidlLocation;
#endif
using android::OK;
using android::sp;
using android::wp;
...
struct GnssFlpServiceCallback : public ILocHidlFlpServiceCallback {
    ...
    Return<void> gnssLocationTrackingCb_4_0(const LocHidlLocation_v4_0& gnssLocation) override;
};
...
Return<void> GnssFlpServiceCallback::gnssLocationTrackingCb_4_0(
        const LocHidlLocation_v4_0& gnssLocation) {
    LOC_LOGD("%s]: (flags: %02x)", __FUNCTION__, gnssLocation.v1_0.locationFlagsMask);
    Location location;
    memset(&location, 0, sizeof(Location));
    translateLocation(location, gnssLocation.v1_0);
...
Return<void> OsNpGlueCallback::onSetRequest(const LocHidlIzatRequest& request) {
    ...
    lock_guard<std::mutex> guard(sOsNpLock);
    if (sOsNpCbs) {
...
struct GnssConfigServiceCallback : public ILocHidlGnssConfigServiceCallback {
    Return<void> getGnssSvTypeConfigCb(
            const hidl_vec<android::hardware::gnss::V1_0::GnssConstellationType>&
            disabledSvTypeList) override;
    Return<void> getGnssSvTypeConfigCb_4_0(
            const hidl_vec<LocHidlGnssConstellationType>& disabledSvTypeList) override;
    Return<void> getRobustLocationConfigCb(
            const ILocHidlGnssConfigServiceCallback::RobustLocationInfo& info) override;
};

Return<void> GnssConfigServiceCallback::getGnssSvTypeConfigCb(
        const hidl_vec<android::hardware::gnss::V1_0::GnssConstellationType>& disabledSvTypeList) {
...
Return<void> GnssConfigServiceCallback::getGnssSvTypeConfigCb_4_0(
        const hidl_vec<LocHidlGnssConstellationType>& disabledSvTypeList) {
    return Void();
}

Return<void> GnssConfigServiceCallback::getRobustLocationConfigCb(
        const ILocHidlGnssConfigServiceCallback::RobustLocationInfo& info) {
    if (sGnssConfigCbs) {
...
sp<ILocHidlFlpService>& getLocHidlFlpIface() {
    if (locHidlFlpIface == nullptr) {
        if (gnssHidlHal != nullptr) {
            locHidlFlpIface = gnssHidlHal->getExtensionLocHidlFlpService_4_0();
        }
        if (locHidlFlpIface == nullptr) {
...
sp<ILocHidlGnssConfigService>& getLocHidlGnssConfigIface() {
    if (lochidlgnssConfigIface == nullptr) {
        if (gnssHidlHal != nullptr) {
            lochidlgnssConfigIface = gnssHidlHal->getExtensionLocHidlGnssConfigService_4_0();
        }
        if (lochidlgnssConfigIface == nullptr) {
...
void HidlAPI::setFlpCbs(GnssCbBase* callbacks) {
    ...
    auto init = flpVendorIface->init_4_0(gnssFlpServiceCb);
#else
...
bool HidlAPI::gnssInjectTime(int64_t timeMs, int64_t timeReferenceMs, int32_t uncertaintyMs) {
    if (gnssHal != nullptr) {
...
//=================================================================================================
    //IAGnss
bool HidlAPI::agnssDataConnClosed() {
    if (agnssIface != nullptr) {
...
bool HidlAPI::agnssDataConnFailed() {
    if (agnssIface != nullptr) {
...
bool HidlAPI::agnssSetServer(int32_t type, const std::string& hostname, int32_t port) {
    if (agnssIface != nullptr) {
...
bool HidlAPI::agnssDataConnOpen(uint64_t networkHandle, const std::string& apn,
        int32_t apnIpType) {
    if (agnssIface != nullptr) {
    ...
    //IAGnssRil
void HidlAPI::agnssRilSetRefLocation(int type, int mcc, int mnc, int lac, int cid) {
    if (agnssRilIface != nullptr) {
...
bool HidlAPI::agnssRilSetSetId(int type, const std::string& setid) {
    if (agnssRilIface != nullptr) {
...
bool HidlAPI::agnssRilUpdateNetworkAvailability(bool available, const std::string& apn) {
    if (agnssRilIface != nullptr) {
...
bool HidlAPI::agnssRilUpdateNetworkState(uint64_t networkHandle, bool connected,
    ...
    //IGnssConfiguration
bool HidlAPI::gnssConfigSetBlacklist(
    ...
    //IGnssDebug
void  HidlAPI::gnssDebugGetDebugData(getDebugDataCb _hidl_cb) {
    ...
    //IGnssMeasurement
void HidlAPI::gnssMeasurementClose() {
    if (gnssMeasurementIface != nullptr) {
...
bool HidlAPI::gnssVisibilityCtrlEnableNfwLocationAccess(const std::vector<std::string>& proxyApps) {
    if (visibCtrlIface != nullptr) {
...
void HidlAPI::getRobustLocationConfig() {
    auto iface = getLocHidlGnssConfigIface();
    if (iface == nullptr) {
        ALOGE("ILocHidlGnssConfig interface NULL!");
        return;
...
void HidlAPI::setRobustLocationConfig(bool enable, bool enableForE911) {
    auto iface = getLocHidlGnssConfigIface();
    if (iface == nullptr) {
        ALOGE("ILocHidlGnssConfig interface NULL!");
        return;
