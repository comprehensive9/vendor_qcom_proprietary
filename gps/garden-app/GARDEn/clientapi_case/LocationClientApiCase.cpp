...
#define CLIENTAPI_TEST_YEAROFHW_OUTPUT  "yearofhw"
...
static void printHelp() {
    menuPrint("\n\n"
        ...
        "yearofhw: get the year of HW info\n"
        "p: ping test \n"
        "q: Quit\n"
...
void LocationClientApiCase::testGetYearOfHw() {
   gardenPrint("testGetYearOfHw enter\n");
    if (!mPLocationClient_1) {
        mPLocationClient_1 = new GnssLocationClientAPI();
        sleep(1);
    }
    if (mPLocationClient_1) {
        uint16_t yearOfHw = mPLocationClient_1->getYearOfHw();
        gardenPrint("testGetYearOfHw result: %d\n", yearOfHw);
    }
}
...
void LocationClientApiCase::menuAddGeofence(int clientId) {
    ...
    if (1 == clientId) {
        mPLocationClient_1->listAllActiveGeofences();
    } else {
        mPLocationClient_2->listAllActiveGeofences();
    }
    ...
    if (1 == clientId) {
        mPLocationClient_1->addGeofence(0, latitude, longitude,
                radius, type, responsiveness, time);
    } else {
        mPLocationClient_2->addGeofence(0, latitude, longitude,
                radius, type, responsiveness, time);
    }
}
...
void LocationClientApiCase::menuModifyGeofence(int clientId) {
    ...
    uint32_t numActiveGf = 0;
    char buf[16], *p;

    // list all active geofences
    if (1 == clientId) {
        numActiveGf = mPLocationClient_1->listAllActiveGeofences();
    } else {
        numActiveGf = mPLocationClient_2->listAllActiveGeofences();
    }
    if (0 == numActiveGf) {
    ...
    if ((0 == geofenceId) || (geofenceId > numActiveGf)) {
        menuPrint("Error: Invalid geofence Id entered !!");
        return;
    }
    ...
    while (!exit_loop) {
        ...
        if (str[0] != '1' && str[0] != '2') {
            if (str[0] == 'Q' || str[0] == 'q') {
                ...
                continue;
            } else if (!strcmp(str, CLIENTAPI_TEST_YEAROFHW_OUTPUT)) {
                testGetYearOfHw();
                continue;
            } else {
                printHelp();
                ...
                case 1: {
                    if (*pch == 'G' || *pch == 'g' || *pch == 'E' || *pch == 'e') {
                        if (*pch == 'G' || *pch == 'g') {
                            ...
                            char buf[16], *p = nullptr;
                            menuPrint("Route NMEA Strings to NMEA Port? (y/n) (default n) :");
                            fflush (stdout);
                            p = fgets (buf, 16, stdin);
                            if (p == nullptr) {
                                GARDEN_ERROR("Error: fgets returned nullptr !!");
                            } else if (p[0] == 'y') {
                                mPLocationClient_1->setAutoModeOptions(false, true);
                            }
                        }
                        if (strlen(pch) > 1) {
                            pch++;
