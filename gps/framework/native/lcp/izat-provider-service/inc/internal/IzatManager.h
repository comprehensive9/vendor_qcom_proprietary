...
public:
    ...
    uint32_t setUserPref(bool userPref, responseCallback* callback);
    uint32_t propagateOptInStatus(bool userPref, responseCallback* callback = nullptr);
    // notify the subscribers by loading FW_glue
    typedef void (notifyOptinStatusToSubscriberExt)(std::list<IDataItemCore*> dataItemList);
