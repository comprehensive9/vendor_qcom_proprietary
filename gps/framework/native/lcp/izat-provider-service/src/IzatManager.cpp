...
IzatManager::IzatManager(IOSFramework* pOSFrameworkObj) :
    ...
    mIzatContext.mMsgTask = (MsgTask*)
             LocContext::getLocContext(LocContext::mLocationHalName)->getMsgTask();
    ...
    struct InitMsg : public LocMsg {
        ...
        inline virtual void proc() const override {
            // instantiate XTadapter for opt-in propagation
            void *handle = nullptr;
            createXtAdapterInstance* createInstanceFn =
                (createXtAdapterInstance*)dlGetSymFromLib(handle, "libxtadapter.so",
                    "createXtAdapterInstance");
            if (NULL != handle) {
                ...
                LOC_LOGe("Failed to get handle to libxtadapter.so");
            }
...
int32 IzatManager::unsubscribeListener(IOSListener* osLocationListener) {
 ...
 uint32_t IzatManager::setUserPref(bool userPref, responseCallback* callback) {
    static setXtAdapterUserPref* setUserPrefFn;
    if (NULL == setUserPrefFn) {
        void *handle = NULL;
        setUserPrefFn = (setXtAdapterUserPref*)dlGetSymFromLib(handle,
...
uint32_t IzatManager::propagateOptInStatus(bool userPref, responseCallback* callback) {
    ENTRY_LOG();
    LOC_LOGd("OptInStatus: %d", userPref);
    static notifyOptinStatusToSubscriberExt* notifySubscriberFn;
    std::list<IDataItemCore *> EnhdataItem_list;
    ENHDataItem enh_data_item;
    enh_data_item.mEnabled = userPref;
    EnhdataItem_list.push_back(&enh_data_item);
    uint32_t sessionId = setUserPref(enh_data_item.mEnabled, callback);
    if (NULL == notifySubscriberFn) {
        void *handle = NULL;
        notifySubscriberFn = (notifyOptinStatusToSubscriberExt*)dlGetSymFromLib(handle,
            "liblocationservice_glue.so", "notifyToSubscriberExt");
    }
    if (NULL != notifySubscriberFn) {
        (*notifySubscriberFn)(EnhdataItem_list);
    } else {
        LOC_LOGe("notifySubscriberFn is NULL!");
    }
...
void IzatManager::registerGnssStatusCallback() {
    ...
    if (NULL == getter) {
        getter = (registerGnssStatuscb*) dlGetSymFromLib(libHandle,
            "android.hardware.gnss@" GNSS_HIDL_VERSION "-impl-qti.so",
                "registerGnssStatusCallback");
    }
    if (NULL != getter) {
        auto funcPtr = [this](bool status) {
                           this->propagateOptInStatus(status);
                       };
        (*getter)(funcPtr);
...
#else
    LOC_LOGd("GNSS_HIDL_VERSION is not defined");
#endif
}
