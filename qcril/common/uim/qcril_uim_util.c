...
)
{
  ...
  QCRIL_LOG_INFO("slot: %d iccid_len: %d ", slot, qcril_uim.card_iccid_info[slot].iccid_len );

#ifndef QMI_RIL_UTF
  /* Copy contents from ICCID cache */
  if(qcril_uim.card_iccid_info[slot].iccid_len > 0)
  {
    if(qcril_uim.card_iccid_info[slot].iccid_len >
       *iccid_len_ptr)
    {
    ...
    {
      file_data_len = qcril_uim.card_iccid_info[slot].iccid_len;
    }
  ...
  QCRIL_LOG_INFO("Return ICCID from cache");
...
)
{
  ...
  switch (qcril_err_code)
  {
    ...
    case QMI_ERR_NOT_SUPPORTED_V01:
      ril_error_value = QCRIL_UIM_RIL_E_REQUEST_NOT_SUPPORTED;
      break;
