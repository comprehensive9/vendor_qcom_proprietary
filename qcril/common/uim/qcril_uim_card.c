...
#ifndef QMI_RIL_UTF
    /* Running for loop for slotIdx 0 & 1 to get ICCID value for both cards from modem
       and store in both qcrild instances. This will be useful for legacy QCRIL GET_SLOT_STATUS query */

    for(slotIdx = 0; slotIdx < (QMI_UIM_MAX_SLOT_COUNT - 1) ; slotIdx++ )
    {
      qcril_uim.card_iccid_info[slotIdx].iccid_len = 0;

      if((RIL_E_SUCCESS == qcril_uim_read_iccid(slotIdx,
                                                iccid_data,
                                                &iccid_len)) &&
         (iccid_len > 0) &&
         (iccid_len <= QCRIL_UIM_ICCID_LEN))
      {
        ...
        memcpy(qcril_uim.card_iccid_info[slotIdx].iccid_data,
               iccid_data,
               iccid_len);
        qcril_uim.card_iccid_info[slotIdx].iccid_len = iccid_len;
      }
    }
#endif /* QMI_RIL_UTF */
  }
