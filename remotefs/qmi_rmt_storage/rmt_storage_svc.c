...
static const int RMTS_UIO_DEV_NODE_OPEN_MAX_RETRY_COUNT = 100;
static const int RMTS_UIO_DEV_NODE_OPEN_RETRY_SLEEP_IN_US = 1000000;
...
static int do_mmap(char *dev_path, struct uio_mmap_info *mmap_info)
{
	...
	int i, max_retry_limit, retry_sleep_interval_us;

	/* UIO device node creation can be delayed if ueventd daemon is
	is busy and has not yet processed the UEVENT notification from
	kernel and opening the device node during this time will result
	in ENOENT error for which we will keep retrying for the configured
	retry count with the configured sleep interval between retries.*/

	max_retry_limit = RMTS_UIO_DEV_NODE_OPEN_MAX_RETRY_COUNT;
	retry_sleep_interval_us = RMTS_UIO_DEV_NODE_OPEN_RETRY_SLEEP_IN_US;

	for (i = 0; i < max_retry_limit; ++i) {
		mmap_info->uio_fd = open(dev_path, O_RDWR | O_DSYNC);
		if (mmap_info->uio_fd >= 0)
			break;

		if (mmap_info->uio_fd < 0) {
			KMSGE("[%d,%s] Unable to open\n", errno, dev_path);
			KMSGE("[%d,%s] Unable to open\n", errno, strerror(errno));
			LOGE("[%d,%s] Unable to open\n", errno, dev_path);
			LOGE("[%d,%s] Unable to open\n", errno, strerror(errno));

			if (errno == ENOENT) {
				usleep(retry_sleep_interval_us);
				continue;
			}
...
int main(int argc, char **argv)
{
	...
	if (ret < 0) {
		LOGE("Mmap Failed for rmts (%d)...pausing execution!", ret);
		KMSGE("Mmap Failed for rmts (%d)...pausing execution!", ret);
		pause();

		LOGE("Mmap Failed for rmts (%d). exiting!", ret);
		exit(1);
	}
	else {
