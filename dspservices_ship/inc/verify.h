...
#ifdef __ANDROID__
/*android */
#if (defined VERIFY_PRINT_INFO) || (defined VERIFY_PRINT_ERROR) || (defined VERIFY_PRINT_WARN)|| (defined VERIFY_PRINT_ERROR_ALWAYS)
...
#ifdef VERIFY_PRINT_ERROR_ALWAYS
...
#if (defined VERIFY_PRINT_INFO) || (defined VERIFY_PRINT_ERROR) || (defined VERIFY_PRINT_ERROR_ALWAYS)
#include <stdio.h>
#endif
...
#define VERIFY_EPRINTF_ALWAYS(format, ...) (void)0
#endif
	   ...
	   do {\
        ...
        #define VERIFYM(val,err_code,format, ...) \
           do {\
                  ...
                  if(0 == (val)) {\
                         nErr = err_code;\
						 VERIFY_EPRINTF_ALWAYS(":Error: %x: " #val "\n", nErr);\
						 goto bail;\
                  } else {\
                         VERIFY_IPRINTF(":info: passed: " #val "\n");\
                  }\
