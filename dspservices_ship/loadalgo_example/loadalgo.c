...
typedef struct loadalgo_handle loadalgo_handle_t;
static loadalgo_handle_t* ld_handle;
...
static int ld_get_rpclib_sym(loadalgo_handle_t* ld_handle, const char *rpclib)
{
    int nErr = 0;
    ld_handle->librpchandle =  dlopen(rpclib, RTLD_NOW);
    if (ld_handle->librpchandle)
    {
        *(void **)(&ld_handle->loadalgo_open) =
                               dlsym(ld_handle->librpchandle,"loadalgo_open");
        if (ld_handle->loadalgo_open == NULL) {
            nErr = LOAD_ERROR;
            ALOGE("Error: 0x%x : %s : dlsym failed to find loadalgo_open\n", nErr, __func__);
            goto bail;
        }
        *(void **)(&ld_handle->loadalgo_init) =
                                dlsym(ld_handle->librpchandle,"loadalgo_init");
        if (ld_handle->loadalgo_init == NULL) {
            nErr = LOAD_ERROR;
            ALOGE("Error: 0x%x : %s : dlsym failed to find loadalgo_init\n", nErr, __func__);
            goto bail;
        }
        *(void **)(&ld_handle->loadalgo_physbuffer) =
                                dlsym(ld_handle->librpchandle,"loadalgo_physbuffer");
        if (ld_handle->loadalgo_physbuffer == NULL) {
            nErr = LOAD_ERROR;
            ALOGE("Error: 0x%x : %s : dlsym failed to find loadalgo_physbuffer\n", nErr, __func__);
            goto bail;
        }
        *(void **)(&ld_handle->loadalgo_Gaussian7x7u8) =
                                dlsym(ld_handle->librpchandle,"loadalgo_Gaussian7x7u8");
        if (ld_handle->loadalgo_Gaussian7x7u8 == NULL) {
            nErr = LOAD_ERROR;
            ALOGE("Error: 0x%x : %s : dlsym failed to find loadalgo_Gaussian7x7u8\n", nErr, __func__);
            goto bail;
        }
        *(void **)(&ld_handle->loadalgo_deinit) =
                                dlsym(ld_handle->librpchandle,"loadalgo_deinit");
        if (ld_handle->loadalgo_deinit == NULL) {
            nErr = LOAD_ERROR;
            ALOGE("Error: 0x%x : %s : dlsym failed to find loadalgo_deinit\n", nErr, __func__);
            goto bail;
        }
        *(void **)(&ld_handle->loadalgo_close) =
                                dlsym(ld_handle->librpchandle,"loadalgo_close");
        if (ld_handle->loadalgo_close == NULL) {
            nErr = LOAD_ERROR;
...
bail:
    if (nErr != 0) {
        dlclose(ld_handle->librpchandle );
        ld_handle->librpchandle  = NULL;
    }
    return nErr;
}

static int ld_get_lib_sym(loadalgo_handle_t* ld_handle, const char *qseecomlib)
{
    int nErr = 0;
    ld_handle->libhandle = dlopen(qseecomlib, RTLD_NOW);
    if (ld_handle->libhandle) {
        *(void **)(&ld_handle->QSEECom_start_app) =
                               dlsym(ld_handle->libhandle,"QSEECom_start_app");
        if (ld_handle->QSEECom_start_app == NULL) {
            nErr = LOAD_ERROR;
            ALOGE("Error: 0x%x : %s : Failed to find QSEECom_start_app\n", nErr, __func__);
            goto bail;
        }
        *(void **)(&ld_handle->QSEECom_shutdown_app) =
                                dlsym(ld_handle->libhandle,"QSEECom_shutdown_app");
        if (ld_handle->QSEECom_shutdown_app == NULL) {
            nErr = LOAD_ERROR;
            ALOGE("Error: 0x%x : %s : Failed to find QSEECom_shutdown_app\n", nErr, __func__);
            goto bail;
        }
        *(void **)(&ld_handle->QSEECom_send_cmd) =
                                dlsym(ld_handle->libhandle,"QSEECom_send_cmd");
        if (ld_handle->QSEECom_send_cmd == NULL) {
            nErr = LOAD_ERROR;
            ALOGE("Error: 0x%x : %s : Failed to find QSEECom_send_cmd\n", nErr, __func__);
            goto bail;
        }
        *(void **)(&ld_handle->QSEECom_send_modified_cmd) =
                                dlsym(ld_handle->libhandle,"QSEECom_send_modified_cmd");
        if (ld_handle->QSEECom_send_modified_cmd == NULL) {
            nErr = LOAD_ERROR;
            ALOGE("Error: 0x%x : %s : Failed to find QSEECom_send_modified_cmd\n", nErr, __func__);
            goto bail;
        }
        *(void **)(&ld_handle->QSEECom_send_modified_cmd_64) =
                                dlsym(ld_handle->libhandle,"QSEECom_send_modified_cmd_64");
        if (ld_handle->QSEECom_send_modified_cmd_64 == NULL) {
            nErr = LOAD_ERROR;
            ALOGE("Error: 0x%x : %s : Failed to find QSEECom_send_modified_cmd_64\n", nErr, __func__);
            goto bail;
        }
        *(void **)(&ld_handle->QSEECom_set_bandwidth) =
                                dlsym(ld_handle->libhandle,"QSEECom_set_bandwidth");
        if (ld_handle->QSEECom_set_bandwidth == NULL) {
            nErr = LOAD_ERROR;
...
bail:
    if (nErr != 0) {
        dlclose(ld_handle->libhandle );
        ld_handle->libhandle  = NULL;
    }
    return nErr;
}
                                  ...
                                  buf_type_t type, uint64_t* state)
{
    ...
    handle = (struct QSEECom_handle *)(ld_handle->qseecom);
    if (!handle) {
        nErr = INVALID_INPUT;
    ...
    nErr = (*ld_handle->QSEECom_send_modified_cmd_64)(handle, shared_buf_req, req_buf_len, shared_buf_rsp, rsp_buf_len, &ion_fd_info);
...
/**
  ...
  * @param ld_handle: Handle associated with the HLOS client app.
  * @retval : 0 in case of success.
  */
static int start_ta(loadalgo_handle_t* ld_handle){
    int nErr, i = 0;

    for(i = 0; i < MAX_NUMBER_PATHS; i++) {
        nErr = (*ld_handle->QSEECom_start_app)((struct QSEECom_handle **)&ld_handle->qseecom,
                        search_paths[i], "loadalgota64", LD_SB_LENGTH);
        if (nErr == 0) {
            break;
        }
        /* some targets have restriction of TA name to 8 character */
        nErr = (*ld_handle->QSEECom_start_app)((struct QSEECom_handle **)&ld_handle->qseecom,
                        search_paths[i], "loadalgo", LD_SB_LENGTH);
        if (nErr == 0) {
            break;
        }
    }
...
/**
  ...
  * @retval : 0 in case of success.
  */
static int start_proxy(loadalgo_handle_t* ld_handle) {
    int nErr = 0;

    nErr = (*ld_handle->loadalgo_open)(loadalgo_URI "&_dom=cdsp", &ld_handle->cdsp);
    if (nErr) {
        ALOGE("%s: Failed to open cdsp domain with nErr: 0x%x\n", __func__, nErr);
        return nErr;
    }
    proxyInit = 1;
    printf("%s: created proxy PD on CDSP with handle 0x%x\n", __func__, (int)ld_handle->cdsp);

    nErr = (*ld_handle->loadalgo_init)(ld_handle->cdsp);
    if (nErr) {
...
/**
  ...
  * @retval : 0 in case of success.
  */
static int initialize_securepd(loadalgo_handle_t* ld_handle) {
    int nErr = 0;
    uint64_t state = 0;
    ...
    nErr = (*ld_handle->QSEECom_set_bandwidth)(ld_handle->qseecom, true);
    if (nErr) {
    ...
    nErr = (*ld_handle->loadalgo_physbuffer)(ld_handle->cdsp, heapHandle.fd, 0, heapHandle.size, HEAP);
    if (nErr) {
    ...
    nErr = (*ld_handle->loadalgo_physbuffer)(ld_handle->cdsp, poolHandle.fd, 0, poolHandle.size, POOL);
     if (nErr) {
...
bail:
    if((*ld_handle->QSEECom_set_bandwidth)(ld_handle->qseecom, false)) {
...
static int ld_proxy_open()
{
    ...
    if (!loadalgoInit) {
        ld_handle = (loadalgo_handle_t *)malloc(sizeof(loadalgo_handle_t));
        if (ld_handle == NULL) {
            nErr = ALLOCATE_ERROR; //mem failed
            ALOGE("%s: Failed to allocate memory for loadalgo handle with nErr:%d\n", __func__, nErr);
            goto bail;
        }
        ld_handle->qseecom = NULL;
        ld_handle->libhandle = NULL;
        nErr = ld_get_lib_sym(ld_handle, qseecomlib);
        if (nErr) {
        ...
        nErr = ld_get_rpclib_sym(ld_handle, rpclib);
        if (nErr) {
        ...
        nErr = start_ta(ld_handle);
        if (nErr) {
            ALOGE("%s: Failed to load loadalgo app with nErr:%d\n", __func__, nErr);
            goto bail;
        }
        printf("%s: started loadalgo TA\n", __func__);

        nErr = start_proxy(ld_handle);
        if (nErr) {
        ...
        nErr = initialize_securepd(ld_handle);
        if (nErr) {
...
/**
  ...
  */
static void ld_proxy_close()
{
    int nErr = 0;
    if (ld_handle) {
        if (TAinit) {
            if (ld_handle->qseecom) {
                printf("%s: shutting down TA with QSEECom handle %p...\n", __func__, ld_handle->qseecom);
                nErr = (*ld_handle->QSEECom_shutdown_app)((struct QSEECom_handle **)&ld_handle->qseecom);
                if (nErr) {
        ...
        if (proxyInit) {
            ...
            nErr = (*ld_handle->loadalgo_deinit)(ld_handle->cdsp);
            if (nErr) {
                ALOGE("%s: Failed to deinitialize proxy with nErr 0x%x\n", __func__, nErr);
            }
            printf("%s: deinitialized proxy PD\n", __func__);

            nErr = (*ld_handle->loadalgo_close)(ld_handle->cdsp);
            if (nErr) {
...
static int start_load_algo_elf(char* path)
{
    ...
    nErr = ld_proxy_open();
    if (nErr) {
        ALOGE("%s: Failed to start loadalgo/proxy with nErr:%d\n", __func__, nErr);
        goto bail;
    }
    ...
    nErr = (*ld_handle->loadalgo_physbuffer)(ld_handle->cdsp, secureHandle.fd, 0, secureHandle.size, ALGO);
    if (nErr) {
...
int run_gaussian_algo(char* path)
{
    ...
    for (i = 0; i < LOOPS; i++)
    {
        ...
        nErr = (*ld_handle->loadalgo_Gaussian7x7u8)(ld_handle->cdsp, srcMemHandle.fd, 0, srcMemHandle.size, srcWidth,
...
static bool retry_sequence_after_user_request() {
    ...
    if (c == 'Y' || c == 'y') {
        ld_proxy_close();
        printf("%s: CDSP is down, waiting for 5 seconds before restarting\n", __func__);
        sleep(5);
        restart = true;
