...
static void initialize(void) {
    ...
    if (!libhandle) {
        ...
        *(void **) (&perf_hint_offload) = dlsym(libhandle, "perf_hint_offload");
        if ((rc = dlerror()) != NULL) {
             QLOGE("NRP: Unable to get perf_hint_offload function handle.\n");
             dlclose(libhandle);
             libhandle = NULL;
             return;
        }
        ...
        *(void **) (&perf_get_prop) = dlsym(libhandle, "perf_get_prop");
        if ((rc = dlerror()) != NULL) {
             QLOGE("NRP: Unable to get perf_get_prop function handle.\n");
             dlclose(libhandle);
             libhandle = NULL;
             return;
        }
...
const char *perf_prop_strings[] = {
    "vendor.debug.enable.lm",
    "vendor.iop.enable_uxe",
    "vendor.enable.prefetch",
    "vendor.iop.enable_prefetch_ofr",
    "vendor.perf.iop_v3.enable",
    "vendor.perf.iop_v3.enable.debug",
    "vendor.debug.enable.memperfd",
    "vendor.perf.pfar.enable",
    "vendor.perf.fps_switch.enable",
...
//increment this value on adding new properties
#define MAX_PERF_PROP 26
...
static void callperfoffloadhints(int params_per_call) {
    int j = 0;
    int size = (sizeof(perf_hints)/sizeof(perf_hints[0]));
    ...
    for (int i=0; i<params_per_call;i++) {
        perf_hint_offload(perf_hints[j]+i, "ANDR-PERF-TP-STRESS", duration, -1,0,NULL);
    }
}
...
void property_test() {
    ...
    for (i = 0 ; i < 7; i++) {
        prop_id = rand() % MAX_PERF_PROP ;
        strlcpy(property_val, perf_get_prop(perf_prop_strings[prop_id] , "Undefined").value, PROPERTY_VALUE_MAX);
        property_val[PROPERTY_VALUE_MAX-1] = '\0';
        printf("prop: %s: %s\n",perf_prop_strings[prop_id], property_val);
    }
}
static void *perflock_function(void *i) {
    ...
    for (int k=0; k<calls_per_thread; k++) {
#if TESTCONFIGSTORE
        file_no = rand() % 50;
        snprintf(buffer,200, "cp /vendor/etc/perf/all_xml/targetconfig_%d.xml /vendor/etc/perf/targetconfig.xml", rand() % 50);
        system(buffer);
        if((rand() % 1009) % 2 == 0) {
            system("stop perf-hal-2-0");
            system("start perf-hal-2-0");
            printf("[%d] restart perf-hal sleep [%d] file [%d] \n",threadno,sleep_time_in_s, file_no);
        ...
        for (j = 0; j < 50; j++) {
            duration = 3000 + rand()%7000;
            printf("thread:%d calling perfoffload hints:%d\n",threadno, j);
            callperfoffloadhints(params_per_call);
        }
        for (j = 0; j < 2; j++) {
            duration = 3000 + rand()%5000;
        ...
        for (j = 0; j < 2; j++) {
            printf("thread:%d calling property:%d\n",threadno, j);
            property_test();
        }
...
static void *perflock_function_1(void *i) {
    int j;
    int params_per_call = 2;
...
static void *perflock_function_2(void *i) {
    int j;
    int params_per_call = 2;
    ...
    for (int k=0; k<calls_per_thread; k++) {
        ...
        {
            int duration_2 = 10000;
            struct timespec sleep_time;
            QLOGE("NRP: Sleep");
            sleep_time.tv_sec = duration_2 / TIME_MSEC_IN_SEC;
            sleep_time.tv_nsec = (duration_2 % TIME_MSEC_IN_SEC) * TIME_NSEC_IN_MSEC;
            nanosleep(&sleep_time, NULL);
        }
        for (j = 0; j < 2; j++) {
            duration = 3000 + rand()%5000;
...
static void *perflock_function_3(void *i) {
    int j;
    int params_per_call = 2;
...
int main(int argc, char *argv[]) {
    ...
    for (i = 0; i < num_threads; i++) {
        ...
        *arg = i;
        int testcases = NUM_CASES;
        switch(i % testcases) {
            case 1:
                pthread_create(&perflock_thread[i], &attr, perflock_function_1, (void *)arg);
                break;
            case 2:
                pthread_create(&perflock_thread[i], &attr, perflock_function_2, (void *)arg);
                break;
            case 3:
                pthread_create(&perflock_thread[i], &attr, perflock_function_3, (void *)arg);
                break;
            default:
