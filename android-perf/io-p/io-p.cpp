...
#define PKG_LEN           1024
#define BOOT_SLEEP        500
#define MICRO_FOR_MILLI   1000
...
unsigned int preferred_app_count = 12;
uint64_t time_delta;
...
int configure_preferred_apps() {
    /*Get MemTotal*/
    FILE *memInfo = fopen("/proc/meminfo", "r");
    int memTotal = -1;
    if (memInfo == NULL) {
        QLOGE("Could not open /proc/meminfo ..\n");
        return -1;
    }
    char line[APP_NAME_LEN];
    while (fgets(line, sizeof(line), memInfo)) {
        char *end = NULL;
        char *token = strtok_r(line, ":", &end);
        if ((end != NULL) && (token != NULL) && (strncmp(token, MEMTOTAL, strlen(MEMTOTAL)) == 0)) {
    ...
    fclose(memInfo);
    if (memTotal > 0 && memTotal <= MEM_4GB) {
        preferred_app_count = 5;
    } else if (memTotal > MEM_4GB && memTotal <= MEM_6GB) {
        preferred_app_count = 8;
    } else {
        preferred_app_count = 12;
    }
    return 0;
}
            ...
            case UXENGINE_CMD_PERFLOCK_TRIGGER:
            {
                ...
                if(msg->opcode == UXE_TRIGGER || msg->opcode == UXE_ULMK_TRIGGER)
                {
                    ...
                    pthread_cond_signal(&ux_trigger_cond);
                         ...
                         msg->opcode == UXE_SPEED_TRIGGER) {
                    char *final_out[preferred_app_count];
                    char individual_time_slot_fav_apps[preferred_app_count * APP_NAME_LEN];
                    int uba_return = 0, i = 0;
                    char *speed_apps;
                    const int DAY_SLOTS = 2;
                    const int TIME_SLOTS = 4;

                    speed_apps = (char *) malloc (strcat_sz * sizeof(char));
                    if (speed_apps == NULL) {
                        pthread_cond_signal(&ux_trigger_cond);
                        break;
                    }
                    memset(speed_apps, 0, strcat_sz);
                    speed_apps[0] = '\0';
                    ...
                    int speed_factor = PREFERRED_SPEED_APP_COUNT / (TIME_SLOTS*DAY_SLOTS);
                    char * weekday[] = {"true", "false"};
                    for (int time_slot_iter = 1; time_slot_iter <= TIME_SLOTS; time_slot_iter++) {
                        for(int week_day_iter = 0; week_day_iter < DAY_SLOTS; week_day_iter++) {
                            uba_return = get_preferred_apps(final_out, 1, weekday[week_day_iter],
                                                            time_slot_iter, false);
                            ...
                            for (i = 0; i < uba_return ; i++) {
                                if (final_out[i][0] != '\0') {
                                    QLOGI("Final OUTPUT: Apps returned: %s\n", final_out[i]);
                                    /* Check if final_out[i] is part of io_pkg_tbl.
                                       If it doesn't exist, add to separate string A. This string
                                       contains apps that need to be speed compiled. */
                                    if (j < speed_factor) {
                                        if (get_package(final_out[i], NULL)) {
                                            set_iop_tbl_speed_bit(final_out[i]);
                                            j++;
                                        } else {
                                            if (!strstr(speed_apps, final_out[i])) {
                                                strlcat(speed_apps,final_out[i],strcat_sz);
                                                strlcat(speed_apps,"/",strcat_sz);
                                                j++;
                                            }
                                        }
                                    }

                                    if (!strstr(preferred_apps, final_out[i])) {
                                        strlcat(preferred_apps,final_out[i],strcat_sz);
                                        /*TODO preferred_apps string is of strcat_sz size. Above
                                           Strlcat buffer length should be 8 * 8 * 256 = 8 * 2048,
                                           So using strcat_sz, as it is already 8 * 2048. If
                                           PKG_NAME_LEN changes, then strcat_sz changes. Then,
                                           we need to adjust buffer length according to it.*/
                                        strlcat(preferred_apps,"/",strcat_sz);
                                    }
                                    strlcpy(final_out[i], "", APP_NAME_LEN);
                                }
                            }
                            //separating delimiter for fav apps of different time slots
                            strlcat(preferred_apps,"=",strcat_sz);
                        }
                    }

                    QLOGI("UXEngine: List of all preferred apps in uxe_server: %s\n",
                           preferred_apps);
                    if (msg->opcode == UXE_SPEED_TRIGGER) {
                        char *iop_out[PREFERRED_SPEED_APP_COUNT];
                        char iop_apps[PREFERRED_SPEED_APP_COUNT * APP_NAME_LEN];


                        for (i = 0; i < PREFERRED_SPEED_APP_COUNT; i++) {
                           iop_out[i] = &iop_apps[i * APP_NAME_LEN];
                           iop_out[i][0] = '\0';
                        }
                        ...
                        int iop_return = 0;
                        /* Obtain list of apps from io_pkg_tbl whose pkg_bit_count is 0.
                           This list is the apps that need to be de-compiled. Check last
                           compile timestamp and if > 24hrs mark for decompilation.
                           Add to deletion string */
                        iop_return = get_pspeed_apps(iop_out, cur_time);
                        QLOGI("No. of old PApps to be decompiled: %d\n", iop_return);

                        for (int k = 0; k < iop_return; k++) {
                            strlcat(speed_apps, "=", APP_NAME_LEN);
                            strlcat(speed_apps, iop_out[k], APP_NAME_LEN);
                        }

                        strlcpy(preferred_apps, speed_apps, strcat_sz);

                        reset_speed_bits();
                        QLOGI("UXEngine: List of all preferred apps for speed/despeed %s\n",
                               preferred_apps);
                    }
                    free(speed_apps);
                    pthread_cond_signal(&ux_trigger_cond);
       ...
       //wait for perflock commands
        ...
        switch (cmd) {
            case UXENGINE_CMD_PERFLOCK_EVENTS:
            {
                ...
                if(opcode == UXE_EVENT_BINDAPP)
                {
                    ...
                    if (bindApp_count) {
                        ...
                        if (ux_lat.bindApp_dur != 0 && (LAT_LOW_THRESHOLD < lat_thres &&
                            LAT_HIGH_THRESHOLD > lat_thres) && bindApp_dur < 4000) {
                            avg_bindApp = ((bindApp_count) * ux_lat.bindApp_dur + bindApp_dur) /
                                           (bindApp_count+1);
                        } else {
                ...
                if (opcode == UXE_EVENT_DISPLAYED_ACT)
                {
                    ...
                    if ((bindApp_dur == 0 && launching) || !pkg_match) {
                        ...
                        if(bindApp_count)
                            ...
                            goto disAct_cleanup;
                        avg_disAct = ux_lat.disAct_dur + bindApp_dur;
                        non_empty_launch = 0;
                    }
                    ...
                    if (pkg_count - 1) {
                        ...
                        if (avg_disAct != 0 && pkg_count != 0 &&
                            (LAT_LOW_THRESHOLD < lat_thres && LAT_HIGH_THRESHOLD > lat_thres) &&
                             disAct_dur < 5000) {
                            avg_disAct = ((pkg_count - 1) * avg_disAct + disAct_dur) / pkg_count;
                            disAct_dur = avg_disAct;
                        }
                        QLOGI("UXEngine: Average displayed activity after: %d\n",
                               avg_disAct);
                        if(avg_disAct != 0)
                            disAct_dur = avg_disAct;
                    }
                    ...
                    update_ux_lat_details(in_pkg_name, bindApp_dur, disAct_dur, 0,
                                          non_empty_launch);
                ...
                if(opcode == UXE_EVENT_PKG_INSTALL)
                {
                    ...
                    // If app update, remove from speed compile list.
                    if (install_code == 1)
                        remove_pkg(in_pkg_name);
                    /*
                        TODO: App newly installed. High chances of being frequently used.
                        Increase priority.
                        IOP file capture logic can benefit from knowing pkg install.
                    */
                }
                if (opcode == UXE_EVENT_PKG_SPEED_ADD)
                {
                    QLOGI("UXEngine: Received pkg speed mode add - %s\n", in_pkg_name);
                    pkg_details pkg_info;
                    strlcpy(pkg_info.pkg_name,in_pkg_name,APP_NAME_LEN);
                    time(&pkg_info.last_time_launched);
                    update_pkg_details(pkg_info);
                    // Store old latency data, and reset with new data.
                }
                if (opcode == UXE_EVENT_PKG_SPEED_DEL)
                {
                    QLOGI("UXEngine: Received pkg speed mode del - %s\n", in_pkg_name);
                    remove_pkg(in_pkg_name);
                    // Restore old latency data
                }
                break;
            }
            case IOP_CMD_PERFLOCK_IOPREFETCH_START:
            {
                ...
                strlcpy(enable_prefetch_property,
                        perf_get_prop("vendor.enable.prefetch" , "0").value, PROPERTY_VALUE_MAX);
                enable_prefetch_property[PROPERTY_VALUE_MAX-1]='\0';

                enable_prefetcher = strtod(enable_prefetch_property, NULL);

                strlcpy(enable_prefetch_ofr_property,
                        perf_get_prop("vendor.iop.enable_prefetch_ofr" , "0").value,
                        PROPERTY_VALUE_MAX);
                ...
                if(msg->pid < 0)
                {
                    ...
                    time(&pkg_info.last_time_launched);
                    compute_time_day_slot(week_day, &time_slot);
                    QLOGI("UXEngine Updating Details: pkg_name: %s, week_day: %s, \
                           time_slot: %d %s\n",
                           pkg_info.pkg_name, week_day, time_slot, tmp_pkg_name);
...
static void *Alloccb() {
    void *mem = (void *) new(std::nothrow) iop_msg_t;
    return mem;
}
...
int iop_server_init() {
    ...
    int loop_count = 20, elapsed_time = 0;
    QLOGI("IOP server starting");
    int ret = configure_preferred_apps();
    if (ret < 0)
        QLOGE("Preferred Apps are not configured at Runtime, using default value = %u", preferred_app_count);

    strcat_sz = (preferred_app_count *(PKG_NAME_LEN+1));
    ...
    memset(preferred_apps, 0, strcat_sz * sizeof(char));
    preferred_apps[0] = '\0';

    IOPevqueue.GetDataPool().SetCBs(Alloccb, Dealloccb);

    /*Create Database before starting the IOP or UXE server*/
    for (;;) {
        if(!is_boot_complete()) {
            usleep(BOOT_SLEEP * MICRO_FOR_MILLI);
            elapsed_time += BOOT_SLEEP;
            if (elapsed_time > 1 * 60 * MICRO_FOR_MILLI) { // if the device is not booted for 1 min smth is wrong
                QLOGE("Device taking longer time for Bootup. Returning..");
                goto error;//return the for loop
            }
            continue;
        }
        QLOGI("Boot_completed. Create IOP DB");
        if (is_db_init == false) {
            if(create_database() == 0) {
                is_db_init = true;
                QLOGI("Successfully Created IOP database");
                break;
            } else {
                loop_count--;
                usleep(50 * MICRO_FOR_MILLI);
                if (loop_count < 0) { //This Check is to avoid creteDB() forever.
                    QLOGE("Error in IOP DB Creation. Returning..");
                    goto error;// If IOP DB is not created for 20 attempts after boot complete, smth wrong.
                }
            }
        } else {
            break; // Already created DB.
        }
    }
    QLOGI("Successfully Created IOP Database");
    ...
    if (uxe_disabled()) {
        ...
        strlcpy(property,perf_get_prop("vendor.iop.uxe_trigger_freq" , "1").value,
                PROPERTY_VALUE_MAX);
        property[PROPERTY_VALUE_MAX-1]='\0';
...
error:
    QLOGE("Unable to create control service (stage=%d, rc1=%d rc2=%d uba_rc=%d)",
           stage, rc1, rc2, uba_rc);
    return 0;
}
...
char * uxe_server_submit_request(iop_msg_t *msg) {
    ...
    pthread_mutex_unlock(&ux_trigger_mutex);
    return preferred_apps;
    /*TODO - While returning we are returning by reference. So when a new request comes
      back to back, we again memset preferred_apps which can corrupt the global variable.
      (the string we are still copying) */
}
...
int iop_server_submit_request(iop_msg_t *msg) {
    ...
    if (perf_debug_output) {
        snprintf(trace_buf, TRACE_BUF_SZ, "IOP HAL: Received pkg_name = %s pid = %d",
                 msg->pkg_name,msg->pid);
        QLOGE("%s", trace_buf);
    }
