...
#define IO_PREFETCHER_QUERY_REMOVE_PACKAGE_PKG_TBL   "DELETE from io_pkg_tbl \
WHERE pkg_name = '%s'"
...
#define IOP_QUERY_SET_SPEED_BIT "UPDATE io_pkg_tbl \
SET pkg_use_count = 1 \
WHERE pkg_name = '%s' AND (pkg_last_use > 0)"

#define IOP_QUERY_RESET_SPEED_BIT "UPDATE io_pkg_tbl \
SET pkg_use_count = 0 \
WHERE (pkg_last_use > 0)"

#define IOP_QUERY_GET_DESPEED "SELECT * FROM io_pkg_tbl \
WHERE (pkg_use_count = 0 \
AND (%lu > pkg_last_use) \
AND (%lu - pkg_last_use > 172800) \
)"

#define IOP_QUERY_SPEED_BLACKLIST "UPDATE io_pkg_tbl \
SET pkg_use_count = -1 \
SET pkg_last_use = -1 \
WHERE pkg_name = '%s'"

#define IOP_QUERY_SPEED_GET_BLACKLIST "SELECT * FROM io_pkg_tbl \
WHERE pkg_name = '%s' \
AND (pkg_use_count = -1) \
AND (pkg_last_use = -1)"
...
#define UX_LATENCY_QUERY_GET_LAT_PRED "SELECT pkg_name,predict_success,predict_fail FROM ux_lat_tbl WHERE pkg_name='%s'"
...
WHERE week_day = '%s' AND (timeslot_count_%d > 0) \
...
#define UX_LATENCY_QUERY_GET_MEMORY_ST  "SELECT memory_st FROM ux_lat_tbl WHERE pkg_name='%s'"
...
int speed_blacklist(char *pkg_name);
...
static int close_database()
{
  ...
  db_conn = NULL;// sqlite3_close_v2() will make sure and deallocate the DB later on, if its BUSY. So not checking for its return code.
...
int get_file_list(char *pkg_name, file_details *file_info_list, int size)
{
    ...
    char query_str[QUERY_LEN] = IO_PREFETCHER_QUERY_GET_FILE_LIST;
...
{
    char query_str[QUERY_LEN] = IO_PREFETCHER_QUERY_TOTAL_FILE;
    int num_files = 0;
    char *error=NULL;
    sqlite3_stmt *stmt=NULL;
...
int get_total_pkgs()
{
    char query_str[QUERY_LEN] = IO_PREFETCHER_QUERY_TOTAL_PKG;
    int num_pkg = 0;
    char *error=NULL;
    sqlite3_stmt *stmt=NULL;
...
{
    char query_str[QUERY_LEN] = IO_PREFETCHER_QUERY_GET_PKG_LIST;
    int num_pkg;
    char *error=NULL;
    sqlite3_stmt *stmt=NULL;
    int counter = 0;
...
{
    char pkg_query_str[QUERY_LEN] = IO_PREFETCHER_QUERY_REMOVE_PACKAGE_PKG_TBL;

    snprintf(pkg_query_str,sizeof(pkg_query_str),IO_PREFETCHER_QUERY_REMOVE_PACKAGE_PKG_TBL,pkg_name);
...
{
    char query_str[QUERY_LEN] = IO_PREFETCHER_QUERY_REMOVE_PACKAGE_FILE_TBL;
...
{
    char query_str[QUERY_LEN] = IO_PREFETCHER_QUERY_MARK_FOR_DELETE;
...
{
    char query_str[QUERY_LEN] = IO_PREFETCHER_QUERY_MARK_FOR_DISABLE;
    if (file_name) {
...
{
    char query_str[QUERY_LEN] = IO_PREFETCHER_QUERY_MARK_FOR_RESTUDY;
...
{
    char query_str[QUERY_LEN] = IO_PREFETCHER_QUERY_MARK_CACHE_DROPPED;
...
{
    char query_str[QUERY_LEN] = IO_PREFETCHER_QUERY_UPDATE_FILE_MODIFIED;
...
int update_file_details(char * pkg_name,file_details *file_info[], int size)
{
    ...
    char *error=NULL;
    sqlite3_stmt *stmt=NULL;
    char trace_buf[QUERY_LEN];
    char query_str[QUERY_LEN*2] = IO_PREFETCHER_QUERY_UPDATE_FILE_DETAILS;
    iop_query_exec("BEGIN TRANSACTION;");
    for(i=0;i<size;i++) {
...
{
    char query_str[QUERY_LEN*2] = IO_PREFETCHER_QUERY_UPDATE_PKG_DETAILS;
...
int get_package(char *pkg_name, pkg_details *pkg_info)
{
    ...
    char query_str[QUERY_LEN] = IO_PREFETCHER_QUERY_GET_PKG;
    char * error;
    sqlite3_stmt *stmt=NULL;
    int count = 0;
...
void set_iop_tbl_speed_bit(char *pkg_name)
{
    char query_str[QUERY_LEN] = IOP_QUERY_SET_SPEED_BIT;

    snprintf(query_str,sizeof(query_str),IOP_QUERY_SET_SPEED_BIT,pkg_name);
    iop_query_exec(query_str);
}

void reset_speed_bits()
{
    char query_str[QUERY_LEN] = IOP_QUERY_RESET_SPEED_BIT;
    iop_query_exec(query_str);
}

int get_pspeed_apps(char **out_list, time_t cur_time)
{
    pkg_details *pkg_info;
    char query_str[QUERY_LEN] = IOP_QUERY_GET_DESPEED;
    char * error;
    sqlite3_stmt *stmt=NULL;
    int count = 0;

    snprintf(query_str,sizeof(query_str),IOP_QUERY_GET_DESPEED,cur_time,cur_time);
    QLOGI("\n get_pspeed_apps %s \n",query_str);
    if(NULL == db_conn) open_db();
    ...
    while (sqlite3_step(stmt)==SQLITE_ROW) {
       const char *pkg_name = (const char *)sqlite3_column_text(stmt, 0);
       if (pkg_name != NULL) {
           strlcpy(out_list[count], pkg_name, APP_NAME_LEN);
           count++;
       }
    }
    sqlite3_reset(stmt);
...
int remove_ux_pkg(char *pkg_name, char *week_day)
{
    ...
    char pkg_query_str1[QUERY_LEN*2] = UX_ENGINE_QUERY_REMOVE_PACKAGE_PKG_TBL;
    char pkg_query_str2[QUERY_LEN*2] = UX_LATENCY_QUERY_REMOVE_PACKAGE_PKG_TBL;
...
int get_top_ux_pkg_list(pkg_ux_top_details *pkg_list, char *week_day, int time_slot, int size, int start)
{
    ...
    char *error = NULL;
    sqlite3_stmt *stmt = NULL;
    char query_str[QUERY_LEN*2] = UX_ENGINE_QUERY_GET_TOP_PKG_LIST;
    snprintf(query_str,sizeof(query_str),UX_ENGINE_QUERY_GET_TOP_PKG_LIST,time_slot,week_day,time_slot,time_slot);
...
int get_total_ux_pkgs(int uniq)
{
    char *error = NULL;
    sqlite3_stmt *stmt = NULL;
    char query_str[QUERY_LEN] = UX_ENGINE_QUERY_TOTAL_PKG;
    int num_pkgs = 0;
...
int get_total_pkg_use_count(char *pkg_name)
{
    ...
    sqlite3_stmt *stmt = NULL;
    int launch_count = 0;
    char query_str[QUERY_LEN*2] = UX_LATENCY_QUERY_GET_PKG_COUNT;
...
int get_total_pkg_bindapp_count(char *pkg_name)
{
    ...
    sqlite3_stmt *stmt = NULL;
    int bindApp_count = 0;
    char query_str[QUERY_LEN*2] = UX_LATENCY_QUERY_GET_BINDAPP_COUNT;
...
int get_ux_lat_pkg(char *pkg_name, pkg_ux_lat_details *pkg_info)
{
    ...
    sqlite3_stmt *stmt = NULL;
    int counter = 0;
    char query_str[QUERY_LEN] = UX_LATENCY_QUERY_GET_LAT;
    ...
    while (counter == 0 && sqlite3_step(stmt) == SQLITE_ROW) {
...
int get_ux_lat_pred(char *pkg_name, pkg_ux_lat_pred_details *pkg_info)
{
    char *error = NULL;
    sqlite3_stmt *stmt = NULL;
    int counter = 0;
    char query_str[QUERY_LEN] = UX_LATENCY_QUERY_GET_LAT_PRED;
    snprintf(query_str,sizeof(query_str),UX_LATENCY_QUERY_GET_LAT_PRED,pkg_name);
...
{
    char query_str[QUERY_LEN*2] = UX_LATENCY_QUERY_SET_PKG_UPDATE;
...
{
    char query_str1[QUERY_LEN*2] = UX_LATENCY_QUERY_UPDATE_PKG_DETAILS;
    char query_str2[QUERY_LEN*2] = UX_LATENCY_QUERY_INSERT_PKG;
...
{
    char query_str[QUERY_LEN*2] = UX_LATENCY_QUERY_UPDATE_PRED_DETAILS;
    QLOGI("update_lat_pred_details");

    pkg_ux_lat_details pkg_info;
    pkg_ux_lat_pred_details pkg_pred_info;

    get_ux_lat_pkg(pkg_name, &pkg_info);
    get_ux_lat_pred(pkg_name, &pkg_pred_info);

    if (succ > 0 && weight == 0) {
        snprintf(query_str,sizeof(query_str),UX_LATENCY_QUERY_UPDATE_PRED_DETAILS
                                             ,(float)pkg_info.bindApp_dur
                                             ,(float)pkg_info.disAct_dur
                                             ,(int)pkg_pred_info.pred_success
                                             ,(int)pkg_pred_info.pred_fail
                                             ,pkg_name);
        QLOGI("\nUpdate Query = %s \n",query_str);
        iop_query_exec(query_str);
    }
    else if (succ == 0 && weight == 0){
        snprintf(query_str,sizeof(query_str),UX_LATENCY_QUERY_UPDATE_PRED_DETAILS
                                             ,0.0
                                             ,0.0
                                             ,(int)pkg_pred_info.pred_success
        ...
        // Check latency benefit
        int pkg_count = get_total_pkg_use_count(pkg_name);
        if (pkg_count < 10 && 1.05*pkg_info.disAct_dur > pkg_pred_info.pred_fail) {
           // benefit less than 5%. Blacklist app from speed mode.
           speed_blacklist(pkg_name);
        }
    }
    return 0;
}
...
{
    char query_str1[QUERY_LEN*2] = UX_LATENCY_QUERY_UPDATE_GAME_DETAILS;
...
{
    char query_str1[QUERY_LEN*2] = UX_ENGINE_QUERY_UPDATE_PKG_DETAILS;
    char query_str2[QUERY_LEN*2] = UX_ENGINE_QUERY_INSERT_PKG;
...
{
    char query_str[QUERY_LEN*2] = UX_ENGINE_QUERY_RESET_PKG_DETAILS;
    //Insert
...
int get_ux_package(char *pkg_name)
{
    char *error = NULL;
    sqlite3_stmt *stmt = NULL;
    char query_str[QUERY_LEN] = UX_ENGINE_QUERY_GET_PKG;
    int size = 3;
    int counter = 0;
...
int get_ux_pkg_list(pkg_ux_details *pkg_list, int size)
{
    ...
    sqlite3_stmt *stmt = NULL;
    int counter = 0;
    char query_str[QUERY_LEN] = UX_ENGINE_QUERY_GET_PKG_LIST;
...
double get_ux_launch_count_avg(char *week_day)
{
    ...
    sqlite3_stmt *stmt = NULL;
    double avg = 0;
    char query_str[QUERY_LEN] = UX_ENGINE_QUERY_AVERAGE_LAUNCH_COUNT;
...
{
    char query_str[QUERY_LEN] = UX_ENGINE_QUERY_UNINSTALL_PKG;
...
int get_memory_st(char *pkg_name)
{
    char *error = NULL;
    sqlite3_stmt *stmt = NULL;
    int memory_st = 0;
    char query_str[QUERY_LEN] = UX_LATENCY_QUERY_GET_MEMORY_ST ;
    snprintf(query_str,sizeof(query_str),UX_LATENCY_QUERY_GET_MEMORY_ST,pkg_name);
    ...
    while (sqlite3_step(stmt) == SQLITE_ROW) {
        memory_st = sqlite3_column_int(stmt, 0);
    }
...
int speed_blacklist(char *pkg_name)
{
    char query_str[QUERY_LEN] = IOP_QUERY_SPEED_BLACKLIST;
    snprintf(query_str,sizeof(query_str),IOP_QUERY_SPEED_BLACKLIST,pkg_name);
    iop_query_exec(query_str);
    return 0;
}

int is_blacklisted(char *pkg_name)
{
    char *error = NULL;
    sqlite3_stmt *stmt = NULL;
    int count = 0;
    char query_str[QUERY_LEN] = IOP_QUERY_SPEED_GET_BLACKLIST;
    snprintf(query_str,sizeof(query_str),IOP_QUERY_SPEED_GET_BLACKLIST,pkg_name);
