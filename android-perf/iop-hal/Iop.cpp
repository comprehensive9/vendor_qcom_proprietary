...
Return<int32_t> Iop::uxEngine_events(int32_t opcode, int32_t pid, const hidl_string& pkg_name, int32_t lat) {
    if (enable_uxe == -1) {
        ...
        strlcpy(property,perf_wait_get_prop("vendor.iop.enable_uxe" , "0").value,
                PROPERTY_VALUE_MAX);
...
Return<void> Iop::uxEngine_trigger(int32_t opcode, uxEngine_trigger_cb _hidl_cb) {
    if(enable_uxe == -1) {
         PropVal property;
         property = perf_wait_get_prop("vendor.iop.enable_uxe", "0");
         enable_uxe = strtod(property.value, NULL);
    }
    if(enable_uxe == 0) {
       _hidl_cb("");
       return Void();
...
Iop::Iop() {
    ...
    if (mHandle.iop_server_init != NULL ) {
        (*(mHandle.iop_server_init))();
    }
}
