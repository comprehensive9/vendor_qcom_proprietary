...
ReferenceFeature::ReferenceFeature(FeatureInfo currFeatureInfo) :
    ...
    if (mFeatureInputDb) {
        mMetaMeter = new(std::nothrow) ReferenceMetaMeter(mfeatureInfo, mFeatureInputDb, mFeatureState);
    }
    ...
    if (mFeatureInputDb && mAction) {
        ...
        if(outputDb) {
            mAlgorithm = new(std::nothrow) ReferenceAlgo(mFeatureInputDb, outputDb, mFeatureState);
        }
    }
}
...
void ReferenceFeature::createFeatureInputDb() {
    ...
    mFeatureInputDb = new(std::nothrow) LMDatabase<time_t>(0,
