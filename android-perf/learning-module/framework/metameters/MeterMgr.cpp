...
void MeterMgr::createMeterHelper(MeterInfo& meterInfo) {
    ...
    meterHelper->meterThread = new(std::nothrow) ThreadHelper(mFInfo.name + " " + meterInfo.name.c_str(),false);
...
long int MeterMgr::getSyncData() {
    long int data = -1;
    if(getState() == ACTIVE) {
       data = mMetaMeter->getSyncData(mMeterInstanceList);
    }
    return data;
}
