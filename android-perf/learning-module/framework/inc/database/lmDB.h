...
lmdbError LMDatabase<T...>::createLMdbTables() {
    ...
    int sqlErr;
    const char* tblNm = LMDBTbl::getTableName().c_str();
    try {
        if (lmdb_conn) {
            Name s = Q_C_TBL_HDR;
            s += tblNm;
            s += LMDB_STRINGIFY(Q_C_TBL_COLUMNS);
            LMDB_SQL_EXEC_STMT_CHK_OK_OR_RTN_ERR(tblNm, sqlErr, lmdb_conn, s.c_str(), NULL, NULL, NULL);
        }
    } catch (std::exception &e) {
        QLOGE("Caught exception: %s in %s",e.what(), __func__);
    } catch (...) {
        QLOGE("Caught Exception in %s",__func__);
    }
    return LMDB_NO_ERROR;
}
...
lmdbError LMDatabase<T...>::writeCachedDRToDisk() {
    ...
    try {
        if (lmdb_conn) {
            ...
            LMDB_SQL_PREPARE_STMT_CHK_OK_RTN_ERR(tblNm, sqlErr, lmdb_conn, s.c_str(), s.length(), &stmt_hdl_insert, NULL);

            s = Q_U_VAL_HDR;
            s += tblNm;
            s += LMDB_SPACE;
            s += LMDB_STRINGIFY(Q_U_VAL_FLD);
            LMDB_SQL_PREPARE_STMT_CHK_OK_RTN_ERR(tblNm, sqlErr, lmdb_conn, s.c_str(), s.length(), &stmt_hdl_update, NULL);

            LMDB_SQL_EXEC_TXN_CHK_OK(tblNm, sqlErr, lmdb_conn, LMDB_SQL_BEGIN_IMM_TXN_STMT, NULL,NULL,NULL);
            ...
            do {
                ...
                for(int i = 0; i < KEY_TYPE_INT_RSVD; i++, colBindIndex++) {
                    if (stKeyPtr->keys[i] != LMDB_INTKEY_DEFVAL) {
                        LMDB_SQL_STEP_PSTMT_CHK_OK(tblNm, sqlErr, bind_int,  stmt_hdl, colBindIndex, (stKeyPtr->keys[i]));
                    }
                }

                if (!(stKeyPtr->appNm.empty())) {
                    LMDB_SQL_STEP_PSTMT_CHK_OK(tblNm, sqlErr, bind_text, stmt_hdl, colBindIndex, stKeyPtr->appNm.c_str(), stKeyPtr->appNm.length(), LMDB_SQL_SMC_NOFREE);
                }
                colBindIndex++;

                if (!(stKeyPtr->rsvdNm.empty())) {
                    LMDB_SQL_STEP_PSTMT_CHK_OK(tblNm, sqlErr, bind_text, stmt_hdl, colBindIndex, stKeyPtr->rsvdNm.c_str(), stKeyPtr->rsvdNm.length(), LMDB_SQL_SMC_NOFREE);
                }
                colBindIndex++;

                LMDB_SQL_STEP_PSTMT_CHK_OK(tblNm, sqlErr, bind_blob, stmt_hdl, colBindIndex, stDataPtr, sizeof(T1), LMDB_SQL_SMC_NOFREE);
                colBindIndex++;

                if (stHdrPtr->rID) {
                    LMDB_SQL_STEP_PSTMT_CHK_OK(tblNm, sqlErr, bind_int,  stmt_hdl, colBindIndex, (stHdrPtr->rID));
                }
                colBindIndex++;

                LMDB_SQL_STEP_PSTMT_CHK_DONE(tblNm, sqlErr, step, stmt_hdl);
                LMDB_SQL_STEP_PSTMT_CHK_OK(tblNm, sqlErr,  reset, stmt_hdl);
            } while (iterate);
            LMDBTbl::freeRow(v);

            LMDB_SQL_EXEC_TXN_CHK_OK(tblNm, sqlErr, lmdb_conn, LMDB_SQL_COMMIT_TXN_STMT, NULL,NULL,NULL);

            LMDB_SQL_STEP_PSTMT_CHK_OK(tblNm, sqlErr1, finalize, stmt_hdl_insert);
            LMDB_SQL_STEP_PSTMT_CHK_OK(tblNm, sqlErr, finalize, stmt_hdl_update);
...
int LMDatabase<T...>::getCountDR(LMDBDataRowKey& k) {
    ...
    try {
        if (lmdb_conn) {
            ...
            LMDB_SQL_PREPARE_STMT_CHK_OK(tblNm, sqlErr, lmdb_conn, s.c_str(), s.length(), &stmt_hdl, NULL);
            ...
            /*while (1)*/ {
                ...
                LMDB_SQL_STEP_PSTMT_CHK_ROW(tblNm, sqlErr, step, stmt_hdl);
                if (LMDB_SQL_ROW == sqlErr) {
                    LMDB_SQL_STEP_COLFN_NOCHECK(tblNm, Count, column_int, stmt_hdl, COL0_SEL_INDEX);
                } /*else { //LMDB_SQL_ROW?
                    LMDB_SQL_STEP_PSTMT_CHK_OK(tblNm, sqlErr,  reset, stmt_hdl);
                    break;
                    }*/
            }
            LMDB_SQL_STEP_PSTMT_CHK_OK(tblNm, sqlErr, finalize, stmt_hdl);
        }
    } catch (std::exception &e) {
        QLOGE("Caught exception: %s in %s",e.what(), __func__);
    } catch (...) {
...
typename LMDatabase<T...>::template vecT<T1> LMDatabase<T...>::readAllocateDR(LMDBDataRowKey& k, DRSize reqCount, lmdbOrderByCol orderByCol, lmdbOrderByDir orderByDir) {
    ...
    try {
            ...
            for (int i = 0; i < reqCount; i++) {
                ...
                if (LMDB_SQL_ROW == sqlErr) {
                    ...
                    for(int i = 0; i < KEY_TYPE_INT_RSVD; i++, colSelectIndex++) {
                        ...
                        LMDB_SQL_STEP_COLFN_NOCHECK(tblNm, cUCPtr, column_text, stmt_hdl, colSelectIndex);
                        if (cUCPtr) {
                            LMDB_SQL_STEP_COLFN_NOCHECK(tblNm, stKeyPtr->keys[i], column_int, stmt_hdl, colSelectIndex);
                        }
                    }
                    LMDB_SQL_STEP_COLFN_NOCHECK(tblNm, cUCPtr, column_text, stmt_hdl, colSelectIndex);
                    colSelectIndex++;
                    ...
                    LMDB_SQL_STEP_COLFN_NOCHECK(tblNm, cVPtr, column_blob, stmt_hdl, colSelectIndex);
                    colSelectIndex++;
                    LMDB_memscpy(stDataPtr, sizeof(*stDataPtr), cVPtr, sizeof(*stDataPtr));

                    LMDB_SQL_STEP_COLFN_NOCHECK(tblNm, stHdrPtr->rID, column_int64, stmt_hdl, colSelectIndex);
                    colSelectIndex++;
                } else { //LMDB_SQL_DONE?
                    LMDB_SQL_STEP_PSTMT_CHK_OK(tblNm, sqlErr,  reset, stmt_hdl);
                    break;
        ...
        QLOGE("Error in std::string operations in %s", __func__);
    }
    return node;
}
#ifdef LMDB_DEBUG_SQL_SHADOW_RD
...
int LMDatabase<T...>::deleteDeallocateDR(vecT<T1>& v) {
    ...
    try {
        if (lmdb_conn) {
            ...
            s += tblNm;
            s += LMDB_SPACE;
            s += LMDB_STRINGIFY(Q_DR_VAL_CLAUSE);
            ...
            for(int i = 0; i < vSize; i++)
            {
                ...
                if (stHdrPtr->rID) {
                    LMDB_SQL_STEP_PSTMT_CHK_OK(tblNm, sqlErr, bind_int,  stmt_hdl, COL1_BIND_INDEX, (stHdrPtr->rID));
                    LMDB_SQL_STEP_PSTMT_CHK_DONE(tblNm, sqlErr, step, stmt_hdl);
                    LMDB_SQL_STEP_PSTMT_CHK_OK(tblNm, sqlErr,  reset, stmt_hdl);
                }
            }
...
typename LMDatabase<T...>::vecNm LMDatabase<T...>::getTxtColUniqList(lmdbTxtCol col, lmdbOrderByCol orderByCol, lmdbOrderByDir orderByDir) {
    ...
    const char *colNm = lmdbTxtColNm[col];
    ...
    try {
        ...
        if (lmdb_conn) {
            ...
            for(DRSize i = 0; i < Count; i++) {
                ...
                if (LMDB_SQL_ROW == sqlErr) {
                    ...
                    LMDB_SQL_STEP_COLFN_NOCHECK(tblNm, cUCPtr, column_text, stmt_hdl, COL0_SEL_INDEX);
        ...
        QLOGE("Caught exception: %s",e.what());
    } catch (...) {
        QLOGE("Error in std::string operations");
    }
    return node;
}
#ifdef LMDB_DEBUG_SQL_SHADOW_COL_LIST
