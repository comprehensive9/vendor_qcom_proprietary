...
private:
    ...
    int createGetUniqueDR(const std::string& appName, DbRowVect& dbRow, bool readonly = false);
    int createRow(LMDBDataRowKey& key);
    int makeRowUnique(const DbRowVect& dbRows, LMDBDataRowKey& key);

    AppState readState(const std::string& appName);
    int writeState(const std::string& appName, AppState target);

public:
    FeatureState(const std::string& featureName);
    ~FeatureState();
    void setState(const std::string& appName, AppState state) {
        writeState(appName, state);
    }
    AppState getState(const std::string& appName) {
        return readState(appName);
    }
    void deleteRow(const std::string& appName);
};
