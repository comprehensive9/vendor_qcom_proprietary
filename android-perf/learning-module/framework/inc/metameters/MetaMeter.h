...
protected:
    ...
    bool isValidMetameterState(const std::string& appName) {
...
public:
    ...
    /*
     * To support sync call, feature will directly invoke getSyncData()
     * since it doesnt have the meterList info with it. This internally
     * will trigger the getSyncData(std::vector<Meter*> meterList)
     * implemented by the MetaMeter instance for a given feature.
     */
    virtual long int getSyncData(std::vector<Meter*> meterList) {return 0;}
    int getSyncData();
};
#endif
