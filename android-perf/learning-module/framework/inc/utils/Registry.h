...
protected:
    ...
    static NameClassMap* getMap() {
        if(!classList) {
            classList = new(std::nothrow) NameClassMap;
        }
        return classList;
    }
...
template<typename TBase, typename TInfo, typename TDerived> TBase* createClassObj(TInfo info) { return new(std::nothrow) TDerived(info); }

template<typename TBase, typename TInfo, typename TDerived>
class Registry : RegistryBase <TBase, TInfo> {
public:
    Registry(const char* className) {
        if (className == NULL) {
            DEBUGV(LOG_TAG_REG,"Error: className null");
            return;
        }
        std::lock_guard<std::mutex> lock(RegistryBase<TBase, TInfo>::mMutex);
        DEBUGV(LOG_TAG_REG,"Registering Class %s", className);
        ...
        if(classMap) {
            ...
            DEBUGE(LOG_TAG_REG,"getMap() returned nullptr, Class %s not registered", className);
        }
    }
};
