...
FeatureInfo FeatureXML::getFeatureInfo() {
    ...
    if(mXMLRoot != NULL) {
        ...
        for(XMLElement* ignoredAppsElem = mXMLRoot->FirstChildElement("IgnoredApps"); ignoredAppsElem != NULL; ignoredAppsElem = ignoredAppsElem->NextSiblingElement("IgnoredApps")) {
            /* XML:
             *            <App>
             */
            ...
            /* Read the IgnoredAppslist for valid target*/
            if (valid_target) {
         ...
         *        <SpuriousHintApps>
         */
        /* Read Each spurious hint app from the xml file */
        XMLElement* spuriousAppsElem = mXMLRoot->FirstChildElement("SpuriousHintApps");
        if(spuriousAppsElem != NULL) {
            /* XML:
             *            <App>
             */
            for(XMLElement* currApp = spuriousAppsElem->FirstChildElement("App"); currApp != NULL; currApp = currApp->NextSiblingElement("App")) {
                std::string appName = getElementText(currApp);
                DEBUGV(LOG_TAG_FX, "appName = %s", appName.c_str());
                currFeatureInfo.spuriousAppsList.push_back(appName);
            }
        }
