...
FeatureState::FeatureState(const string& featureName) : mStateDb(nullptr) {
    mFeatureName = featureName;

    DEBUGV(LOG_TAG_FS, "createStateDb %s", mFeatureName.c_str());
    lmdbError dbError = LMDB_NO_ERROR;

    mStateDb = new(std::nothrow) AppStateDb(0, mFeatureName + "State.db", mFeatureName.c_str());
    if (mStateDb) {
 ...
 */
int FeatureState::createGetUniqueDR(const string& appName, DbRowVect& dbRow, bool readonly) {
    if(!mStateDb) {
...
AppState FeatureState::readState(const string& appName) {
    DbRowVect dbRow;
    if(!mStateDb) {
    ...
    createGetUniqueDR(appName, dbRow, true);
    if(dbRow.size() == 1) {
...
int FeatureState::writeState(const string& appName, AppState target) {
    DbRowVect dbRow;
    if(!mStateDb) {
    ...
    lock_guard<mutex> lock(mMutex);
    createGetUniqueDR(appName, dbRow);
    if(dbRow.size() == 1) {
...
void FeatureState::deleteRow(const std::string& appName){
    if(!mStateDb) {
        DEBUGE(LOG_TAG_FS,"%s:Fatal: StateDb is NULL", mFeatureName.c_str());
        return;
    }
