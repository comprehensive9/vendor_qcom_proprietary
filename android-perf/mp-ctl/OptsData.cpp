    ...
    for (idx = 0; idx < MAX_MINOR_RESOURCES; idx++) {
        ...
        case MEMLAT_START_INDEX + L3_MEMLAT_MAXFREQ_OPCODE:
            ...
            for (int j = 0; j < tc.getNumCluster(); j++) {
                node_i = 0;
                for (int i = 0; i < NODE_MAX && node_path[i] != '\0'; i++, node_i++) {
...
int OptsData::init_l3_path() {
    ...
    for (int j = 0; j < tc.getNumCluster(); j++) {
        ...
        if (ret == SUCCESS) {
            ...
            for (int i = 0; i < l3_avail_freqs_n[j]; i++) {
                l3_avail_freqs[j][i] = avail_freqs[i];
                QLOGI("L3 CPU%d, avail freq is %d",t.getFirstCoreIndex(j), l3_avail_freqs[j][i]);
            }
        }
    }
    return ret;
...
float OptsData::get_fps_hyst_time() {
    QLOGI("OptsData::get_fps_hyst_time %f", fps_hyst_time_sec);
    return fps_hyst_time_sec;
}

void OptsData::set_fps_hyst_time(float fht) {
    QLOGI("OptsData::set_fps_hyst_time new_value: %f", fht);
