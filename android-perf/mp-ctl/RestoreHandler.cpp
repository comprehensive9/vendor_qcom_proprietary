...
void ResetHandler::reset_to_default_values(OptsData &d) {
    ...
    if (ftell(defval) == 0) {
        ...
        for(int j = 0; j < tc.getNumCluster(); j++) {
            snprintf(tmp, NODE_MAX, "l3Cluster%dminf", j);
            write_to_file(defval, tmp, d.l3_minfreq_path[j]);
            snprintf(tmp, NODE_MAX, "l3Cluster%dmaxf", j);
        ...
        if (d.core_ctl_present > 0) {
            ...
            snprintf(tmp_s, NODE_MAX, "%d:%d", tc.getCoresInCluster(0), tc.getCoresInCluster(1));
