...
Request * internal_perf_lock_acq_verification(int &handle, int duration, int list[], int num_args,
                                              ...
                                              bool renew, enum client_t client = REGULAR_CLIENT);
...
void readSfHwcPid() {
    ...
    if (pFile != NULL) {
        fscanf(pFile,"%d", &d.sfPid);
        fclose(pFile);
        QLOGI("SurfaceFlinger pid %d", d.sfPid);
    }

    pFile = fopen(HWC_PID_FILE, "r");
    if (pFile != NULL) {
        fscanf(pFile,"%d", &d.hwcPid);
        fclose(pFile);
        QLOGI("Hardware Composer pid %d", d.hwcPid);
    }
    return;
}
...
float getCurrentFps() {
    ...
    ret = DisplayConfig::ClientInterface::Create("MPCTL", nullptr, &client);

    if (client != NULL) {
        client->GetActiveConfig(DisplayConfig::DisplayType::kPrimary, &dpy_index);
        if (dpy_index >= 0) {
            ret = client->GetDisplayAttributes(dpy_index, DisplayConfig::DisplayType::kPrimary, &dpy_attr);
            if (ret == 0 && dpy_attr.vsync_period) {
                QLOGI("VSYNC : %d", dpy_attr.vsync_period);
                fps = NSEC_TO_SEC/dpy_attr.vsync_period;
                QLOGI("Panel Attributes vsyncPeriod: %d Fps: %f", dpy_attr.vsync_period, fps);
            }
        }
...
int internal_perf_lock_acq(int handle, int duration, int list[], int num_args,
    ...
    req = internal_perf_lock_acq_verification(handle, duration, list, num_args,
                                              client_pid, client_tid, false, client);
    if (req != NULL ) {
...
Request* internal_perf_lock_acq_verification(int &handle, int duration, int list[], int num_args,
                                              pid_t client_pid, pid_t client_tid, bool renew, enum client_t client) {
    ...
    if (renew == true && request_list.CanSubmit() == false && isDisplayOnReq(req) == false) {
        QLOGE("Perflock Submit request limit reached.");
        request_list.Dump();
        delete req;
        req = NULL;
        ATRACE_END();
...
int internal_perf_lock_acq_apply(int handle, Request *req) {
    ...
    if (1 == request_list.GetNumActiveRequests())
    ...
    while (true) {
        ...
        do {
            ...
            if (rc < 0) {
                ...
                rc = pread(fds.fd, buf, NODE_MAX, 0);
                for (int i=0; i < tc.getNumCluster(); i++) {
                      if (!i) {
...
int perfmodule_submit_request(mpctl_msg_t *msg) {
    ...
    if ((NULL == evData) || (NULL == evData->mEvData)) {
        QLOGE("perf request submission limit reached.");
        return handle;
    }
    ...
    switch (cmd) {
        case MPCTL_CMD_PERFLOCKHINTACQ:
            ...
            if (preActionRetVal == PRE_ACTION_SF_HWC_VAL) {
                QLOGI("Special handling for SF HWC pid hint");
                ...
                handle = -1;
                QLOGI("Preaction FPS, dont update FPS");
                break;
            }
            ...
            if ((pMsg->pl_time == 0 || pMsg->pl_time > MAX_OFFLOAD_TIMEOUT) && pMsg->offloadFlag == true) {
                QLOGE("Cannot submit offload Hint=0x%X of %d duration, Max Allowed duration: %d", pMsg->hint_id, pMsg->pl_time, MAX_OFFLOAD_TIMEOUT);
                handle = -1;
                break;
            }
            if (pMsg->pl_time < 0) {
        ...
        case MPCTL_CMD_PERFLOCKACQ:
            ...
            req = internal_perf_lock_acq_verification(pMsg->pl_handle, pMsg->pl_time, pMsg->pl_args, size,
                    pMsg->client_pid, pMsg->client_tid, pMsg->renewFlag);
            handle = pMsg->pl_handle;
            if (req != NULL) {
                pMsg->userdata = (void *) req;
                if (isDisplayOnReq(req) == false) {
