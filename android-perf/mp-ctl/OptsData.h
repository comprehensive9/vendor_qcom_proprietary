...
#define FREAD_STR(fn, pstr, len, rc)    { FILE *fd;                                     \
                                          rc = -1;                                      \
                                          fd = fopen(fn, "r");                          \
                                          if (fd != NULL) {                             \
...
#define FWRITE_STR(fn, pstr, len, rc)   { FILE *fd;                                     \
                                          rc = -1;                                      \
                                          fd = fopen(fn, "w");                          \
                                          if (fd != NULL) {                             \
...
private:
    ...
    unsigned int  l3_avail_freqs[MAX_CLUSTER][MAX_FREQ_L3L];
    int  l3_avail_freqs_n[MAX_CLUSTER];
    ...
    short upmigrate_val[MAX_CLUSTER] = {0};
    short downmigrate_val[MAX_CLUSTER] = {0};
    ...
    int  sch_coloc_busy_hyst_cpu_busy_pct_sl[MAX_CLUSTER];

    char sch_coloc_busy_hyst_cpu_ns_s[MAX_CLUSTER][NODE_MAX];
    int sch_coloc_busy_hyst_cpu_ns_sl[MAX_CLUSTER];
