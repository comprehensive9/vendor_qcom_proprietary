...
bool Request::TranslateUpDownMigrate(int upmigrate_idx[], int downmigrate_idx[]) {
    int up_idx, down_idx;
    unsigned int up_value, down_value, override_value;
    ...
    for (int i = 0; i < mCluster; i++) {
        ...
        if (up_idx != -1 && down_idx != -1) {
            ...
            if (up_value <= 0 || up_value > numeric_limits<short>::max() ||
                ...
                QLOGE("Invalid value found for migrate opcode");
                return false;
            }
            ...
            QLOGE("Request rejected.Pass both up and down migrate values for %d cluster",i);
            return false;
        }
    }
    return true;
...
bool Request::TranslateGrpUpDownMigrate(int grp_upmigrate_idx, int grp_downmigrate_idx) {
    unsigned int up_value, down_value, override_value;
    ...
    if (up_value <= 0 || up_value > numeric_limits<short>::max() ||
            down_value <= 0 || down_value > numeric_limits<short>::max()) {
        QLOGE("Invalid value found for group migrate opcode");
        return false;
    }
    override_value = (down_value | (up_value << 16));
...
bool Request::Process(int nargs, int list[]) {
    ...
    int upmigrate_idx[mCluster];
    int downmigrate_idx[mCluster];
    memset(upmigrate_idx, -1, sizeof(upmigrate_idx));
    memset(downmigrate_idx, -1, sizeof(downmigrate_idx));
    int grp_upmigrate_idx = -1, grp_downmigrate_idx = -1;
    ...
    for (i = 0; i < mNumArgs; i++) {
        if (LOCK_NEW_VERSION == state) {
            ...
            if (Target::getCurTarget().isUpDownCombSupported() &&
                ...
                int minor = mResources[i].GetMinor();
                if (minor == SCHED_UPMIGRATE_OPCODE) {
                    upmigrate_idx[mResources[i].GetCluster()] = i;
                    migrate_flag = true;
                }
                if (minor == SCHED_DOWNMIGRATE_OPCODE) {
                    downmigrate_idx[mResources[i].GetCluster()] = i;
                    migrate_flag = true;
                }
            }
        ...
        if (mResources[i].GetQIndex() == ALWAYS_ALLOW_INDEX) {
            QLOGI("Recieved always allow request");
