...
Return<::vendor::qti::hardware::servicetracker::V1_2::Status> Servicetracker::registerCallback(
                                    const sp<::vendor::qti::hardware::servicetracker::V1_2::IActivityEventInfoCallback>& callback,
                                    ::vendor::qti::hardware::servicetracker::V1_2::NotifyType aNotify,
                                    int32_t userdata) {

    if (callback == nullptr) {
            LOG(WARNING) << "Invalid callback pointer ";
        return ::vendor::qti::hardware::servicetracker::V1_2::Status::ERROR_INVALID_ARGS;
    }
    std::lock_guard<std::mutex> _lock(callbacks_lock_);
    if (std::any_of( callbacks_.begin(), callbacks_.end(), [&](const CallbackData& c){

            return interfacesEqual(c.callback, callback);
        })) {
        return ::vendor::qti::hardware::servicetracker::V1_2::Status::CALLBACK_EXIST;
    } else {
          callbacks_.emplace_back(callback,aNotify,userdata);
          if(mDebug) {
              LOG(INFO)<< "New callback registered : " << &callback ;
          }
    ...
    return ::vendor::qti::hardware::servicetracker::V1_2::Status::SUCCESS;
}

Return<::vendor::qti::hardware::servicetracker::V1_2::Status> Servicetracker::unregisterCallback(
                                    const sp<::vendor::qti::hardware::servicetracker::V1_2::IActivityEventInfoCallback>& callback) {

    if(mDebug) {
        LOG(INFO) << "Unregister callback :"<< &callback;
    }
    if (callback == nullptr) {
            LOG(WARNING)<<"Invalid callback pointer ";
        return ::vendor::qti::hardware::servicetracker::V1_2::Status::ERROR_INVALID_ARGS;
    ...
    return unregisterCallbackInternal(callback) ? ::vendor::qti::hardware::servicetracker::V1_2::Status::SUCCESS :
                                                   ::vendor::qti::hardware::servicetracker::V1_2::Status::ERROR_NOT_AVAILABLE;

}

Return<void> Servicetracker::OnActivityStateChange(
                                    ::vendor::qti::hardware::servicetracker::V1_2::ActivityStates aState,
                                    const ::vendor::qti::hardware::servicetracker::V1_2::ActivityDetails& aDetails,
                                    const ::vendor::qti::hardware::servicetracker::V1_2::ActivityStats& aStats, bool early_notify) {
    if(mDebug) {
        LOG(INFO) << "Activity state is changed to : " << int(aState);

        LOG(INFO) << "Activity launchedFromPid : " << aDetails.launchedFromPid << "And launchedFromUid" << aDetails.launchedFromUid;
        LOG(INFO) << "Activity packageName : " << aDetails.packageName << "And processName" << aDetails.processName;
        LOG(INFO) << "Activity intent : " << aDetails.intent << "And className" << aDetails.className;
        LOG(INFO) << "Activity versionCode : " << aDetails.versioncode;

        LOG(INFO) << "Activity createTime : " << aStats.createTime << "And lastVisibleTime" << aStats.lastVisibleTime;
        LOG(INFO) << "Activity launchCount : " << aStats.launchCount << "And" << aStats.lastLaunchTime;
    }
    ...
    for (auto it = callbacks_.begin(); it != callbacks_.end();) {
        ...
        switch((*it).aNotify) {
            case ::vendor::qti::hardware::servicetracker::V1_2::NotifyType::EARLY :
                if(early_notify) {
                    ret = ((*it).callback)->notifyActivityEvent(aState, aDetails, aStats, early_notify, (*it).userdata);
                    move_next = true;
                } else {
                      ++it;
                }
                break;
            case ::vendor::qti::hardware::servicetracker::V1_2::NotifyType::LATE :
                if(!early_notify) {
                    ret = ((*it).callback)->notifyActivityEvent(aState, aDetails, aStats, early_notify, (*it).userdata);
                    move_next = true;
                } else {
                ...
                break;
            case ::vendor::qti::hardware::servicetracker::V1_2::NotifyType::BOTH :
                ret = ((*it).callback)->notifyActivityEvent(aState, aDetails, aStats, early_notify , (*it).userdata);
                move_next = true;
                break;
        }

        if (move_next && !ret.isOk()) {
              if(ret.isDeadObject())
                  it = callbacks_.erase(it);
              else {
                  if(mDebug) {
                      LOG(INFO) << "Transaction for callback" << &((*it).callback) << "failed with : " << ret.description();
                  }
                  ++it;
              }
        } else if (move_next) {
...
bool  Servicetracker::unregisterCallbackInternal(const sp<IBase>& callback) {
...
void Servicetracker::serviceDied(uint64_t /* cookie */, const wp<IBase>& who) {
...
IServicetracker* HIDL_FETCH_IServicetracker(const char* /* name */) {
    mDebug = property_get_bool("persist.vendor.debug.servicetracker", false);
