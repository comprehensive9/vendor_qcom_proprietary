...
static void qmi_cci_client_free(qmi_cci_client_type *clnt)
{
    ...
    * from the list */
    ...
    if(clnt->release_cb)
    {
      QMI_CCI_LOGD("QCCI qmi_cci_client_free: Calling release_cb\n");
    ...
    FREE(clnt);
    QMI_CCI_LOGD("QCCI qmi_cci_client_free: Done\n");
}
...
)
{
  ...
  clnt->priv.ref_count++;
  UNLOCK(&clnt->priv.ref_count_lock);
  QMI_CCI_LOGD("qmi_cci_get_ref: ref count increased %d\n", clnt->priv.ref_count);
...
static void qmi_cci_put_ref(qmi_cci_client_type *clnt)
{
  ...
  UNLOCK(&clnt->priv.ref_count_lock);
  QMI_CCI_LOGD("qmi_cci_put_ref: ref count decreased %d\n", ref);
...
static void release_cb_internal(void *cb_data)
{
  ...
  if(signal)
  {
    QMI_CCI_LOGD("QCCI release_cb_internal: Setting sig\n");
...
)
{
  ...
  QMI_CCI_LOGD("qmi clnt rel: Completed cleanup_client_lists\n");
  ...
  qmi_cci_put_ref(clnt);
  QMI_CCI_LOGD("qmi clnt rel: completed\n");
...
)
{
  ...
  if(rc == QMI_NO_ERR)
  {
    QMI_CCI_LOGD("QCCI qmi_client_release: Going for sig wait\n");
    QMI_CCI_OS_SIGNAL_WAIT(&signal, 0);
    QMI_CCI_LOGD("QCCI qmi_client_release: sig wait done\n");
  }
