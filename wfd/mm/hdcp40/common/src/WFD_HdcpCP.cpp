...
#ifdef KERNEL_5_4
    m_nQceFd = open("/dev/qce", O_RDWR | O_SYNC);
    if (m_nQceFd == -1)
    {
        MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_ERROR,
                     "Failed to open /dev/qce: %s", strerror(errno));
        m_eHdcpSessionState = HDCP_SESSION_STATE_ERROR;
        return;
    }
#endif
...
CWFD_HdcpCp::~CWFD_HdcpCp(void)
{
  /*De Init */
    ...
    if (m_nQceFd != -1)
    {
        ...
        {
            MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_ERROR,
                         "Failed to close /dev/qce: %s", strerror(errno));
        }
        m_nQceFd = -1;
    }
#endif
  ...
  unsigned long  ulPayloadLen )
{
       ...
       (NULL != aPESPvtHeader))
  {
...

    {
...
#ifdef KERNEL_5_4
      smmu_va_info_t inputVaInfo;
      smmu_va_info_t outputVaInfo;

      if(STREAM_AUDIO == ucStreamType)
      {
#ifdef HDCP_DEBUG
          MM_MSG_PRIO(MM_GENERAL, MM_PRIO_ERROR,
                      "|||||| *Encrypt: Using PA for Audio only||||||");
#endif
      ...
      else if(STREAM_VIDEO == ucStreamType)
      {
#ifdef HDCP_DEBUG
          MM_MSG_PRIO(MM_GENERAL, MM_PRIO_ERROR,
                      "||||||Encrypt: Using crypto smmu VA for Video||||||");
#endif

          // Map input buffer/plaintext secure buff to Crypto SMMU VA
          struct qcedev_map_buf_req map{};
          map.fd[0] = (int32_t)pucPESClrPayload;
          map.fd[1] = (int32_t)pucPESEncrytPayload;
          map.num_fds = 2;
          map.fd_size[0] = ulPayloadLen;
          map.fd_size[1] = ulPayloadLen;
          map.fd_offset[0] = 0;
          map.fd_offset[1] = 0;

          int err = ioctl(m_nQceFd, QCEDEV_IOCTL_MAP_BUF_REQ, &map);
          if (err == -1)
          {
              MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_ERROR,
                           "Failed to map Crypto SMMU for i/o buffer: %s", strerror(errno));
              QC_HDCP_CP_ErrorUndefined;
          }
          uint64_t inputBufferCryptoVA = map.buf_vaddr[0];
          uint64_t outputBufferCryptoVA = map.buf_vaddr[1];

#ifdef HDCP_DEBUG
          MM_MSG_PRIO2(MM_GENERAL, MM_PRIO_ERROR,
                       "DEBUG: pucPESEncrytPayload = %p encryptInputCryptoVA = %p",
                       (void*)pucPESClrPayload, (void*)inputBufferCryptoVA);
          MM_MSG_PRIO2(MM_GENERAL, MM_PRIO_ERROR,
                       "DEBUG: pucPESClrPayload= %p encryptOutputCryptoVA = %p",
                       (void*)pucPESEncrytPayload, (void*)outputBufferCryptoVA);
#endif // HDCP_DEBUG

          inputVaInfo = {inputBufferCryptoVA, 1}; //secure
          outputVaInfo = {outputBufferCryptoVA, 0}; //non-secure

          sPlainTextInfo.type = ARM_HDCP_BUFFER_TYPE_CRYPTO_VIRTUAL_POINTER;//ARM_HDCP_BUFFER_TYPE_SHARED_MEMORY_HANDLE;
          sPlainTextInfo.handle = (uint64_t)&inputVaInfo;//pucPESClrPayload;
          sPlainTextInfo.offset = 0;

          sCipherTextInfo.bBypassCipher = false;
          sCipherTextInfo.handle = (uint64_t)&outputVaInfo;//pucPESEncrytPayload;
          sCipherTextInfo.type = ARM_HDCP_BUFFER_TYPE_CRYPTO_VIRTUAL_POINTER;//ARM_HDCP_BUFFER_TYPE_SHARED_MEMORY_HANDLE;
      } //end for video stream
#else // KERNEL_5_4
...
#ifdef HDCP_DEBUG
          MM_MSG_PRIO2(MM_GENERAL, MM_PRIO_ERROR,
                       "DEBUG:Calling ARM_HDCP_Transmitter_Encrypt for stream type = %d: Params: ulPayloadLen = 0x%x",
                       ucStreamType, ulPayloadLen);
#endif

        ulStatus = ARM_HDCP_Transmitter_Encrypt(hStreamHandle,
                                             ulPayloadLen,
                                             &sPlainTextInfo, /*pInputBufferInfo */
                                             &sCipherTextInfo); /*pOutputBufferInfo*/

#ifdef HDCP_DEBUG
        MM_MSG_PRIO(MM_GENERAL, MM_PRIO_ERROR,
                    "DEBUG:Returned from ARM_HDCP_Transmitter_Encrypt");
#endif

#ifdef KERNEL_5_4
    if (STREAM_VIDEO == ucStreamType) {
        struct qcedev_unmap_buf_req unmap{};
        unmap.fd[0] = (int32_t)pucPESClrPayload;
        unmap.fd[1] = (int32_t)pucPESEncrytPayload;
        unmap.num_fds = 2;

        int err = ioctl(m_nQceFd, QCEDEV_IOCTL_UNMAP_BUF_REQ, &unmap);
        if (err) {
            MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_ERROR,
                "Failed to unmap Crypto VA: %s", strerror(errno));
        }
    }
#endif // KERNEL_5_4
