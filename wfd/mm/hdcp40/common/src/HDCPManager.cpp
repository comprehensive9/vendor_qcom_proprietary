                                   ...
                                   int nStreamId)
{
    ...
    if( pIn && pIn->handle && pOut && pOut->handle && nLen )
    {
        ...
        if (nStreamId == HDCP_VIDEO_TRACK_ID){
            // Map input buffer/plaintext secure buff to Crypto SMMU VA
            struct qcedev_map_buf_req map{};
            map.fd[0] = (int32_t)pIn->handle;
            map.fd[1] = (int32_t)pOut->handle;
            map.num_fds = 2;
            map.fd_size[0] = nLen;
            map.fd_size[1] = nLen;
            map.fd_offset[0] = 0;
            ...
            {
                MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_ERROR,
                             "Failed to map Crypto VA: %s", strerror(errno));
                HDCPMAN_CRITICAL_SECT_LEAVE;
                return HDCP_FAIL;
            }
            ...
            MM_MSG_PRIO2(MM_GENERAL, MM_PRIO_ERROR,
                         "DEBUG: decryptInputCryptoVA = %p, decryptOutputCryptoVA = %p",
                         (void*)inputBufferCryptoVA, (void*)outputBufferCryptoVA);
#endif

            inputVaInfo = {inputBufferCryptoVA, 0};///*non-secure
            outputVaInfo = {outputBufferCryptoVA, 1};///*secure

            sInBufInfo.bBypassCipher = nPesPvtDataLen ? false: true;
            sInBufInfo.handle = (uint64_t)&inputVaInfo;//pIn->handle;
            sInBufInfo.type = pIn->isFd?
                ARM_HDCP_BUFFER_TYPE_CRYPTO_VIRTUAL_POINTER : ARM_HDCP_BUFFER_TYPE_VIRTUAL_POINTER;///*ARM_HDCP_BUFFER_TYPE_SHARED_MEMORY_HANDLE

            sOutBufInfo.handle = (uint64_t)&outputVaInfo;//pOut->handle;
            sOutBufInfo.type = pOut->isFd?
                ARM_HDCP_BUFFER_TYPE_CRYPTO_VIRTUAL_POINTER : ARM_HDCP_BUFFER_TYPE_VIRTUAL_POINTER;///*ARM_HDCP_BUFFER_TYPE_SHARED_MEMORY_HANDLE
            sOutBufInfo.offset = nOutputOffset;

        } else if(nStreamId == HDCP_AUDIO_TRACK_ID) {
            sInBufInfo.bBypassCipher = nPesPvtDataLen ? false: true;
        ...
        {
            memcpy(sInBufInfo.PES_PrivateData, pesPrivateData, nPesPvtDataLen);
        }
    ...
    if (nStreamId == HDCP_VIDEO_TRACK_ID) {
        ...
        unmap.fd[0] = (int32_t)pIn->handle;
        unmap.fd[1] = (int32_t)pOut->handle;
        unmap.num_fds = 2;

        int err = ioctl(m_nQceFd, QCEDEV_IOCTL_UNMAP_BUF_REQ, &unmap);
        if (err) {
            MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_ERROR,
                "Failed to unmap i/o Crypto VA: %s", strerror(errno));
        }
    }
#endif // KERNEL_5_4
        ...
        MM_MSG_PRIO6(MM_GENERAL, MM_PRIO_ERROR,
               "HDCPManager:Decrypt Err Invalid Args,PES Private Data"
                     " = %p, pIn = %p:%p, pOut = %p:%p, pLen = %lu",
                                     pesPrivateData, pIn, (pIn? pIn->handle : 0),
                                     pOut, (pOut? pOut->handle : 0), nLen );
    }
    m_eState = HDCP_STATE_CONNECTED;
    HDCPMAN_CRITICAL_SECT_LEAVE
#else
