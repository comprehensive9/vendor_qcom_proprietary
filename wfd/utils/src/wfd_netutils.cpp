...
bool getIPSockPair(bool bPair, int *sock1, int * sock2,
                    int *nPort1, int *nPort2, bool tcp, NetType netType)
{
   char ipAddr[24] = {0};
   getLocalIpAddress((char*)ipAddr, sizeof(ipAddr), netType);

   char ifName[IFNAMSIZ];
   getP2pInterfaceName(ifName, sizeof(ifName), netType);

   MM_MSG_PRIO2(MM_GENERAL, MM_PRIO_MEDIUM,
                 "NetUtils:getIPSockPair ipaddr = %s ifname = %s", ipAddr, ifName);
   ...
   if(bPair)
   {
            ...
            portNum < RTP_PORT_END_NUM; portNum+=2)
       {
           ...
           if (netType == WIFI_P2P)
           {
               bindRet = setsockopt(RTPSocket, SOL_SOCKET, SO_BINDTODEVICE, ifName,
                                    strlen(ifName));
               MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_MEDIUM, "Bind to nw iface = %d", bindRet);
           }
           ...
           else
           {
               ...
               if (netType == WIFI_P2P)
               {
                   bindRet = setsockopt(RTPSocket, SOL_SOCKET, SO_BINDTODEVICE,
                                        ifName, strlen(ifName));
                   MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_MEDIUM,
                                 "Bind to nw iface _1  = %d", bindRet);
               }
