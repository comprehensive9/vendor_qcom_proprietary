        ...
        if (params[i].len > 0)
        {
          ...
          if(( qdp_created_profile &&
                (calling_function == QDP_PROFILE_LOOKUP_FOR_SET_INITIAL_ATTACH ||
                 calling_function == QDP_PROFILE_LOOKUP_FOR_SETUP_DATA_CALL)) ||
              (calling_function == QDP_PROFILE_LOOKUP_FOR_SET_DATA_PROFILE) ||
              ((qdp_allow_update_modem_profile || carrier_config_flag )))
          {
...
)
{
  ...
  /*return QDP_ATTACH_REQUIRED if any other params apart frm APN mismatches */
  if(!apnMatched && ret == QDP_ATTACH_REQUIRED)
  {
    ret = QDP_FAILURE;
  }
...
)
{
  ...
  do
  {
    ...
    if(req.profile.profile_type == WDS_PROFILE_TYPE_3GPP_V01)
    {
      ...
      int retVal = qdp_3gpp_match_lte_attach_profile_params(ril_apn,
                                                            ril_user,
                                                            ril_pass,
                                                            ril_auth,
                                                            ril_ipfamily,
...
)
{
  *profileId = (unsigned int)wdsProfile.profile_index;
  if(apnName != NULL && wdsSettings.apn_name_valid) {
    std_strlcpy(apnName, wdsSettings.apn_name, QMI_WDS_APN_NAME_MAX_V01+1);
  }
...
{
  return qdp_init3(default_port,isAPAssist,FALSE);
}
...
)
{
  ...
  do
  {
    ...
    if (QDP_SUCCESS != qdp_qmi_wds_client_init(&global_qmi_wds_hndl))
    {
      QDP_LOG_ERROR("%s(): WDS client init failed",
                    __func__);
      break;
    }
    ...
    qdp_allow_update_modem_profile = profile_update_value;
    QDP_LOG_DEBUG("qdp_allow_update_modem_profile is set to %d",qdp_allow_update_modem_profile);
    qdp_inited = TRUE;
...
boolean get_carrier_config_enabled()
{
  return carrier_config_flag;
}
