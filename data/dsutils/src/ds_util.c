...
/******************************************************************************
  ...
  ---------------------------------------------------------------------------
  Copyright (c) 2008, 2010-2011, 2013-2022 Qualcomm Technologies, Inc.
  All Rights Reserved.
...
static const ds_target_str_map_t ds_target_str_map[] =
{
    ...
    { DS_TARGET_LAHAINA_PT,       "lahaina_pt"      },
    { DS_TARGET_MONACO,          "monaco"         },
    { DS_TARGET_QTANG,          "qtang"         },
};
      ...
      else
      {
        ...
        switch((int)soc_id)
        {
         ...
         case DS_TARGET_SOC_ID_SMP_LAHAINA:
          ...
          case DS_TARGET_SOC_ID_VORDONISI:
          case DS_TARGET_SOC_ID_YUPIK_LTE:
          case DS_TARGET_SOC_ID_STRAIT:
          case DS_TARGET_SOC_ID_KATMAI:
          case DS_TARGET_SOC_ID_BLAIR_LITE:
...
)
{
  ...
  if (target == DS_TARGET_LAHAINA || target == DS_TARGET_LAHAINA_PT)
  {
    memset(&n, 0, sizeof(n));
    if (uname(&n))
    {
    ...
    else if (strstr(n.release, "qgki") != NULL)
    {
      global_imtype = DS_IMAGE_TYPE_PERF;
      return DS_IMAGE_TYPE_PERF;
    }
    /* GKI build does not contain either the qgki or the
     * debug/consolidate keywords
     * Ex: 5.10.81-android12-9-g2aa85638ab39 or
     * 5.4.147-android11-2-00015-g983a7e7c981b-ab8005766
     */
    global_imtype = DS_IMAGE_TYPE_GKI;
    return DS_IMAGE_TYPE_GKI;
  ...
  if (target == DS_TARGET_QTANG)
  {
    // For non-jgki, treat it as GKI to aviod DFS reg for remote socket.
    if (strstr(n.release, "jgki") == NULL)
    {
      global_imtype = DS_IMAGE_TYPE_GKI;
      return DS_IMAGE_TYPE_GKI;
    }
