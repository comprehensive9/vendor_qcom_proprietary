...
static boolean                is_adpl_read_ph_fd_fail = FALSE;
...
)
{
 ...
 /*-------------------------------------------------------------------------*/
  ...
  if (ret < 0)
  {
    if (is_adpl_read_ph_fd_fail != TRUE )
    {
        LOG_MSG_ERROR("Failed to read from the dev file. Errno %d",
                   errno, 0, 0);
    }
    is_adpl_read_ph_fd_fail = TRUE;
    return ADPL_FAILURE;
  }
...
int odl_process_ph_reset()
{
  ...
  else if(odl_state_config->is_odl_ipa_enabled == ODL_DOWN)
  {
    if (odl_state_config->odl_enabled)
    {
      ...
      if (ret_val != ADPL_SUCCESS)
      {
        LOG_MSG_ERROR("Failed to close ODL port through DPM", 0, 0, 0);
        return ADPL_FAILURE;
      }
