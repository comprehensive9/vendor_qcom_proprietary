...
/*===========================================================================
  Copyright (c) 2011-2014, 2020 Qualcomm Technologies, Inc. All Rights
...
)
{
   ...
   else
   {
      ...
      cfgdb_xmlfile = ds_strndup(xml_file, (size_t)CFGDB_MAX_STRING_SIZE);
...
)
{
   ...
   if (NULL != cfgdb_root)
   {
      ...
      char *dup_param_id = ds_strndup(param_id, (size_t)CFGDB_MAX_STRING_SIZE);
...
)
{
   ...
   /* If there is no existing entry for the given type, create a new one */
            ...
            NULL == (node->type = ds_strndup(type, (size_t)CFGDB_MAX_STRING_SIZE)))
   {
...
)
{
   ...
   if (0 == std_strnicmp(node->type, CFGDB_ELE_LIST, CFGDB_ELE_LIST_LEN))
   {
      ...
      else
      {
         ...
         if (NULL == (size->name = ds_strndup(CFGDB_SPL_ATTR_SIZE, CFGDB_SPL_ATTR_SIZE_LEN)) ||
             NULL == (size->type = ds_strndup(CFGDB_TYPE_INT, CFGDB_TYPE_INT_LEN))           ||
