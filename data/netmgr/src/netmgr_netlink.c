    ...
    case NETMGR_USER_CMD:
      ...
      *buflen = size;
      ...
      if(NETMGR_EVT_PARAM_MTU & event_info->param_mask) {
        ...
        size_t pos = sizeof(event_info->mtu);
        memcpy( rtad, &event_info->mtu, sizeof(event_info->mtu) );
        memcpy( rtad + pos, &event_info->mtu4, sizeof(event_info->mtu4) );
        memcpy( rtad + 2*pos, &event_info->mtu6, sizeof(event_info->mtu6) );
      }
      ...
      switch( rtah->rta_type ) {
        ...
        case RTA_OIF:
	     ...
	     netmgr_log_med("RTA_OIF idx:%d link_name:%s \n", route_info->attr_info.oif,
				if_indextoname(route_info->attr_info.oif, linkname));
	     break;
        case RTA_IIF:
	     route_info->attr_info.iif = *(int*)RTA_DATA(rtah);
	     route_info->attr_info.param_mask |= NETMGR_ROUTE_PARAM_IIF;
	     netmgr_log_med("RTA_IIF %d link_name:%s\n", route_info->attr_info.iif,
				if_indextoname(route_info->attr_info.iif, linkname));
	     break;
        case RTA_CACHEINFO:
        ...
        case RTA_METRICS:
	     ...
	     for (rtah = (struct rtattr*)RTA_DATA(rtah); RTA_OK(rtah, rta_nested_len); rtah = RTA_NEXT(rtah, rta_nested_len)) {
