...
/*---------------------------------------------------------------------------
   Constant representing the default rev rmnet MTU
 ---------------------------------------------------------------------------*/
#define NETMGR_KIF_REV_RMNET_DEFAULT_MTU 3000
...
netmgr_kif_get_mtu(int link)
{
  ...
  if (link < NETMGR_MAX_LINKS_PER_MODEM_EXT) {
    iface_mtu = netmgr_qmi_cfg.links[link].wds_info.iface_mtu;
    netmgr_log_high("netmgr_kif_get_mtu: mtu=[%u]", iface_mtu);
    return iface_mtu;
  }
  else {
...
netmgr_kif_ioctl_set_rmnet_mtu(int link, unsigned int mtu4, unsigned int mtu6)
{
  ...
  {
    netmgr_log_med("%s: RMNET_IOCTL_SET_OFFLOAD set mtu IPv4 %d IPv6 %d for link %s on phys_dev[%s]",
...
netmgr_kif_set_mtu(int link, boolean post_nl_ev)
{
  ...
  netmgr_log_low("%s() Got MTUs for Link #%d: IPV4 =  %u, IPV6 = %u\n",
                 __func__, link, mtu4, mtu6);
  ...
  else if ((NETMGR_MTU_INVALID == mtu4) && (NETMGR_MTU_INVALID == mtu6) )
  {
    ...
    /* When using TEE target to forward packets to the Modem, a copy
     * of the packet is sent with DF bit (don't fragment) bit set. If there
     * are multiple IKE AUTH fragments, these are reassembled by the
     * Kernel first before sending a copy to the Modem using TEE. If the
     * final packet happens to be bigger than the reverse rmnet MTU then the
     * packets gets dropped (as the DF bit is set). To avoid this problem
     * increase the reverse rmnet MTU to the maximum IKE packet expected,
     * which is 3000 bytes.*/
    if (NETMGR_KIF_IS_REV_RMNET_LINK(link))
    {
       mtu = NETMGR_KIF_REV_RMNET_DEFAULT_MTU;
       /*For reverse rmnet links, android framework wont set thus do it yourself*/
       if( NETMGR_FAILURE ==
  ...
  if (mtu > max_mtu && !NETMGR_KIF_IS_REV_RMNET_LINK(link))
  {
  ...
  netmgr_kif_ioctl_set_rmnet_mtu(link, netmgr_qmi_get_mtu(link, QMI_IP_FAMILY_PREF_IPV4),
                                 netmgr_qmi_get_mtu(link, QMI_IP_FAMILY_PREF_IPV6));
...
#endif /* FEATURE_DS_LINUX_ANDROID */
  netmgr_qmi_cfg.links[link].wds_info.iface_mtu = mtu;
...
#ifdef FEATURE_DS_LINUX_ANDROID
  if (TRUE == post_nl_ev && !NETMGR_KIF_IS_REV_RMNET_LINK(link))
  {
...
/*===========================================================================
  FUNCTION  netmgr_kif_is_ipv6_addr_globa_
===========================================================================*/
/*!
@brief
...
netmgr_kif_nl_route_recv_msg(netmgr_nl_msg_t  *nlmsg_info_ptr)
{
  ...
  if (netmgr_kif_is_default_route_event(route_info)) {
    ...
    if (!strncmp(linkname, NETMGR_KIF_INTF_RMNET_DATA,
      ...
      if (NETMGR_KIF_IS_REV_RMNET_LINK(NETMGR_KIF_IS_REV_RMNET_LINK(link))) {
...
)
{
  ...
  switch (cmd_data->cmd_id)
  {
    ...
    case NETMGR_USER_CMD_QUERY_LINK_IFACE_MTU_ONLY:
    {
      ...
      wds_info = &netmgr_qmi_cfg.links[cmd_data->link].wds_info;
...
embms_skip:
  ...
  if (NETMGR_KIF_IS_REV_RMNET_LINK(link))
  {
    ...
    */
    wds_info_ptr = netmgr_qmi_get_wds_info(link);
...
)
{
  ...
  sa->nl_sa_state_id.spi_tx = sa_config.spi_tx;
  sa->nl_sa_state_id.spi_rx = sa_config.spi_rx;
  switch (sa_config.encapsulation_mode)
  {
...
if (netmgr_main_cfg.netmgr_recovery_enabled)
{
  ...
  if (NETMGR_FAILURE == netmgr_xfrm_add_states(&sa_config))
  {
...
)
{
  ...
  if (netmgr_main_cfg.netmgr_recovery_enabled)
  {
    ...
    else if (AF_INET6 == ip_family)
    {
      ...
      inet_ntop(AF_INET6, &addr_info_ptr->ipv6.if_addr.addr.v6_addr8,
                link_info->link_addr, NETMGR_MAX_STR_LENGTH);
      snprintf(prefix, sizeof(prefix), "/%u", addr_info_ptr->ipv6.if_mask);
