...
/*===========================================================================
  FUNCTION  nm_mdmprxy_hal_prov_setForwarding()
===========================================================================*/
/*!
@brief
  set forwarding between the wlan and r_rmnet_data interfaces
...
/*=========================================================================*/
static int
nm_mdmprxy_hal_prov_setForwarding(int link, char *link_name)
{
  const char *wlanifname = NULL;
  int rc = NETMGR_SUCCESS;

#ifdef FEATURE_DATA_IWLAN
  /* To differentiate between add_route, called during wlan UP and
     ifcfg pre-handler, wlan_ifname info from the link is used
     this will be NULL in case of wlan UP and non-null when called
     from ifcfg pre-handler(rev_ip_ind) */

  wlanifname = netmgr_kif_get_wlan_ifname_by_link(link);

  if (NETMGR_MAIN_CFG_IS_IWLAN_CONCURRENCY_SET())
  {
    /* If this call is during wlan UP, we should not set forwarding */
    if (wlanifname &&
         !NM_MDMPRXY_GET_ROUTING_RULES_INSTALLED_FOR_WLAN(link))
    {
      netmgr_log_med("%s(): SetForwardingBetweenInterfaces link:%d name:%s",
                     __func__, link, link_name);
      rc = NetmgrNetdClientSetForwardingBetweenInterfaces(link_name,
                     wlanifname, TRUE);
      if (NETMGR_SUCCESS != rc) {
         netmgr_log_err("%s(): Failed to add forwarding rule from link [%s] to [%s]",
                     __func__, link_name, wlanifname);
          return rc;
      }
      NM_MDMPRXY_SET_ROUTING_RULES_INSTALLED_FOR_WLAN(link, TRUE);
    } else if (!wlanifname)
    {
      netmgr_log_med("%s(): WLAN UP event, delay SetForwardingBetweenInterfaces !!",
                     __func__);
    }
  }
  else
...
)
{
        ...
        || (AF_INET6 == af && TRUE == NM_MDMPRXY_GET_ROUTING_RULES_INSTALLED_V4(link)))
  {
    ...
    /* Forwarding between interfaces is not installed during wlan
       interface UP, it is delayed till netmgr receives rev_ip_connect_ind.
       Check and attempt installation of forwarding rules now */
    rc = nm_mdmprxy_hal_prov_setForwarding(link, link_name);

    if (NETMGR_SUCCESS != rc) {
         netmgr_log_err("%s(): Failed to add forwarding rule from link [%s] to wlan",
                     __func__, link_name);
          goto bail;
    }
    rc = NETMGR_SUCCESS;
...
)
{
  ...
  /* Reset wlan routing rule status when Interface is removed from OEM network */
  if (NETMGR_MAIN_CFG_IS_IWLAN_CONCURRENCY_SET()) {
    NM_MDMPRXY_SET_ROUTING_RULES_INSTALLED_FOR_WLAN(link, false);
    netmgr_log_med("%s(): Unset wlan routing rules [%s]", __func__, link_name);
  }
...
static int nm_mdmprxy_hal_prov_netd_link_is_hdlr(void* arg)
{
  ...
  {
    if (NM_MDMPRXY_GET_ROUTING_RULES_INSTALLED_FOR_WLAN(ifinfo->link))
       NM_MDMPRXY_SET_ROUTING_RULES_INSTALLED_FOR_WLAN(ifinfo->link, FALSE);
