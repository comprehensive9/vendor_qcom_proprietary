...
)
{
  ...
  char* iifname = NM_MDMPRXY_IIFACE;
  ...
  if (iiface_refcount == 0)
  {
    ...
    if ((iifname = netmgr_kif_get_wlan_ifname_by_link(link)) == NULL)
    {
      /* The wlan information for link should be available by now, cannot be invalid */
      netmgr_log_err("%s(): invalid wlan info for the link %d", __func__, link);
      return NETMGR_FAILURE;
    }
#endif /* FEATURE_DATA_IWLAN */

    iiface_handle = nm_mdmprxy_install_iiface_rules(iifname, nm_mdmprxy_route_oem_mark);
...
)
{
  ...
  if (NETMGR_KIF_CB_IF_TECH_TYPE_LBO == tech_type)
  {
    ...
    if ((iifname = netmgr_kif_get_wlan_ifname_by_link(link)) == NULL)
    {
      netmgr_log_err("%s(): invalid wlan info", __func__);
      return NETMGR_FAILURE;
    }
#endif /* FEATURE_DATA_IWLAN */
    if (NETMGR_SUCCESS
...
int nm_mdmprxy_route_prov_common_link_oos_hdlr(void* arg)
{
  ...
  {
#ifdef FEATURE_DATA_IWLAN
    if ((iifname = netmgr_kif_get_wlan_ifname_by_link(ifinfo->link)) == NULL)
    {
      netmgr_log_err("%s(): Invalid wlan info for local-breakout iface!",
                      __func__);
      goto bail;
    }
#endif /* FEATURE_DATA_IWLAN */
