...
*/
/*=========================================================================*/
int netmgr_main_set_ko_loading_properties(void)
{
  ds_target_t target = ds_get_target();
  int reti = NETMGR_SUCCESS;
  ...
  netmgr_log_high("%s(): unload ko modules before loading", __func__);
  ...
  if (netmgr_main_cfg.rmnet_shs)
  {
    ...
    {
        if (netmgr_main_cfg.rmnet_shs == 1) {
        ...
        {
            netmgr_log_high("%s(): bypassing set property for shsusrd load", __func__);
        }
    }
    else
    {
  ...
  if (netmgr_main_cfg.rmnet_offload && !netmgr_main_cfg.rmnet_perf)
  {
    if (NETMGR_SUCCESS != netmgr_rmnet_set_ko_loading_property(NETMGR_MAIN_OFFLOAD_KO_PROPERTY, NETMGR_MAIN_KO_PROPERTY_SET_ONE))
    {
      reti = NETMGR_FAILURE;
    }
  }
  ...
  if (netmgr_main_cfg.rmnet_perf && !netmgr_main_cfg.rmnet_offload)
  {
    do {
