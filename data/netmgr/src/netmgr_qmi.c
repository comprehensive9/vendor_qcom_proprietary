...
netmgr_qmi_reset_link_info (int link)
{
  ...
  netmgr_qmi_cfg.links[link].wds_info.iface_mtu = NETMGR_MTU_INVALID;
...
)
{
  ...
  switch (ind_data->status.connection_status)
  {
    ...
    case WDS_CONNECTION_STATUS_DISCONNECTED_V01:
    {
      ...
      netmgr_qmi_cfg.links[ link ].wds_info.addr_info.ipv4.if_addr.type = NETMGR_IPV4_ADDR;
      netmgr_qmi_cfg.links[ link ].wds_info.addr_info.ipv6.if_addr.type = NETMGR_IPV6_ADDR;
      ...
      WDS_TECHNOLOGY_NAME_WLAN_LOCAL_BRKOUT_V01 == rev_ip_ind->tech_name)
  {
    ...
    if ( rev_ip_ind->ipv4_addr_valid )
    {
      ...
      memcpy( (char*)&addr.s_addr,
                    (char*)&addr_info_ptr->ipv4.if_addr.addr.v4,
                    sizeof(addr.s_addr) );
...
)
{
   ...
   * indication will then teardown interface as normal.
   */
  GET_TEARDOWN_IFACE_VAL(clnt, netmgr_qmi_cfg.links[ link ].wds_info.addr_info.valid_mask, tiface);
...
unsigned int netmgr_qmi_get_link_mtu_by_family(int link, int ip_fam)
{
  ...
  if (link < NETMGR_MAX_LINKS_PER_MODEM_EXT) {
    if (ip_fam == NETMGR_IPV4_ADDR)
      mtu = netmgr_qmi_cfg.links[link].wds_info.mtu_ipv4;
    else if (ip_fam == NETMGR_IPV6_ADDR)
      mtu = netmgr_qmi_cfg.links[link].wds_info.mtu_ipv6;
    else {
...
/*===========================================================================
  FUNCTION  netmgr_qmi_get_wds_info
===========================================================================*/
/*!
@brief
  Retrives the wds info for Modem netwok interface
...
*/
/*=========================================================================*/
netmgr_wds_qmi_drv_info_t* netmgr_qmi_get_wds_info( int link )
{
  if( NETMGR_SUCCESS == netmgr_qmi_verify_link( link ) )
