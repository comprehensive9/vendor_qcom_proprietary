...
)
{
      ...
      !NETMGR_KIF_IS_REV_RMNET_LINK(link_num) )
  {
    ...
    {
      if (netmgr_qmi_cfg.links[link_num].qos_info.qos_flows[i])
      {
        free(netmgr_qmi_cfg.links[link_num].qos_info.qos_flows[i]);
        netmgr_qmi_cfg.links[link_num].qos_info.qos_flows[i] = NULL;
      }
    }
...
)
{
  ...
  if ( netmgr_main_get_qos_enabled())
  {
    netmgr_log_med("%s():  configure link %d dynamic %d ondemand %d", __func__, link_num,
                  ...
                  netmgr_qmi_cfg.link_array[link_num].on_demand );
    if ( netmgr_main_cfg.dynamic_link_creation && netmgr_qmi_cfg.link_array[link_num].on_demand )
    {
...
)
{
  ...
  if (netmgr_qmi_cfg.links[cmd_buf->data.link].wds_info.v4_pkt_conn)
  {
    if (NETMGR_SUCCESS !=netmgr_qmi_get_modem_link_info((uint8)cmd_buf->data.link, WDS_IP_FAMILY_IPV4_V01))
    {
      netmgr_log_err("qmi query modem link info failed!\n ");
    }
  }

  if (netmgr_qmi_cfg.links[cmd_buf->data.link].wds_info.v6_pkt_conn)
  {
    if (NETMGR_SUCCESS !=netmgr_qmi_get_modem_link_info((uint8)cmd_buf->data.link, WDS_IP_FAMILY_IPV6_V01))
    {
      netmgr_log_err("qmi query modem link info failed!\n ");
    }
  }
