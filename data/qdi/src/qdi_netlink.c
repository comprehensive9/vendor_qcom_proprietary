    ...
    while (RTA_OK(rta, rtattrlen))
    {
      switch (rta->rta_type)
      {
        case IFA_ADDRESS:
          ...
          {
            QDI_LOG_LOW("Matched becuase ifname %s size %zu dev_name %s size %zu", ifname, strlen(ifname), dev_name, strlen(dev_name));
            ...
            QDI_LOG_LOW("Not matched becuase ifname %s size %zu dev_name %s size %zu", ifname, strlen(ifname), dev_name, strlen(dev_name));
          }
          break;
