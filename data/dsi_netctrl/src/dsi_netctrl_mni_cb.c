...
)
{
     ...
     )
  {
    ...
    DSI_LOG_DEBUG("DEBUG : ref_count=%d for st=0x%p", st->priv.ref_count, st);

    if( !st->priv.ref_count )
    {
...
)
{
  ...
  do
  {
    ...
    if(DSI_ERROR == dsi_validate_data_srvc_hndl(st))
    {
        DSI_LOG_ERROR("%s","Handle already released, Ignoring processing");
        break;
    }
...
)
{
  ...
  {
    DSI_LOCK_MUTEX(&(st->priv.mutex));
    *ref_count = st->priv.ref_count;
    DSI_UNLOCK_MUTEX(&(st->priv.mutex));
    DSI_LOG_VERBOSE("dsi_is_valid_client_found_for_iface: valid client=0x%p found at "
                    "store index = %d ref_count = %d ip_version = %d st->priv.ref_count = %d",
                    st, i, *ref_count, st->priv.ip_version, st->priv.ref_count);
  }
  else
  {
  ...
  do
  {
    ...
    case WDS_CONNECTION_STATUS_DISCONNECTED_V01:
      {
        ...
        /* Since the pkt_srvc indication is a broadcast indication, we could get
           a spurious indication when we haven't initiated a call in a multi-PD
           scenario. We need to make sure to increment the ref_count only if the
           call was originated using this instance of the library */
    ...
    case WDS_CONNECTION_STATUS_CONNECTED_V01:
      if (ind_data->pkt_srvc_status.status.reconfiguration_required != TRUE)
      {
        ...
        {
          /* Increament Count only for first indication in case of Route Lookup
             success with valid interface and Another call alreay in process*/
          DSI_LOG_ERROR("client_ref_count = %d", client_ref_count);
          if(0 == client_ref_count)
          {
            DSI_LOG_ERROR("incrementing ref count client_ref_count = %d",
                          client_ref_count);
            ...
            {
              dsi_update_call_tech(i, ind_data->pkt_srvc_status.tech_name,
                                   ip_version);
            }
            DSI_UNLOCK_MUTEX(DSI_GET_MUTEX(i));
...
)
{
  ...
  do
  {
    ...
    if(!ind_data->sai_list.freq_sai_list_valid &&
       !ind_data->sai_list.freq_sai_list_ex_valid)
    {
