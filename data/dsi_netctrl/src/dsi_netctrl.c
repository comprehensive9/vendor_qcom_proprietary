...
)
{
  ...
  if (!DSI_IS_HNDL_VALID(st))
  {
    DSI_LOG_ERROR("%s", "dsi_get_call_end_reason : received invalid hndl");
    goto error;
  }
...
/*===========================================================================
  FUNCTION:  dsi_validate_data_srvc_hndl
===========================================================================*/
/** @ingroup dsi_validate_data_srvc_hndl

    Check if data_srvc_hndl is valid and present in store table.

    @note1hang
    If the data srvc hndl is not present in the dsi_store table,
    we will return ERROR to avoid referencing invalid hndl.
...
*/
/*=========================================================================*/
int dsi_validate_data_srvc_hndl
(
  dsi_hndl_t hndl
)
{
  ...
  if(!hndl)
  {
      DSI_LOG_ERROR("%s","Invalid Data Srvc hndl");
      goto error;
  }
  for(index = 0;index < DSI_MAX_DATA_CALLS; index++)
  {
      ...
      {
        DSI_LOG_DEBUG("Found Valid hndl in store table: hndl=[%p] at index=%d", hndl, index);
        ret = DSI_SUCCESS;
        break;
      }
  }
...
)
{
  ...
  do
  {
    if (!st_hndl)
    {
        DSI_LOG_ERROR("%s", "invalid store hndl" );
        break;
    }
    ...
    if (( st_hndl->self != st_hndl ))
    {
      DSI_LOG_ERROR("%s","rel_data_srvc_hndl: Invalid store handle ");
      break;
    }
