...
/*===========================================================================
  ...
  Copyright (c) 2010-2015, 2017, 2019-2021 Qualcomm Technologies, Inc.
  All Rights Reserved.
        ...
        for (i = 0; i < DSI_MAX_DATA_CALLS; i++)
        {
             ...
             )
          {
                ...
                (( ipf == DSI_IP_FAMILY_V6 ) &&( DSI_GET_WDS_V6_TXN(st) != DSI_INVALID_WDS_TXN )) ) )
            {
              ...
              DSI_LOG_ERROR("Invalid Txn:Dropping %d evt in %d state",dsi_event,st->priv.call_state);
            ...
            else if ((DSI_STATE_CALL_CONNECTED == st->priv.call_state) && (DSI_EVT_NET_IS_CONN == dsi_event))
            {
  ...
  do
  {
    ...
    switch(event)
    {
      ...
      case NETMGR_USER_CMD:
        DSI_LOG_ERROR("%s", "NETMGR_USER_CMD received, netmgr_state [%d]\n", info->cmd_data.netmgr_restart_state);
        ...
        if (netmgr_state == NETMGR_POST_RESTART_STATE_INIT_DONE)
        {
          ...
          DSI_LOG_DEBUG("%s","NETMGR_POST_RESTART_STATE_INIT_DONE received, dsi inited");
        }
        else
        if (netmgr_state == NETMGR_POST_RESTART_STATE_INIT_START)
        {
          dsi_inited = DSI_FALSE;
          DSI_LOG_DEBUG("%s","NETMGR_POST_RESTART_STATE_INIT_START received, dsi not inited");
        }
        break;
