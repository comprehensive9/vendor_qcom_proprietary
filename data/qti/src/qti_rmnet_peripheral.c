...
extern boolean                 is_fusion_supported;
extern boolean                 is_ep_trans_enabled;
extern boolean                 is_cmd_args_passed;
...
=========================================================================*/
int qti_rmnet_process_ph_reset()
{
  int                             ret_val;
  int                             ret;
  int                             line_state;
...
----------------------------------------------------------------------------*/
      if(line_state == 1)
      {
        ...
        {
          /*Get EP info to update is_ep_trans_enabled flag for JOLOKIA, LYKAN, FEERO targets */
          ...
          if (is_cmd_args_passed)
          {
            LOG_MSG_INFO1("is_ep_trans_enabled flag already enabled with command line arguments"
                           "skipping dynamic detection", 0, 0, 0);
          }
          else if (!is_ep_trans_enabled && rmnet_state_config->ep_info.ph_ep_info.ep_type
                                      == DATA_EP_TYPE_BAM_DMUX)
          {
            is_ep_trans_enabled = TRUE;
            if (qti_rmnet_qmi_init(rmnet_state_config) != QTI_SUCCESS)
            {
...
----------------------------------------------------------------------------*/
    if (rmnet_state_config->is_LA_LE_fusion_target)
    {
