...
/* 40 Mib */
#define FSYNC_THRESHOLD_LIMIT 41943040
...
int generate_ramdump(int index, ramdump_s *dump, char *tm)
{
	...
	while ((numBytes = read(pfd[index].fd, rd_buf, BUFFER_SIZE)) > 0) {
		...
		totalBytes += numBytes;
		/*
		* Do fsync approximately for every 40MiB, as 250MB size
		* of ramdumps are taking long time. This will ensure
		* fsync operation is performed as part of 120 seconds
		* modem ramdump collection timeout.
		*/
		fsyncThresholdBytes += numBytes;
		if (fsyncThresholdBytes >= FSYNC_THRESHOLD_LIMIT) {
			ALOGI("Copied %d bytes for ramdump file %s; do periodic fsync",
				fsyncThresholdBytes, f_name);
			fsync(fd);
			fsyncThresholdBytes = 0;
		}
	}
	ALOGI("Copied %d bytes for ramdump file %s; do final fsync", totalBytes, f_name);
	/* Make sure things are written */
	fsync(fd);
