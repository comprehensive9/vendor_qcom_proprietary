...
TEST(Usecase_OpenClose)
{
    ...
    u32Ret = u32VppUsecase_Init(pstCtx1, 0);
    DVP_ASSERT_NEQUAL(u32Ret, VPP_OK);
    u32Ret = u32VppUsecase_Init(pstCtx2, 0);
    DVP_ASSERT_NEQUAL(u32Ret, VPP_OK);
    u32Ret = u32VppUsecase_Init(pstCtx3, 0);
    DVP_ASSERT_NEQUAL(u32Ret, VPP_OK);
    u32Ret = u32VppUsecase_Init(pstCtx4, 0);
    DVP_ASSERT_NEQUAL(u32Ret, VPP_OK);
    u32Ret = u32VppUsecase_Init(pstCtx5, 0);
    DVP_ASSERT_NEQUAL(u32Ret, VPP_OK);
    u32Ret = u32VppUsecase_Init(pstCtx6, 0);
    DVP_ASSERT_NEQUAL(u32Ret, VPP_OK);
    u32Ret = u32VppUsecase_Init(pstCtx7, 0);
    DVP_ASSERT_NEQUAL(u32Ret, VPP_OK);
...
TEST(Usecase_Framerate)
{
    ...
    // 2 cases: disable/enable idle timeout
    for (uint32_t i = 0; i < 2; i++)
    {
        u32TimeoutMs = (i == 0) ? 0 : 1000;
        LOGI("Running test with idle timeout %s",
             (u32TimeoutMs == 0) ? "disabled" : "enabled");

        pstCtx1 = vpp_init(0,cb);
        pstCtx2 = vpp_init(0,cb);

        // Re-init UC session with idle timeout disabled/enabled
        vVppUsecase_Term(pstCtx1);
        vVppUsecase_Term(pstCtx2);
        u32Ret = u32VppUsecase_Init(pstCtx1, u32TimeoutMs);
        DVP_ASSERT_EQUAL(u32Ret, VPP_OK);
        u32Ret = u32VppUsecase_Init(pstCtx2, u32TimeoutMs);
        DVP_ASSERT_EQUAL(u32Ret, VPP_OK);
        ...
        DVP_ASSERT_EQUAL(u32Ret, VPP_OK);
        u32Ret = u32VppUsecase_ObtainProcessingService(pstCtx1);
        DVP_ASSERT_EQUAL(u32Ret, VPP_OK);

        // Configure 1920x1080 as second context
        u32Ret = u32VppUsecase_Configure(pstCtx2, pstUc1, stpCtrl1, pstVppParam2);
        DVP_ASSERT_EQUAL(u32Ret, VPP_OK);
        u32Ret = u32VppUsecase_ObtainProcessingService(pstCtx2);
        DVP_ASSERT_EQUAL(u32Ret, VPP_OK);
        ...
        // Framerate of 0 should error
        u32Ret = u32VppUsecase_UpdateFramerate(pstCtx1, pstUc1, 0, eVppBufType_Progressive);
        DVP_ASSERT_NEQUAL(u32Ret, VPP_OK);
...
TEST(Usecase_Credits)
{
    ...
    for (uint32_t i = 0; i < 2; i++)
    {
        ...
        vVppUsecase_Term(pstCtx1);
        vVppUsecase_Term(pstCtx2);
        vVppUsecase_Term(pstCtx3);
        u32Ret = u32VppUsecase_Init(pstCtx1, u32TimeoutMs);
        DVP_ASSERT_EQUAL(u32Ret, VPP_OK);
        u32Ret = u32VppUsecase_Init(pstCtx2, u32TimeoutMs);
        DVP_ASSERT_EQUAL(u32Ret, VPP_OK);
        u32Ret = u32VppUsecase_Init(pstCtx3, u32TimeoutMs);
        DVP_ASSERT_EQUAL(u32Ret, VPP_OK);
...
TEST(Usecase_ObtainProcessingService)
{
    /*
     * Test ObtainProcessingService() as well as expiry functionality.
     */
    t_StVppCtx *apstCtx[2] = {};
    t_StVppCtx stUnknownCtx = {};
    const uint32_t u32NumCtx = sizeof(apstCtx) / sizeof(apstCtx[0]);
    const struct vpp_callbacks cb = { NULL, NULL, NULL, NULL };
    uint32_t u32Ret;
    const uint32_t u32TimeoutMs = 1000;
    t_StVppHqvCtrl *pstCtrl1, *pstCtrl2;
    t_StVppParam *pstVppParam1, *pstVppParam2;
    t_StVppUsecase *pstUc1 = NULL;
    ...
    u32Ret = usecase_test_init(&pstCtrl1, &pstCtrl2, &pstVppParam1, &pstVppParam2);
    DVP_ASSERT_EQUAL(u32Ret, VPP_OK);
    usecase_test_set_resolution(pstVppParam1, 3840, 2160);

    for (uint32_t i = 0; i < u32NumCtx; i++)
    {
        apstCtx[i] = vpp_init(0, cb);
        DVP_ASSERT_PTR_NNULL(apstCtx[i]);
        // Term Usecase so we can init() later
        vVppUsecase_Term(apstCtx[i]);
    }

    for (uint32_t i = 0; i < u32NumCtx; i++)
    {
        // Init all Usecases with idle timeout enabled
        u32Ret = u32VppUsecase_Init(apstCtx[i], u32TimeoutMs);
        DVP_ASSERT_EQUAL(u32Ret, VPP_OK);

        // Configure should pass for both, irregardless of credit allocation
        // situation
        u32Ret = u32VppUsecase_Configure(apstCtx[i], pstUc1, pstCtrl1, pstVppParam1);
        DVP_ASSERT_EQUAL(u32Ret, VPP_OK);
    }

    // There's only enough credit allocation for 1 session, so only one of them
    // should pass
    u32Ret = u32VppUsecase_ObtainProcessingService(apstCtx[0]);
    DVP_ASSERT_EQUAL(u32Ret, VPP_OK);
    u32Ret = u32VppUsecase_ObtainProcessingService(apstCtx[1]);
    DVP_ASSERT_NEQUAL(u32Ret, VPP_OK);

    // Check that after expiry, the same session can re-take the credits
    usleep(u32TimeoutMs * 1000 + ONE_SECOND_IN_MICROSECONDS);
    u32Ret = u32VppUsecase_ObtainProcessingService(apstCtx[0]);
    DVP_ASSERT_EQUAL(u32Ret, VPP_OK);
    u32Ret = u32VppUsecase_ObtainProcessingService(apstCtx[1]);
    DVP_ASSERT_NEQUAL(u32Ret, VPP_OK);

    // Wait for first session to expire, to allow second session to get credits
    usleep(u32TimeoutMs * 1000 + ONE_SECOND_IN_MICROSECONDS);
    u32Ret = u32VppUsecase_ObtainProcessingService(apstCtx[1]);
    DVP_ASSERT_EQUAL(u32Ret, VPP_OK);
    u32Ret = u32VppUsecase_ObtainProcessingService(apstCtx[0]);
    ...
    // Verify ObtainProcessingService() fails for invalid contexts
    u32Ret = u32VppUsecase_ObtainProcessingService(NULL);
    DVP_ASSERT_NEQUAL(u32Ret, VPP_OK);
    u32Ret = u32VppUsecase_ObtainProcessingService(&stUnknownCtx);
    DVP_ASSERT_NEQUAL(u32Ret, VPP_OK);
...
TEST(Usecase_Refresh)
{
    ...
    // ctx0 obtains the credits while ctx1 cannot
    u32Ret = u32VppUsecase_ObtainProcessingService(apstCtx[0]);
    DVP_ASSERT_EQUAL(u32Ret, VPP_OK);
    u32Ret = u32VppUsecase_ObtainProcessingService(apstCtx[1]);
    DVP_ASSERT_NEQUAL(u32Ret, VPP_OK);
    ...
    {
        usleep(u32TimeoutMs * 1000 / 2);

        // Refresh to extend expiry
        u32Ret = u32VppUsecase_Refresh(apstCtx[0]);
        DVP_ASSERT_EQUAL(u32Ret, VPP_OK);

        // ctx1 should keep failing because ctx0 keeps refreshing
        u32Ret = u32VppUsecase_ObtainProcessingService(apstCtx[1]);
        DVP_ASSERT_NEQUAL(u32Ret, VPP_OK);
    }

    // Verify that Refresh() works on a session without credits
    u32Ret = u32VppUsecase_ObtainProcessingService(apstCtx[1]);
    DVP_ASSERT_NEQUAL(u32Ret, VPP_OK);
    u32Ret = u32VppUsecase_Refresh(apstCtx[1]);
    DVP_ASSERT_EQUAL(u32Ret, VPP_OK);

    // Verify that Refresh() works on a session with no idle timeout
    vVppUsecase_Term(apstCtx[1]);
    u32Ret = u32VppUsecase_Init(apstCtx[1], 0);
    DVP_ASSERT_EQUAL(u32Ret, VPP_OK);
    u32Ret = u32VppUsecase_Refresh(apstCtx[1]);
    DVP_ASSERT_EQUAL(u32Ret, VPP_OK);

    // Verify Refresh() errors with invalid ctx's
    u32Ret = u32VppUsecase_Refresh(NULL);
    DVP_ASSERT_NEQUAL(u32Ret, VPP_OK);
    u32Ret = u32VppUsecase_Refresh(&stUnknownCtx);
    DVP_ASSERT_NEQUAL(u32Ret, VPP_OK);
...
//    TEST_CASE(Usecase_Find),
    ...
    TEST_CASE(Usecase_ObtainProcessingService),
    TEST_CASE(Usecase_Refresh),
    TEST_CASE_NULL(),
};
