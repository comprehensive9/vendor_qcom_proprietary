...
//            {"r4oft", required_argument, 0, PARAM_RECONFIG4_OUT_FILE_TYPE},
            ...
            /* frame rate of the input buffers
             * This just sets the timestamps of each input buffer such that it
             * results in this frame rate */
            {"framerate", required_argument, 0, PARAM_FRAME_RATE},
            /* allow bypass
             * This essentially disables one of the conditions in the while-loop
             * of the func_test which causes a hang if too many buffers are
             * bypassed. This means if a certain func_test execution is known to
             * bypass, this bypass argument can be used so that the func_test
             * doesn't hang. */
            {"bypass", required_argument, 0, PARAM_EXPECT_BYPASS},
            {0, 0, 0, 0}
        };
...
*/
            ...
            case PARAM_FRAME_RATE:
                ...
                LOGI ("Frame rate %u", pCtx->u32FrameRate);
                break;
            case PARAM_EXPECT_BYPASS:
                pCtx->bExpectBypass = atoi(optarg);
                LOGI ("Expect bypass %u", pCtx->bExpectBypass);
                break;
            default:
