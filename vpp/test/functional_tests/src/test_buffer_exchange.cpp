...
TEST_SETUP(BufferExchangeTestInit)
{
    ...
    u32FrameRate = TS_FREQ_7HZ;
    if (pstTestCtx->u32FrameRate > 0)
    {
        u32FrameRate = pstTestCtx->u32FrameRate;
    }
    u32 = ts_mgr_init(&tsmgr, u32FrameRate, BUFFER_POOL_SZ);
    DVP_ASSERT_EQUAL(u32, VPP_OK);
...
TEST(BufferExchange1_Testcase)
{
    ...
    if((u32TestMask & FUNCTEST_ROTATE_BASIC) || (u32TestMask & FUNCTEST_ROTATE_ALL))
    {
        ...
        if (!bUsecaseBypassesInput)
        {
            bUsecaseBypassesInput = pstTestCtx->bExpectBypass;
        }
