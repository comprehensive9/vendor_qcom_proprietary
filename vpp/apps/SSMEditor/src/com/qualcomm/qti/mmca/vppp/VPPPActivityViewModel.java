...
public class VPPPActivityViewModel implements Parcelable {
    ...
    /**
     * Get a corresponding preset index from string.
     * @return List of the names of all Presets for the Config.
     */
    public int getPresetIndexById(String inputPresetId) {
        inputPresetId = inputPresetId.replaceAll("\\s+","");
        List<Preset> presetList = mConfig.getPresets();
        for (int i = 0; i < presetList.size(); i++) {
            String presetIdFromList = presetList.get(i).getId().replaceAll("\\s+","");
            if (inputPresetId.equalsIgnoreCase(presetIdFromList)) {
                return i;
            }
        }
        return -1;
