 ...
 ***********************************************************************/
// INDEX=current state, VALUE=next state when transitioning down
static const t_EVppConcState aeStateTransitionDownTable[VPPCONC_STATE_MAX] = {
    [VPPCONC_STATE_ACTIVE] = VPPCONC_STATE_CONFIGURED,
    [VPPCONC_STATE_CONFIGURED] = VPPCONC_STATE_INITED,
    [VPPCONC_STATE_INITED] = VPPCONC_STATE_NULL,
};
...
/*!
* @brief       Find the ConcSession associated with this ctx
*
* @description Find the ConcSession associated with this ctx within the static
*              concurrency list. This function needs to be locked prior to being
*              called.
*
* @in          pstCtx The context to look for
*
* @return      pointer to the ConcSession if exists, NULL otherwise
*/
static t_StVppConcSession *pstFindSession(t_StVppCtx *pstCtx)
{
    t_StVppConcSession *pstSession;

    VPP_RET_IF_NULL(pstCtx, NULL);

    pstSession = concurrency_main.pHead;
    while (pstSession != NULL)
    {
        if (pstSession->pstUcCtx == pstCtx)
        {
            LOGD("found session for ctx=%p", pstCtx);
            return pstSession;
        }
        pstSession = pstSession->pNext;
    }

    LOGE("failed to find session for ctx=%p", pstCtx);
    return NULL;
}

/*!
* @brief       Get session count info across concurrent sessions, per IP
*
* @description Get session count info across concurrent sessions, per IP. This
*              iterates through all concurrent sessions and determines the
*              number of sessions opened for each IP as well as the max number
*              of concurrent sessions each IP can support, taking into account
*              the max session value set by the usecase of each session.
*
* @in          o_pu32NumSessions        output, number of sessions per IP
*              u32NumSessions           Size of above array
*              o_pu32MaxConcSessions    output, max number of sessions per IP
*              u32MaxConcSessionsSize   Size of above array
*              pstCtx                   optional, the context to ignore
*
* @return      OK if success, ERR otherwise
*/
static uint32_t u32GetConcIpSessionInfo(uint32_t *o_pu32NumSessions,
                                        uint32_t u32NumSessionsSize,
                                        uint32_t *o_pu32MaxConcSessions,
                                        uint32_t u32MaxConcSessionsSize,
                                        t_StVppCtx *pstCtx)
{
    uint32_t i;
    t_StVppConcSession *pstSession;

    if (o_pu32NumSessions == NULL || u32NumSessionsSize < VPP_IP_BLOCK_MAX ||
        o_pu32MaxConcSessions == NULL || u32MaxConcSessionsSize < VPP_IP_BLOCK_MAX)
    {
        LOGE("invalid param, num_sessions=%p, num_size=%u, max_sessions=%p, max_size=%u",
             o_pu32NumSessions, u32NumSessionsSize, o_pu32MaxConcSessions,
             u32MaxConcSessionsSize);
        return VPP_ERR_PARAM;
    }

    memset(o_pu32NumSessions, 0, sizeof(uint32_t)*VPP_IP_BLOCK_MAX);
    memset(o_pu32MaxConcSessions, 0, sizeof(uint32_t)*VPP_IP_BLOCK_MAX);

    for (pstSession = concurrency_main.pHead; pstSession != NULL; pstSession = pstSession->pNext)
    {
        if (pstCtx != NULL && pstSession->pstUcCtx == pstCtx)
        {
            LOGD("ignoring ctx=%p during num sessions counting", pstCtx);
        }
        else
        {
            if (pstSession->eState == VPPCONC_STATE_CONFIGURED ||
                pstSession->eState == VPPCONC_STATE_ACTIVE)
            {
                // Session is in use by another context, count num sessions
                for (i = 0; i < VPP_IP_BLOCK_MAX; i++)
                {
                    if (pstSession->au32MaxConcSession[i] != 0)
                    {
                        // This IP block is being used by this session
                        o_pu32NumSessions[i]++;

                        if (o_pu32MaxConcSessions[i] == 0)
                        {
                            // No other checked sessions are using this IP
                            o_pu32MaxConcSessions[i] = pstSession->au32MaxConcSession[i];
                        }
                        else if (pstSession->au32MaxConcSession[i] < o_pu32MaxConcSessions[i])
                        {
...
/*!
* @brief       Accumulate credits used for each IP
*
* @description Accumulate credits used for each IP across all concurrent
*              sessions, ignoring one session's credit use if a context is given
*
* @in          o_pu32Credits    Array of uint32_t's to write into
*              u32CreditsSize   Size of above array
*              pstCtx           optional, the context to ignore
*
* @return      OK if success, ERR otherwise
*/
static uint32_t u32GetUsedIpCredits(uint32_t *o_pu32Credits,
                                    uint32_t u32CreditsSize,
                                    t_StVppCtx *pstCtx)
{
    uint32_t i;
    t_StVppConcSession *pstSession;

    if (o_pu32Credits == NULL || u32CreditsSize < VPP_IP_BLOCK_MAX)
    {
        LOGE("invalid param, credits=%p, size=%u", o_pu32Credits, u32CreditsSize);
        return VPP_ERR_PARAM;
    }

    memset(o_pu32Credits, 0, sizeof(uint32_t)*VPP_IP_BLOCK_MAX);

    for (pstSession = concurrency_main.pHead; pstSession != NULL; pstSession = pstSession->pNext)
    {
        if (pstCtx != NULL && pstSession->pstUcCtx == pstCtx)
        {
            LOGD("ignoring ctx=%p during IP credit accumulation", pstCtx);
        }
        else
        {
            if (pstSession->eState == VPPCONC_STATE_ACTIVE)
            {
                // Session is in use by another context, accumulate used credits
                for (i = 0; i < VPP_IP_BLOCK_MAX; i++)
                {
                    o_pu32Credits[i] += pstSession->au32Credit[i];
                }
            }
        }
    }
                                      ...
                                      t_StVppParam *pstParam, const t_StVppIpBlock *pstIp)
{
    ...
    while (pstCurrentSession != NULL)
    {
        if ((pstCurrentSession->pstUcCtx != NULL) && (pstCurrentSession != pstConfigureSession) &&
           (pstCurrentSession->eState == VPPCONC_STATE_CONFIGURED ||
            pstCurrentSession->eState == VPPCONC_STATE_ACTIVE))
        {
...
/*!
* @brief       Check if idle-timeout is enabled for this session
*
* @description Verify if idle-timeout is enabled for this session by checking if
*              the u32IdleTimeoutMs is set to a non-zero value
*
* @in          pstSession The session being checked
*
* @return      TRUE if idle timeout is enabled, FALSE otherwise
*/
static uint32_t bVppUsecase_IdleTimeoutEnabled(t_StVppConcSession *pstSession)
{
    return pstSession->u32IdleTimeoutMs > 0;
}

/*!
* @brief       Determines if a state transition is possible
*
* @description Determines if a state transition is possible following the state
*              transitions from the source state through the valid state
*              transitions to see if the destination state is reachable.
*
* @in          eSrcState    Source state
*              eDstState    Destination state
*
* @return      TRUE if transition from source to destination state is possible,
*              FALSE otherwise
*/
static uint32_t bVppUsecase_CanTransitionStateDown(t_EVppConcState eSrcState,
                                                   t_EVppConcState eDstState)
{
    t_EVppConcState ePreviousState;
    t_EVppConcState eCurrentState = eSrcState;

    if (eSrcState >= VPPCONC_STATE_MAX || eDstState >= VPPCONC_STATE_MAX)
    {
        LOGE("invalid states: src=%u, dst=%u", eSrcState, eDstState);
        return VPP_FALSE;
    }

    while (eCurrentState != eDstState)
    {
        if (eCurrentState == VPPCONC_STATE_NULL)
        {
            return VPP_FALSE;
        }

        ePreviousState = eCurrentState;
        eCurrentState = aeStateTransitionDownTable[eCurrentState];

        if (eCurrentState == ePreviousState)
        {
            // To prevent possible infinite-loops
            LOGE("current and previous state is same, state=%u", eCurrentState);
            return VPP_FALSE;
        }
    }
...
/*!
* @brief       Transition state down the hierarchy
*
* @description Transition the session's state down the state hierarchy, doing
*              tasks that are needed to move states.
*
* @in          pstSession   The session being transitioned
*              eTargetState State to move to
*
* @return      OK if successful, ERR otherwise
*/
static uint32_t u32VppUsecase_TransitionStateDown(t_StVppConcSession *pstSession,
                                                  t_EVppConcState eTargetState)
{
    uint32_t u32Ret = VPP_OK;

    VPP_RET_IF_NULL(pstSession, VPP_ERR_PARAM);

    if (!bVppUsecase_CanTransitionStateDown(pstSession->eState, eTargetState))
    {
        LOGE("cannot transition from state=%u to state=%u", pstSession->eState,
             eTargetState);
        return VPP_ERR_PARAM;
    }

    while (pstSession->eState != eTargetState)
    {
        LOGD("Transitioning down from state=%u", pstSession->eState);
        switch (pstSession->eState)
        {
            case VPPCONC_STATE_ACTIVE:
                if (bVppUsecase_IdleTimeoutEnabled(pstSession))
                {
                    u32Ret = u32VppTimer_Stop(pstSession->pvIdleTimer);
                    if (u32Ret != VPP_OK)
                    {
                        LOGE("failed to stop idle timer, ret=%u", u32Ret);
                        return u32Ret;
                    }
                }
                break;
            case VPPCONC_STATE_CONFIGURED:
                // do nothing
                break;
            case VPPCONC_STATE_INITED:
                // do nothing
                break;
            default:
                LOGE("can't transition down from this state=%u", pstSession->eState);
                return VPP_ERR;
        }

        pstSession->eState = aeStateTransitionDownTable[pstSession->eState];
    }
...
/*!
* @brief       Configure a use case, validating static params
*
* @description Configure, acquire resources, and update concurrency for a
*              usecase, using static params
*
...
*/
static uint32_t u32VppUsecase_ConfigureStatic(t_StVppCtx *pstCtx,
                                              t_StVppUsecase *pstUc,
                                              ...
                                              t_StVppParam *pstParam)
{
    ...
    VPP_RET_IF_NULL(pstCtrl, VPP_ERR_PARAM);
    ...
    // Get used session count and max concurrent sessions count
    u32Ret = u32GetConcIpSessionInfo(au32UsedIPSessions,
                                     sizeof(au32UsedIPSessions)/sizeof(au32UsedIPSessions[0]),
                                     au32ConcMaxSessions,
                                     sizeof(au32ConcMaxSessions)/sizeof(au32ConcMaxSessions[0]),
                                     pstCtx);
    if (u32Ret != VPP_OK)
    {
        LOGE("failed to get conc IP session info, ret=%u", u32Ret);
        return u32Ret;
    }
    ...
    for (i = 0; i < VPP_IP_BLOCK_MAX; i++)
    {
        if (au32UcMaxSessions[i] != 0)
        {
            ...
            // Check if current usecase's max sessions count causes concurrent
            // max sessions count to decrease
            if (au32ConcMaxSessions[i] == 0 || au32ConcMaxSessions[i] > au32UcMaxSessions[i])
            {
    ...
    pstConfigureSession = pstFindSession(pstCtx);
    ...
    if (pstConfigureSession->eState != VPPCONC_STATE_INITED)
    {
        LOGE("invalid state=%u", pstConfigureSession->eState);
    ...
    pstConfigureSession->eState = VPPCONC_STATE_CONFIGURED;

    return VPP_OK;
}

/*!
* @brief       Configure a use case, validating dynamic params
*
* @description Configure, acquire resources, and update concurrency for a
*              usecase, using dynamic params
*
* @in          pstCtx The context being configured
*
* @return      OK if use case configured, ERR otherwise
*/
static uint32_t u32VppUsecase_ConfigureDynamic(t_StVppCtx *pstCtx)
{
    uint32_t u32Ret;
    uint32_t u32Fps;
    ...
    pstConfigureSession = pstFindSession(pstCtx);
    if (!pstConfigureSession)
    {
        LOGE("cannot find session");
        return VPP_ERR_INVALID_CFG;
    }

    u32Ret = u32GetUsedIpCredits(au32UsedCredit,
                                 sizeof(au32UsedCredit)/sizeof(au32UsedCredit[0]),
                                 pstCtx);
    if (u32Ret != VPP_OK)
    {
        LOGE("failed to get used IP credits, ret=%u", u32Ret);
        return u32Ret;
    }

    if (pstConfigureSession->eState != VPPCONC_STATE_CONFIGURED)
    {
        LOGE("invalid state=%u", pstConfigureSession->eState);
        return VPP_ERR_STATE;
    }

    pstUc = pstConfigureSession->pstUc;

    // Check credits, concurrent sessions
    if (u32GetUcCreditsPerFrame(pstUc, pstConfigureSession->tResolution,
                                au32RequiredCredit))
    {
        return VPP_ERR;
    }
    ...
    for (i = 0; i < VPP_IP_BLOCK_MAX; i++)
    {
        ...
        {
            LOGD("NOT ENOUGH CREDITS FOR IP %u, max=%u, used=%u, required=%u", i,
    ...
    if (bVppUsecase_IdleTimeoutEnabled(pstConfigureSession))
    {
        u32Ret = u32VppTimer_Restart(pstConfigureSession->pvIdleTimer);
        if (u32Ret != VPP_OK)
        {
            LOGE("failed to start idle timer, ret=%u", u32Ret);
            return u32Ret;
        }
    }
    ...
    pstConfigureSession->eState = VPPCONC_STATE_ACTIVE;

    return VPP_OK;
}

static void vVppUsecase_OnIdleTimerExpiry(void *pvData)
{
    t_StVppConcSession *pstSession = (t_StVppConcSession *)pvData;

    pthread_mutex_lock(&concurrency_main.mutex);
    LOGI("timer expired, losing post-proc services, ctx=%p", pstSession);
    u32VppUsecase_TransitionStateDown(pstSession, VPPCONC_STATE_CONFIGURED);
...
*              u32IdleTimeoutMs Number of seconds for idle timeout, 0 means
*                               disable idle timeout
*
* @return      None
*/
uint32_t u32VppUsecase_Init(t_StVppCtx *pstCtx, uint32_t u32IdleTimeoutMs)
{
    uint32_t u32Ret;
    ...
    pstNewSession->u32IdleTimeoutMs = u32IdleTimeoutMs;
    if (bVppUsecase_IdleTimeoutEnabled(pstNewSession))
    {
        t_StVppTimerCallback stCallback;
        t_StVppTimerDuration stDuration;

        stCallback.pvData = pstNewSession;
        stCallback.pfOnTimerExpiry = vVppUsecase_OnIdleTimerExpiry;

        stDuration.u32Seconds = pstNewSession->u32IdleTimeoutMs / 1000;
        stDuration.u32NanoSeconds = (pstNewSession->u32IdleTimeoutMs % 1000) * 1000000;

        pstNewSession->pvIdleTimer = pvVppTimer_Init(stCallback, stDuration);
        if (!pstNewSession->pvIdleTimer)
        {
            LOGE("failed to init idle timer");
            u32Ret = VPP_ERR;
            goto err_idle_timer;
        }
    }

    pstNewSession->eState = VPPCONC_STATE_INITED;
...
void vVppUsecase_Term(t_StVppCtx *pstCtx)
{
    ...
    if (bVppUsecase_IdleTimeoutEnabled(pstCurrentSession))
    {
        vVppTimer_Term(pstCurrentSession->pvIdleTimer);
    }
...
{
    uint32_t u32Ret;
    uint32_t bIsInActiveState = VPP_FALSE;
    t_StVppConcSession *pstCurrentSession;

    pthread_mutex_lock(&concurrency_main.mutex);

    pstCurrentSession = pstFindSession(pstCtx);
    if (!pstCurrentSession)
    {
        LOGE("can't find UC session");
        pthread_mutex_unlock(&concurrency_main.mutex);
        return VPP_ERR_INVALID_CFG;
    }

    // Stay in ACTIVE state if Configure() is called in ACTIVE state, so that an
    // ACTIVE session doesn't lose its dynamic resources due to a Configure()
    bIsInActiveState = (pstCurrentSession->eState == VPPCONC_STATE_ACTIVE);

    u32Ret = u32VppUsecase_TransitionStateDown(pstCurrentSession,
                                               VPPCONC_STATE_INITED);
    if (u32Ret != VPP_OK)
    {
        LOGE("failed to transition to INITED state, ret=%u", u32Ret);
        pthread_mutex_unlock(&concurrency_main.mutex);
        return u32Ret;
    }

    u32Ret = u32VppUsecase_ConfigureStatic(pstCtx, pstUc, pstCtrl, pstParam);
    if (u32Ret != VPP_OK)
    {
        LOGE("failed ConfigureStatic(), ret=%u", u32Ret);
        pthread_mutex_unlock(&concurrency_main.mutex);
        return u32Ret;
    }

    if (!bVppUsecase_IdleTimeoutEnabled(pstCurrentSession) || bIsInActiveState)
    {
        u32Ret = u32VppUsecase_ConfigureDynamic(pstCtx);
        if (u32Ret != VPP_OK)
        {
            LOGE("failed ConfigureDynamic(), ret=%u", u32Ret);
            pthread_mutex_unlock(&concurrency_main.mutex);
            return u32Ret;
        }
    }
...
uint32_t u32VppUsecase_Release(t_StVppCtx *pstCtx)
{
    ...
    if (u32Ret != VPP_OK)
    {
        LOGE("failed to transition to inited, ret=%u", u32Ret);
        pthread_mutex_unlock(&concurrency_main.mutex);
        LOG_EXIT_RET(u32Ret);
    }
...
{
    uint32_t u32Ret;
    uint32_t u32MaxFpsIndex, u32MaxFps;
    t_StVppConcSession *pstConfigureSession;

    LOG_ENTER();

    VPP_RET_IF_NULL(pstCtx, VPP_ERR_PARAM);
    VPP_RET_IF_NULL(pstUc, VPP_ERR_PARAM);
    VPP_RET_IF_NULL(u32Fps, VPP_ERR_PARAM);
    ...
    u32Ret = u32VppUsecase_TransitionStateDown(pstConfigureSession,
                                               VPPCONC_STATE_CONFIGURED);
    if (u32Ret != VPP_OK)
    {
        LOGE("failed to transition to CONFIGURED, ret=%u", u32Ret);
        pthread_mutex_unlock(&concurrency_main.mutex);
        return u32Ret;
    }
    ...
    u32MaxFps = pstUc->au32MaxInFps[u32MaxFpsIndex][pstConfigureSession->tResolution];
    ...
    if (u32Ret != VPP_OK)
    {
        LOGE("ConfigureDynamic() failed, so can't process, ret=%u", u32Ret);
        pthread_mutex_unlock(&concurrency_main.mutex);
        return u32Ret;
    }
...
/*!
* @brief       Refresh a session's idle timer
*
* @description Refresh a session's idle timer, so that the expiry is pushed
*              further
*
* @in          pstCtx The context being refreshed
*
* @return      OK if refreshed successfuly, ERR otherwise
*/
uint32_t u32VppUsecase_Refresh(t_StVppCtx *pstCtx)
{
    uint32_t u32Ret;
    t_StVppConcSession *pstCurrentSession;
    ...
    if (bVppUsecase_IdleTimeoutEnabled(pstCurrentSession) &&
        pstCurrentSession->eState == VPPCONC_STATE_ACTIVE)
    {
        u32Ret = u32VppTimer_Restart(pstCurrentSession->pvIdleTimer);
        if (u32Ret != VPP_OK)
        {
            LOGE("failed to refresh idle timer, ret=%u", u32Ret);
            pthread_mutex_unlock(&concurrency_main.mutex);
            return u32Ret;
        }
    }
...
/*!
* @brief       Obtain dynamic resources to allow processing
*
* @description Obtain dynamic resources to allow this session to do processing
*
* @in          pstCtx The context to allow processing
*
* @return      OK if successful, ERR otherwise
*/
uint32_t u32VppUsecase_ObtainProcessingService(t_StVppCtx *pstCtx)
{
    uint32_t u32Ret;
    t_StVppConcSession *pstSession;
    LOG_ENTER();
    ...
    pstSession = pstFindSession(pstCtx);
    if (pstSession == NULL)
    {
        LOGE("failed to find session for ctx=%p", pstCtx);
        pthread_mutex_unlock(&concurrency_main.mutex);
    ...
    if (pstSession->eState == VPPCONC_STATE_ACTIVE)
    {
        LOGI("ctx=%p already active, skip obtaining dynamic resources", pstCtx);
        pthread_mutex_unlock(&concurrency_main.mutex);
        return VPP_OK;
    }
    ...
    if (pstSession->eState == VPPCONC_STATE_CONFIGURED)
    {
        ...
        if (u32Ret != VPP_OK)
        {
            LOGE("configure dynamic failed, ret=%u", u32Ret);
            pthread_mutex_unlock(&concurrency_main.mutex);
            return u32Ret;
        }
    }
    ...
    {
        LOGE("ctx=%p, invalid state=%u", pstCtx, pstSession->eState);
        pthread_mutex_unlock(&concurrency_main.mutex);
        return VPP_ERR_STATE;
    }
