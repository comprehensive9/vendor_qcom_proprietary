...
static void vVpp_InputBufferDone(void *pv, t_StVppBuf *pstBuf)
{
    ...
    u32 = u32VppUsecase_Refresh(pstCtx);
    LOGE_IF(u32 != VPP_OK, "failed to refresh for ctx=%p", pstCtx);
                                 ...
                                 t_StVppBuf *pstIntBuf)
{
        ...
        && (u32Count > 0))
    {
        ...
        if (u32Ret != VPP_OK)
        {
            LOGD("New frame rate %dFPS not supported.", u32NewFrameRate);
        }
        else
        {
...
static uint32_t u32Vpp_GetIdleTimeout()
{
    uint32_t bIdleTimeoutEnabled;
    uint32_t u32IdleTimeoutMs;

    bIdleTimeoutEnabled = bVppConfigStore_GetBool("vpp", "enable_idle_timeout", VPP_FALSE);
    u32IdleTimeoutMs = u32VppConfigStore_GetUnsignedInt("vpp", "idle_timeout_ms", 0);

    if (bIdleTimeoutEnabled && u32IdleTimeoutMs > 0)
    {
        LOGI("enabling idle timeout, enable=%u, timeout=%u",
             bIdleTimeoutEnabled, u32IdleTimeoutMs);
    }
    else
    {
        LOGI("disabling idle timeout, enable=%u, timeout=%u",
             bIdleTimeoutEnabled, u32IdleTimeoutMs);
        bIdleTimeoutEnabled = VPP_FALSE;
        u32IdleTimeoutMs = 0;
    }

    return u32IdleTimeoutMs;
}
...
void *vpp_init(uint32_t flags, struct vpp_callbacks cb)
{
    ...
    u32Ret = u32VppStats_Init(pstCtx);
    LOGE_IF(u32Ret != VPP_OK, "ERROR: unable to initialize stats block, ret=%u", u32Ret);

    u32VppStats_SetEnable(pstCtx, stDbgProp.u32StatsEn);

    LOGI("flags=0x%x", flags);

    u32Ret = u32VppUsecase_Init(pstCtx, u32Vpp_GetIdleTimeout());
    if (u32Ret != VPP_OK)
        goto ERR_UC_INIT;
...
uint32_t vpp_queue_buf(void *ctx, enum vpp_port port, struct vpp_buffer *buf)
{
    ...
    u32Ret = u32VppUsecase_Refresh(VPP_CTX_GET(ctx));
    LOGE_IF(u32Ret != VPP_OK, "failed to refresh for ctx=%p", ctx);
    ...
    if (port == VPP_PORT_INPUT)
    {
        ...
        u32Ret = u32VppUsecase_ObtainProcessingService(VPP_CTX_GET(ctx));
        if (u32Ret == VPP_OK)
        {
            if (pstCb->stFrameRate.u32Bypass != 0)
            {
                LOGD("obtained post-proc service");
                pstCb->stFrameRate.u32Bypass = 0;
            }
        }
        else
        {
            ...
            {
                LOGD("failed to get post-proc service, bypassing");
                pstCb->stFrameRate.u32Bypass = 1;
            }
