...
static const char HOSTAPD_CLI_GLOBAL_SOCK_PATH[] = "/data/vendor/wifi/wigig_hostapd/global";
...
int WigigSuppConnector::setup_interface()
{
    // nothing needed for supplicant, as it starts with wigig interface
    // already added
    return 0;
}
...
int WigigSuppConnector::connect_to_supplicant()
{
    ...
    while (true) {
        // ignore errors from setup_interface,
        // it will be detected as failure to connect later.
        setup_interface();
...
int WigigHostapdConnector::setup_interface()
{
    // hostapd service starts without any interfaces, in order
    // to support VTS tests which use lazy HAL to load hostapd
    // by requesting a HAL service. Since wigig may be down when
    // the VTS test is running, if wigig interface was added by default,
    // hostapd will exit immediately because it will not find the interface,
    // and the VTS test will fail.
    // here we add the wigig interface dynamically by sending a command
    // to the hostapd global control interface.
    ...
    if (!property_get(mConfig.supp_prop_name, supp_status, NULL)
            || strcmp(supp_status, "running")) {
        LOG(ERROR) << "Hostapd not running, cannot connect";
        return -1;
    }

    global_ctrl_conn = wpa_ctrl_open2(HOSTAPD_CLI_GLOBAL_SOCK_PATH,
        mConfig.supp_cli_sock_path);
    LOG(INFO) << "Trying to connect to global path: " << HOSTAPD_CLI_GLOBAL_SOCK_PATH;
    if (global_ctrl_conn == NULL) {
        LOG(ERROR) << "Unable to open global connection to hostapd on "
              << HOSTAPD_CLI_GLOBAL_SOCK_PATH << ": " << strerror(errno);
        return -1;
    }

    LOG(INFO) << "Opened global connection to hostapd on " << HOSTAPD_CLI_GLOBAL_SOCK_PATH;

    // send command to add wigig interface to hostapd
    snprintf(cmd, sizeof(cmd), "ADD %s config=%s", mConfig.primary_iface,
             HOSTAPD_CONF_FILE);
    ret = wpa_ctrl_request(global_ctrl_conn, cmd, strlen(cmd), reply, &reply_len, NULL);
    if (ret < 0 || strncmp(reply, "FAIL", 4) == 0) {
        LOG(ERROR) << "Unable to add interface: " << mConfig.primary_iface << "ret: " << ret;
        ret = -1;
        goto out;
    }
    // when adding interface from the CLI it is disabled by default,
    // send command to enable it
    reply_len = sizeof(reply);
    snprintf(cmd, sizeof(cmd), "IFNAME=%s ENABLE", mConfig.primary_iface);
    ret = wpa_ctrl_request(global_ctrl_conn, cmd, strlen(cmd), reply, &reply_len, NULL);
    if (ret < 0 || strncmp(reply, "FAIL", 4) == 0) {
        LOG(ERROR) << "Unable to enable interface: " << mConfig.primary_iface << "ret: " << ret;
        ret = -1;
        goto out;
    }

    // if there are any other failues, it will be detected later,
    // as failure to connect to interface control socket
    LOG(INFO) << "Added interface: " << mConfig.primary_iface;
    ret = 0;
