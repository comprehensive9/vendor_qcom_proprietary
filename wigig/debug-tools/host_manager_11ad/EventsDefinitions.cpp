...
InterfaceListChangedEvent::InterfaceListChangedEvent(std::vector<std::string> interfaceNames) :
    HostManagerEventBase(""),
    m_interfaceNames(std::move(interfaceNames))
{}

void InterfaceListChangedEvent::ToJsonInternal(Json::Value& root) const
{
    Json::Value interfaceNames(Json::arrayValue);
    for (const auto& ifname : m_interfaceNames)
    {
        interfaceNames.append(ifname);
    }
    root["InterfaceNames"] = interfaceNames;
}
