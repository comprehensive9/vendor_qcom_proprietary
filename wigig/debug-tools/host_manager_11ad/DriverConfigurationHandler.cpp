...
namespace
{
    const char* const WINDOWS_NETWORK_ADAPTER_NAME = "Qualcomm 11ad Wireless";
}
...
bool DriverConfigurationHandler::CheckArgumentValidity(const std::vector<std::string> arguments, unsigned numberOfExpectedArguments, std::string& statusMessage) const
{
    ...
    {
        stringstream ss;
        ss << "Expected " << numberOfExpectedArguments << " parameters for the driver configuration command but got " << arguments.size() << endl;
...
bool DriverConfigurationHandler::HandleTcpCommand(const string& operation, const string& parameterList, string& statusMessage) const
{
    ...
    {
        statusMessage = "No handler exists for command " + operation + ". Use help command for more information.";
        return false;
    }
