...
#include "DebugLogger.h"
    ...
    #include "Unix/UnixDPDKInterfaceMonitor.h"
    #include "UnixRtlDriver.h"
    #include "RtlSimEnumerator.h"
    #include "Unix/UnixDPDKDriver.h"
#endif // _WINDOWS
...
namespace
{
    ...
    std::unique_ptr<DriverAPI> CreateDriver(DeviceType deviceType, const std::string& interfaceName)
    {
        ...
        case DeviceType::DPDK:
            return std::unique_ptr<UnixDPDKDriver>(new UnixDPDKDriver(interfaceName));
#endif // WINDOWS
    ...
    m_deviceRemovedFunc(std::move(deviceRemovedFunc))
{
    ...
    m_unixDPDKInterfaceMonitor.reset(new UnixDPDKInterfaceMonitor());
#endif
}
...
void DriversFactory::StartNetworkInterfaceMonitors()
{
#ifdef __linux
        using namespace std::placeholders;

        m_unixDPDKInterfaceMonitor->StartMonitoring(std::bind(&DriversFactory::OnInterfacesAdded, this, _1, _2),
                                                    std::bind(&DriversFactory::OnInterfacesRemoved, this, _1, _2));

        m_unixNetworkInterfaceMonitor->StartMonitoring(
