...
bool UnixRtlDriver::ExecuteCommand(
    const IRtlSimCommand& cmd, vector<uint32_t>& refReply) const
{
...
bool UnixRtlDriver::ReadSimulatorReply(const IRtlSimCommand& cmd, vector<uint32_t>& reply) const
{
...
OperationStatus UnixRtlDriver::Read(uint32_t address, uint32_t& value)
{
...
OperationStatus UnixRtlDriver::ReadBlock(uint32_t address, uint32_t blockSize, vector<uint32_t>& values)
{
    uint32_t ahbAddress = address;
    bool conversionSuccess = AddressTranslator::ToAhbAddress(address, ahbAddress, TLN_M_B0);
    if (!conversionSuccess)
...
OperationStatus UnixRtlDriver::Write(uint32_t address, uint32_t value)
{
    uint32_t ahbAddress = address;
    bool conversionSuccess = AddressTranslator::ToAhbAddress(address, ahbAddress, TLN_M_B0);
    if (!conversionSuccess)
...
OperationStatus UnixRtlDriver::WriteBlock(uint32_t address, vector<uint32_t> values)
{
    uint32_t ahbAddress = address;
    bool conversionSuccess = AddressTranslator::ToAhbAddress(address, ahbAddress, TLN_M_B0);
    if (!conversionSuccess)
