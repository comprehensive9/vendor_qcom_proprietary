...
CommandHandlerResult CommandHandler::HandleCommand(CommandLineArgs& commandLineArgs)
{
    ...
    if (commandLineArgs.IsDumpHelpRequired())
    {
        // first argument is -dh
        DumpHelp();
        return CH_SUCCESS;
    }
...
void CommandHandler::PrintGeneralHelp()
{
    ...
    for (const auto& x : m_groupCommandBuilders)
    {
        availableGroupsHelp << "\t" << (x.first) << "\n";
    }
...
void CommandHandler::DumpHelp()
{
    cout << "#####################################    Shell 11ad Help    #####################################\n";
    PrintGeneralHelp();

    for (const auto& x : m_groupCommandBuilders)
    {
        cout << x.second->GetFullHelp(x.first);
    }
}
...
void CommandHandler::SetHandlers()
{
    ...
    m_groupCommandBuilders.insert(make_pair("trace", unique_ptr<GroupCommandBuilderBase>(new TraceCommandBuilder())));
    m_groupCommandBuilders.insert(make_pair("cmd", unique_ptr<GroupCommandBuilderBase>(new DriverCommandBuilder())));
}
