...
public:
    ...
    static std::string HelpTemplate(std::string helpText, std::string group="", std::string opcode="", bool isSynopsysPrintRequired = true);

    // gets help for the group and every avalaible opcode in it (used for dump_help)
    std::string GetFullHelp(const std::string& group);
...
private:
    ...
    std::string GetAvailableOpCodesHelp() const;
};
