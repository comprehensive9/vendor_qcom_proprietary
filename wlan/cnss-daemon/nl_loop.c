...
static void initialize_nl_fd (void)
{
...
static inline void initialize_nl_fd (void)
{
}
#endif
...
static int nl_loop_process_msg_svc(struct wlan_hdr *ani_hdr)
{
	...
	if (!ind_table) {
#ifndef CONFIG_CNSS_DP
		/**
		 * Returns success for dp indication handlers when
		 * CONFIG_CNSS_DP is disabled
		 */
		if (ani_hdr->type == WLAN_MSG_WLAN_TP_IND    ||
		    ani_hdr->type == WLAN_MSG_WLAN_TP_TX_IND ||
		    ani_hdr->type == WLAN_MSG_RPS_ENABLE_IND ||
		    ani_hdr->type == WLAN_MSG_CORE_MINFREQ)
		    return 0;
#endif
...
#define CNSS_DAEMON_MAX_POLLFD 6
#else
#define CNSS_DAEMON_MAX_POLLFD 5
#endif
...
int nl_loop_run(void)
{
	...
	struct msghdr msg;
	int ret = 0;
	struct pollfd pfd[CNSS_DAEMON_MAX_POLLFD];
	...
	memset(&pfd, 0, CNSS_DAEMON_MAX_POLLFD * sizeof(pfd[0]));
	...
	pfd[3].fd = cnss_nl80211_get_socket();
	pfd[3].events = POLLIN;

	pfd[4].fd = cnss_plat_ipc_qmi_get_fd();
	pfd[4].events = POLLIN;

#ifdef ENABLE_SUBNET_DETECTION
	pfd[5].fd = cnss_get_gw_update_sockfd();
	pfd[5].events = POLLIN;
#endif
	...
	while (!nl_loop.terminate) {
		...
		result = poll(pfd, CNSS_DAEMON_MAX_POLLFD, -1);
		if (result < 0) {
		...
		if (pfd[4].revents & (POLLIN | POLLHUP | POLLERR)) {
			wsvc_printf_dbg("pfd[4].fd: %d has data to process\n",
					pfd[4].fd);
			cnss_plat_ipc_qmi_msg_process();
		}

#ifdef ENABLE_SUBNET_DETECTION
		if (pfd[5].revents & (POLLIN | POLLHUP | POLLERR)) {
			wsvc_printf_dbg("pfd[5].fd: %d has data to process\n", pfd[5].fd);
			cnss_gw_update_loop(pfd[5].fd);
		}
#endif
	}
...
#else /* #ifndef DISABLE_CLD80211_LIB */

#ifdef ENABLE_SUBNET_DETECTION
#define CNSS_DAEMON_MAX_POLLFD 4
#else
#define CNSS_DAEMON_MAX_POLLFD 3
#endif
...
#ifdef ENABLE_SUBNET_DETECTION
	pfd[3].fd = cnss_get_gw_update_sockfd();
	pfd[3].events = POLLIN;
#endif
