...
int wsvc_debug_init(void)
{
	...
	if (kmsg_fd < 0)
		wsvc_printf_err("failed to open /dev/kmsg file");

	return 0;
}

void wsvc_debug_deinit(void)
{
	if (kmsg_fd < 0)
		return;
...
static void wsvc_kmsg_log(const char *fmt, va_list argp)
{
	char buf[512];
	char log_buf[1024];
	int ret;
	...
	if (ret < 0)
		strlcpy(buf, "wsvc_kmsg_log: vsnprintf() failed.", sizeof(buf));

	snprintf(log_buf, sizeof(log_buf),"%s: %s", LOG_TAG, buf);
	write(kmsg_fd, log_buf, strlen(log_buf));
}
...
void wsvc_printf(int level, const char *fmt, ...)
{
	...
	va_start(argp, fmt);
	 ...
	 if (level <= wsvc_kmsg_logging)
		 wsvc_kmsg_log(fmt, argp);
