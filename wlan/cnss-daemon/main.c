...
#define KMSG_LOGGING_DISABLED 0

int wsvc_debug_level = MSG_DEFAULT;
int wsvc_kmsg_logging = MSG_INFO_HIGH;
...
void configure_default_debug_level(void)
{
	wsvc_debug_level = property_get_int32("persist.vendor.cnss-daemon.debug_level",
					      wsvc_debug_level);
	wsvc_kmsg_logging = property_get_int32("persist.vendor.cnss-daemon.kmsg_logging",
						wsvc_kmsg_logging);
}
#else
void configure_default_debug_level(void)
{
	wsvc_kmsg_logging = KMSG_LOGGING_DISABLED;
}
#endif
...
int main(int argc, char *argv[])
{
	...
	if(cnss_vendor_cmd_event_init()){
		wsvc_printf_err("Failed to init hang issues ap");
		goto event_init_fail;
	}
	...
	if (cnss_user_socket_init(AF_UNIX)) {
		wsvc_printf_err("Failed to init user interface");
		goto user_socket_init_fail;
	}
	...
	if (cnss_plat_ipc_qmi_init() < 0) {
		wsvc_printf_err("Failed to setup CNSS Platform QMI IPC. Fallback to direct FW QMI");
		goto register_fail;
	}
...
user_socket_init_fail:
	cnss_user_socket_deinit();
