...
void initialize_cnsslog_timer(void)
{
		...
		while (!feof(fp)) {
			...
			if (*line == '#')
				...
				if ((1 == path_flag) && (1 == size_flag) &&
				    ...
				    enable_flush_log_flag && real_time_write_flag) {
					break;
				}
			}
			if (!path_flag)
...
static int handle_diag_cld(void) {
	...
	pfd[0].fd = nl_socket_get_fd(cldctx->sock);
	pfd[0].events = POLLIN;

	pfd[1].fd = cldctx->exit_sockets[1];
	pfd[1].events = POLLIN;
	...
	do {
		...
		if (pfd[0].revents & (POLLIN | POLLHUP | POLLERR)) {
			cld80211_handle_event(pfd[0].revents, cldctx->sock, cb);
		}
...
int32_t main(int32_t argc, char *argv[])
{
	...
	while (1) {
		c = getopt_long (argc, argv, "fsScpqxluwdDPFb:T:m:t:n:z:", long_options,
				 &option_index);
		if (c == -1) break;
