...
============================================================================*/
/* $Header: //components/rel/gnss8.mpss/10.1/gnss/slim/common/client/inc/slim_api.h#1 $ */
...
/*
  ...
  Updated:
    02/01/2016   -- #1 initial version
    01/25/2017   -- #2 added three new vehicle services
    03/15/2018   -- #3 added tilt service
    09/28/2019   -- #4 added new vehicle service: Steering_wheel
*/
#define SLIM_API_VERSION_ID  (4)
...
/**
@brief Request for current SLIM service status.

This function requires that client connection is already open
via the slim_Open function.

If client connection is already open, SLIM notifies which SLIM
services are available using client callback function (@see
eSLIM_MESSAGE_ID_SERVICE_STATUS_IND). Client can use @see
slim_ServiceAvailable function to check the availability of a
specific service when it has received the service status
indication.

@param[in]  p_Handle       Handle to identify client.

@return eSLIM_SUCCESS if request is sent successfully. Otherwise
SLIM error code.
*/
SLIM_API slimErrorEnumT slim_GetServiceStatus
(
  slimClientHandleT p_Handle
);
...
via registered callback function. @see slimNotifyCallbackFunctionT
 - Service: eSLIM_SERVICE_VEHICLE_ACCEL or
            ...
            eSLIM_SERVICE_VEHICLE_GEAR or
            eSLIM_SERVICE_VEHICLE_STEERING_WHEEL depending on the request type
 - Type:    eSLIM_MESSAGE_TYPE_RESPONSE
 - Error:   eSLIM_SUCCESS if request was successful, otherwise SLIM error code.
 - Payload: No payload.
...
/**
@brief Enabling or disabling of Tilt service.

Tilt event streaming can be started or stopped with this function.
Response to this configuration request is sent via registered client callback
function and possible error is indicated in the message header data
of the response. If service is started successfully, indications are
reported on-event via registered client callback function.

Final result to Tilt event streaming request is provided to client
via registered callback function. @see slimNotifyCallbackFunctionT
 - Service: eSLIM_SERVICE_TILT
 - Type:    eSLIM_MESSAGE_TYPE_RESPONSE
 - Error:   eSLIM_SUCCESS if request was successful, otherwise SLIM error code.
 - ID/Payload: eSLIM_MESSAGE_ID_TILT_ENABLE_RESP/no payload
               eSLIM_MESSAGE_ID_TILT_DISABLE_RESP/no payload

Tilt indications are provided to client via registered callback
function. @see slimNotifyCallbackFunctionT
 - Service: eSLIM_SERVICE_TILT
 - Type:    eSLIM_MESSAGE_TYPE_INDICATION
 - Error:   SLIM error code if error was detected in indication.
            eSLIM_SUCCESS otherwise.
 - ID/Payload:
    - eSLIM_MESSAGE_ID_TILT_IND/slimTiltDataStructT

@param[in]  p_Handle   Handle to identify client.
@param[in]  pz_Request Data for Tilt configuration.
@param[in]  u_TxnId    A transaction ID for the sensor service request. This id
                       is provided to client when a response to this request is
                       sent.
@return eSLIM_SUCCESS if request was sent successfully. Otherwise SLIM error code.
*/
SLIM_API slimErrorEnumT slim_EnableTilt
(
  slimClientHandleT p_Handle,
  const slimEnableTiltRequestStructT *pz_Request,
  uint8_t u_TxnId
);
