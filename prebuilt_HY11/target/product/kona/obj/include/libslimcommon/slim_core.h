...
============================================================================*/
/* $Header: //components/rel/gnss8.mpss/10.1/gnss/slim/common/core/inc/slim_core.h#1 $ */
...
#define SLIM_INIT_SERVICE_DISCOVERY_INTERVAL_MS (30000) /* 30s */
#define SLIM_INIT_SERVICE_DISCOVERY_COUNT       (3)
...
/* Macros for getting SLIM Test Mode config settings */
#define SLIM_TEST_MODE_ENABLE()                     (slim_GetTestConfig()->u_TestMode)
#define SLIM_TEST_IS_PROVIDER_DISABLE(e_Provider)   (SLIM_TEST_MODE_ENABLE() && \
                        SLIM_MASK_IS_SET(slim_GetTestConfig()->q_ProviderDisableMask, e_Provider))
...
@param[in] pz_Txn Pointer to the request transaction data.
*/
void slim_CoreGetServiceStatus
(
  const slim_GetServiceStatusTxnStructType *pz_Txn
);
...
/**
@brief Engine function for enabling Tilt reporting.

SLIM core implementation for enabling Tilt reporting.
Engine forwards the reporting request to currently active
sensor service provider.

@param[in] pz_Txn Pointer to the request transaction data.
*/
void slim_CoreEnableTiltRequest
(
  const slim_EnableTiltTxnStructType *pz_Txn
);
