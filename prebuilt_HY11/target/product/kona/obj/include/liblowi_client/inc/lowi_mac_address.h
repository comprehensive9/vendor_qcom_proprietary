 ...
 * These two macros are just mapping of Mac Address content from unsigned char[6] to string format
 =============================================================================================*/
#define LOWI_MACADDR_FMT_ALL "%02x:%02x:%02x:%02x:%02x:%02x"
#define LOWI_MACADDR_ALL(mac_addr) \
  mac_addr[0], mac_addr[1], mac_addr[2], mac_addr[3], mac_addr[4], mac_addr[5]

#define hexChar(i) \
  i < 10 ? i + '0' : (i-10) + 'a'
#define LOWI_MACADDR_FMT "%c%c:%c%c:%c%c:%c%c:%c%c:%c%c"
#define LOWI_MACADDR(mac_addr) \
  hexChar(mac_addr[0]/16), hexChar(mac_addr[0]%16), \
  hexChar(mac_addr[1]/16), hexChar(mac_addr[1]%16), \
  hexChar(mac_addr[2]/16), hexChar(mac_addr[2]%16), \
  (lowi_debug_level > 4) ? hexChar(mac_addr[3]/16):'*', \
  (lowi_debug_level > 4) ? hexChar(mac_addr[3]%16):'*', \
  (lowi_debug_level > 4) ? hexChar(mac_addr[4]/16):'*', \
  (lowi_debug_level > 4) ? hexChar(mac_addr[4]%16):'*', \
  hexChar(mac_addr[5]/16), hexChar(mac_addr[5]%16)
