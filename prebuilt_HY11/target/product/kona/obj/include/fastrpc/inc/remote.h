...
enum session_control_req_id {
	...
	FASTRPC_CONTROL_LOAD_VOTE	=	255, // Enable load time vote feature (added temporarily for a specific feature on a particular target)
};
...
// Used with FASTRPC_CONTROL_LOAD_VOTE req ID, for enabling and disabling load time vote on DSP
struct remote_rpc_control_load_vote {
   int domain;				// Remote subsystem domain ID, -1 to set params for all domains
   int enable; 				// enable load time vote in user pd
};
