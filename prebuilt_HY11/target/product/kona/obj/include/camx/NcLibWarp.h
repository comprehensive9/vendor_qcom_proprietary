...
/** Types of generic grids */
enum NcLibGridTypeBits
{
    NCLIB_GRID_SAMPLES_ON_EDGES = 1, /**< Set this bit if a grid has samples on the perimeter of the mapped area,
                                      *   leave it unset if the samples are in within the mapped area */
    NCLIB_GRID_CENTERED = 2          /**< Set this bit if the grid values are relative to the image center,
                                      *   leave it unset if the values are relative to the image top left corner */
};
...
/**
*  @brief   Resample a grid to an ICA grid
*
*  @param [in]      rawGridX           X part of the source grid
*  @param [in]      rawGridY           Y part of the source grid
*  @param [in]      gridSize           Number of samples in the source grid.
*  @param [in]      gridType           Combination of optional NcLibGridTypeBits.
*  @param [in]      gridArea           The size of the image on which the grid is defined, and the area of the grid relative to
*                                      the image. This size is assumed to be mapped to ICA virtual domain.
*  @param [in]      icaGridGeometry    Required output grid size, either NcLibIcaGrid35x27 or NcLibIcaGrid67x51
*  @param [out]     icaGrid            Ouput grid
*
*  @return NC_LIB_SUCCESS in case of success, error code in case of failure.
*/
int32_t NcLibResampleGridForIca(
    const double* rawGridX,
    const double* rawGridY,
    NcLibImageSize gridSize,
    uint32_t gridType,
    NcLibWindowRegion gridArea,
    NcLibIcaGridGeometry icaGridGeometry,
    NcLibIcaGrid* icaGrid);
