...
void SoftFlacDecoder::printStreamInfo(const FLACDec_ParserInfo& info, bool configChange) {
    ALOGD_IF(configChange, "configuring with new configuration");
    ALOGD("Channels %d, SampleRate %d, BitsPerSample%d", info.i32NumChannels,
            info.i32SampleRate, info.i32BitsPerSample);
    ALOGD("MinBlockSize %d, MinBlockSize %d", info.i32MinBlkSize, info.i32MaxBlkSize);
    ALOGD("MinFrameSize %d, MaxFrameSize %d", info.i32MinFrmSize, info.i32MaxFrmSize);
}

void SoftFlacDecoder::setStreamInfo(uint8_t* buffer, int offset) {
...
bool SoftFlacDecoder::parseMetaData(uint8_t* buffer, size_t len) {
    bool status = false;
    int offset = 0;
    uint32_t blockSize = 0;
    if (!memcmp(buffer, FLAC_SIGN_BYTES, FLAC_SIGN_SIZE)) {
        status = true;
    ...
    while (offset <= len-FLAC_SIGN_SIZE) {
        uint8_t blockType = buffer[offset] & 0x7F;
        blockSize = (uint32)((((uint32)buffer[offset+1]) << 16) |
                                 (((uint32)buffer[offset+2]) << 8) |
                                 (((uint32)buffer[offset+3])));
        offset += FLAC_SIGN_SIZE;
        if (MD_BLK_STREAMINFO == blockType && (offset+blockSize) <= len)
            setStreamInfo(buffer, offset);
        offset += blockSize;
    }

    if (parserInfoToPass != mCsdInfo) {
        mState = STATE_CC_CHANGED;
        parserInfoToPass.i32SampleRate = mCsdInfo.i32SampleRate;
        parserInfoToPass.i32NumChannels = mCsdInfo.i32NumChannels;
        parserInfoToPass.i32BitsPerSample = mCsdInfo.i32BitsPerSample;
        printStreamInfo(mCsdInfo, true);
    }
    return status;
}
...
SoftFlacDecoder::SoftFlacDecoder(
    ...
    memcpy(&mCsdInfo, &parserInfoToPass, sizeof(FLACDec_ParserInfo));
    OMX_ERRORTYPE status = preInit();
    ALOGW_IF(status != OMX_ErrorNone, "%s preInit Failed", __func__);
}
...
OMX_ERRORTYPE SoftFlacDecoder::preInit() {
    ALOGV("SoftFlacDecoder::preinit() Enter");
...
OMX_ERRORTYPE SoftFlacDecoder::init(FLACDec_ParserInfo* parserInfoToPass) {
    ALOGV("%s enter in state %d", __func__, mState);
    int result = 0;

    if (mDecoderEnd && mInitStatus){
        (*mDecoderEnd)(&flacDecState);
        ALOGD("flac decoder will reinit");
    }
...
int32_t SoftFlacDecoder::updatePointers(flacByteStreamBuffer *pByteBuffer, uint32_t readBytes, int32_t result) {
    ...
    if ((result == FLACDEC_SUCCESS) || (result == FLACDEC_EOF)) {
        ...
        ALOGE("SoftFlacDecoder: Erroneous decode!");
    ...
    switch ((int)index) {
        ...
        case (OMX_INDEXTYPE)QOMX_IndexParamAudioFlacDec:
        {
            ...
            if (0 == flacDecParam->nPortIndex) {
                ...
                printStreamInfo(parserInfoToPass);
            } else {
        ...
        if (mNotifyInputBufferEmpty) {
            ...
            if (inHeader->pBuffer) {
                ...
                if ((inHeader->nFlags & OMX_BUFFERFLAG_CODECCONFIG) != 0) {
                    ...
                    if (!parseMetaData(inBuffer, availLength)) {
                        ALOGE("%s parseMetaData error",__func__);
                        mSignalledError = true;
                        notify(OMX_EventError, OMX_ErrorStreamCorrupt, 0, NULL);
                        return;
                    }
                    bool changed = (mState == STATE_CC_CHANGED);
                    ALOGI("current state %d, changed %d", mState, changed);
                    if ((mState == STATE_PRE_INIT) || (mState == STATE_CC_CHANGED)) {
                        if (OMX_ErrorNone != init(&parserInfoToPass)) {
                            ALOGE("Failed to initialize Flac Decoder");
                            return;
                        }
                        if (changed)
