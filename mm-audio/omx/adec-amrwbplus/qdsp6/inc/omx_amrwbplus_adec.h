...
Copyright (c) 2010, 2012, 2014, 2017-2018, 2020, Qualcomm Technologies, Inc. All Rights Reserved.
All Rights Reserved.
...
public:
    ...
    inline void dec_num_il_in_buf()
    {
      pthread_mutex_lock(&in_buf_count_lock);
      if (nNumInputBuf == 0)
        DEBUG_PRINT_ERROR("%s: Input buffer count is zero", __FUNCTION__);
      else
        --nNumInputBuf;
      pthread_mutex_unlock(&in_buf_count_lock);
      return;
    }
    inline unsigned int get_num_il_out_buf()
    {
      unsigned int num;
    ...
    inline void inc_num_il_out_buf()
    {
      pthread_mutex_lock(&out_buf_count_lock);
      ++nNumOutputBuf;
      pthread_mutex_unlock(&out_buf_count_lock);
    ...
    inline void dec_num_il_out_buf()
    {
      pthread_mutex_lock(&out_buf_count_lock);
      if (nNumOutputBuf == 0)
        DEBUG_PRINT_ERROR("%s: Output buffer count is zero", __FUNCTION__);
      else
        --nNumOutputBuf;
      pthread_mutex_unlock(&out_buf_count_lock);
      return;
    }
    inline unsigned int get_num_il_in_buf()
    {
      unsigned int num;
      pthread_mutex_lock(&in_buf_count_lock);
      num = nNumInputBuf;
    ...
    inline unsigned int get_num_drv_in_buf()
    {
      unsigned int num;
      pthread_mutex_lock(&in_buf_count_lock);
      num = drv_inp_buf_cnt;
    ...
    inline void dec_num_drv_out_buf()
    {
      ...
      if (drv_out_buf_cnt == 0)
        DEBUG_PRINT_ERROR("%s: Output buffer counter is zero", __FUNCTION__);
      else
        --drv_out_buf_cnt;
      pthread_mutex_unlock(&out_buf_count_lock);
      return;
    }
    inline unsigned int get_num_drv_out_buf()
    {
      unsigned int num;
      pthread_mutex_lock(&out_buf_count_lock);
      num = drv_out_buf_cnt;
...
private:
    ...
    unsigned int                        drv_inp_buf_cnt;
    unsigned int                        drv_out_buf_cnt;
