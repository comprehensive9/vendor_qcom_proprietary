/*============================================================================
  ...
  Copyright (c) 2010-2014, 2016-2020, 2022 Qualcomm Technologies, Inc. All Rights Reserved.
...
bool COmxAmrwbplusDec::prepare_for_ebd(OMX_BUFFERHEADERTYPE *bufHdr)
{
    ...
    if (bufHdr->nFlags & OMX_BUFFERFLAG_EOS )
    {
        if (bufHdr->nFilledLen)
        {
            ...
            DEBUG_PRINT("%s::drv_inp_buf_cnt[%u]FilledLen[%d]", __FUNCTION__,
...
void COmxAmrwbplusDec::frame_done_cb(OMX_BUFFERHEADERTYPE *bufHdr, bool flg)
{
    ...
    if(m_cb.FillBufferDone)
    {
        ...
        m_fbd_cnt++;
        dec_num_il_out_buf();
        DEBUG_PRINT("%s[%p] Cnt[%d] NumOutBuf[%u]", __FUNCTION__, this,
...
bool COmxAmrwbplusDec::execute_omx_flush(OMX_IN OMX_U32 param1, bool cmd_cmpl)
{
    ...
    if (param1 == OMX_ALL)
    {
        ...
        while (1)
        {
            ...
            DEBUG_PRINT("Flush:nNumOutputBuf=%u nNumInputBuf=%u\n",\
            nNumOutputBuf,nNumInputBuf);
            DEBUG_PRINT("Flush:drv_out_buf_cnt=%u drv_inp_buf_cnt=%u\n",\
...
bool COmxAmrwbplusDec::execute_input_omx_flush()
{
    ...
    do
    {
        ...
        {
          pthread_mutex_unlock(&m_inputlock);
          if(get_num_drv_in_buf())
          {
            usleep(100); //Wait for driver input buffer to be free
            DEBUG_PRINT("\n i/p Flush: drv_inp_buf_cnt is %u state %x\n",
                        get_num_drv_in_buf(), m_state);
          }
        }
    }while(tot_qsize>0);
    DEBUG_PRINT("*************************\n");
...
bool COmxAmrwbplusDec::execute_output_omx_flush()
{
    ...
    do
    {
        ...
        {
            pthread_mutex_unlock(&m_outputlock);
            if(get_num_drv_out_buf())
            {
              usleep(100); //Wait for driver output buffer to be free
              DEBUG_PRINT("\n o/p Flush: drv_out_buf_cnt is %u state %x\n",
              get_num_drv_out_buf(), m_state);
            }
        }
    }while(tot_qsize>0);
    DEBUG_PRINT("*************************\n");
...
void* COmxAmrwbplusDec::alloc_ion_buffer(unsigned int bufsize)
{
    ...
    if(bufsize > UINT_MAX - 4095)
    {
        DEBUG_PRINT_ERROR("\n one or more parameter not valid\n");
        free(ion_data);
        ion_data = NULL;
        return NULL;
    }
                                                          ...
                                                          OMX_BUFFERHEADERTYPE* buffer)
{
    ...
    else
    {
        ...
        inc_num_drv_in_buf();
        DEBUG_PRINT("\n i/p Buffers with amr-wb+ drv = %u\n", get_num_drv_in_buf());

        rc = aio_write(&audio_aio_buf);
        if(rc == false)
        {
            dec_num_drv_in_buf();
            DEBUG_PRINT("ETBP: nNumInputBuf %u, drv_inp_buf_cnt %u\n",
                        get_num_il_in_buf(), get_num_drv_in_buf());
            buffer_done_cb((OMX_BUFFERHEADERTYPE *)bufHdr);
            if (!m_error_propogated) {
                m_error_propogated = true;
                m_cb.EventHandler(&m_cmp, m_app_data,
                                                          ...
                                                          OMX_BUFFERHEADERTYPE* buffer)
{
    ...
    if((search_output_bufhdr(buffer) == true))
    {
        ...
        if(ret == false)
        {
            ...
            dec_num_drv_out_buf();
            dec_num_il_out_buf();
            DEBUG_PRINT("FTBP: nNumOutputBuf %u, drv_out_buf_cnt %u\n",
              get_num_il_out_buf(), get_num_drv_out_buf());
            m_cb.FillBufferDone(&m_cmp, m_app_data, buffer);
            if (!m_error_propogated) {
                m_error_propogated = true;
                m_cb.EventHandler(&m_cmp, m_app_data,
                                                  ...
                                                  OMX_IN OMX_BUFFERHEADERTYPE* buffer)
{
            ...
            )
    {
        m_ftb_cnt++;
        inc_num_il_out_buf();
        DEBUG_PRINT("FTB: nNumOutputBuf %u ftb_cnt %d bufferhdr %p",
          get_num_il_out_buf(), m_ftb_cnt, buffer);
