...
Copyright (c) 2011-2012, 2014, 2017-2018, 2020 Qualcomm Technologies, Inc.
All Rights Reserved.
...
#define OMX_AAC_OUTPUT_BUFFER_SIZE  (8192 + META_OUT_OFFSET)
...
public:
    ...
    pthread_mutex_lock(&in_buf_count_lock);
    if (nNumInputBuf == 0)
        DEBUG_PRINT_ERROR("%s input buffer is zero,\
                            can't be negative", __FUNCTION__);
    else
        --nNumInputBuf;
    pthread_mutex_unlock(&in_buf_count_lock);
    return;
    ...
    if (nNumOutputBuf == 0)
        DEBUG_PRINT_ERROR("%s output buffer is zero,\
                            can't be negative", __FUNCTION__);
    else
        --nNumOutputBuf;
    ...
    if (drv_inp_buf_cnt == 0)
        DEBUG_PRINT_ERROR("%s input buffer counter is zero,\
                            can't be negative", __FUNCTION__);
    else
        --drv_inp_buf_cnt;
    pthread_mutex_unlock(&in_buf_count_lock);
   ...
   inline void inc_num_drv_out_buf()
    ...
    if (drv_out_buf_cnt == 0)
        DEBUG_PRINT_ERROR("%s output buffer counter is zero,\
                            can't be negative", __FUNCTION__);
    else
        --drv_out_buf_cnt;
    pthread_mutex_unlock(&out_buf_count_lock);
