...
RIL_Errno RadioImpl_1_2::convertLcResultToRilError(rildata::LinkCapCriteriaResult_t result) {
...
RadioError RadioImpl_1_2::convertMsgToRadioError(Message::Callback::Status status, RIL_Errno e) {
...
#ifdef HAS_QCRIL_DATA_1_5_RESPONSE_TYPES
rildata::AccessNetwork_t RadioImpl_1_2::convertHidlAccessNetworkToDataAccessNetwork(V1_2::AccessNetwork ran)
{
        ...
        V1_2::AccessNetwork ran)
{
    ...
    if (ril_ran == EUTRAN || ril_ran == NGRAN)
       type = MT_RSRP;
...
bool RadioImpl_1_2::checkThresholdAndHysteresis(int32_t hysteresisMs, int32_t hysteresisDb, const hidl_vec<int32_t> &thresholdsDbm) {
    ...
    {
        QCRIL_LOG_DEBUG("Invalid arguments passed, hysteresisDb: %d must be smaller than the smallest threshold delta: %d ",hysteresisDb,diff);
        ret = false;
    }
...
RadioError RadioImpl_1_2::sanityCheckSignalStrengthCriteriaParams(
    int32_t hysteresisMs,
    int32_t hysteresisDb,
    const hidl_vec<int32_t> &thresholdsDbm,
    V1_2::AccessNetwork ran)
...
int RadioImpl_1_2::sendNetworkScanResult(
    ...
    if (ind) {
        ...
        Return<void> retStatus = ind->networkScanResult_1_2(
                RadioIndicationType::UNSOLICITED_ACK_EXP, resultV1_2);
        checkReturnStatus(retStatus);
    } else {
...
void RadioImpl_1_2::setLinkCapacityReportingCriteriaResponse_1_2(RadioResponseInfo responseInfo) {
    QCRIL_LOG_FUNC_ENTRY();

    auto responseV1_2 = mRadioResponseV1_2;
    if (responseV1_2 != NULL) {
        QCRIL_LOG_INFO("setLinkCapacityReportingCriteriaResponse_1_2");
        Return<void> retStatus =
