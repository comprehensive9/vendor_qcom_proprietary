...
#define QCRIL_USSD_OVER_IMS_SUPPORTED "persist.vendor.radio.is_ussd_supported"
...
Return<void> RadioImpl::sendImsSms(int32_t serial, const ImsSmsMessage& message) {
  ...
  do {
    ...
    auto msg = std::make_shared<RilRequestImsSendSmsMessage>(getContext(serial), message.messageRef,
                                                             tech, message.retry);
    if (msg == nullptr) {
      sendResponse = true;
    ...
    GenericCallback<QcRilRequestMessageCallbackPayload> cb(
        ([this, serial]([[maybe_unused]] std::shared_ptr<Message> msg,
          ...
          if (status == Message::Callback::Status::SUCCESS && resp != nullptr) {
            ...
            errorCode = resp->errorCode;
            if (sendSmsResult) {
              result = makeSendSmsResult(responseInfo, serial, RESPONSE_SOLICITED, sendSmsResult,
                                         errorCode);
            } else {
              populateResponseInfo(responseInfo, serial, RESPONSE_SOLICITED, errorCode);
            }
          ...
          RADIO_HIDL_SEND_RESPONSE(mSlotId, mRadioResponse, sendImsSmsResponse, responseInfo,
                                   result);
        }));
    msg->setCallback(&cb);
    msg->dispatch();
...
bool RadioImpl::isUssdOverImsSupported() {
  boolean ussdOverImsSupported = FALSE;
  qmi_ril_get_property_value_from_boolean(QCRIL_USSD_OVER_IMS_SUPPORTED, &ussdOverImsSupported,
                                          FALSE);
