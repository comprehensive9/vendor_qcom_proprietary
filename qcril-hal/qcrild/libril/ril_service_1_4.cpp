...
void RadioImpl_1_4::sendRadioClientCapabilities() {
    bool isAPAssistCap = true;

    if (mRadioResponseV1_4 == nullptr || mRadioIndicationV1_4 == nullptr) {
        isAPAssistCap = false;
    } else {
        if (dcListChangedCallback == nullptr) {
#ifdef QMI_RIL_UTF
            string rilVersion = qcril_get_property_value("vendor.radio.utf.version", "1.4");
            QCRIL_LOG_DEBUG("RadioImpl_1_4 read property version %s", rilVersion.c_str());
            if (rilVersion == "1.4") {
#endif
            dcListChangedCallback = [](const vector<rildata::DataCallResult_t>& dcList) -> void {
                auto indMsg = std::make_shared<rildata::RadioDataCallListChangeIndMessage_1_4>(dcList);
                if (indMsg != nullptr) {
                    indMsg->broadcast();
                } else {
                    QCRIL_LOG_DEBUG("dcListChangedCallback_1_4 failed to allocate message");
                }
            };
#ifdef QMI_RIL_UTF
            } else {
...
rildata::AccessNetwork_t RadioImpl_1_4::convertHidlAccessNetworkToDataAccessNetwork(
    V1_4::AccessNetwork ran) {
    if (ran == V1_4::AccessNetwork::UNKNOWN) {
        return rildata::AccessNetwork_t::UNKNOWN;
    } else {
        return RadioImpl_1_2::convertHidlAccessNetworkToDataAccessNetwork(
...
V1_4::SetupDataCallResult RadioImpl_1_4::convertDcResultToHidlDcResult_1_4(const rildata::DataCallResult_t& result) {
    V1_4::SetupDataCallResult dcResult = {};
    dcResult.cause = (V1_4::DataCallFailCause) result.cause;
...
Return<void> RadioImpl_1_4::startKeepalive(int32_t serial, const ::android::hardware::radio::V1_1::KeepaliveRequest& keepalive) {
    ...
    if (msg) {
        GenericCallback<StartKeepAliveResp_t> cb([this, serial](std::shared_ptr<Message> msg,
            ...
            if(rsp == nullptr)
            {
              responseInfo = {.serial = serial, .error = RadioError::REQUEST_NOT_SUPPORTED};
            }
            RADIO_HIDL_SEND_RESPONSE(mSlotId, mRadioResponseV1_4, startKeepaliveResponse, responseInfo, ka_status);
        });
        msg->setCallback(&cb);
...
rildata::DataProfileInfo_t RadioImpl_1_4::convertHidlDataProfileInfoToRil(const V1_4::DataProfileInfo& profile) {
...
int RadioImpl_1_4::sendNetworkScanResult(std::shared_ptr<RilUnsolNetworkScanResultMessage> msg) {
    ...
    if (ind) {
        ...
        Return<void> retStatus = ind->networkScanResult_1_4(
                RadioIndicationType::UNSOLICITED_ACK_EXP, resultV1_4);
        checkReturnStatus(retStatus);
    } else {
...
void RadioImpl_1_4::setDataProfileResponse_1_4(RadioResponseInfo responseInfo) {
    QCRIL_LOG_FUNC_ENTRY();

    auto rsp = mRadioResponseV1_4;
    if (rsp) {
        QCRIL_LOG_INFO("setDataProfileResponse_1_4");
        Return<void> retStatus = rsp->setDataProfileResponse(responseInfo);
        checkReturnStatus(retStatus);
    } else {
        RLOGE("setDataProfileResponse_1_4: radioService[%d]->mRadioResponse == NULL", mSlotId);
    }

    QCRIL_LOG_FUNC_RETURN();
}

void RadioImpl_1_4::setInitialAttachApnResponse_1_4(RadioResponseInfo responseInfo) {
    QCRIL_LOG_FUNC_ENTRY();

    auto rsp = mRadioResponseV1_4;
    if (rsp) {
        QCRIL_LOG_INFO("setInitialAttachApnResponse_1_4");
        Return<void> retStatus = rsp->setInitialAttachApnResponse(responseInfo);
        checkReturnStatus(retStatus);
    } else {
        RLOGE("setInitialAttachApnResponse_1_4: radioService[%d]->mRadioResponse == NULL", mSlotId);
    }
...
void RadioImpl_1_4::getDataCallListResponse_1_4(std::shared_ptr<rildata::DataCallListResult_t> responseDataPtr, int32_t serial, Message::Callback::Status status) {
    QCRIL_LOG_FUNC_ENTRY();
    ...
    if (rsp) {
#endif
        QCRIL_LOG_INFO("getDataCallListResponse_1_4");
        qtimutex::QtiSharedMutex *radioServiceRwlockPtr = radio::getRadioServiceRwlock(mSlotId);
        ...
        if ((status == Message::Callback::Status::SUCCESS) &&
            ...
            for (rildata::DataCallResult_t entry: responseDataPtr->call) {
              ...
              dcResultList[i] = convertDcResultToHidlDcResult_1_4(entry);
              i++;
            }
        } else {
        ...
        Return<void> retStatus = rsp->getDataCallListResponse_1_4(responseInfo, dcResultList);
        checkReturnStatus(retStatus);
        radioServiceRwlockPtr->unlock_shared();
    } else {
        RLOGE("getDataCallListResponse_1_4: radioService[%d]->mRadioResponse == NULL", mSlotId);
        RadioImpl_1_3::getDataCallListResponse(responseDataPtr, serial, status);
    }
    QCRIL_LOG_FUNC_RETURN();
}
