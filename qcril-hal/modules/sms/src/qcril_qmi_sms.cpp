...
)
{
  ...
  do
  {
    if (msg->getIndex() <= 0)
    {
        QCRIL_LOG_ERROR("Invalid index for delete SMS on SIM.");
        ril_err = RIL_E_INVALID_ARGUMENTS;
        break;
    }
    ...
    request_msg.index = static_cast<uint32_t>(msg->getIndex()) - 1;
...
)
{
  ...
  do
  {
    if (msg->getIndex() <= 0)
    {
        QCRIL_LOG_ERROR("Invalid index for delete SMS on RUIM.");
        ril_err = RIL_E_INVALID_ARGUMENTS;
        break;
    }
    ...
    if (!ret.second)
    {
        QCRIL_LOG_ERROR("Failed to add an entry for delete SMS on RUIM.");
        ril_err = RIL_E_INTERNAL_ERR;
        break;
    }
    inserted_to_pendinglist = true;
