...
)
{
  ...
  if (params_ptr == nullptr)
  {
    QCRIL_LOG_ERROR("Invalid params_ptr ");
    return QMI_SERVICE_ERR;
  }
  ...
  if (user_cb)
  {
    ...
    QCRIL_LOG_ERROR("Sending Aysnc Msg");
    rc = qmi_client_send_msg_async( client_handle,
  ...
  {
    QCRIL_LOG_ERROR("Sending Sync Msg");
    rc = qmi_client_send_msg_sync( client_handle,
                                   QMI_UIM_PROFILE_CONFIRMATION_CODE_REQ_V01,
                                   &conf_code_req,
                                   sizeof(uim_profile_confirmation_code_req_msg_v01),
    ...
    QCRIL_LOG_ERROR("Return Value %d",rc);
    if(!rc)
    {
...
)
{
  ...
  return rc;
} /* qcril_qmi_uim_set_hlos_prov_preference */
