...
#define QCRIL_UIM_HLOS_PROV_PREF_AUTO     "persist.vendor.radio.hlos_prov_pref_auto"

const uint8_t HLOS_PROV_PREF_MANUAL = 0;

bool qcril_uim_log_adb_on = false;

/*For Android-R Iradio.1.5 based targets, default provisioning shall be Manual ( 0 )*/
bool qcril_uim_hlos_prov_pref_auto = false;
bool is_iradio_hal_1_5_or_up = false;
bool is_hlos_prov_request_sent = false;
...
UimModule::UimModule():uim_req_list("UimModule")
{
  ...
  mMessageHandler = {
       ...
       HANDLER(ClientDisconnectedMessage, UimModule::handleClientDisconnectedMsg),
       HANDLER(RilManualProvisioningMessage, UimModule::handleRilManualProvisioningRequestMsg),
                    };
} /* UimModule::UimModule */
...
void UimModule::UimProcessRilInit(std::shared_ptr<Message> msg_ptr)
{
  ...
  /* Read the Hlos provisioning preference ADB property */

  auto  hlosPrefPropPtr  = std::make_shared<UimGetRilPropertySyncMsg>
                            (QCRIL_UIM_HLOS_PROV_PREF_AUTO);

  std::shared_ptr<std::string>  hlosPrefRespPtr = NULL;

  if (hlosPrefPropPtr != NULL &&
      (hlosPrefPropPtr->dispatchSync(hlosPrefRespPtr) == Message::Callback::Status::SUCCESS)&&
      hlosPrefRespPtr != NULL &&
      strcmp(hlosPrefRespPtr->c_str(), "1") == 0)
  {
    qcril_uim_hlos_prov_pref_auto = true;
    QCRIL_LOG_INFO("hlos_prov_pref_auto set to true (Auto) ");
  }
  else
  {
    qcril_uim_hlos_prov_pref_auto = false;
  }

  QCRIL_LOG_INFO("persist.vendor.radio.hlos_prov_pref_auto is %d", qcril_uim_hlos_prov_pref_auto);
  ...
  if (ind_ptr != NULL)
  {
    switch(ind_ptr->get_ind_id())
    {
      case QMI_UIM_SRVC_UP_IND_MSG:
        ...
        QCRIL_LOG_INFO("Recvd QMI_UIM_SRVC_UP_IND_MSG hlos_prov_pref_auto - %d  IRadio 1.5 Hal: %d",
                        qcril_uim_hlos_prov_pref_auto, is_iradio_hal_1_5_or_up);

        if( (!qcril_uim_hlos_prov_pref_auto) && is_iradio_hal_1_5_or_up )
        {
          is_hlos_prov_request_sent = true;

          QCRIL_LOG_INFO("Calling qcril_uim_set_hlos_provisioning_preference - Manual");
          /* send Hlos provisioning preference QMI message to Modem only if manual */
          /* Hlos prov pref ADB property & NV4398 value are kept similar */
          qcril_uim_set_hlos_provisioning_preference(HLOS_PROV_PREF_MANUAL);
        }
        if (mRestart == true)
        {
      ...
      case QMI_UIM_SRVC_DOWN_IND_MSG:
        mRestart = true;
        mReady = false;
        is_hlos_prov_request_sent = false;
...
/*=========================================================================
  ...
  Function:  isIRadioMessage

===========================================================================*/
boolean isIRadioMessage
(
  uint32_t      msgId
)
{

 QCRIL_LOG_INFO("%s, msgId: %d", __FUNCTION__, msgId);

  switch(msgId)
  {
    /* List of IRadio messages */
...
/*=========================================================================
  ...
  Function:  UimModule::handleClientDisconnectedMsg

===========================================================================*/
void UimModule::handleClientDisconnectedMsg(std::shared_ptr<ClientDisconnectedMessage> msg)
{
  QCRIL_LOG_INFO( "%s :  %s\n", __FUNCTION__, msg->dump().c_str());
  QCRIL_LOG_INFO( "QMI Pending queue mQmiAsyncMsgCount: %d  mQmiPendingList.size: %d \n", \
                                                mQmiAsyncMsgCount, mQmiPendingList.size());

  /* In case of Telephony crash,only Iradio messages are impacted, other UIM messages can
     be processed by UIM custom clients.

     Get the last message in the queue, and iterate through the queue to check and delete
     pending IRadio messages. In case message is not IRadio message, pop and push back to
     the queue till original last pending message is encountered.
  */

  std::shared_ptr<UimQmiUimRequestMsg> lastPendingMsg;

  if(! mQmiPendingList.empty())
  {
    lastPendingMsg = mQmiPendingList.back();
  }
  else
  {
    QCRIL_LOG_INFO("QMI Pending queue is empty, returning");
    return;
  }
  ...
  while(! mQmiPendingList.empty())
  {
    ...
    if(isIRadioMessage(pendingMsg->get_msg_id()))
    {
      mQmiPendingList.pop();
      ...
      QCRIL_LOG_INFO("Removing pending Request message:%d in QMI Queue", pendingMsg->get_msg_id());
    }
    else
    {
      QCRIL_LOG_INFO("Did Not Remove pending Request message:%d in QMI Queue", pendingMsg->get_msg_id());
      mQmiPendingList.pop();
      mQmiPendingList.push(pendingMsg);
    }

    if(pendingMsg == lastPendingMsg)
    {
      QCRIL_LOG_INFO("Iterated complete QMI Queue for Iradio messages");
      break;
    }
  }
  QCRIL_LOG_INFO( "END QMI Pending queue mQmiAsyncMsgCount: %d  mQmiPendingList.size: %d \n", \
                                                mQmiAsyncMsgCount, mQmiPendingList.size());
}/* handleClientDisconnectedMsg */


/*=========================================================================

  Function:  UimModule::handleRilManualProvisioningRequestMsg

===========================================================================*/
void UimModule::handleRilManualProvisioningRequestMsg(std::shared_ptr<RilManualProvisioningMessage> msg)
{
  QCRIL_LOG_INFO( "%s :  %s\n", __FUNCTION__, msg->dump().c_str());

  is_iradio_hal_1_5_or_up = true;

  QCRIL_LOG_INFO("qcril_uim_hlos_prov_pref_auto: %d is_hlos_prov_request_sent: %d",
                  qcril_uim_hlos_prov_pref_auto, is_hlos_prov_request_sent);

  /* If RIL common sent 1.5 IRadio Hal indication late, sendHLOS prov req to Modem if not sent already */
  if((!qcril_uim_hlos_prov_pref_auto) && (!is_hlos_prov_request_sent))
  {
    QCRIL_LOG_INFO("Calling qcril_uim_set_hlos_provisioning_preference - Manual");
    /* send Hlos provisioning preference QMI message to Modem only if manual */
    /* Hlos prov pref ADB property & NV4398 value are kept similar */
