...
extern bool is_iradio_hal_1_5_or_up;
/*===========================================================================
      ...
      new_status_ptr->card[slot].card_state == QMI_UIM_CARD_STATE_ABSENT)
  {
        ...
        ((old_status_ptr->index_gw_pri_prov & 0xFF) < QMI_UIM_MAX_APP_PER_CARD_COUNT))
    {
      ...
      if ((old_status_ptr->num_slots > 1) ||
          (is_iradio_hal_1_5_or_up && old_status_ptr->num_slots > 0))
      {
...
)
{
  ...
  if (old_status_ptr->num_slots <= 1)
  {
    if( is_iradio_hal_1_5_or_up && old_status_ptr->num_slots < 1)
    {
      /*If Single SIM - Android R configuration, treat like DSDS for Manual prov */
    }
    else
    {
    /* No change needed */
...
)
{
  ...
  QCRIL_LOG_DEBUG( "number_of_responses_expected: %d  response_info_len: %d\n",
...
)
{
  ...
  RIL_UIM_Errno                               iccidReadStatus       = RIL_UIM_E_INTERNAL_ERR;
  ...
  if (status_changed)
  {
       ...
       qcril_uim.card_status.card[slot].card_state == QMI_UIM_CARD_STATE_PRESENT)
    {
      ...
      if ((status_change_ind_ptr->num_slots > 1) ||
          (is_iradio_hal_1_5_or_up && status_change_ind_ptr->num_slots > 0))
      {
