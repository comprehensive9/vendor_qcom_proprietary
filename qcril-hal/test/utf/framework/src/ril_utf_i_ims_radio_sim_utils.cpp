...
V1_6::CallType convertProtoToHidlCallType(const ims_CallType& callType) {
    switch (callType) {
      ...
      default:
        return (V1_6::CallType) V1_0::utils::convertProtoToHidlCallType(callType);
    }
}
...
void convertProtoToHidlServiceInfoList(const ims_Info** const in,
                                       hidl_vec<V1_6::ServiceStatusInfo>& out) {
    if (in != nullptr) {
        uint32_t count = V1_0::utils::getCount(in);
        out.resize(count);
        for(uint32_t i = 0; i < count; ++i) {
            out[i] = {};
            convertProtoToHidlServiceStatusInfo(*(in[i]), out[i]);
        }
    }
}

V1_6::RadioTechType convertProtoToHidlRadioTech(const ims_RadioTechType& radioTechType) {
  switch (radioTechType) {
    case ims_RadioTechType_RADIO_TECH_NR5G:
      return RadioTechType::RADIO_TECH_NR5G;
    default:
      return (V1_6::RadioTechType) V1_0::utils::convertProtoToHidlRadioTech(radioTechType);
  }
}

void convertProtoToHidlRegistration(const ims_Registration& inReg, V1_6::RegistrationInfo& outReg) {
    if (inReg.has_state) {
        outReg.state = V1_0::utils::convertProtoToHidlRegState(inReg.state);
    } else {
        outReg.state = V1_0::RegState::INVALID;
    }
    ...
    V1_0::utils::convertProtoToHidlString((const char*)inReg.errorMessage.arg, outReg.errorMessage);

    if (inReg.has_radioTech) {
        outReg.radioTech = V1_6::utils::convertProtoToHidlRadioTech(inReg.radioTech);
    } else {
        outReg.radioTech = V1_6::RadioTechType::RADIO_TECH_INVALID;
    }
...
void convertProtoToHidlStatusForAccessTech(const ims_StatusForAccessTech& in,
        V1_6::StatusForAccessTech& out) {
    if (in.has_networkMode) {
        out.networkMode = V1_6::utils::convertProtoToHidlRadioTech(in.networkMode);
    } else {
        out.networkMode = V1_6::RadioTechType::RADIO_TECH_INVALID;
    }

    if (in.has_status) {
        out.status = V1_0::utils::convertProtoToHidlStatusType(in.status);
    } else {
    ...
    if (in.has_registered) {
      V1_6::utils::convertProtoToHidlRegistration(in.registered, out.registration);
    }
}

void convertProtoToHidlServiceStatusInfo(const ims_Info& in, V1_6::ServiceStatusInfo& out) {
    out.hasIsValid = in.has_isValid;
    if (in.has_isValid) {
        out.isValid = in.isValid;
    }

    if (in.has_type) {
        out.type = V1_0::utils::convertProtoToHidlServiceType(in.type);
    } else {
        out.type = V1_0::ServiceType::INVALID;
    }

    if (in.has_callType) {
        out.callType = V1_6::utils::convertProtoToHidlCallType(in.callType);
    } else {
    ...
    qcril_binary_data_type* userdataPtr =
            (qcril_binary_data_type*)in.userdata.arg;
    if (userdataPtr != nullptr && userdataPtr->data != nullptr) {
        out.userdata.setToExternal(userdataPtr->data, userdataPtr->len);
    }
    ...
    if(in.has_rttMode) {
       out.rttMode = (in.rttMode) ? V1_0::RttMode::RTT_MODE_FULL : V1_0::RttMode::RTT_MODE_DISABLED;
    } else {
    ...
    if (ptr != nullptr) {
        ...
        for(uint32_t i = 0; i < count; ++i) {
            out.accTechStatus[i] = {};
            V1_6::utils::convertProtoToHidlStatusForAccessTech(*(ptr[i]), out.accTechStatus[i]);
        }
    }
}

void convertProtoToHidlCallDetails(const ims_CallDetails& in, CallDetails& out) {
    if (in.has_callType) {
        out.callType = V1_6::utils::convertProtoToHidlCallType(in.callType);
    } else {
    ...
    if (in.has_callDomain) {
        out.callDomain = V1_0::utils::convertProtoToHidlCallDomain(in.callDomain);
    } else {
        out.callDomain = V1_0::CallDomain::CALL_DOMAIN_INVALID;
    }

    if (in.has_extrasLength) {
        out.extrasLength = in.extrasLength;
    } else {
        out.extrasLength = INT32_MAX;
    }
    ...
    for(uint32_t i = 0; i < count; ++i) {
        out.extras[i].setToExternal(ptr[i], strlen(ptr[i]));
    }

    V1_6::utils::convertProtoToHidlServiceInfoList((const ims_Info**)
    ...
    if(in.has_rttMode) {
       out.rttMode = V1_0::utils::convertProtoToHidlRttMode(in.rttMode);
    } else {
        out.rttMode = V1_0::RttMode::RTT_MODE_INVALID;
    }
    QCRIL_LOG_INFO("Out rrtMode %d ", out.rttMode);
    ...
    if (ptr1 != NULL) {
        out.sipAlternateUri.setToExternal(ptr1, strlen(ptr1));
        QCRIL_LOG_INFO("Out Sip Redirect Uri %s ", out.sipAlternateUri.c_str());
    }

}

ims_Error convertProtoToHidlDial(const ims_Dial &in, V1_6::DialRequest &out) {
  ims_Error ret = ims_Error_E_SUCCESS;
  ...
  do {
    ...
    if (in.has_callDetails) {
      out.hasCallDetails = true;
      V1_6::utils::convertProtoToHidlCallDetails(in.callDetails, out.callDetails);
    }
    // Presentation
    out.presentation = V1_0::IpPresentation::IP_PRESENTATION_INVALID;
    if (in.has_presentation) {
    ...
    out.redialInfo.callFailReason = V1_5::CallFailCause::CALL_FAIL_ERROR_UNSPECIFIED;
    out.redialInfo.callFailRadioTech = V1_6::RadioTechType::RADIO_TECH_UNKNOWN;
    if (in.has_redialInfo) {
      out.redialInfo.callFailReason =
          V1_5::utils::convertProtoToHidlCallFailCause(in.redialInfo.callFailReason);
      out.redialInfo.callFailRadioTech =
          V1_6::utils::convertProtoToHidlRadioTech(in.redialInfo.callFailRadioTech);
    }
  } while (0);
...
ims_CallProgressInfoType convertHidlToProtoCallProgressInfoType(V1_7::CallProgressInfoType in) {
  switch(in) {
    case V1_7::CallProgressInfoType::CALL_REJ_Q850:
      return ims_CallProgressInfoType_CALL_REJ_Q850;
    case V1_7::CallProgressInfoType::CALL_WAITING:
      return ims_CallProgressInfoType_CALL_WAITING;
    case V1_7::CallProgressInfoType::CALL_FORWARDING:
      return ims_CallProgressInfoType_CALL_FORWARDING;
    case V1_7::CallProgressInfoType::REMOTE_AVAILABLE:
      return ims_CallProgressInfoType_REMOTE_AVAILABLE;
    case V1_7::CallProgressInfoType::INVALID:
      return ims_CallProgressInfoType_UNKNOWN;
  }
  return ims_CallProgressInfoType_UNKNOWN;
}

bool convertHidlToProtoCall(const V1_7::CallInfo &in, ims_CallList_Call &out) {
  out.has_state = (in.state != V1_0::CallState::CALL_STATE_INVALID);
  out.state = V1_0::utils::convertHidlToProtoCallState(in.state);
  ...
  if (in.callProgInfo.type != V1_7::CallProgressInfoType::INVALID) {
    out.callProgInfo.type =
        V1_7::utils::convertHidlToProtoCallProgressInfoType(in.callProgInfo.type);
  ...
  out.isPreparatory = in.isPreparatory;
  out.has_crsData = !(in.crsData.type & V1_7::CrsType::INVALID) &&
      (in.crsData.originalCallType != V1_0::CallType::CALL_TYPE_INVALID);
  out.crsData.originalCallType = V1_0::utils::convertHidlToProtoCallType(in.crsData.originalCallType);
  out.crsData.type = in.crsData.type;
  return true;
}

bool convertHidlToProtoCallList(const ::android::hardware::hidl_vec<V1_7::CallInfo> &in,
                                ims_CallList &out) {
  int len = in.size();
