...
WR_ONCE_UNKNOWN_DWARF = 8  # use DWARF for WU
RD_ONLY_AFTER_INIT = 9
RD_WR = 10
...
class MP_RD_ONLY_AFTER_INIT(MP):
	def __init__(self, name, vaddr, size, writers, attrs, sha256):
		MP.__init__(self, name, vaddr, size, attrs)
		self.mp_type = RD_ONLY_AFTER_INIT
		self.sha256 = None
		self.writers = writers
		return
	def dump(self):
		print('MP_RD_ONLY_AFTER_INIT({0}:{1}:{2})'
			   .format(self.mp_type, self.vaddr, self.size))
		return
