...
class Entry():
	...
	def __init__(self, mp_type, name, attrs, value = None, section_or_symbol = mpgen_mpdb.SYMBOL, split_step = 0, split_reserve_size = 0, writer = '', obj_search_inc_filter = []):
		...
		self.obj_search_inc_filter = obj_search_inc_filter  # search path filter for DWARF
		self.wu_ranges = []  # the list for valid WU ranges generated by DWARF
...
class Catalog(list):
	def __init__(self):
		...
		self.wu_dwarf = []  # symbols converted to WU lists using DWARF
		return
	...
	def ro_init_section(self, section_name, attrs = '0x0'):
		el = Entry(mpgen_mpdb.RD_ONLY_AFTER_INIT, section_name, attrs, None, mpgen_mpdb.SECTION)
		self.append(el)
		return

	def rw_section(self, section_name, attrs = '0x0'):
		el = Entry(mpgen_mpdb.RD_WR, section_name, attrs, None, mpgen_mpdb.SECTION)
		self.append(el)
		return
	...
	def ro_init_symbol(self, symbol_name, attrs = '0x0'):
		el = Entry(mpgen_mpdb.RD_ONLY_AFTER_INIT, symbol_name, attrs)
		self.append(el)
		return
	...
	def kp_process_stack_conf(self, kp_proc_stack_obj_search_inc_filter):
		"""
			Set's inclusive filter for filenames/ paths that contain symbols needed for
			QHEE KP process stack unwinding.
		"""
		self.kp_proc_stack_obj_search_inc_filter = kp_proc_stack_obj_search_inc_filter

	def write_once_unknown_dwarf(self, symbol_name, obj_search_inc_filter, attrs = '0x0'):
		"""
			WU for all non pointer members of a symbol
		"""
		el = Entry(mpgen_mpdb.WR_ONCE_UNKNOWN_DWARF, symbol_name, attrs, obj_search_inc_filter = obj_search_inc_filter)
		self.append(el)
...
def get_catalog(config):
	...
	cat.ro_symbol('linux_banner', attrs = 'MP_SECTION_ATTRIBUTE_ATTESTATION')  # RTIC + QHEE
	cat.ro_symbol('linux_proc_banner', attrs = 'MP_SECTION_ATTRIBUTE_ATTESTATION')  # RTIC + QHEE

	# == selinux_enforcing (before kernel 4.19) ==
	...
	# == ss_initialized (before kernel 4.19) ==
	...
	# == selinux_hooks (before and after kernel 4.19) ==
	...
	cat.ro_init_symbol('selinux_hooks', attrs = 'MP_SECTION_ATTRIBUTE_ATTESTATION')  # QHEE only (RTIC pointers affected by KASLR)
	cat.ro_init_symbol('selinux_state', attrs = 'MP_SECTION_ATTRIBUTE_ATTESTATION')

	# == selinux_state (after kernel 4.19) ==
	# cat.write_once_unknown('selinux_state', writer = 'selinux', attrs = 'MP_SECTION_ATTRIBUTE_ALLOW_ONE | MP_SECTION_ATTRIBUTE_NRTIC')  # QHEE only for now
	cat.write_once_unknown_dwarf('selinux_state', ['security/selinux/hooks.c'], attrs = 'MP_SECTION_ATTRIBUTE_ALLOW_ONE')  # RTIC + QHEE

	# == selinux_avc (after kernel 4.19) ==
	# cat.write_once_unknown_dwarf('selinux_avc', ['security/selinux/avc.c'], attrs = 'MP_SECTION_ATTRIBUTE_NQHEE ')  # RTIC only
	...
	# Other config
	# Provide filepath search filter where to look the data for process unwinding (doesn't support whildcards)
	cat.kp_process_stack_conf(['/init/init_task.c', '/init/main.c', '/init/version.c', ])
	...
	if (dynamic_ftrace_configs):
		...
		cat.wu_section('.head.text', attrs = "NONE")
		cat.ro_init_section('.text', attrs = 'MP_SECTION_ATTRIBUTE_ATTESTATION | MP_SECTION_ATTRIBUTE_KP | MP_SECTION_ATTRIBUTE_CODE')
		cat.rw_section('.data', attrs = 'MP_SECTION_ATTRIBUTE_KP | MP_SECTION_ATTRIBUTE_ENFORCE')
		cat.rw_section('.bss', attrs = 'MP_SECTION_ATTRIBUTE_KP | MP_SECTION_ATTRIBUTE_ENFORCE')
		cat.ro_init_section('.rodata', attrs = 'MP_SECTION_ATTRIBUTE_ATTESTATION | MP_SECTION_ATTRIBUTE_KP | MP_SECTION_ATTRIBUTE_ENFORCE')
		...
		cat.ro_section('.head.text', attrs = "NONE")
		cat.ro_section('.text', attrs = "MP_SECTION_ATTRIBUTE_ATTESTATION")
		# May 22 18 Marked as non strict. It looks like HLOS unloads this region.
