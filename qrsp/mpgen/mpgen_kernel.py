...
class ExceptionDwarfParsingFailed(Exception):
	pass
...
class Kernel:
	...
	def get_symbol_file_pos(self, symbol_name):
		...
		if symbol is None:
			print("ERROR: Symbols unable to be found")
			return None
		section = self.section_tbl.find_by_name(symbol.section)
		if section is None:
			print("ERROR: Sections unable to be found")
			return None
	...
	def get_kernel(self, vmlinux) :
		...
		if (outdated(HEADERS_FILENAME, KERNEL_IMAGE)) :
			print("SECTIONS\n")
                        try:
			    mpgen_shell.dsystemf("%s -S %s > %s", SYMDUMP, KERNEL_IMAGE, HEADERS_FILENAME)

                        except:
                            print("llvm-readelf NOT FOUND")
